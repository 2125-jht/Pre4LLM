...
#用于跨batch统计
def batch_sid_recall_stats_from_logits(
    pred_logit_sid0,            # [B, 8192]
    user_click_list_label,       # [B, L]  sid0 labels, valid iff > 0, may contain duplicates
    top_k=(1, 10),
    vocab_size=8192,
):
    """
    返回：
      batch_recall_sums[k]: [V] —— 本 batch 内每个 sid0 在 label 列表里“命中 topK”的次数
    口径与 recall_at_list_k 完全一致（逐 label 位置计数）。
    """
    pred_logit_sid0 = tf.cast(pred_logit_sid0, tf.float32)
    labels = tf.cast(user_click_list_label, tf.int32)  # [B, L]

    # -------- 1) 有效 label：>0 --------
    valid = tf.greater(labels, 0)  # [B, L] bool

    # -------- 2) 分母：label 出现次数（不去重）--------
    flat_sid = tf.boolean_mask(labels, valid)  # [N]，N=本 batch 所有有效 label 位置数


    # -------- 3) 预测 topK 下标 --------
    max_k = max(top_k)
    top_idx = tf.nn.top_k(pred_logit_sid0, k=max_k).indices  # [B, max_k]

    batch_recall_sums = []
    for k in top_k:
        topk_idx = top_idx[:, :k]  # [B, k]

        # 对齐 recall_at_list_k 的命中逻辑：
        # top_k_exp: [B,k,1], label_exp:[B,1,L] -> match:[B,k,L]
        top_k_exp = tf.expand_dims(topk_idx, axis=2)     # [B, k, 1]
        label_exp = tf.expand_dims(labels, axis=1)       # [B, 1, L]
        match = tf.equal(top_k_exp, label_exp)           # [B, k, L]
        hit_per_label = tf.reduce_any(match, axis=1)     # [B, L] bool（每个 label 位置是否命中 topK）

        # 只统计有效 label（>0）
        flat_hit = tf.boolean_mask(hit_per_label, valid)                 # [N] bool
        flat_hit_f = tf.cast(flat_hit, tf.float32)                       # [N]

        # 分子：按 sid0 分桶，把命中次数加起来（不去重）
        batch_recall_sums.append(  tf.unsorted_segment_sum(data=flat_hit_f,segment_ids=flat_sid,num_segments=vocab_size)  )

    return  batch_recall_sums
