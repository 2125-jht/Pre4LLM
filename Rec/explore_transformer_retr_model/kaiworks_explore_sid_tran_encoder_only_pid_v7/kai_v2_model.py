...
if args.with_kai_v2:
    ...
    def filter_mask_wrapper(dataset):
        ...
        def mask_fn(batch):
            ...
            # action_cnt = label_play_over_7s*(label_play_over_7s+label_finish+label_like+label_follow+label_comment+label_collect+label_download+label_profile_enter)
...
if is_training:
    ...
    # _, _ = model.beam_search_fast(user_colossus_play_time_list, user_colossus_duration_list, user_colossus_channel_list, beam_size=128, temperature=1)  # 束搜索推理（训练时也运行用于调试）
    ...
    user_click_fea = all_param_dict["user_colossus_pid_list"]

    batch_size = tf.shape(user_colossus_play_time_list)[0]

    play_time_shape = tf.reshape(tf.shape(user_colossus_play_time_list), [1, -1])
    play_time_shape_repeat = tf.cast(tf.tile(play_time_shape, [batch_size, 1]), tf.float32)

    duration_shape = tf.reshape(tf.shape(user_colossus_duration_list), [1, -1])
    duration_shape_repeat = tf.cast(tf.tile(duration_shape, [batch_size, 1]), tf.float32)

    channel_shape = tf.reshape(tf.shape(user_colossus_channel_list), [1, -1])
    channel_shape_repeat = tf.cast(tf.tile(channel_shape, [batch_size, 1]), tf.float32)

    click_shape = tf.reshape(tf.shape(user_click_fea), [1, -1])
    click_shape_repeat = tf.cast(tf.tile(click_shape, [batch_size, 1]), tf.float32)
    # play_time_double = tf.cast(user_colossus_play_time_list, dtype=tf.float32)
    # duration_double =  tf.cast(user_colossus_duration_list, dtype=tf.float32)
    # channel_double = tf.cast(user_colossus_channel_list, dtype=tf.float32)
    ...
    user_targets = [
        ...
        ("play_time_shape", play_time_shape_repeat),
        ("duration_shape", duration_shape_repeat),
        ("channel_shape",  channel_shape_repeat),
        ("click_shape", click_shape_repeat)
    ]
