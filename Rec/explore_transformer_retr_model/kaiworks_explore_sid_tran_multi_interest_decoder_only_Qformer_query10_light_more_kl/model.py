...
class MultiInterestModel(object):
    ...
    def model(self, photo_sid, label, photo_semantic_id_int):
        ...
        # === 计算 coarse_interest 在 dim=1(query维度) 上的 KL 散度损失 ===
        coarse_attn = tf.reduce_sum(tf.square(coarse_interest), axis=-1)
        coarse_probs = tf.nn.softmax(coarse_attn, axis=1)
        uniform_probs = tf.ones_like(coarse_probs) / tf.cast(self._query_token_numb, tf.float32)
        kl_loss_per_sample = tf.reduce_sum(coarse_probs * tf.log(coarse_probs / (uniform_probs + 1e-10) + 1e-10), axis=1)
        ...
        # 统计每个coarse_interest被选中的次数占比
        for i in range(self._query_token_numb):
            selected_count = tf.reduce_sum(tf.cast(tf.equal(best_head, i), tf.float32))
            selected_ratio = selected_count / tf.cast(B, tf.float32)
            print_tensor("selected_ratio/query_%d" % i, selected_ratio)

        

        for i in range(self._query_token_numb):
            cos_sim_i=cos_sim[:,i]
            print_tensor("interest_pref_sim/query_%d" %i, tf.reduce_mean(cos_sim_i))
        ...
        # 计算加权平均损失，并加入 KL 散度正则化
        main_loss = tf.reduce_sum((losses[0] + losses[1] + losses[2]) * loss_mask) / (tf.reduce_sum(loss_mask) + 1e-9)
        kl_loss_masked = tf.reduce_sum(kl_loss_per_sample * loss_mask) / (tf.reduce_sum(loss_mask) + 1e-9)
        print_tensor('loss/kl_loss', kl_loss_masked)
        kl_weight = 1
        loss = main_loss + kl_weight * kl_loss_masked
        print_tensor('loss/main_loss', main_loss)
    ...
    def beam_search_fast_share_head(self, beam_size=512, temperature=1):
        ...
        * step>0 时保持固定 beam_size。
        ...
        返回：
            ...
            probs         – 同形状，逐 token 的 softmax 概率（便于做温度/多样性分析）
            query_indices – shape [B, Q*beam_size]，标记每个序列对应的 query 索引（0 代表第一个 query）
        """
        # ------------- 常量 & 子模型 -------------
        offsets = [0,
                self._vocab_sizes[0],
        ...
        # 创建 query 索引，标记每个序列对应的是哪个 query (0代表第一个query)
        # 对于每个 batch，前 beam_size 个对应 query 0，接下来 beam_size 个对应 query 1，以此类推
        query_range = tf.range(self._query_token_numb, dtype=tf.int32)  # [Q]
        query_range_expanded = tf.expand_dims(query_range, 1)  # [Q, 1]
        query_indices_1d = tf.reshape(tf.tile(query_range_expanded, [1, beam_size]), [-1])  # [Q*beam]
        query_indices = tf.tile(tf.expand_dims(query_indices_1d, 0), [B, 1])  # [B, Q*beam]

        K = tf.shape(all_scores)[1]                 # K = Q*beam
        sorted_scores, sorted_idx = tf.nn.top_k(all_scores, k=K)  # desc, [B,K]

        # 用 sorted_idx 重排 seqs/probs/query_indices
        batch_idx = tf.tile(tf.expand_dims(tf.range(B), 1), [1, K])             # [B,K]
        gather_2d = tf.stack([batch_idx, sorted_idx], axis=2)                   # [B,K,2]
        ...
        return all_seqs, all_probs, query_indices
