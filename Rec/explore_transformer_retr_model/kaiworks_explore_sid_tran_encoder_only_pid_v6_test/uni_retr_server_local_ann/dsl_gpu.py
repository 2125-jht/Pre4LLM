            ...
            self.uni_predict_fused(
              ...
              ## embedding 拉取相关配置，对应 fetch_mio_embedding 部分
               embedding_fetchers = [dict(
                ...
                timeout_ms = 50,
                slots_config = [config for config in slots_config if config["compress_group"] == "USER"],
              ...
              batching_config = dict(
                batch_timeout_micros = 10,													                    # 如果开启 batching，则配置 batching 的 timeout
              ...
              executor_config = dict(
                context_per_device=3,  				# 每张卡上设置的 context 个数，
                memory_size_per_context=7120, # 每个 context 占用的显存，单位 MB
              ),
            )
            .log_debug_info(
              common_attrs = [config["input_name"] for config in slots_config if config["compress_group"] == "USER"],
      ...
      return self.copy_user_meta_info(
        ...
        .cast_attr_type(
          attr_type_cast_configs=[
            ...
            {
              "to_type": "double",
              "from_common_attr": "colossus_channel_list",
