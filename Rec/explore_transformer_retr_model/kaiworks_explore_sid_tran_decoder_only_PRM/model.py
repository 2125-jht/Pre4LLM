    ...
    def beam_search_fast(self, beam_size=512, temperature=1):
        ...
        * PRM 重新打分后保留 beam_size 条进入下一步。
        ...
        for step, V in enumerate(self._vocab_sizes):
            ...
            # 当前候选只包含到本 step 的 prefix，直接 sum-pool 就是当前 sid path 的 reward target。
            cand_sid_embeddings = tf.nn.embedding_lookup(self._vocab_embedding, cand_seqs[:, :, 1:])
            prm_target_embedding = tf.reduce_sum(cand_sid_embeddings, axis=2)                # [B, cand, C]
            ...
            prm_probs = tf.nn.softmax(prm_logits, axis=-1)

            # --- PRM 选全局 top-beam_size，进入下一层解码 ---
            prm_path_probs, prm_best_idx = tf.nn.top_k(prm_probs, k=beam_size)
            ...
            prm_path_log_probs = tf.log(tf.maximum(prm_path_probs, 1e-12))     # [B, beam]
