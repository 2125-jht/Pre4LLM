...
# 获取模型output
# all_param_dict, feature_emb_size_dict = get_param_dict()
model = MultiInterestModel(print_ops=print_ops)
...
if is_training:
    ...
    user_effective_play_semantic_id_list_int = tf.cast(config.get_dense_fea("user_effective_play_semantic_id_list", dim=200, dtype=tf.int64), dtype=tf.int32)
    photo_semantic_id_int = tf.cast(config.get_dense_fea("photo_semantic_id", dim=1, dtype=tf.int64), dtype=tf.int32)
    user_effective_play_semantic_id_list_int_last = getLast(user_effective_play_semantic_id_list_int, 64)
    ...
    user_effective_play_semantic_id_list = processInput(user_effective_play_semantic_id_list_int_last)
    photo_semantic_id = processInput(photo_semantic_id_int)
    label = processLabel(photo_semantic_id_int)

    loss = model.model(user_effective_play_semantic_id_list, photo_semantic_id, label, photo_semantic_id_int)
    # user_sid = model.beam_search(user_effective_play_semantic_id_list)
    ...
    with tf.control_dependencies(print_ops):
        ...
        mask = tf.less(tf.random_uniform(label_shape), 0.8)  # True 的概率是 70%
        # 为大于 0.5 的部分生成 [0.5, 1.0] 的值
        high_vals = tf.random_uniform(label_shape, minval=0.5, maxval=1.0)

        # 为小于等于 0.5 的部分生成 [0.0, 0.5] 的值
        low_vals = tf.random_uniform(label_shape, minval=0.0, maxval=0.5)
        # 根据 mask 选择 high 或 low
    ...
    if args.with_kai_v2:
        ...
        dense_var_list = config.get_collection(config.GraphKeys.TRAINABLE_VARIABLES)
    ...
    if args.dryrun:
        ...
        user_effective_play_semantic_id_list = config.get_dense_fea("user_effective_play_semantic_id_list", dim=64, dtype=tf.float32)
        user_effective_play_semantic_id_list_int_origin = tf.cast(user_effective_play_semantic_id_list, tf.int32)
        user_effective_play_semantic_id_list_int = processInput(user_effective_play_semantic_id_list_int_origin)
        user_sid = model.beam_search(user_effective_play_semantic_id_list_int)
        user_sid = user_sid[:, :, 1:]
        # user_sid = tf.reshape(user_sid, [-1, tf.shape(user_sid)[1] * tf.shape(user_sid)[2]])
        output_sid = processOutputV2(user_sid)
        ...
        user_targets = [
            ("user_emb", output_sid), 
            ("user_input_origin", user_effective_play_semantic_id_list_int_origin),
            ("user_sid_input", user_effective_play_semantic_id_list_int),
            ("user_emb_origin", user_sid_origin)
        ]
