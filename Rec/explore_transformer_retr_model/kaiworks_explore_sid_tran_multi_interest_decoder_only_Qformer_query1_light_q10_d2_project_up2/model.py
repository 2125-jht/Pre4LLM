...
class MultiInterestModel(object):
    ...
    def model(self, photo_sid, label, photo_semantic_id_int):
        ...
        coarse_interest_model = QFormer(num_layers=10, dim=self._dim, num_heads=8, dropout_rate=0.1, hidden_dim=self._dim*2, training=True)
        coarse_interest = coarse_interest_model.forward(coarse_interest_input, encoder_output, src_mask, training=True) # [b, 1, dim*2]

        interest_embed = tf.layers.dense(coarse_interest, self._dim, name='project') # [b, 1, dim]从兴趣提取语义到sid生成
        

        #构造fine_item_input的输入，
        fine_item_input = tf.concat([interest_embed, sid_embedding[:, :-1, :]], axis=1)  # [b, code_size, dim]


        with tf.variable_scope("fine_item_decoder", reuse=tf.AUTO_REUSE):

            fine_item_model = DecoderModel(num_layers=2, dim=self._dim, num_heads=8, dropout_rate=0.1, hidden_dim=self._dim*2, training=True)
            decoder_output = fine_item_model.forward(fine_item_input, encoder_output, src_mask, training=True)  # [b, code_size, dim]

        coarse_norm = tf.nn.l2_normalize(tf.squeeze(interest_embed), axis=-1)        # [b, dim]
