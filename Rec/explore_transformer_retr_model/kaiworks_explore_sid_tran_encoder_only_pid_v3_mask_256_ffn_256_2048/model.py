...
class MultiInterestModel(object):
    ...
    def model(self, photo_sid, label, photo_semantic_id_int):
        ...
        decoder_model = DecoderModel(num_layers=4, dim=self._dim, num_heads=8, dropout_rate=0.1, hidden_dim=self._dim*8, training=True)
    ...
    def beam_search_fast(self, beam_size=512, temperature=1):
        ...
        * step>0 时保持固定 beam_size。
        ...
        decoder_model = DecoderModel(num_layers=4, dim=self._dim, num_heads=8,
                                    dropout_rate=0.1, hidden_dim=self._dim * 8)
    ...
    def beam_search_fast_sampling(self, beam_size=512, temperature=1):
        """
        O(batch·beam·logV) 近似复杂度的束搜索（显存与 beam_size 线性）
        ...
        * step>0 时保持固定 beam_size。
        ...
        for step, V in enumerate(self._vocab_sizes):
            ...
            if step == 0:
                ...
                var_logp = tf.reduce_mean(tf.square(logp - mean_logp))

                noise_standard = tf.random.normal(tf.shape(logp))

                noise = noise_standard * tf.sqrt(var_logp / 10) + mean_logp
