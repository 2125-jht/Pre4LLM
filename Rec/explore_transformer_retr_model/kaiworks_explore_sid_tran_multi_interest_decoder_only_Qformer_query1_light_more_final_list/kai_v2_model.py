...
# 训练时用最近 10 个点击样本扩展 sid0 set-label，同时从 user click sequence 头部裁掉，避免 label 泄漏
NUMB_TEST_PHOTO = 10
...
if is_training:
    ...
    user_profile_v1_click_semantic_id_list_int = config.get_dense_fea(
        "user_profile_v1_click_semantic_id_list", dim=200, dtype=tf.int64)
    
    # === 数据预处理 ===
    photo_semantic_id = processInput(photo_semantic_id_int)  # 处理输入序列
    label = processLabel(photo_semantic_id_int)  # 处理标签
    user_profile_v1_click_semantic_id0_list = processListLabel(user_profile_v1_click_semantic_id_list_int)
    user_click_list_label_int = user_profile_v1_click_semantic_id_list_int[:, :NUMB_TEST_PHOTO]
    user_click_list_label = user_profile_v1_click_semantic_id0_list[:, :NUMB_TEST_PHOTO]

    # === 模型前向传播 ===
    print("=== 模型前向传播 ===")
    loss, result_dict = model.model(
        photo_semantic_id,
        label,
        photo_semantic_id_int,
        user_click_list_label=user_click_list_label,
        user_click_list_label_int=user_click_list_label_int,
        numb_test_photo=NUMB_TEST_PHOTO,
        usingsoftmax=False
    )  # 计算训练损失
    ...
    user_sid, user_sid_prob = model.beam_search_fast_share_head(beam_size=512, temperature=1)
