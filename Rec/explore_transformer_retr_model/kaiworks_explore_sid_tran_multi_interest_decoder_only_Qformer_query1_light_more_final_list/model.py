...
class MultiInterestModel(object):
    ...
    def model(self, photo_sid, label, photo_semantic_id_int,
              user_click_list_label=None, user_click_list_label_int=None,
              numb_test_photo=0, usingsoftmax=True):
        """
        主训练模型前向传播
        ...
        Args:
            ...
            user_click_list_label: 最近点击视频的 sid0 label，shape=[batch_size, numb_test_photo]
            user_click_list_label_int: 最近点击视频的原始 semantic id，shape=[batch_size, numb_test_photo]
            numb_test_photo: 从用户点击序列头部裁掉的最近点击数，避免 label 泄漏
            usingsoftmax: sid0 是否仍使用单点 softmax CE；False 时使用最近点击 set loss
        ...
        fine_item_input = tf.concat([coarse_interest, sid_embedding[:, :-1, :]], axis=1)  # [b, code_size, dim]
        ...
        coarse_norm = tf.nn.l2_normalize(tf.squeeze(coarse_interest), axis=-1)        # [b, dim]
        ...
        for step in range(len(self._vocab_sizes)):
            with tf.variable_scope('proj_%d' % step):
                ...
                if step == 0 and (not usingsoftmax) and user_click_list_label is not None and user_click_list_label_int is not None:
                    label_main = tf.expand_dims(label[:, step], axis=1)  # [B, 1]
                    set_labels = tf.concat([label_main, user_click_list_label], axis=1)  # [B, 11]

                    photo_semantic_id_int_flat = tf.reshape(photo_semantic_id_int, [-1])
                    set_label_mask = tf.concat(
                        [
                            tf.expand_dims(photo_semantic_id_int_flat > 0, axis=1),
                            user_click_list_label_int > 0
                        ],
                        axis=1
                    )
                    set_labels = tf.where(
                        set_label_mask,
                        set_labels,
                        tf.fill(tf.shape(set_labels), tf.constant(-1, dtype=tf.int32))
                    )
                    list_loss_mask = tf.cast(tf.reduce_any(set_label_mask, axis=1), tf.float32)

                    multi_hot = tf.reduce_max(
                        tf.one_hot(set_labels, self._vocab_sizes[step]),
                        axis=1
                    )
                    unique_sid0_count = tf.reduce_sum(multi_hot, axis=1)
                    list_mask_sum = tf.reduce_sum(list_loss_mask) + 1e-9
                    print_tensor(
                        "sid0/unique_sid0_count_masked_mean",
                        tf.reduce_sum(unique_sid0_count * list_loss_mask) / list_mask_sum
                    )

                    min_top_k = set_labels.get_shape().as_list()[1]
                    min_top_k = min_top_k if min_top_k is not None else tf.shape(set_labels)[1]
                    recall_at_k_multi_label(
                        ...
                        list_loss_mask,
                        top_k=[16, 128, 256],
                        dynamic_top_k=min_top_k,
                        name="predict_recall_list_%d" % step
                    )

                    log_prob = tf.nn.log_softmax(pred_logit/temperature, axis=-1)
                    loss_i = -tf.reduce_sum(log_prob * multi_hot, axis=1)
                    step_loss = tf.reduce_sum(loss_i * list_loss_mask) / (tf.reduce_sum(list_loss_mask) + 1e-9)
                    losses.append(step_loss)

                    print_tensor("loss/loss_%d" % step, step_loss)
                    print_tensor("list_loss_mask", list_loss_mask)
                    ...
                    step_loss = tf.reduce_sum(loss_i * loss_mask) / (tf.reduce_sum(loss_mask) + 1e-9)
                    losses.append(step_loss)
