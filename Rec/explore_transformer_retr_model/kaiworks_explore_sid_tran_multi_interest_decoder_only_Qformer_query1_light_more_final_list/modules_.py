...
def recall_at_k_multi_label(predict, multi_hot_label, label_count, loss_mask,
                            top_k=[16, 128, 256], dynamic_top_k=None,
                            name="default"):
    """
    计算 multi-label Top-K 召回率。

    Args:
        predict: 预测logits，形状为[batch_size, vocab_size]
        multi_hot_label: 去重后的multi-hot标签，形状为[batch_size, vocab_size]
        label_count: 每条样本有效且去重后的label数量，形状为[batch_size]
        loss_mask: 损失掩码，形状为[batch_size]
        top_k: 固定K值列表
        dynamic_top_k: 动态K的最大候选宽度；每条样本实际K由label_count决定
        name: 指标名称，用于TensorBoard显示
    """
    ...
    batch_idx_base = tf.range(batch_size, dtype=tf.int32)
    label_count_safe = label_count + 1e-9
    mask_sum = tf.reduce_sum(loss_mask) + 1e-9

    if dynamic_top_k is not None:
        _, top_indices = tf.nn.top_k(predict, k=dynamic_top_k)
        top_width = tf.shape(top_indices)[1]
        batch_idx = tf.tile(tf.expand_dims(batch_idx_base, 1), [1, top_width])
        gather_idx = tf.stack([batch_idx, tf.cast(top_indices, tf.int32)], axis=2)
        hit = tf.gather_nd(multi_hot_label, gather_idx)
        top_position_mask = tf.sequence_mask(
            tf.cast(label_count, tf.int32),
            maxlen=dynamic_top_k,
            dtype=tf.float32
        )
        hit = hit * top_position_mask
        recall_per_sample = tf.reduce_sum(hit, axis=1) / label_count_safe
        recall_value = tf.reduce_sum(recall_per_sample * loss_mask) / mask_sum
        tf.summary.scalar("top_k/{}_min_unique".format(name), recall_value)

    for k in top_k:
        _, top_indices = tf.nn.top_k(predict, k=k)
        batch_idx = tf.tile(tf.expand_dims(batch_idx_base, 1), [1, k])
        gather_idx = tf.stack([batch_idx, tf.cast(top_indices, tf.int32)], axis=2)
        hit = tf.gather_nd(multi_hot_label, gather_idx)
        recall_per_sample = tf.reduce_sum(hit, axis=1) / label_count_safe
        recall_value = tf.reduce_sum(recall_per_sample * loss_mask) / mask_sum
        tf.summary.scalar("top_k/{}_{}".format(name, k), recall_value)
