...
class PRMModel():
    ...
    def build_variables(self):
        ...
        with tf.variable_scope("prm_model", reuse=tf.AUTO_REUSE):
            ...
            # LayerNorm after cross-attention residual connection (removed: old checkpoint has no cross_attn_ln variables)
    ...
    def forward_with_kv(self, target_embedding, K_pre, V_pre, src_mask, training):
        ...
        with tf.variable_scope("prm_model", reuse=tf.AUTO_REUSE):
            ...
            # Residual connection (no LayerNorm to match old checkpoint)
            target_attn = target_embedding + target_attn
            score = mlp(
                "target_score",
                target_attn,
