...
user_static_fea_names_dec = [
    "user_id_dec",          # 用户ID
    "user_gender_dec",      # 用户性别
    "user_age_segment_dec", # 用户年龄段
    "user_level_dec"        # 用户等级
]
...
class SIDRecModel(object):
    ...
    def __init__(self, feature_emb_dict, feature_emb_size_dict, dim=256, selected_size=64, vocab_sizes=[8192, 8192, 8192], print_ops=[]):
        """
        初始化
        ...
        self._total_len = 1 + selected_size
        ...
        # 初始化为 0，交由训练自发学习偏置；便于与 LayerNorm 配合
        # 0 = enc:user_static（编码器user token）
        # 1 = enc:profile_token（编码器侧点击序列 item token）
        self._token_type_embedding = tf.get_variable(
        ...
        # 可学习的位置编码
        self._enc_pos_embedding = tf.get_variable(
            name="enc_pos_embedding",
            shape=[self._total_len, self._dim],
            initializer=tf.random_uniform_initializer(minval=-1.0/self._dim, maxval=1.0/self._dim),
            trainable=True
        )  # [L, D]
        
        self._dec_pos_embedding = tf.get_variable(
            name="dec_pos_embedding",
            shape=[1 + len(self._vocab_sizes), self._dim],           # <START> + 解码步数
            initializer=tf.random_uniform_initializer(minval=-1.0/self._dim, maxval=1.0/self._dim),
            trainable=True
        )
    ...
    def model(self, photo_sid, label, photo_semantic_id_int):
        ...
        user_static_emb = tf.reshape(user_static_emb, [B, 1, self._dim]) # 调整形状为[batch_size, 1, dim]，作为序列的一个元素

        # === 2. 用户的profile ===================
        user_profile_fea = tf.concat([self._feature_emb_dict[fea] for fea in user_click_fea_names], axis=2)   # [B, L, dim] 最新→最旧 
        
        # 仅保留“有效序列的最后 selected_size 条”
        profile_max_len  = self._selected_size                                             
        profile_raw_len  = tf.cast(self._feature_emb_size_dict['user_profile_v1_click_pid_list'], tf.int32)  # 可能是 [B,1] 也可能是 [B]
        profile_valid_len = tf.reshape(profile_raw_len, [-1])      # 强制展平成 [B]
        profile_used_len   = tf.minimum(profile_valid_len, profile_max_len)               # [B] 
        user_profile_fea = user_profile_fea[:,:profile_max_len,:]  # 最新→最旧 仅保留 selected_size 条
        
        # 通过MLP处理点击特征
        user_profile_emb = mlp('user_click_emb', user_profile_fea, [4*self._dim], self._dim, activation=tf.nn.leaky_relu)
        user_profile_emb = tf.reverse(user_profile_emb, axis=[1])     # reverse 使得 最旧→最新    
        
        with tf.variable_scope('sample_len'):
            print_tensor("profile_valid_len", profile_valid_len)
            print_tensor("profile_used_len", profile_used_len)                          # 打印结果
        
        # === 3. 构建编码器输入 ===
        # 将user特征和点击行为特征拼接
        seq_emb = tf.concat([user_static_emb, user_profile_emb], axis=1)
        # token type emb
        user_type_ids = tf.fill([B, 1], 0)                                             # 0 = enc:user_static
        profile_type_ids = tf.fill([B, profile_max_len], 1)                            # 1 = enc:profile_token
        token_type_ids = tf.concat([user_type_ids, profile_type_ids], axis=1)          # concat到一起
        enc_type_emb = tf.nn.embedding_lookup(self._token_type_embedding, token_type_ids) # 获取emb
        # 位置编码，广播到 batch
        enc_pos_emb = tf.expand_dims(self._enc_pos_embedding, 0)  # [1, L, D]
        
        # 叠加：token + type + position  → LayerNorm → Dropout
        encoder_input = layer_norm(seq_emb+enc_pos_emb+enc_type_emb, scope="enc_emb_ln")
        encoder_input = tf.layers.dropout(encoder_input, rate=0.1, training=True, name="enc_emb_dropout")
        ...
        # profile mask：右对齐
        profile_mask = tf.sequence_mask(profile_used_len, maxlen=profile_max_len, dtype=tf.int8)     # [B,max_len] 左 1 右 0
        profile_mask = tf.reverse(profile_mask, axis=[1])                              # 右对齐

        # 拼成整条序列的 mask 向量，形状 [B, total_len]
        seq_mask   = tf.concat([user_tok, profile_mask], axis=1)  # 例: [1 0 0 1 1 1]
        ...
        # === 4. 构建解码器输入 ===
        # 构建dec侧的user info
        user_static_fea_dec = tf.concat([self._feature_emb_dict[fea] for fea in user_static_fea_names_dec], axis=1)
        user_static_emb_dec = mlp('user_static_emb_dec', user_static_fea_dec, [2*self._dim], self._dim, activation=tf.nn.leaky_relu)
        user_static_emb_dec = tf.reshape(user_static_emb_dec, [B, 1, self._dim]) # 调整形状为[batch_size, 1, dim]
        # 起始token
        start_token_indice = tf.tile(tf.constant(self._total_vocab_size, shape=(1, 1), dtype=tf.int32), [B, 1])
        start_token_emb = tf.nn.embedding_lookup(self._vocab_embedding, start_token_indice)
        start_emb = user_static_emb_dec + start_token_emb                       # [B,1,D]
        # sid嵌入向量
        photo_sid_emb = tf.nn.embedding_lookup(self._vocab_embedding, photo_sid) # [B,T,D]
        # 位置编码，广播到 batch
        dec_pos_emb = tf.expand_dims(self._dec_pos_embedding, 0)  # [1, L, D]
        # 构建dec输入
        # 拼接后统一做 LayerNorm
        decoder_input = tf.concat([start_emb, photo_sid_emb], axis=1)       # [B,1+T,D]
        decoder_input = layer_norm(decoder_input+dec_pos_emb, scope="dec_emb_ln")
        decoder_input = tf.layers.dropout(decoder_input, rate=0.1, training=True, name="dec_emb_dropout")
        ...
        # 计算编码器输出的余弦相似度（用于调试）
        encoder_output_sim = tf.reshape(encoder_output, [B, -1])
        ...
        decoder_output = decoder_model.forward(decoder_input[:,:-1,:], encoder_output, src_mask, training=True) # [batch_size, seq_len-1, dim]
        ...
        loss_mask = tf.reshape(loss_mask, [-1])  # [B]
    ...
    def beam_search_fast(self, beam_size=512, temperature=1):
        ...
        * step>0 时保持固定 beam_size。
        ...
        user_static_emb = tf.reshape(user_static_emb, [B, 1, self._dim])
        ...
        for step, V in enumerate(self._vocab_sizes):
            ...
            # 循环内 step：
            pos_slice = self._dec_pos_embedding[step:step+1, :]            # [1, D]  step=0 对应 <START>
            pos_slice = tf.reshape(pos_slice, [1, 1, self._dim])           # [1,1,D]
            pos_bcast = tf.tile(pos_slice, [B*cur_beam, 1, 1])             # [B*beam,1,D]
            
            if step == 0:
                user_static_beam = tf.reshape(
                    tf.tile(user_static_emb_dec, [1, cur_beam, 1]),                        # [B, beam, D]（去掉了时序维）
                    [B * cur_beam, 1, self._dim]
                )
                dec_in = dec_in + user_static_beam
                      
            dec_in = layer_norm(dec_in+pos_bcast, scope="dec_emb_ln")
