...
def dump_prm_variable_order(graph, tag):
    """
    dump 当前图中 PRM 相关变量的建图顺序，用于训练/推理图交叉对比。
    线上按 dense bin 顺序加载权重，若训练/推理建图顺序不一致会导致投影层与 PRM 层权重错位。
    重点检查 prm_model/* 的顺序在两个图里完全一致。
    """
    print("===== PRM Variable Order [%s] =====" % tag)
    prm_vars = [v for v in tf.compat.v1.global_variables() if "prm_model" in v.name]
    if not prm_vars:
        print("  (no prm_model variables found)")
        return
    for i, v in enumerate(prm_vars):
        print("  [%2d] %-80s %s" % (i, v.name, v.shape))
    print("  total prm_model variables: %d" % len(prm_vars))
...
def main():
    ...
    with g_train.as_default():
        ...
        with tf.Session(graph=g_train, config=cfg) as sess:
            ...
            # dump 训练图 PRM 变量顺序，用于与推理图交叉对比
            dump_prm_variable_order(g_train, "train")
    ...
    with g_infer.as_default():
        ...
        with tf.Session(graph=g_infer, config=cfg) as sess:
            ...
            # dump 推理图 PRM 变量顺序，用于与训练图交叉对比
            dump_prm_variable_order(g_infer, "infer")
