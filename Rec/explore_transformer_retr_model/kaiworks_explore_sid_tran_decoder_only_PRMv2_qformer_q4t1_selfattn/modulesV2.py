...
def multi_head_attention_with_kv(queries, K_pre, V_pre, num_heads, src_mask, dropout_rate, training=False):
    """
    与 multi_head_attention 一致，但跳过 w_k/w_v 投影：
    K_pre / V_pre 已是 [N, H, Tk, Dh]（split_heads 之后），由外部 project_kv 一次性算好。
    用于 PRM 的 cross-attention，避免对 encoder_output 重复做 K/V 投影（先投影再 tile）。
    """
    def split_heads(x, num_heads):
        batch_size = tf.shape(x)[0]
        depth = x.get_shape().as_list()[-1] // num_heads
    ...
    with tf.variable_scope("multi_head_attention", reuse=tf.AUTO_REUSE):
        ...
        Q = tf.layers.dense(queries, depth, use_bias=False, name="w_q")
        Q = split_heads(Q, num_heads)
        # K_pre / V_pre 已由 project_kv 投影并 split_heads，直接用，不再做 w_k/w_v 投影
...
def scaled_attention(q,            # [B*beam, H, T_q, Dh]  (单步)
    ...
    if src_mask is not None:       # Padding mask / Cross-attn mask
        ...
        # —— 兼容旧版 TF（TileOp 不支持 int8，如 1.12）：上面两行替换成下面四行 ——
        # src_mask = tf.cast(src_mask, tf.float32)
        # src_mask = tf.tile(src_mask, [cur_beam, 1, 1, 1])
        # src_mask = tf.broadcast_to(src_mask, [B*cur_beam, H, T_q, T_k])
        # attn_scores = apply_mask_add(attn_scores, src_mask)
...
class PRMModel():
    ...
    def build_variables(self):
        ...
        with tf.variable_scope("prm_model", reuse=tf.AUTO_REUSE):
            # LayerNorm variables for cross-attention residual
            with tf.variable_scope("cross_atten_ln/layer_norm", reuse=tf.AUTO_REUSE):
                tf.get_variable("gamma", [self.dim], initializer=tf.ones_initializer())
                tf.get_variable("beta", [self.dim], initializer=tf.zeros_initializer())
    ...
    def project_kv(self, hidden_states):
        """
        对 encoder_output [B, L, dim] 一次性投影 K/V 并 split_heads，返回 [B, H, L, Dh]。
        供 forward_with_kv 复用，避免在 tile 后重复投影（Level 1：先投影再 tile，仅省 FLOPs）。
        内部先调 build_variables() 固定 PRM 全部变量的 dense bin 顺序，
        使训练/推理图建图顺序一致（线上按顺序加载权重，避免投影层与 PRM 层错位）。
        """
        def split_heads(x, num_heads):
            batch_size = tf.shape(x)[0]
            depth = x.get_shape().as_list()[-1] // num_heads
        ...
        with tf.variable_scope("prm_model/multi_head_attention", reuse=tf.AUTO_REUSE):
            ...
            K = split_heads(K, self.num_heads)   # [B, H, L, Dh]
            V = split_heads(V, self.num_heads)   # [B, H, L, Dh]
        return K, V

    def forward_with_kv(self, target_embedding, K_tiled, V_tiled, src_mask, training):
        """
        与 forward 一致，但 K/V 用 project_kv 预投影的结果（已 split_heads + tile 到 [N, H, L, Dh]）。
        Q 仍在内部用 w_q 投影。target_embedding shape [N, 1, dim]。
        """
        with tf.variable_scope("prm_model", reuse=tf.AUTO_REUSE):
            ...
            # residual + LayerNorm (Post-Norm, same pattern as DecoderLayer)
...
# ======================================================================
#  QFormer 相关组件（从方法A移植）
#  用于将 encoder_output [B, L, D] 压缩为少量可学习兴趣 token [B, K, D]
#  使 Decoder 和 PRM 的 cross-attn Tk 从 L=201 降至 K，大幅减少推理耗时
# ======================================================================

def rms_norm(x, scope, eps=1e-6):
    """RMSNorm，QFormer 内部使用 pre-norm 时的归一化层"""
...
class QformerLayer:
    """
    单层 QFormer：self-attention + cross-attention + FFN（三件套，对齐 BLIP-2）。

    无 self-attn 时，K 个 query 独立对同一 encoder 做 cross-attn，会倾向于 attend
    到 encoder 中最显著的相同 token，导致 K 个 query 输出趋同（同质化）。
    加入 query 间 self-attn 后，K 个 query 先互相"商量"分工，再 cross-attn encoder，
    自然能分化出不同兴趣偏好。

    变量 scope 设计（向后兼容热启动）：
      - self-attn: <layer>/self_atten_ln/..., <layer>/self_attention/multi_head_attention/...
      - cross-attn: <layer>/cross_atten_ln/..., <layer>/multi_head_attention/...  （原路径不变）
      - ffn: <layer>/ffn_ln/..., <layer>/feed_forward_network/...                 （原路径不变）
    """
    def __init__(self, name, dim, num_heads, hidden_dim, dropout_rate, training=False):
        self.name = name
        self.dim = dim
...
class QFormer:
    """
    多层 QFormer：将 encoder_output [B, L, D] 压缩为 K 个兴趣 token [B, K, D]。

    用法：
        qformer = QFormer(num_layers=2, dim=256, num_heads=8,
                          dropout_rate=0.1, hidden_dim=512)
        enc_compressed = qformer.forward(
            query_tokens,   # [B, K, D]，可学习 query
            encoder_output, # [B, L, D]，原始 user context
            src_mask,       # [B, 1, 1, L]
            training=True)  # [B, K, D]
    """
    def __init__(self, num_layers, dim, num_heads, hidden_dim, dropout_rate, training=False):
        ...
        self.layers = [
            QformerLayer(
                f"qformer_layer_{i}", dim, num_heads, hidden_dim, dropout_rate, training=training)
            for i in range(num_layers)
        ]

    def forward(self, query_embedding, hidden_states, src_mask, training):
        for i in range(self.num_layers):
            query_embedding = self.layers[i].forward(
                query_embedding, hidden_states, src_mask, training=training)
