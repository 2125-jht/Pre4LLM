...
# global param
predict_server_name = "grpc_exploreGRRetr10Query"
model_queue_prefix="kaiworks_explore_sid_tran_multi_interest_decoder_only_Qformer_query10_light_more"
            ...
            self.uni_predict_fused(
              ...
              ## embedding 拉取相关配置，对应 fetch_mio_embedding 部分
               embedding_fetchers = [dict(
                ...
                colossusdb_embd_model_name="explore_sid0_v1",
                colossusdb_embd_table_name="gr_multi_query10_light_more_cpu_new",
              ...
              executor_config = dict(
                context_per_device=32,  				# 每张卡上设置的 context 个数，
                memory_size_per_context=32, # 每个 context 占用的显存，单位 MB
              ),
            )
            .log_debug_info(
...
service.build(output_file=os.path.join(model_dir, "dynamic_json_config_cpu.json"))
