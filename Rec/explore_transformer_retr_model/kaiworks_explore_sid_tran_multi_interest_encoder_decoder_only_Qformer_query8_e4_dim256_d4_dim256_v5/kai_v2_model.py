...
if is_training:
    ...
    user_sid, user_sid_prob, query_indices, query_probs = model.beam_search_fast_share_head(beam_size=500, temperature=1, head_top_k=2)
    ...
    query_indices = tf.cast(query_indices, tf.float32)
    query_probs = tf.cast(query_probs, tf.float32)
    print('user_sid:', user_sid)
    print('query_indices:', query_indices)
    print('query_probs:', query_probs)
