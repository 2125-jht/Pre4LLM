...
class MultiInterestModel(object):
    ...
    def model(self, photo_sid, label, photo_semantic_id_int):
        ...
        # === 3. 构建编码器输入 ===
        # 在序列最前面插入一个 query token，encoder 输出的第 0 位作为用户兴趣 token。
        query_token_id = tf.constant(self._total_vocab_size, dtype=tf.int32)
        query_token_indice = tf.fill([batch_size, 1], query_token_id)
        query_token_emb = tf.nn.embedding_lookup(self._embedding, query_token_indice)
        ...
        # 整个序列长度 = 1（query token）+ 1（user token）+ max_len（点击序列）
        total_len  = 2 + max_len                       # int, e.g. 7 when max_len=5
        ...
        # ② query token 和 user 静态 token 永远有效，直接补 1
        query_tok  = tf.ones([B, 1], dtype=tf.int8)  # [B,1]
        user_tok   = tf.ones([B, 1], dtype=tf.int8)  # [B,1]

        # ③ 拼成整条序列的 mask 向量，形状 [B, total_len]
        seq_mask   = tf.concat([query_tok, user_tok, click_mask], axis=1)  # 例: [1 1 1 0 0 0 0]
        ...
        interest_token = encoder_output[:, :1, :] # [batch_size, 1, dim]
        decoder_encoder_output = encoder_output[:, 1:, :] # decoder cross-attn 不包含 interest token
        decoder_src_mask = tf.reshape(seq_mask[:, 1:], [B, 1, 1, total_len - 1])
        ...
        # === 6. Transformer解码器 ===
        # decoder 首位使用 interest token 替换原来的 BOS，其余位置保持 teacher forcing 输入。
        photo_sid_embedding = tf.nn.embedding_lookup(self._embedding, photo_sid)
        decoder_input = tf.concat([interest_token, photo_sid_embedding], axis=1)
        ...
        decoder_output = decoder_model.forward(decoder_input, decoder_encoder_output, decoder_src_mask, training=True) # [batch_size, seq_len, dim]
    ...
    def beam_search_fast(self, beam_size=512, temperature=1):
        ...
        * step>0 时保持固定 beam_size。
        ...
        for step, V in enumerate(self._vocab_sizes):
            ...
            if step == 0:
                sid0_top = tf.cast(tf.reshape(topk_tok, [B, -1]), tf.float32)
        ...
        return gen_part_loc, probs, sid0_top
