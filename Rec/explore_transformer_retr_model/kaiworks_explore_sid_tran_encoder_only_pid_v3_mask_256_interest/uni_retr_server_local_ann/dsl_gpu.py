...
# global param
predict_server_name = "grpc_exploreGRRetr256Dim"
model_queue_prefix="kaiworks_explore_sid_trans_yzc_v3_256"
model_dir = "predict/conf_gpu"
param_json = "parameter_config.json"
PXTRS = ['user_sid_origin', "user_sid_prob", "sid0_top"]
sign_format = "mio"
shard_offset = 0
queue_shard_num = 1
EMBEDDING_SHARDS = 1
            ...
            self.uni_predict_fused(
              ...
              ## embedding 拉取相关配置，对应 fetch_mio_embedding 部分
               embedding_fetchers = [dict(
                ...
                colossusdb_embd_table_name="gr_256_new",
    ...
    def prepare(self):
      return self.copy_user_meta_info(
        ...
        .fetch_remote_embedding(
          ...
          input_attr_name = "playing_photo_id_list",
          id_converter = {"type_name": "mioEmbeddingIdConverter"},
          output_attr_name = "user_profile_v1_click_semantic_id_list",
          client_side_shard = True,
          size = 3,
        ) \
        .enrich_attr_by_lua(
          import_common_attr = ["user_profile_v1_click_semantic_id_list"],
          # function_for_item 的值也可用 "{{}}" 格式指定为某个 common_attr
          function_for_common = "calculate",
          export_common_attr = ['top_10', 'total_count', 'top_10_count', 'total_avg', 'top_10_avg'],
          lua_script = """
            function calculate()
              local sid0_list = {}
    ...
    def retrieve(self):
      ...
      .cast_attr_type(
        ...
        attr_type_cast_configs=[
          {
            "to_type": "int",
            "from_common_attr": "sid0_top",
            "to_common_attr": "sid0_top_int"
          },
        ]
      ) \
      .pack_common_attr(
        input_common_attrs = ["sid0_top_int"],
        output_common_attr = "sid0_top_int_10",
        limit_num = 10,
      ) \
      .enrich_attr_by_lua(
        import_common_attr = ["top10_sid0", 'sid0_top_int_10'],
          # function_for_item 的值也可用 "{{}}" 格式指定为某个 common_attr
        function_for_common = "calculate",
        export_common_attr = ['same_count'],
        lua_script = """
          function calculate()
            local set = {}
            local count = 0
            -- 将数组 a 的元素存储在哈希表中
            for _, value in ipairs(sid0_top_int_10) do
                set[value] = true
            end
            -- 遍历数组 b，检查每个元素是否在哈希表中
            for _, value in ipairs(top10_sid0) do
                if set[value] then
                    count = count + 1
                    set[value] = nil  -- 防止重复计数相同元素
                end
            end
            return count
          end
      ...
      .enrich_attr_by_lua(
        ...
        # function_for_item 的值也可用 "{{}}" 格式指定为某个 common_attr
        function_for_common = "calculate",
        export_common_attr = ["sid1_num", "sid12_num","top1","top2","top3","top4","top5","top6","top7","top8","top9","top10","all_top10","all_top20",],
        lua_script = """
          function calculate()
            local seen_1 = {}
            local seen_12 = {}
