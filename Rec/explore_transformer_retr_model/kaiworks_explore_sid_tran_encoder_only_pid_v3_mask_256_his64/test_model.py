...
def build_dummy_inputs(batch_size=4, max_len=200, dim=256,
    ...
    ph["user_id_dec"]          = tf.placeholder(tf.float32, [None, d_user_each], name="ph_user_id_dec")
    ph["user_gender_dec"]      = tf.placeholder(tf.float32, [None, d_user_each], name="ph_user_gender_dec")
    ph["user_age_segment_dec"] = tf.placeholder(tf.float32, [None, d_user_each], name="ph_user_age_segment_dec")
    ph["user_level_dec"]       = tf.placeholder(tf.float32, [None, d_user_each], name="ph_user_level_dec")
    ...
    fd = {
        ...
        ph["user_id_dec"]:          rng.randn(B, d_user_each).astype(np.float32),
        ph["user_gender_dec"]:      rng.randn(B, d_user_each).astype(np.float32),
        ph["user_age_segment_dec"]: rng.randn(B, d_user_each).astype(np.float32),
        ph["user_level_dec"]:       rng.randn(B, d_user_each).astype(np.float32),
    ...
    feature_emb_dict = {
        ...
        "user_id_dec":          ph["user_id_dec"],
        "user_gender_dec":      ph["user_gender_dec"],
        "user_age_segment_dec": ph["user_age_segment_dec"],
        "user_level_dec":       ph["user_level_dec"],
...
def main():
    ...
    with g.as_default():
        ...
        # 2) 构建模型（假定 Decoder 已经替换为 MoE SwiGLU + Top-2，并在 router 里 add_to_collection("lb_loss", ...)）
        ...
        # 5) Beam Search（两个版本）
        gen_loc_greedy, probs_greedy = model.beam_search_fast(beam_size=5, temperature=1.0)
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            for t in range(1, num_steps + 1):
                ...
                if t % log_every == 0:
                    print("[Train] step=%d  lb=%.6f  total=%.6f" %
                        (gs, lb_v, total_v))
