...
import collections
import random 
#sys.path.append("/home/fengkai/data/code/svn/ks/ks/common_reco/ann_retrieve/dragonfly/")
#from ann import *
current_dir = os.path.dirname(__file__)
#sys.path.append('/home/liyunhao/data/ks/dragon/tools/pypi/')
sys.path.append('/home/fengkai/data/code/git/python/dragon/tools/pypi/')

# global param
predict_server_name = "grpc_exploreGRRetrBeam1024"
model_queue_prefix="kaiworks_explore_sid_trans_yzc_v3"
model_dir = "predict/conf_gpu900"
            ...
            self.uni_predict_fused(
              ...
              ## embedding 拉取相关配置，对应 fetch_mio_embedding 部分
               embedding_fetchers = [dict(
                ...
                colossusdb_embd_table_name="yzc_sid_v3_2",
  ...
  # .enrich_attr_by_py(
    ...
    def retrieve(self):
      ...
      .enrich_attr_by_lua(
        ...
        export_item_attr = ["query_sid_list", "query_sid", "prob1", "prob2", "prob3", "prob"],
        lua_script = """
          function calculate()
            ...
            prob = math.log(prob1 * prob2 * prob3)
            return query_sid_list, query_int, prob1, prob2, prob3, prob
          end
        """
      ) \
      .pack_item_attr(
        ...
        mappings = [
          {
            "from_item_attr": "prob",
            "to_common_attr": "min_prob",
            "aggregator": "min",
          },
          {
            "from_item_attr": "prob",
            "to_common_attr": "max_prob",
            "aggregator": "max",
          }
        ]
      ) \
      .enrich_attr_by_lua(
        import_common_attr = ["max_prob", "min_prob"],
        import_item_attr = ["prob"],
        function_for_item = "calculate",
        export_item_attr = ["class"],
        lua_script = """
          function calculate()
            local interval = (max_prob - min_prob) / 50
            local class = math.floor((prob - min_prob) / interval)
            return class
          end
        """
      ) \
      ...
      .shuffle() \
      .zigzag_arranger(
        cluster_attr_name = "index"
      )
      

    def post_process(self):
      return self.deduplicate(on_item_attr="author__id") \
        .log_debug_info(
          common_attrs = [
            "user_sid_origin",
            "request_num",
...
service.build(output_file=os.path.join(model_dir, "dynamic_json_config_gpu_beam_1024.json"))
