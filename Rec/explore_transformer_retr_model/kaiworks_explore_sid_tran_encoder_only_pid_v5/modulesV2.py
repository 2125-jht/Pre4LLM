...
def apply_mask_add(logits, src_mask, neg_large=-1e9):
    """
    logits: [B,H,Lq,Lk]
    src_mask: [B,1,Lq,Lk]  (0=pad, 1=valid; any dtype)
    """
...
def scaled_attention(q,            # [B*beam, H, T_q, Dh]  (单步)
    ...
    if src_mask is not None:       # Padding mask / Cross-attn mask
        ...
        src_mask = tf.tile(tf.expand_dims(src_mask, 1), [1, cur_beam, 1, 1, 1])
        src_mask = tf.reshape(src_mask, [B*cur_beam, 1, 1, T_k])
...
class EncoderLayer:
    ...
    def forward(self, x, src_mask, training):
        with tf.variable_scope(f"{self.name}", reuse=tf.AUTO_REUSE):
            ...
            out1 = layer_norm(x + attn_output, name="atten_ln")
            ffn_output = self.ffn(out1, training=training)
            out2 = layer_norm(out1 + ffn_output, name="ffn_ln")
        return out2
...
class DecoderLayer:
    ...
    def forward(self, x, enc_output, src_mask, training=False):
        with tf.variable_scope(f"{self.name}", reuse=tf.AUTO_REUSE):
            ...
            out1 = layer_norm(x + attn_output, name="self_atten_ln")
            ...
            out2 = layer_norm(out1 + cross_attn_output, name="cross_atten_ln")
            ffn_output = self.ffn(out2, training=training)
            out3 = layer_norm(out2 + ffn_output, name="ffn_ln")
    ...
    def step(self, x_t, cur_beam,         # [B,1,D] 当步输入
        ...
        with tf.variable_scope(f"{self.name}", reuse=tf.AUTO_REUSE):
            ...
            out1 = layer_norm(x_t + self_out, name="self_atten_ln")
            ...
            out2 = layer_norm(out1 + cross_out, name="cross_atten_ln")
        
            # ---------- ③ FFN ----------
            ffn_out = self.ffn(out2, training=False)   # 仅算当前 token
            y_t     = layer_norm(out2 + ffn_out, name="ffn_ln")
