...
def calc_sim_cos(a):
    ...
    bs = tf.shape(a)[0] 
    # 为了提高推理速度，解码器输出设置为float16了，所以bs要设置dtype。但encoder没有改所以不能强制编码
    mask = tf.ones_like(sim_matrix) - tf.eye(bs, dtype=sim_matrix.dtype)  # 对角线为0，其他位置为1的掩码
...
def mlp(name, net, hidden_units, output_unit=None, activation=tf.nn.relu):
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        for i, k in enumerate(hidden_units):
            ...
            net = tf.layers.dense(net, k, activation=activation, name=layer_name,trainable=False)
        
        # 可选的输出层（通常不使用激活函数）
        if output_unit != None:
            net = tf.layers.dense(net, output_unit, activation=None, name=scope + '_final',trainable=False)
    return net
