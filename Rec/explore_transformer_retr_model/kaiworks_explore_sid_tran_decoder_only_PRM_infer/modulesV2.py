...
class PRMModel():
    ...
    def forward_with_encoder_cache(self, target_embedding, k_enc_cache, v_enc_cache, src_mask, cur_beam, training):
        def split_heads(x, num_heads):
            batch_size = tf.shape(x)[0]
            T_x = tf.shape(x)[1]
            depth = x.get_shape().as_list()[-1] // num_heads
        ...
        with tf.variable_scope("prm_model", reuse=tf.AUTO_REUSE):
            with tf.variable_scope("multi_head_attention", reuse=tf.AUTO_REUSE):
                # 只计算 query，而 key/value 从缓存中直接获取
                q_cur = tf.layers.dense(target_embedding, self.dim, use_bias=False, name="w_q",trainable=False)
                q_cur = split_heads(q_cur, self.num_heads)
                ...
                # 通过广播复用缓存，适应 beam search 中的多条路径
                ...
                target_attn = scaled_attention(q_cur, k_enc, v_enc,
                                            ...
                                            dropout_rate=self.dropout_rate,
                                            training=training)
                target_attn = tf.layers.dense(target_attn, self.dim, name="w_o",trainable=False)
