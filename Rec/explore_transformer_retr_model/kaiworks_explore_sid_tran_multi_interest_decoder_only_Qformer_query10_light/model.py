...
class MultiInterestModel(object):
    ...
    def beam_search_fast(self, beam_size=512, temperature=1):
        ...
        * step>0 时保持固定 beam_size。
        ...
        # === 6. 粗粒度解码器 ===
        # 使用4层Transformer解码器生成序列表示
        coarse_interest_model = QFormer(num_layers=10, dim=self._dim, num_heads=8, dropout_rate=0.1, hidden_dim=self._dim*2)
        ...
        for i in range(self._query_token_numb):
            ...
            for step, V in enumerate(self._vocab_sizes):
                ...
                with tf.variable_scope(scope_name):
                    fine_item_model = DecoderModel(num_layers=2, dim=self._dim, num_heads=8, dropout_rate=0.1, hidden_dim=self._dim*2)
