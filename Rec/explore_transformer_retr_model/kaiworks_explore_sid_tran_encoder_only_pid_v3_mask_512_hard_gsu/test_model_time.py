...
def build_dummy_inputs(batch_size=4, max_len=200, dim=512,
    ...
    ph["user_sid"] = tf.placeholder(tf.int64, [None, max_len], name="ph_user_sid")
    # 点击有效长度
    ...
    fd = {
        ...
        ph["user_sid"]: rng.randint(1, 500, size=(B, max_len)).astype("int64"),
...
def main():
    ...
    with g.as_default():
        ...
        # 5) Beam Search（两个版本）
        gen_loc_greedy, probs_greedy = model.beam_search_fast(ph["user_sid"], beam_size=512, temperature=1.0)
