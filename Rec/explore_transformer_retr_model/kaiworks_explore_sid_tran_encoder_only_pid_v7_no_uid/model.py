...
class MultiInterestModel(object):
    ...
    def __init__(self, feature_emb_dict, feature_emb_size_dict, dim=256, selected_colossus_size=64, selected_profile_size=200, vocab_sizes=[8192, 8192, 8192], print_ops=None):
        """
        初始化多兴趣模型
        ...
        self._dim = dim
        self._selected_colossus_size = selected_colossus_size
        self._selected_profile_size = selected_profile_size
    ...
    def model(self, user_colossus_play_time_list, user_colossus_duration_list, user_colossus_channel_list, photo_sid, label, photo_semantic_id_int):
        ...
        # === 2. 用户点击行为特征处理 ===
        # 2.1 拼接原始点击特征
        user_colossus_fea = tf.concat([self._feature_emb_dict[fea] for fea in user_colossus_fea_names], axis=2)   # [B, 1000, dim*2]

        B = tf.shape(user_colossus_fea)[0]
        ...
        colossus_max_len   = self._selected_colossus_size               # 例如 64
        colossus_raw_len   = tf.cast(self._feature_emb_size_dict['user_colossus_pid_list'], tf.int32)  # [B] 或 [B,1]
        colossus_valid_len = tf.reshape(colossus_raw_len, [-1])                 # [B]
        ...
        # play_time / duration 已 ×1000 → int32(ms)
        user_colossus_fea, colossus_longview_len, colossus_used_len = filter_long_and_crop_vec(
            seq_emb   = user_colossus_fea,                 # [B, 1000, fea_dim]
            play_time = play_time_seq,                  # [B, 1000]
            duration  = duration_seq,                   # [B, 1000]
            channel   = channel_seq,                    # [B, 1000]
            valid_len = colossus_valid_len,                      # [B]
            max_len   = colossus_max_len,
            pad_value = 0.0,
            name="user_click_recent_long_vec"
        )
        
        # 通过MLP处理点击特征
        user_colossus_emb = mlp('user_click_emb', user_colossus_fea, [4*self._dim], self._dim, activation=tf.nn.leaky_relu) # [B, 1000, dim]

        with tf.variable_scope('sample_len'):
            print_tensor("colossus_valid_len", colossus_valid_len)
            print_tensor("colossus_longview_len", colossus_longview_len)   # 每个 batch 样本长播原始条数
            print_tensor("colossus_used_len", colossus_used_len)           # 截断后实际使用条数
        
        # ================ 用户的profile v1 ===================
        user_profile_fea = tf.concat([self._feature_emb_dict[fea] for fea in user_profile_fea_names], axis=2)   # [B, L, dim]
        
        # 2.2 仅保留“有效序列的最后 selected_size 条”
        profile_max_len  = self._selected_profile_size                                             
        ...
        user_realtime_fea = tf.concat([self._feature_emb_dict[fea] for fea in user_realtime_fea_names], axis=2)
        
        # 2) 序列长度（≤ 20）
        realtime_max_len = 20
        realtime_raw_len = tf.cast(self._feature_emb_size_dict['user_profile_v1_explore_click_pid_list'], tf.int32)
        realtime_valid_len = tf.reshape(realtime_raw_len, [-1])      # 强制展平成 [B]
        realtime_used_len = tf.minimum(realtime_raw_len, realtime_max_len)       # [B]
        
        user_realtime_emb = mlp('user_click_emb', user_realtime_fea, [4*self._dim], self._dim, activation=tf.nn.leaky_relu)  # [B,20,dim]
        user_realtime_emb = tf.reverse(user_realtime_emb, axis=[1])  # reverse 使得 最旧→最新
        
        with tf.variable_scope('sample_len'):
            print_tensor("realtime_valid_len", realtime_valid_len)
            print_tensor("realtime_used_len", realtime_used_len)
            print_tensor("all_used_len", colossus_used_len + profile_used_len + realtime_used_len)
            
        # === 3. 构建编码器输入 ===
        encoder_input = tf.concat([user_colossus_emb, user_profile_emb, user_realtime_emb], axis=1)
        ...
        # === 3-A. padding mask 更新 ===============================
        # 总序列长度
        total_len = colossus_max_len + profile_max_len + realtime_max_len

        # 历史（长播）mask：右对齐
        colossus_mask = tf.sequence_mask(colossus_used_len, maxlen=colossus_max_len, dtype=tf.int8)     # [B,max_len] 左 1 右 0
        colossus_mask = tf.reverse(colossus_mask, axis=[1])                              # 右对齐
        ...
        # 实时 mask：右对齐
        realtime_mask = tf.sequence_mask(realtime_used_len, maxlen=realtime_max_len, dtype=tf.int8)
        realtime_mask = tf.reverse(realtime_mask, axis=[1])

        # 拼整条 mask
        seq_mask = tf.concat([colossus_mask, profile_mask, realtime_mask], axis=1)  # [B,total_len]
    ...
    def beam_search_fast(self, user_colossus_play_time_list, user_colossus_duration_list, user_colossus_channel_list, beam_size=512, temperature=1):
        ...
        * step>0 时保持固定 beam_size。
        ...
        # play‑time / duration 原始张量 [B, 1000]，与 click‑emb 对齐
        play_time_seq = user_colossus_play_time_list*1000      # int32, ms
        duration_seq  = user_colossus_duration_list*1000       # int32, ms
        channel_seq = user_colossus_channel_list
        ...
        def filter_long_and_crop_vec(seq_emb,
            ...
            ----
            seq_emb   : [B, L, D]  float32/16   点击序列 embedding
            play_time : [B, L]     int32        播放时长 (ms)
            duration  : [B, L]     int32        视频总时长 (ms)
            channel   : [B, L]     int32        频道 id
            valid_len : [B]        int32        每条序列真实长度 (≤ L)
            max_len   : int                       只保留最近 max_len 条
            pad_value : float                     左侧 padding 常量
