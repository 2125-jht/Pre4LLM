...
class MultiInterestModel(object):
    ...
    def model(self, photo_sid, label, photo_semantic_id_int):
        ...
        fine_item_input = tf.nn.embedding_lookup(self._sid_embedding, photo_with_start_token)


        fine_item_model = DecoderModel(num_layers=6, dim=self._stage_two_dim, num_heads=8, dropout_rate=0.1, hidden_dim=self._stage_two_dim*2, training=True)
        fine_item_mask = tf.ones([B, 1, 1, self._query_token_numb], dtype=tf.float32)
        decoder_output = fine_item_model.forward(fine_item_input, interest_embed, fine_item_mask, training=True)  # [b, code_size, dim]
