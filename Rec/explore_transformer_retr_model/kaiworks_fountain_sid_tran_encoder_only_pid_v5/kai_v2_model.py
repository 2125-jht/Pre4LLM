...
if args.with_kai_v2:
    ...
    def filter_mask_wrapper(dataset):
        ...
        def mask_fn(batch):
            # ===== 1. 原有标签 =====
            label_like          = tf.cast(batch['context_info__like'],          tf.int32)
            label_follow        = tf.cast(batch['context_info__follow'],        tf.int32)
            label_comment       = tf.cast(batch['context_info__comment'],       tf.int32)
            label_collect       = tf.cast(batch['context_info__collect'],       tf.int32)
            label_download      = tf.cast(batch['context_info__download'],      tf.int32)
            label_profile_enter = tf.cast(batch['context_info__profile_enter'], tf.int32)

            playing_time = tf.cast(batch['context_info__playing_time'], tf.int32)   # ms
            duration_ms  = tf.cast(batch['photo_info__duration_ms'],   tf.int32)   # ms
            ...
            # bin_id     = tf.raw_ops.Bucketize(input=duration_ms, boundaries=boundaries)          # [B] 
            # base_thr   = tf.gather(thr_table, bin_id)                                            # [B] 
            # ===== 3. 计算 “长播” 阈值并生成 label_long =====
            thr_7s  = tf.constant(7000,  dtype=duration_ms.dtype)   # 7 s
            thr_60s = tf.constant(60000, dtype=duration_ms.dtype)   # 60 s

            # 3.1 计算每条样本对应的阈值 base_thr
            base_thr = tf.where(
                duration_ms < thr_7s,                              # dur < 7 s
                tf.fill(tf.shape(duration_ms), thr_7s),            # → 阈值固定 7 s
                tf.where(
                    duration_ms < thr_60s,                         # 7 s ≤ dur < 60 s
                    duration_ms,                                   # → 阈值 = dur
                    tf.fill(tf.shape(duration_ms), thr_60s)        # dur ≥ 60 s → 阈值固定 60 s
                )
            )

            # —— 3.3 是否满足长播
            label_long = tf.where(tf.greater_equal(playing_time, base_thr), tf.ones_like(playing_time), tf.zeros_like(playing_time))       # [B] float32
            ...
            # debug
            print_ops.append(tf.print("label_like", label_like, summarize=100))
            print_ops.append(tf.print("playing_time", playing_time, summarize=100))
            print_ops.append(tf.print("duration_ms", duration_ms, summarize=100))
            print_ops.append(tf.print("label_long", label_long, summarize=100))
            print_ops.append(tf.print("interact_cnt", interact_cnt, summarize=100))
            print_ops.append(tf.print("mask", mask, summarize=100))
...
if is_training:
    ...
    # === 获取User相关特征 ===
    user_colossus_play_time_list = config.get_dense_fea("colossus_play_time_list", dim=1000, dtype=tf.int64) # [B, 1000]
    user_colossus_duration_list = config.get_dense_fea("colossus_duration_list", dim=1000, dtype=tf.int64) # [B, 1000]
    user_colossus_channel_list = config.get_dense_fea("colossus_channel_list", dim=1000, dtype=tf.int64) # [B, 1000]
    ...
    # === 模型前向传播 ===
    print("=== 模型前向传播 ===")
    loss, result_dict = model.model(user_colossus_play_time_list, user_colossus_duration_list, user_colossus_channel_list,
                                    photo_semantic_id, label, photo_semantic_id_int)  # 计算训练损失
    ...
    kai.add_run_hook(DumpTensorHook('dump_tensors', {
        ...
        'pid': pid,
        'photo_sid': label,
        'sid0_indices': result_dict['sid0_indices'],
        'sid0_probs': result_dict['sid0_probs'],
        'sid1_indices': result_dict['sid1_indices'],
        'sid1_probs': result_dict['sid1_probs'],
        'sid2_indices': result_dict['sid2_indices'],
        'sid2_probs': result_dict['sid2_probs'],
        'truth0_probs': result_dict['truth0_probs'],
        'truth1_probs': result_dict['truth1_probs'],
        'truth2_probs': result_dict['truth2_probs'],
    }), 'custom_dump_tensor_hook')

    
    # === 控制依赖和目标设置 ===
    with tf.control_dependencies(model.get_print_ops()):
        targets = []
    ...
    user_sid, user_sid_prob = model.beam_search_fast(user_colossus_play_time_list, user_colossus_duration_list, user_colossus_channel_list, beam_size=512, temperature=1)
