...
# =======================
def rms_norm(x, scope, eps=1e-6, with_bias=True):
    """
    RMSNorm: 只按均方根归一化，不减均值
    x: [..., D]
    """
    with tf.variable_scope(f"{scope}/rms_norm", reuse=tf.AUTO_REUSE):
        dim = x.get_shape().as_list()[-1]
        gamma = tf.get_variable("gamma", [dim], initializer=tf.ones_initializer())
        if with_bias:
            beta = tf.get_variable("beta", [dim], initializer=tf.zeros_initializer())

        rms = tf.sqrt(tf.reduce_mean(tf.square(x), axis=[-1], keepdims=True) + eps)
        y = x / rms
        y = y * gamma
        if with_bias:
            y = y + beta
...
def apply_mask_add(logits, src_mask, neg_large=-1e9):
    """
    logits: [B,H,Lq,Lk]
    src_mask: [B,1,1,Lk] 或已广播到 [B,H,Lq,Lk] (0=pad, 1=valid; any dtype)
    """
...
def multi_head_attention(queries, keys, values, num_heads, src_mask, dropout_rate, training=False):
    ...
    def scaled_dot_product_attention(Q, K, V, src_mask):
        ...
        src_mask_b = tf.broadcast_to(src_mask, [B, H, T_q, T_k])
        scaled_attention_logits = apply_mask_add(scaled_attention_logits, src_mask_b)
...
def scaled_attention(q,            # [B*beam, H, T_q, Dh]  (单步)
    ...
    # ---------- 2. 掩码 ----------
    if causal:
        pass  # 当前 q 在末尾，可省略；保留以示可扩展

    if src_mask is not None:       # Padding mask / Cross-attn mask
        src_mask_rep = tf.repeat(src_mask, cur_beam, axis=0) # [B*beam, 1, 1, Tk]
        src_mask_rep = tf.broadcast_to(src_mask_rep, [B*cur_beam, H, T_q, T_k])
        attn_scores = apply_mask_add(attn_scores, src_mask_rep)
    ...
    _, _, H_, Dh = context.shape.as_list()
    context = tf.reshape(context, [tf.shape(q)[0], T_q, H_*Dh]) # [B*beam,Tq,D_model]
...
class DecoderLayer:
    ...
    def forward(self, x, enc_output, src_mask, training=False):
        with tf.variable_scope(f"{self.name}", reuse=tf.AUTO_REUSE):
            ...
            x_norm = rms_norm(x, scope="self_atten_ln")
            attn_output = self.self_attention(x_norm, x_norm, x_norm,
                                              self.num_heads, self.dropout_rate, training=training)
            y = x + attn_output

            # Cross-Attention (Pre-Norm)
            y_norm = rms_norm(y, scope="cross_atten_ln")
            cross_attn_output = self.cross_attention(y_norm, enc_output, enc_output,
                                                     self.num_heads, src_mask, self.dropout_rate, training=training)
            z = y + cross_attn_output

            # FFN (Pre-Norm)
            z_norm = rms_norm(z, scope="ffn_ln")
            ffn_output = self.ffn(z_norm, training=training)
            out = z + ffn_output
    ...
    def step(self, x_t, cur_beam,         # [B,1,D] 当步输入
        ...
        with tf.variable_scope(f"{self.name}", reuse=tf.AUTO_REUSE):
            # -------- Self-Attention (Pre-Norm) --------
            x_t_norm = rms_norm(x_t, scope="self_atten_ln")

            with tf.variable_scope("multi_head_attention_causality", reuse=tf.AUTO_REUSE):
                depth = x_t.shape[-1]
                q_t = tf.layers.dense(x_t_norm, depth, use_bias=False, name="w_q")
                k_t = tf.layers.dense(x_t_norm, depth, use_bias=False, name="w_k")
                v_t = tf.layers.dense(x_t_norm, depth, use_bias=False, name="w_v")
            ...
            with tf.variable_scope("multi_head_attention", reuse=tf.AUTO_REUSE):
                q_cur = tf.layers.dense(y_norm, self.dim, use_bias=False, name="w_q")
                ...
                k_enc = tf.broadcast_to(k_enc, [B0, cur_beam, H0, L_enc, self.head_dim])
                v_enc = tf.broadcast_to(v_enc, [B0, cur_beam, H0, L_enc, self.head_dim])
                k_enc = tf.reshape(k_enc, [B0*cur_beam, H0, L_enc, self.head_dim])
                v_enc = tf.reshape(v_enc, [B0*cur_beam, H0, L_enc, self.head_dim])
            ...
            # -------- FFN (Pre-Norm) --------
            z_norm = rms_norm(z, scope="ffn_ln")
            ffn_out = self.ffn(z_norm, training=False)   # 保持与你原代码一致
            y_t     = z + ffn_out
