...
class MultiInterestModel(object):
    ...
    def _multi_interest_encode_v2(self, photo_sid, label, photo_semantic_id_int):
        ...
        colossus_pid = self._feature_emb_dict["truncate_user_colossus_pid_list"]
        colossus_aid = self._feature_emb_dict["truncate_user_colossus_aid_list"]
        colossus_channel = self._feature_emb_dict["truncate_user_colossus_channel_list"]

        truncate_colossus_play_time = tf.cast(self._truncate_dense_feature["truncate_colossus_play_time_list"], tf.int32)
        truncate_colossus_duration = tf.cast(self._truncate_dense_feature["truncate_colossus_duration_list"], tf.int32)
        truncate_colossus_label = tf.cast(self._truncate_dense_feature["truncate_colossus_label_list"], tf.int32)
        completion = tf.cast(tf.cast(truncate_colossus_play_time, tf.float32) / (tf.cast(truncate_colossus_duration, tf.float32) + 1e-5) * 10, tf.int32)

        # [batch_size, history_size, embedding_dim]
        interact_feature = label_encoding(truncate_colossus_label)
        completion_feature = play_time_encoding(truncate_colossus_play_time, truncate_colossus_duration)

        # score_feature = self._pooling([colossus_channel, interact_feature, completion_feature], self._history_size, "photo_score_feature")
        score_feature_list = [colossus_channel, interact_feature, completion_feature]
        # [batch_size, history_size, 1]
        score_unsqueezed = self._single_score_module(score_feature_list)
        score = tf.squeeze(score_unsqueezed, axis=2)
        print_tensor("score", score)

        # [batch_size, history_size, embedding_dim]
        colossus_photo_feature = self._pooling([colossus_pid, colossus_aid], self._history_size, "photo_feature")

        # [batch_size, selected_history_size]
        indices = gumbel_top_k(score, self._selected_size, 1.0)
        print_tensor("indices", indices)
        # 为了让打分有梯度，我们把colossus_photo_feature 与score 做一个乘积
        colossus_photo_feature = score_unsqueezed * colossus_photo_feature
        # [batch_size, selected_size, embedding_dim]
        # selected_photo_feature = colossus_photo_feature
        selected_photo_feature = self._select_feature(colossus_photo_feature, indices)

        encoder_output = self._transformer_with_special_token(selected_photo_feature, photo_embedding, user_embedding)
        ...
        decoder_output = transformer_decoder(decoder_embedding=decoder_input,
                                            seq_input_embeddings=encoder_output,
