...
class MultiInterestModel(object):
    ...
    def __init__(self, feature_emb_dict, feature_emb_size_dict, dim=256, vocab_sizes=[8192, 8192, 8192], print_ops=None):
        ...
        self._query_token_numb=16
        self._stage_one_dim=dim
        self._stage_two_dim=dim
        
        # 创建统一的嵌入矩阵，包含所有语义ID的嵌入向量
        # 使用均匀分布初始化，范围为[-1/dim, 1/dim]
        self._query_embedding = tf.get_variable(
            shape=[self._query_token_numb, self._stage_one_dim], 
            name='query_embedding',
            initializer=tf.random_uniform_initializer(minval=-1.0/self._stage_one_dim, maxval=1.0/self._stage_one_dim), 
            trainable=True
        )
        self._sid_embedding = tf.get_variable(
            shape=[self._total_vocab_size+1, dim], 
            name='sid_embedding',
            initializer=tf.random_uniform_initializer(minval=-1.0/self._stage_two_dim, maxval=1.0/self._stage_two_dim), 
            trainable=True
        )
    ...
    def model(self, photo_sid, label, photo_semantic_id_int):
        ...
        # === 3. 构建query token ===
        query_token_indice_1d = tf.range(start=0, limit=self._query_token_numb, delta=1, dtype=tf.int32)  # [q]
        ...
        query_token_emb = tf.nn.embedding_lookup(self._query_embedding, query_token_indice)  # [b,q,dim]
        ...
        input_sid_embed = tf.nn.embedding_lookup(self._sid_embedding, photo_with_start_token) #[b,len(vocab_sizes),_stage_two_dim]
