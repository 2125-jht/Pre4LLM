...
class MultiInterestModel(object):
    ...
    def model(self, photo_sid, label, photo_semantic_id_int, label_click):
        """
        主训练模型前向传播
        ...
        Args:
            ...
            label_click: label是否被点击了，点击为1，int64
        Returns:
            loss: 训练损失值
        """
        ...
        decoder_output = decoder_model.forward(decoder_input[:,:-1,:], encoder_output, src_mask, training=True) # [batch_size, seq_len, dim]
        ...
        # 根据点击标记区分正/负样本（点击=正样本=1）
        pos_mask = tf.cast(tf.reshape(label_click, [-1]) > 0, tf.float32)  # [B]
        neg_mask = 1.0 - pos_mask

        # 统计并打印 pos/neg 的数量（仅统计有效样本）
        valid_cnt = tf.reduce_sum(tf.cast(loss_mask, tf.int32))
        pos_cnt = tf.reduce_sum(tf.cast(pos_mask * loss_mask, tf.int32))
        neg_cnt = tf.reduce_sum(tf.cast(neg_mask * loss_mask, tf.int32))
        with tf.variable_scope('label_count'):
            print_tensor("count/valid_total", valid_cnt)
            print_tensor("count/poslabel", pos_cnt)
            print_tensor("count/neglabel", neg_cnt)
        
        # 设置系数和eps
        alpha = self._hard_neg_alpha
        eps = 1e-9
        ...
        # 对每个语义层级分别计算损失
        pos_losses_only = []                                   # 累计正样本逐级 loss（不含 hard neg）
        pos_den = tf.reduce_sum(pos_mask * loss_mask) + eps    # 正样本分母（按有效样本计）
        neg_den = tf.reduce_sum(neg_mask * loss_mask) + eps    # 负样本分母（按有效样本计）
        ...
        for step in range(len(self._vocab_sizes)):
            with tf.variable_scope('proj_%d' % step):
                ...
                # 3. 打印
                print_tensor("probs/true_token_prob_%d" % step, tf.reduce_sum(true_p * loss_mask) / (tf.reduce_sum(loss_mask) + eps))
                result_dict['true%d_probs' % step] = true_p
                ...
                # 分别打印正样本 & 负样本的 true/max
                print_tensor("probs_pos/true_token_prob_%d" % step,
                             tf.reduce_sum(true_p * pos_mask * loss_mask) / pos_den)
                print_tensor("probs_neg/true_token_prob_%d" % step,
                             tf.reduce_sum(true_p * neg_mask * loss_mask) / neg_den)

                print_tensor("probs_pos/max_token_prob_%d" % step,
                             tf.reduce_sum(tf.squeeze(max_probs, -1) * pos_mask * loss_mask) / pos_den)
                print_tensor("probs_neg/max_token_prob_%d" % step,
                             tf.reduce_sum(tf.squeeze(max_probs, -1) * neg_mask * loss_mask) / neg_den)
                ...
                # pos_loss_i = tf.nn.softmax_cross_entropy_with_logits(
                #     labels=one_hot_labels, logits=pred_logit / temperature
                # )  # [B]
                pos_loss_i = -tf.math.log(tf.clip_by_value(true_p, eps, 1.0))
                
                # 打印与“历史未加负样本”对齐的逐级正样本 loss（只看正样本）
                print_tensor("loss_pos/loss_%d" % step, tf.reduce_sum(pos_loss_i * pos_mask * loss_mask) / pos_den)
                
                # 保存逐级正样本 loss，便于后面求总和
                pos_losses_only.append(pos_loss_i)

                # 负样本：仅第 0 级使用 -log(1-p)，其他级 0
                if step == 0:
                    neg_loss_i = -tf.math.log(tf.clip_by_value(1.0 - true_p, eps, 1.0))
                else:
                    neg_loss_i = tf.zeros_like(true_p)
                # neg_loss_i = -tf.math.log(tf.clip_by_value(1.0 - true_p, eps, 1.0))
                    
                print_tensor("loss_neg/loss_%d" % step, tf.reduce_sum(neg_loss_i * neg_mask * loss_mask) / neg_den)    
                
                # 混合正/负样本 loss
                loss_i = pos_mask * pos_loss_i + alpha * neg_mask * neg_loss_i  # [B]
                ...
                recall_at_k(pred_logit, label[:, step], loss_mask,           self._print_ops,
                            top_k=[1, 16, 128], name="predict_recall_%d" % step)
                recall_at_k(pred_logit, label[:, step], pos_mask * loss_mask, self._print_ops,
                            top_k=[1, 16, 128], name="predict_recall_pos_%d" % step)
                recall_at_k(pred_logit, label[:, step], neg_mask * loss_mask, self._print_ops,
                            top_k=[1, 16, 128], name="predict_recall_neg_%d" % step)
                
        print_tensor("loss_mask", loss_mask)
        
        # === 总体 loss 打印 ===
        # 与历史对齐的“正样本总 loss”（逐级相加，但只统计正样本）
        pos_total_per_example = tf.add_n(pos_losses_only)                     # [B]
        pos_total_mean = tf.reduce_sum(pos_total_per_example * pos_mask * loss_mask) / pos_den
        print_tensor("loss_pos/loss", pos_total_mean)                        # 历史对齐的总正样本 loss

        # 计算加权平均损失
        loss = tf.reduce_sum((losses[0] + losses[1] + losses[2]) * loss_mask) / (tf.reduce_sum(loss_mask) + eps)
