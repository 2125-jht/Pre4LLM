...
def print_tensor(name, tensor):
    ...
    # 始终记录 scalar（对非标量取均值，避免 tensorboard 爆节点）
    tf.summary.scalar(name, tf.reduce_mean(t))

    # 只有在“确实是向量/矩阵”时再打直方图；标量就跳过或 reshape 成长度1
    nd = t.shape.ndims
    if nd is not None:
        if nd > 0:
            tf.summary.histogram(name, t)
        else:
            # 可选：若你确实想看直方图，改用独立 tag，避免与 scalar 冲突
            tf.summary.histogram(name + "_hist", tf.reshape(t, [1]))
    else:
        # 动态未知 rank 的情况，安全起见 reshape 成 1D 再打到 _hist
        tf.summary.histogram(name + "_hist", tf.reshape(t, [-1]))
...
def recall_at_k(predict, label, loss_mask, print_ops, top_k=[5, 15], name="default"):
    ...
    for k in top_k:
        ...
        # 4. 计算加权召回率（只考虑有效样本）
        recall_at_k_value = tf.reduce_sum(tf.cast(correct_any, tf.float32) * loss_mask) / (tf.reduce_sum(loss_mask) + 1e-9)
