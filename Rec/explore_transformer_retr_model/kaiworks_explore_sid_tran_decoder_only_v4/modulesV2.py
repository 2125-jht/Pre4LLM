import tensorflow as tf
import numpy as np
from modules_ import *

def layer_norm(x, name, eps=1e-6):
    with tf.variable_scope(f"{name}", reuse=tf.AUTO_REUSE):
        gamma = tf.get_variable("gamma", [x.get_shape()[-1]], initializer=tf.ones_initializer())
        beta = tf.get_variable("beta", [x.get_shape()[-1]], initializer=tf.zeros_initializer())

        mean, variance = tf.nn.moments(x, axes=[-1], keep_dims=True)
        normalized = (x - mean) / tf.sqrt(variance + eps)
        output = gamma * normalized + beta
    return output
    
def apply_mask_add(logits, atten_mask, neg_large=-1e6):
    """
    logits: [B,H,Lq,Lk]
    atten_mask: [B,H,Lq,Lk]  (0=pad, 1=valid; any dtype)
    """
    atten_mask_f = tf.cast(atten_mask, logits.dtype)
    neg = tf.constant(neg_large, dtype=logits.dtype)
    return logits + (1. - atten_mask_f) * neg

# def masked_softmax(logits, mask, axis=-1, eps=1e-6):
#     """logits: [..., T_k]; mask: 同形或可广播到 logits，取 0/1。
#        返回在 masked 区域内归一化的概率；若该行全 0，则整行返回 0。"""
#     logits = logits - tf.reduce_max(logits, axis=axis, keepdims=True)  # 数值稳定
#     mask_f = tf.cast(mask, logits.dtype)
#     exp = tf.exp(logits) * mask_f
#     denom = tf.reduce_sum(exp, axis=axis, keepdims=True)
#     return exp / (denom + eps)  # 若 denom=0 → 全 0
# def masked_softmax(logits, mask, axis=-1, eps=1e-6):
#     """
#     logits: [..., T_k]
#     mask  : 同形或可广播到 logits，0/1
#     返回：在 mask 内归一化；若某一行 mask 全 0，则整行输出 0，并阻断该行梯度。
#     """
#     mask = tf.cast(mask, logits.dtype)
#     # 数值稳定：先做减 max
#     logits = logits - tf.reduce_max(logits, axis=axis, keepdims=True)
#     # 只对 mask=1 的位置取 exp
#     exp = tf.exp(logits) * mask
#     denom = tf.reduce_sum(exp, axis=axis, keepdims=True)  # 可能为 0
#     probs = exp / (denom + eps)
#     # 行是否有有效 key（至少一个 1）
#     has_any = tf.cast(tf.reduce_any(mask > 0, axis=axis, keepdims=True), logits.dtype)
#     # 对 “整行全 0” 的行：输出 0，且不向后传梯度
#     probs = tf.stop_gradient((1.0 - has_any) * probs) + has_any * probs
#     # 最后再保一手：把非有限数清零
#     probs = tf.where(tf.math.is_finite(probs), probs, tf.zeros_like(probs))
#     return probs
def masked_softmax(logits, mask, axis=-1, eps=1e-6):
    scaled_attention_logits = apply_mask_add(logits, mask)
    return tf.nn.softmax(scaled_attention_logits, axis=-1)

def multi_head_attention(queries, keys, values, num_heads, atten_mask, dropout_rate, training=False):
    
    def split_heads(x, num_heads):
        batch_size = tf.shape(x)[0]
        depth = x.get_shape().as_list()[-1] // num_heads
        reshaped = tf.reshape(x, [batch_size, -1, num_heads, depth])
        return tf.transpose(reshaped, [0, 2, 1, 3])
    
    training = tf.constant(training, dtype=tf.bool)
    
    def scaled_dot_product_attention(Q, K, V, atten_mask):
        matmul_qk = tf.matmul(Q, K, transpose_b=True)
        dk = tf.cast(tf.shape(K)[-1], tf.float32)
        scaled_attention_logits = matmul_qk / tf.math.sqrt(dk)
        
        B = tf.shape(scaled_attention_logits)[0]
        H = tf.shape(scaled_attention_logits)[1]
        T_q = tf.shape(scaled_attention_logits)[-2]
        T_k = tf.shape(scaled_attention_logits)[-1]
        
        # 扩展到 [B, H, T_q, T_k] 用 [1,1,T_q,T_k] 让广播去做
        atten_mask = tf.broadcast_to(atten_mask, [B, H, T_q, T_k])
        # scaled_attention_logits = apply_mask_add(scaled_attention_logits, atten_mask)
        # attention_weights = tf.nn.softmax(scaled_attention_logits, axis=-1)
        attention_weights = masked_softmax(scaled_attention_logits, atten_mask, axis=-1)        
        attention_weights = tf.layers.dropout(attention_weights, rate=dropout_rate,
                                training=training, name="attn_dropout")
        
        output = tf.matmul(attention_weights, V)
        return output, attention_weights
    
    with tf.variable_scope("multi_head_attention", reuse=tf.AUTO_REUSE):
        depth = queries.get_shape().as_list()[-1]
        Q = tf.layers.dense(queries, depth, use_bias=False, name="w_q")
        K = tf.layers.dense(keys,    depth, use_bias=False, name="w_k")
        V = tf.layers.dense(values,  depth, use_bias=False, name="w_v")

        Q = split_heads(Q, num_heads)
        K = split_heads(K, num_heads)
        V = split_heads(V, num_heads)

        scaled_attention, attention_weights = scaled_dot_product_attention(Q, K, V, atten_mask)
        scaled_attention = tf.transpose(scaled_attention, [0, 2, 1, 3])

        concat_attention = tf.reshape(scaled_attention, [tf.shape(queries)[0], -1, depth])
        output = tf.layers.dense(concat_attention, depth, name="w_o")

    return output
    
    
def feed_forward_network(dim, hidden_dim, dropout_rate, training=False):
    def ffn(x, training=training):
        training = tf.constant(training, dtype=tf.bool)
        with tf.variable_scope(f"feed_forward_network", reuse=tf.AUTO_REUSE):
            x = tf.layers.dense(x, hidden_dim, activation=tf.nn.relu, name="w_up")
            x = tf.layers.dropout(x, rate=dropout_rate,
                                    training=training, name="ffn_dropout")
            x = tf.layers.dense(x, dim, name="w_down")
            # x = tf.nn.dropout(x, rate=dropout_rate)
            # x = tf.cond(training, lambda: tf.nn.dropout(x, rate=dropout_rate), lambda: x)
            return x
    return ffn

def scaled_attention(q,            # [B*beam, H, T_q, Dh]  (单步)
                     k,            # [B*beam, H, T_k, Dh] (已缓存)
                     v,            # [B*beam, H, T_k, Dh]
                     cur_beam,
                     causal=False,
                     atten_mask=None,        # [B, 1, 1, T_k]
                     dropout_rate=0.0,
                     training=False):
    """
    单步 / 小批量通用的缩放点积注意力
    --------------------------------------------------------
        q : 当前 step 的 query (B,H,1,Dh)
        k : 历史 K cache        (B,H,Tk,Dh)
        v : 历史 V cache        (B,H,Tk,Dh)
    返回：
        context : (B,1,D_model) 已经合并 H 头
    """
    training = tf.convert_to_tensor(training, dtype=tf.bool)
    
    B = tf.shape(q)[0] // cur_beam
    H = tf.shape(q)[1]
    T_q = tf.shape(q)[2]
    T_k = tf.shape(k)[2]

    # ---------- 1. 点积并缩放 ----------
    attn_scores = tf.matmul(q, k, transpose_b=True)        # [B*beam,H,Tq,Tk]
    dk = tf.cast(tf.shape(k)[-1], tf.float32)
    attn_scores = attn_scores / tf.sqrt(dk)                # 缩放

    # ---------- 2. 掩码 ----------
    if causal:                     # 自回归用下三角
        # Tk = tf.shape(k)[-2]；这里 q 只有长度1，所以只需屏蔽「未来」
        # 亦可省略，因为 q 在末尾
        pass                       # 保留以示可扩展

    if atten_mask is not None:       # Padding mask / Cross-attn mask

        atten_mask = tf.tile(tf.expand_dims(atten_mask, 1), [1, cur_beam, 1, 1, 1])
        atten_mask = tf.reshape(atten_mask, [B*cur_beam, 1, -1, T_k])
        
        atten_mask = tf.broadcast_to(atten_mask, [B*cur_beam, H, T_q, T_k])
        # attn_scores = apply_mask_add(attn_scores, atten_mask)

    # ---------- 3. softmax & dropout ----------
    # attn_weights = tf.nn.softmax(attn_scores, axis=-1)      # [B*beam,H,Tq,Tk]
    attn_weights = masked_softmax(attn_scores, atten_mask if atten_mask is not None else 1, axis=-1) # [B*beam,H,Tq,Tk]
    attn_weights = tf.layers.dropout(attn_weights,
                                        rate=dropout_rate,
                                        training=training)

    # ---------- 4. 加权求和 ----------
    context = tf.matmul(attn_weights, v)        # [B*beam,H,Tq,Dh]
    context = tf.transpose(context, [0, 2, 1, 3])   # → [B*beam,Tq,H,Dh]
    _, _, H, Dh = context.shape.as_list()
    context = tf.reshape(context, [tf.shape(q)[0], T_q, H*Dh]) # [B*beam,Tq,D_model]

    return context          # 返回给上层 (仍是 3D，后续可接线性层)

class DecoderOnlyLayer:
    def __init__(self, name, dim, num_heads, hidden_dim, dropout_rate, training=False):
        super(DecoderOnlyLayer, self).__init__()
        self.name = name
        self.dim = dim
        self.num_heads = num_heads
        self.head_dim = self.dim // self.num_heads
        self.hidden_dim = hidden_dim
        self.dropout_rate = dropout_rate
        self.mha = multi_head_attention
        self.ffn = feed_forward_network(dim, hidden_dim, dropout_rate)

    def forward(self, x, atten_mask, training):
        with tf.variable_scope(f"{self.name}", reuse=tf.AUTO_REUSE):
            attn_out = self.mha(x, x, x, self.num_heads, atten_mask, self.dropout_rate, training=training)
            out1 = layer_norm(x + attn_out, name="atten_ln")
            ffn_out = self.ffn(out1, training=training)
            out2 = layer_norm(out1 + ffn_out, name="ffn_ln")
        return out2
    
    def forward_with_cache(self, x_t, cur_beam,         # [B,L,D] 输入
             layer_id,
             atten_mask,
             cache, training=False):
        """
        cache: dict 存各层的 {k_self, v_self, ffn_out}
        """
        def split_heads(x, num_heads):
            batch_size = tf.shape(x)[0]
            T_x = tf.shape(x)[1]
            depth = x.get_shape().as_list()[-1] // num_heads
            reshaped = tf.reshape(x, [batch_size, T_x, num_heads, depth])
            return tf.transpose(reshaped, [0, 2, 1, 3])
                    
        with tf.variable_scope(f"{self.name}", reuse=tf.AUTO_REUSE):
            with tf.variable_scope("multi_head_attention", reuse=tf.AUTO_REUSE):
                # # ---------- ① Self-Attention ----------
                # ---------- ① q/k/v ----------
                depth = x_t.shape[-1]
                q_t = tf.layers.dense(x_t, depth, use_bias=False, name="w_q")
                k_t = tf.layers.dense(x_t, depth, use_bias=False, name="w_k")
                v_t = tf.layers.dense(x_t, depth, use_bias=False, name="w_v")

                q_t = split_heads(q_t, self.num_heads)   # [B*beam,H,L,Dh]
                k_t = split_heads(k_t, self.num_heads)
                v_t = split_heads(v_t, self.num_heads)

                # ---------- ② 取 / 初始化 Self-KV 缓存 ----------
                k_key = f"k_self_{layer_id}"
                v_key = f"v_self_{layer_id}"

                B_beam = tf.shape(q_t)[0]                # == B*beam
                B      = B_beam // cur_beam
                H      = self.num_heads
                L      = tf.shape(q_t)[-2]
                Dh     = self.head_dim                   # dim // H

                # reshape 到 5-D 以便 concat & gather：[B,beam,H,L,Dh]
                k_t_5d = tf.reshape(k_t, [B, cur_beam, H, L, Dh])
                v_t_5d = tf.reshape(v_t, [B, cur_beam, H, L, Dh])

                if k_key in cache:
                    k_prev = cache[k_key]                # [B, beam, H, T-1, Dh]
                    v_prev = cache[v_key]
                    k_cat  = tf.concat([k_prev, k_t_5d], axis=3)   # T += 1
                    v_cat  = tf.concat([v_prev, v_t_5d], axis=3)
                else:
                    k_cat, v_cat = k_t_5d, v_t_5d        # 第一次，历史为空

                cache[k_key] = k_cat                    # 形状始终 [B, beam, H, T, Dh]
                cache[v_key] = v_cat

                # ---------- ③ 缩放点积注意力 ----------
                # 展平成 [B*beam, H, T, Dh] 供 matmul 使用
                k_cat_4d = tf.reshape(k_cat, [B_beam, H, -1, Dh])
                v_cat_4d = tf.reshape(v_cat, [B_beam, H, -1, Dh])

                self_out = scaled_attention(
                    q_t, k_cat_4d, v_cat_4d,
                    cur_beam,atten_mask=atten_mask,
                    dropout_rate=self.dropout_rate,
                    training=training,      # causal Mask 已在 scaled_attention 内部做
                )                           # [B*beam, L, D]

                self_out = tf.layers.dense(self_out, depth, name="w_o")  # [B*beam,L,D]
            
            out = layer_norm(x_t + self_out, name="atten_ln")
            # ---------- ③ FFN ----------
            ffn_out = self.ffn(out, training=False)
            y_t     = layer_norm(out + ffn_out, name="ffn_ln")

        return y_t, cache
    
    
    def step(self, x_t, cur_beam,         # [B,1,D] 当步输入
             layer_id,
             atten_mask,
             cache, training=False):
        """
        cache: dict 存各层的 {k_self, v_self, ffn_out}
        """
        def split_heads(x, num_heads):
            batch_size = tf.shape(x)[0]
            T_x = tf.shape(x)[1]
            depth = x.get_shape().as_list()[-1] // num_heads
            reshaped = tf.reshape(x, [batch_size, T_x, num_heads, depth])
            return tf.transpose(reshaped, [0, 2, 1, 3])
                    
        with tf.variable_scope(f"{self.name}", reuse=tf.AUTO_REUSE):
            with tf.variable_scope("multi_head_attention", reuse=tf.AUTO_REUSE):
                # # ---------- ① Self-Attention ----------
                # ---------- ① 当前 token 的 q/k/v ----------
                depth = x_t.shape[-1]
                q_t = tf.layers.dense(x_t, depth, use_bias=False, name="w_q")
                k_t = tf.layers.dense(x_t, depth, use_bias=False, name="w_k")
                v_t = tf.layers.dense(x_t, depth, use_bias=False, name="w_v")

                q_t = split_heads(q_t, self.num_heads)   # [B*beam,H,1,Dh]
                k_t = split_heads(k_t, self.num_heads)
                v_t = split_heads(v_t, self.num_heads)

                # ---------- ② 取 / 初始化 Self-KV 缓存 ----------
                k_key = f"k_self_{layer_id}"
                v_key = f"v_self_{layer_id}"

                B_beam = tf.shape(q_t)[0]                # == B*beam
                B      = B_beam // cur_beam
                H      = self.num_heads
                Dh     = self.head_dim                   # dim // H

                # reshape 到 5-D 以便 concat & gather：[B,beam,H,1,Dh]
                k_t_5d = tf.reshape(k_t, [B, cur_beam, H, 1, Dh])
                v_t_5d = tf.reshape(v_t, [B, cur_beam, H, 1, Dh])

                if k_key in cache:
                    k_prev = cache[k_key]                # [B, beam, H, T-1, Dh]
                    v_prev = cache[v_key]
                    k_cat  = tf.concat([k_prev, k_t_5d], axis=3)   # T += 1
                    v_cat  = tf.concat([v_prev, v_t_5d], axis=3)
                else:
                    k_cat, v_cat = k_t_5d, v_t_5d        # 第一次，历史为空

                cache[k_key] = k_cat                    # 形状始终 [B, beam, H, T, Dh]
                cache[v_key] = v_cat

                # ---------- ③ 缩放点积注意力 ----------
                # 展平成 [B*beam, H, T, Dh] 供 matmul 使用
                k_cat_4d = tf.reshape(k_cat, [B_beam, H, -1, Dh])
                v_cat_4d = tf.reshape(v_cat, [B_beam, H, -1, Dh])

                self_out = scaled_attention(
                    q_t, k_cat_4d, v_cat_4d,
                    cur_beam,atten_mask=atten_mask,
                    dropout_rate=self.dropout_rate,
                    training=training,      # causal Mask 已在 scaled_attention 内部做
                )                           # [B*beam, 1, D]

                self_out = tf.layers.dense(self_out, depth, name="w_o")  # [B*beam,1,D]
            
            out = layer_norm(x_t + self_out, name="atten_ln")
            # ---------- ③ FFN ----------
            ffn_out = self.ffn(out, training=False)   # 仅算当前 token
            y_t     = layer_norm(out + ffn_out, name="ffn_ln")

        return y_t, cache


class DecoderOnlyModel:
    def __init__(self, num_layers, dim, num_heads, hidden_dim, dropout_rate, training=False):
        super(DecoderOnlyModel, self).__init__()
        self.num_layers = num_layers
        self.dim = dim
        self.layers = [DecoderOnlyLayer(f"decoder_only_layer_{i}", dim, num_heads, hidden_dim, dropout_rate, training=training) for i in range(num_layers)]
        
    def forward(self, decoder_embedding, atten_mask, training):
        for i in range(self.num_layers):
            decoder_embedding = self.layers[i].forward(decoder_embedding, atten_mask, training=training)
        return decoder_embedding
    
    def forward_with_cache(self, x_t, cur_beam, atten_mask, cache):
        for i, layer in enumerate(self.layers):
            x_t, cache = layer.forward_with_cache(x_t, cur_beam, i, atten_mask, cache)
        return x_t, cache
    
    def step(self, x_t, cur_beam, atten_mask, cache):
        for i, layer in enumerate(self.layers):
            x_t, cache = layer.step(x_t, cur_beam, i, atten_mask, cache)
        return x_t, cache
