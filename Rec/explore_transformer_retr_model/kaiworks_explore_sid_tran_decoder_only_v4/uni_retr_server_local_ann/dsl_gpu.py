...
# global param
predict_server_name = "grpc_exploreGRRetrDecoderOnlyV3"
model_queue_prefix="kaiworks_explore_sid_trans_yzc_decoder_only_v3"
...
class ModelRetrFlow(LeafFlow, MioApiMixin, OfflineApiMixin, UniPredictApiMixin, ExploreModelApiMixin, RetrievalApiMixin, GsuApiMixin, EmbeddingApiMixin):
    ...
    def prepare(self):
      return self.copy_user_meta_info(
        ...
        .pack_common_attr(
          input_common_attrs = ["colossus_label_list"],
          output_common_attr = "colossus_label_list_limit",
          limit_num = 1000,
        ) \
        .reverse_list_attr(
          common_attrs=[
        ...
        .cast_attr_type(
          attr_type_cast_configs=[
            {
              "to_type": "double",
              "from_common_attr": "colossus_photo_id_list_limit",
              "to_common_attr": "colossus_photo_id_list",
            },
            {
              "to_type": "double",
            ...
            {
              "to_type": "double",
              "from_common_attr": "colossus_label_list_limit",
        ...
        .fetch_remote_embedding(
          ...
          input_attr_name = "colossus_photo_id_list_limit",
        ...
        .enrich_attr_by_lua(
          import_common_attr = ["colossus_photo_semantic_id_list"],
          function_for_common = "calculate",
          # 将 calculate 函数的返回值依次存入 export_item_attr 指定的 3 个 item_attr 中
          export_common_attr = ["sid_num"],
          lua_script = """
            function calculate(seq, item_key, reason, score)
              local sid_num = 0
              for i = 1, #colossus_photo_semantic_id_list do
                if colossus_photo_semantic_id_list[i] > 0 then
                  sid_num = sid_num + 1
                end
              end
              return sid_num
        ...
        .log_debug_info(
          common_attrs = [
            "sid_num",
            "colossus_photo_semantic_id_list",
          ],
          for_debug_request_only = True,
          respect_sample_logging = False,
          log_tag = "mxj_input_sid"
        ) \
        .perflog_attr_value(
          check_point="sid_num",
          common_attrs=["sid_num"],
        ) \
