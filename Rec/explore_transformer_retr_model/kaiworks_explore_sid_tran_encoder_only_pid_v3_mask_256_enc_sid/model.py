# -*- coding: utf-8 -*-
"""
SID推荐模型 - 基于Transformer的编码器-解码器架构
用于用户兴趣建模和内容推荐
...
class SIDRecModel(object):
    ...
    def __init__(self, feature_emb_dict, feature_emb_size_dict, dim=256, select_size=200, vocab_sizes=[8192, 8192, 8192], print_ops=None):
        """
        初始化模型
        ...
        self._enc_embedding = tf.get_variable(
            shape=[self._total_vocab_size, dim], 
            name='enc_embedding',
            initializer=tf.random_uniform_initializer(minval=-1.0/dim, maxval=1.0/dim), 
            trainable=True
        )
        
        self._dec_embedding = tf.get_variable(
    ...
    def model(self, click_semantic_id_list, photo_sid, label, photo_semantic_id_int):
        ...
        Args:
            click_semantic_id_list:用户点击的视频SID int列表 [batch_size, 200]
            photo_sid: 视频语义ID序列，shape=[batch_size, 3], 每级绝对index
            label: 真实标签，shape=[batch_size, 3]，每级相对index
        ...
        # debug
        self._print_ops.append(tf.print("click_semantic_id_list(first):",
                                        click_semantic_id_list[0], summarize=100))
        # === 1. 用户静态特征处理 ===
        # 拼接所有用户静态特征
        user_static_fea = tf.concat([self._feature_emb_dict[fea] for fea in user_static_fea_names], axis=1)
        # 通过MLP将静态特征映射到指定维度
        user_static_emb = mlp('user_static_emb', user_static_fea, [4*self._dim], self._dim, activation=tf.nn.leaky_relu)
        ...
        def filter_valid_sid_and_recent(seq_fea, sid_seq, valid_len, max_len,
                                        pad_value_fea=0.0, pad_value_sid=0,
                                        new_to_old=False,
                                        name="filter_valid_sid_and_recent"):
            """
            seq_fea   : [B, L, D]  float  原始点击特征（pid/aid等已concat）
            sid_seq   : [B, L]     int64  sid int 序列（0表示缺失/无效）
            valid_len : [B]        int32  每条序列真实长度 (≤ L)
            max_len   : int               只保留最近 max_len 条
            new_to_old: bool              若原始序列是 新→旧，请设 True

            Returns:
              out_fea : [B, max_len, D]  选中 + 左PAD(0) 后的点击特征（最旧→最新）
              out_sid : [B, max_len]     选中 + 左PAD(0) 后的 sid（最旧→最新）
              used_len: [B] int32        实际被选中的有效条数（≤ max_len）
              keep_topk_bool: [B, max_len] bool  有效位mask（左PAD为False）
            """
            with tf.name_scope(name):
                valid_len = tf.reshape(tf.cast(valid_len, tf.int32), [-1])     # [B]
                B = tf.shape(seq_fea)[0]
                L = tf.shape(seq_fea)[1]
                D = tf.shape(seq_fea)[2]

                rng = tf.range(L, dtype=tf.int32)[tf.newaxis, :]               # [1, L]
                valid_mask = tf.less(rng, valid_len[:, tf.newaxis])            # [B, L] bool
                sid_mask   = tf.greater(sid_seq, 0)                            # [B, L] bool
                keep_mask  = tf.logical_and(valid_mask, sid_mask)              # [B, L] bool

                # 以“越大越新”为打分：旧→新时用 rng；新→旧时用 (L-1-rng)
                recency_score = tf.where(new_to_old,
                                         tf.cast((tf.shape(rng)[1] - 1), tf.int32) - rng,
                                         rng)                                  # [1, L]
                recency_score = tf.tile(recency_score, [B, 1])                 # [B, L]

                # 无效位置赋 -1，便于 top_k；top_k 在 value 上排序并返回 index
                masked_score = tf.where(keep_mask, recency_score,
                                        tf.fill(tf.shape(recency_score), tf.constant(-1, tf.int32)))  # [B,L]

                # 取“最近”的 max_len 个位置（desc），再 reverse 成最旧→最新
                topk_vals, topk_pos = tf.nn.top_k(masked_score, k=max_len)     # [B, max_len]
                ordered_pos = tf.reverse(topk_pos, axis=[1])                   # [B, max_len] asc (旧→新)
                ordered_vals = tf.reverse(topk_vals, axis=[1])                 # [B, max_len]

                keep_topk_bool = tf.greater_equal(ordered_vals, 0)             # [B, max_len]
                used_len = tf.reduce_sum(tf.cast(keep_topk_bool, tf.int32), axis=1)  # [B]

                # batch gather
                batch_ids = tf.tile(tf.range(B, dtype=tf.int32)[:, tf.newaxis], [1, max_len])
                gather_nd_idx = tf.stack([batch_ids, ordered_pos], axis=-1)    # [B, max_len, 2]

                gathered_fea = tf.gather_nd(seq_fea, gather_nd_idx)            # [B, max_len, D]
                gathered_sid = tf.gather_nd(sid_seq, gather_nd_idx)            # [B, max_len]

                # 左侧 PAD
                fea_pad = _broadcast_const(gathered_fea, pad_value_fea, gathered_fea.dtype)
                keep_topk = tf.cast(keep_topk_bool, gathered_fea.dtype)        # float
                out_fea = keep_topk[:, :, tf.newaxis] * gathered_fea + (1.0 - keep_topk[:, :, tf.newaxis]) * fea_pad

                out_sid = tf.where(keep_topk_bool,
                                   gathered_sid,
                                   tf.fill(tf.shape(gathered_sid), tf.cast(pad_value_sid, gathered_sid.dtype)))

                return out_fea, out_sid, used_len, keep_topk_bool

        # === 2.1 原始点击特征（pid/aid等） ===
        user_click_fea = tf.concat(
            [self._feature_emb_dict[fea] for fea in user_click_fea_names], axis=2)   # [B, L, D_click]
        B = tf.shape(user_click_fea)[0]

        # debug
        self._print_ops.append(tf.print("user_click_fea(first):",
                                        user_click_fea[0,:,1], summarize=100))

        # === 2.2 选出有效 sid & 最近 select_size 条（左PAD，最旧→最新） ===
        raw_len = tf.cast(self._feature_emb_size_dict['user_profile_v1_click_pid_list'], tf.int32)  # [B]或[B,1]
        valid_len = tf.reshape(raw_len, [-1])  # [B]
        print_tensor("valid_len", valid_len)
        
        selected_click_fea, selected_sids, used_len, keep_mask = filter_valid_sid_and_recent(
            seq_fea   = user_click_fea,                # [B, L, D_click]
            sid_seq   = click_semantic_id_list,        # [B, L]  int
            valid_len = valid_len,                     # [B]
            max_len   = self._select_size,
            pad_value_fea = 0.0,
            pad_value_sid = 0,
            new_to_old=True,                          # 若原始是 新→旧，则为 True
            name="click_sid_recent_vec"
        )

        # debug
        self._print_ops.append(tf.print("selected_click_fea(first):",
                                        selected_click_fea[0,:,1], summarize=100))
        self._print_ops.append(tf.print("selected_sids(first):",
                                        selected_sids[0], summarize=100))

        print_tensor("used_len", used_len)
        
        # === 2.3 拆成多级 sid → emb → 汇聚 ===
        batch_size = tf.shape(selected_click_fea)[0]
        multi_level_sid = processInput(selected_sids, vocab_size=self._vocab_sizes)    # [B, S*3] int32
        
        # debug
        self._print_ops.append(tf.print("multi_level_sid(first):",
                                        multi_level_sid[0], summarize=100))
        
        multi_level_emb = tf.nn.embedding_lookup(self._enc_embedding, multi_level_sid)     # [B, S*3, D]
        multi_level_emb = tf.reshape(multi_level_emb, [batch_size, self._select_size, 3, self._dim])  # [B,S,3,D]

        sid_emb = tf.reduce_sum(multi_level_emb, axis=2)                                # [B, S, D] 汇聚3级
        # 对 padding 位置（sid==0）置零，避免噪声
        sid_valid_mask = tf.cast(tf.greater(selected_sids, 0), sid_emb.dtype)           # [B, S]
        sid_emb = sid_emb * sid_valid_mask[:, :, tf.newaxis]                            # [B, S, D]
        # === 2.4 融合 sid emb 与原点击特征，并过 MLP 得到最终 video emb ===
        fused_click_fea = tf.concat([selected_click_fea, sid_emb], axis=-1)             # [B, S, D_click + D]
        user_click_emb  = mlp('user_click_emb', fused_click_fea, [4*self._dim], self._dim, activation=tf.nn.leaky_relu)
        
        # === 3. 构建编码器输入：user token + 点击序列（旧→新） ===
        encoder_input = tf.concat([user_static_emb, user_click_emb], axis=1)   
        
        # 计算编码器输入的余弦相似度（用于调试）
        encoder_input_sim = tf.reshape(encoder_input, [B, -1])
        print_tensor("encoder_input_sim", calc_sim_cos(encoder_input_sim))
        
        # 右对齐（前0后1），与左PAD布局一致
        max_len = self._select_size
        B = tf.shape(used_len)[0]
        click_mask_left = tf.sequence_mask(lengths=used_len, maxlen=max_len, dtype=tf.int8)
        click_mask      = tf.reverse(click_mask_left, axis=[1])
        user_tok        = tf.ones([B, 1], dtype=tf.int8)
        seq_mask        = tf.concat([user_tok, click_mask], axis=1)     # [B, 1+S]
        src_mask        = tf.reshape(seq_mask, [B, 1, 1, 1 + max_len])  # [B,1,1,T]
        ...
        # 查找嵌入向量
        decoder_input = tf.nn.embedding_lookup(self._dec_embedding, photo_with_start_token)
    ...
    def beam_search_fast(self, click_semantic_id_list, beam_size=512, temperature=1):
        ...
        * step>0 时保持固定 beam_size。
        ...
        for step, V in enumerate(self._vocab_sizes):
            # 只 embed 当前 token
            dec_in = tf.nn.embedding_lookup(self._dec_embedding, seqs[:, :, -1])  # [B,beam,1,C]
