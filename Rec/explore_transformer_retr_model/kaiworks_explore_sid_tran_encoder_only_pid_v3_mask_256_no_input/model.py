...
class MultiInterestModel(object):
    ...
    def model(self, photo_sid, label, photo_semantic_id_int):
        ...
        # fake input
        encoder_input = tf.zeros_like(encoder_input)
        ...
        # fake mask
        seq_mask = tf.zeros_like(seq_mask) # 全 0
        ...
        # fake encoder_output
        encoder_output = tf.zeros_like(encoder_output) # 全 0
    ...
    def beam_search_fast(self, beam_size=512, temperature=1):
        ...
        * step>0 时保持固定 beam_size。
        ...
        # fake enc_out_base
        enc_out_base = tf.zeros_like(enc_out_base) # 全 0
