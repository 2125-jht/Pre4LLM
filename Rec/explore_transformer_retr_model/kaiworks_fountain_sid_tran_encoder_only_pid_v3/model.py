...
user_click_fea_names = [
    "user_fountain_profile_click_pid_list",  # 用户点击的视频ID列表
    "user_fountain_profile_click_aid_list"   # 用户点击的作者ID列表
]
...
class MultiInterestModel(object):
    ...
    def model(self, photo_sid, label, photo_semantic_id_int):
        ...
        Returns:
            loss: 训练损失值
        """
        print_tensor("user_click_list_length", self._feature_emb_size_dict['user_fountain_profile_click_pid_list'])
        # === 1. 用户静态特征处理 ===
        # 拼接所有用户静态特征
