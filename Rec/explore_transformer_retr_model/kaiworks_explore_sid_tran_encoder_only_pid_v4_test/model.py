...
class MultiInterestModel(object):
    ...
    def model(self, user_colossus_play_time_list, user_colossus_duration_list, user_colossus_channel_list, photo_sid, label, photo_semantic_id_int):
        ...
        play_time_seq = user_colossus_play_time_list*1000      # ms
        duration_seq  = user_colossus_duration_list*1000       # ms
        ...
        def filter_channel_whitelist_crop_vec(seq_emb,
                                            play_time, channel,
                                            valid_len,
                                            max_len,
                                            pad_value=0.0,
                                            name="filter_channel_whitelist_crop_vec"):
            """
            Args
            ----
            seq_emb   : [B, L, D]  点击序列 embedding
            play_time : [B, L] int64  播放时长 (ms)
            channel   : [B, L] int64  频道 id
            valid_len : [B]     int32  实际有效长度
            max_len   : int            仅保留最近 max_len 条
            pad_value : float          左 PAD 常量

            Returns
            -------
            out_emb  : [B, max_len, D]
            kept_len : [B] int32   过滤后条数 (未截断)
            used_len : [B] int32   min(kept_len, max_len)
            """
            with tf.name_scope(name):
                ...
                # ---------- ① 构造 keep_mask ----------
                channel_int = tf.cast(channel, tf.int64)
                whitelist = tf.constant([1, 2, 3, 5, 8, 122], dtype=tf.int64)
                # a) 白名单频道
                in_white   = tf.reduce_any(tf.equal(channel_int[..., tf.newaxis], whitelist), axis=-1)  # [B,L] bool
                # b) 频道 37 且播放 ≥ 7s
                is_37      = tf.equal(channel_int, 37)
                pt_ge_7s   = tf.greater_equal(play_time, tf.constant(7000, play_time.dtype))
                cond_37    = tf.logical_and(is_37, pt_ge_7s)                                    # [B,L]
                # keep = 白名单  OR  (精选且 ≥7s)
                keep_mask_raw = tf.logical_or(in_white, cond_37)                                # [B,L]

                # ---------- ② 有效长度掩码 ----------
                rng        = tf.range(L, dtype=tf.int32)[tf.newaxis, :]        # [1,L]
                valid_mask = tf.less(rng, valid_len[:, tf.newaxis])            # [B,L]
                keep_mask  = tf.logical_and(keep_mask_raw, valid_mask)         # [B,L]

                kept_len   = tf.reduce_sum(tf.cast(keep_mask, tf.int32), axis=1)  # [B]
                ...
                used_len  = tf.reduce_sum(tf.cast(keep_topk, tf.int32), axis=1)

                safe_idx  = tf.maximum(ordered_idx, 0)
                ...
                out_emb = keep_topk[:, :, tf.newaxis] * gathered + \
                        (1.0 - keep_topk[:, :, tf.newaxis]) * pad_tensor

                return out_emb, kept_len, used_len
                
        user_click_fea, kept_len, used_len = filter_channel_whitelist_crop_vec(
        ...
        # —— debug print —— #
        print_tensor("kept_len", kept_len)   # 每个 batch 样本长播原始条数
        print_tensor("used_len", used_len)           # 截断后实际使用条数
        
        # debug
        self._print_ops.append(tf.print("user_click_emb(first) after channel_based filter:",
                                        user_click_emb[0, :, 1], summarize=100))
        # === 3. 构建编码器输入 ===
        ...
        click_mask = tf.sequence_mask(used_len, maxlen=max_len, dtype=tf.int8) # [B, max_len]
