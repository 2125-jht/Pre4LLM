...
# global param
predict_server_name = "grpc_exploreGRRetrV3"
...
class ModelRetrFlow(LeafFlow, MioApiMixin, OfflineApiMixin, UniPredictApiMixin, ExploreModelApiMixin, RetrievalApiMixin, GsuApiMixin, EmbeddingApiMixin):
            ...
            self.uni_predict_fused(
              ...
              ## embedding 拉取相关配置，对应 fetch_mio_embedding 部分
               embedding_fetchers = [dict(
                ...
                colossusdb_embd_table_name="gr_trinity",
    ...
    def prepare(self):
      return self.copy_user_meta_info(
        ...
        .enrich_attr_by_lua(
          import_common_attr = ["user_profile_v1_click_pid_list"],
          ...
          export_common_attr = ["colossus_play_time_list", "colossus_label_list", "colossus_duration_list", "colossus_channel_list"],
          lua_script = """
            function calculate()
              local length = #user_profile_v1_click_pid_list
              local colossus_play_time_list = {}
              local colossus_label_list = {}
              local colossus_duration_list = {}
              local colossus_channel_list = {}
              for i = 1, length do
                table.insert(colossus_play_time_list, 100.0)
                table.insert(colossus_label_list, 0.0)
                table.insert(colossus_duration_list, 50.0)
                table.insert(colossus_channel_list, 1.0)
              end
              return colossus_play_time_list, colossus_label_list, colossus_duration_list, colossus_channel_list
            end
          """
        ) \
        .explore_extract_universal_feature(
          ...
          user_info_attrs = {
            ...
            "user_colossus_pid_list_attr": "user_profile_v1_click_pid_list",
            "user_colossus_aid_list_attr": "user_profile_v1_click_aid_list",
          },
          photo_info_attrs = {
          },
...
service.build(output_file=os.path.join(model_dir, "dynamic_json_config_gpu_fake.json"))
