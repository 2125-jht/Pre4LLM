...
class MultiInterestModel(object):
    ...
    def __init__(self, feature_emb_dict, feature_emb_size_dict, dim=256, selected_size=256, vocab_sizes=[8192, 8192, 8192], print_ops=None):
        """
        初始化多兴趣模型
    ...
    def model(self, user_colossus_play_time_list, user_colossus_duration_list, user_colossus_channel_list, photo_sid, label, photo_semantic_id_int):
        ...
        batch_size = tf.shape(user_click_fea)[0]
        ...
        encoder_input = tf.concat([user_click_emb, user_realtime_click_emb], axis=1)
        ...
        # 总序列长度
        total_len = max_len + realtime_max_len   # user token + 历史 + 实时
        ...
        # 拼整条 mask
        seq_mask = tf.concat([click_mask, realtime_mask], axis=1)  # [B,total_len]
