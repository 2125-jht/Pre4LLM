...
#新增统计8192个sid0的频次以及在召回情况，查看在高热sid0和低热sid0上的召回率
sid_cnt = tf.get_variable(shape=[8192], name="sid_cnt", initializer=tf.zeros_initializer, trainable=True)
sid_recall11 = tf.get_variable(shape=[8192], name="recall11_sid", initializer=tf.zeros_initializer, trainable=True)
sid_recall16 = tf.get_variable(shape=[8192], name="recall16_sid", initializer=tf.zeros_initializer, trainable=True)
...
if is_training:
    ...
    valid = tf.reshape(mask, [-1,1])
    flat_sid = tf.boolean_mask(tf.cast(photo_sid0, tf.int32), valid)#[K]

    batch_sid_cnt = tf.cast(
        tf.math.bincount(flat_sid, minlength=8192, maxlength=8192),
        tf.float32
    )
    ...
    # === 模型前向传播 ===
    print("=== 模型前向传播 ===")
    loss, result_dict, pred_prob_sid0 = model.model(photo_semantic_id, label, photo_semantic_id_int)  # 计算训练损失
    ...
    print("=======sid_cnt_debug========",sid_cnt.shape)
    print("=======sid_cnt_debug========",sid_recall11.shape)
    print("=======sid_cnt_debug========",sid_recall16.shape)
    
    import kai.tensorflow as kai
    kai.add_run_hook(DumpTensorHook('dump_tensors', {
        'sid_cnt': tf.reshape(sid_cnt, [1, -1]),
        'sid_recall11': tf.reshape(sid_recall11, [1, -1]) / tf.reshape(sid_cnt, [1, -1]),
        'sid_recall16': tf.reshape(sid_recall16, [1, -1]) / tf.reshape(sid_cnt, [1, -1])
    ...
    with tf.control_dependencies(print_ops):
        ...
        batch_recall_sums = batch_sid_recall_stats_from_logits(pred_prob_sid0,photo_sid0,top_k=(1, 10),vocab_size=8192)

        batch_recall11_sum = batch_recall_sums[0]
        batch_recall16_sum = batch_recall_sums[1]            
    ...
    if args.with_kai_v2:
        ...
        sid_cnt_optimizer = config.optimizer.AssignAddOptimizer(decay_rate=0.999, add_rate=0.001)
        sid_cnt_optimizer.minimize(loss, var_list=[sid_cnt], custom_gradient={sid_cnt.name: batch_sid_cnt})

        sid_recall11_optimizer = config.optimizer.AssignAddOptimizer(decay_rate=0.999, add_rate=0.001)
        sid_recall11_optimizer.minimize(loss, var_list=[sid_recall11], custom_gradient={sid_recall11.name: batch_recall11_sum})

        sid_recall16_optimizer = config.optimizer.AssignAddOptimizer(decay_rate=0.999, add_rate=0.001)
        sid_recall16_optimizer.minimize(loss, var_list=[sid_recall16], custom_gradient={sid_recall16.name: batch_recall16_sum})



        sid_cnt_dense_list=[sid_cnt,sid_recall11,sid_recall16]

        for dense_var in sid_cnt_dense_list:
            print("remove", dense_var, "because sid_cnt_dense_list")
            dense_var_list.remove(dense_var)
    ...
    user_sid, user_sid_prob, sid0_top = model.beam_search_fast(beam_size=512, temperature=1)
