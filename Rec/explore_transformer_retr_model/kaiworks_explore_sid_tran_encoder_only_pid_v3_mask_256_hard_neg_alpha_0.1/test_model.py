...
def build_dummy_inputs(batch_size=4, max_len=200, dim=256,
                       ...
                       neg_ratio=0.5):
    """构建占位符与随机伪数据
    - 新增: label_click (int64), 1=正样本(点击), 0=hard neg
    """
    ...
    # 新增：点击标记（正/负样本）
    ph["label_click"] = tf.placeholder(tf.int64, [None], name="ph_label_click")    # 1=正样本, 0=hard neg
    ...
    fd = {
        ...
        # 所有样本都参与 loss（与模型里的 photo_semantic_id_int 对齐）
...
def main():
    ...
    with g.as_default():
        ...
        ph, fd, feature_emb_dict, feature_emb_size_dict = build_dummy_inputs(
            batch_size=batch_size, max_len=max_len, dim=dim, vocab_sizes=vocab_sizes, neg_ratio=0.5
        )
        ...
        # 可选：设置 hard negative 的 α 系数
        model._hard_neg_alpha = 1e-3

        # 3) 前向与损失（注意：补上 label_click）
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            for t in range(1, num_steps + 1):
                ...
                if t % log_every == 0:
                    print("[Train] step=%d  total=%.6f" % (gs, total_v))
