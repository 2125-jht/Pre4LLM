...
if args.with_kai_v2:
    ...
    def filter_mask_wrapper(dataset):
        ...
        def mask_fn(batch):
            ...
            # 计算用户正向行为计数：仅在播放超3s的前提下统计；播放超7s + 播完 + 点赞 + 关注 + 评论 + 收藏 + 下载 + 主页访问
            ...
            # 如果正向行为计数 < 1，则该样本被标记为需过滤（mask=True）
    ...
    for attr in all_features:
        ...
        if not is_training:
            ...
            if attr.is_common:
                ...
                embed, size_var = config.new_embedding(attr.attr_name, dim=attr.dim, slots=attr.slots, expand=attr.expand, common=attr.is_common, sized=True, dtype=tf.float16,trainable=False)
            else:
                embed, size_var = config.new_embedding(attr.attr_name, dim=attr.dim, slots=attr.slots, expand=attr.expand, sized=True, dtype=tf.float16,trainable=False)
...
# 创建多兴趣推荐模型实例
model = MultiInterestModel(all_param_dict, feature_emb_size_dict, print_ops=print_ops, is_training=is_training)
...
if is_training:
    ...
    # ============================================================
    # photo_semantic_id 标签诊断：仅在前 DIAG_STEPS 个 step 运行
    # ============================================================
    DIAG_STEPS = 20  # 只诊断前 20 个 step，之后完全无开销

    def _diagnose_photo_sid_labels():
        """诊断 photo_semantic_id 标签覆盖率，打印到日志"""
        sid_val = photo_semantic_id_int  # [batch_size, 1]
        uid_val = uid                    # [batch_size, 1]
        pid_val = pid                    # [batch_size, 1]

        has_label = tf.cast(tf.greater(sid_val, 0), tf.float32)  # >0 表示有标签
        label_rate = tf.reduce_mean(has_label)  # 本 batch 标签覆盖率

        # 逐样本统计：哪些有标签、哪些没有
        has_label_vec = tf.reshape(has_label, [-1])       # [B]
        sid_vec = tf.reshape(tf.cast(sid_val, tf.int64), [-1])  # [B]
        uid_vec = tf.reshape(uid_val, [-1])               # [B]
        pid_vec = tf.reshape(pid_val, [-1])               # [B]

        # 找出没有标签的样本索引
        no_label_indices = tf.where(tf.equal(has_label_vec, 0.0))  # [N, 1]
        has_label_indices = tf.where(tf.greater(has_label_vec, 0.0))  # [M, 1]

        num_no_label = tf.shape(no_label_indices)[0]
        num_has_label = tf.shape(has_label_indices)[0]
        total_samples = tf.shape(has_label_vec)[0]

        # 打印汇总信息
        diag_ops = [
            tf.print("\n" + "="*60, output_stream=sys.stdout),
            tf.print("[PhotoSID Diagnosis] step=", worker_global_step,
                     " | batch_size=", total_samples,
                     " | has_label=", num_has_label,
                     " | no_label=", num_no_label,
                     " | label_rate=", label_rate,
                     output_stream=sys.stdout),
            tf.print("="*60, output_stream=sys.stdout),
        ]

        # 打印前 10 个没有标签的样本详情（uid, pid, sid_val）
        no_label_uid = tf.gather(uid_vec, no_label_indices[:10])
        no_label_pid = tf.gather(pid_vec, no_label_indices[:10])
        no_label_sid = tf.gather(sid_vec, no_label_indices[:10])
        diag_ops.append(tf.print("[No Label Samples] (uid, pid, photo_sid):",
                                  tf.transpose(tf.stack([no_label_uid, no_label_pid, no_label_sid], axis=0)),
                                  summarize=30, output_stream=sys.stdout))

        # 打印前 10 个有标签的样本详情
        has_label_uid = tf.gather(uid_vec, has_label_indices[:10])
        has_label_pid = tf.gather(pid_vec, has_label_indices[:10])
        has_label_sid = tf.gather(sid_vec, has_label_indices[:10])
        diag_ops.append(tf.print("[Has Label Samples] (uid, pid, photo_sid):",
                                  tf.transpose(tf.stack([has_label_uid, has_label_pid, has_label_sid], axis=0)),
                                  summarize=30, output_stream=sys.stdout))

        # 解码有标签样本的语义ID层级信息（a, b, c 三层）
        sample_sid_int = tf.gather(sid_vec, has_label_indices[:5])  # 取5个
        sample_a = tf.bitwise.bitwise_and(tf.bitwise.right_shift(tf.cast(sample_sid_int, tf.int64), 30), 0x7FFF)
        sample_b = tf.bitwise.bitwise_and(tf.bitwise.right_shift(tf.cast(sample_sid_int, tf.int64), 15), 0x7FFF)
        sample_c = tf.bitwise.bitwise_and(tf.cast(sample_sid_int, tf.int64), 0x7FFF)
        diag_ops.append(tf.print("[Label Hierarchy] (sid_int -> a, b, c):",
                                  tf.transpose(tf.stack([sample_sid_int, sample_a, sample_b, sample_c], axis=0)),
                                  summarize=20, output_stream=sys.stdout))

        diag_ops.append(tf.print("-"*60 + "\n", output_stream=sys.stdout))

        return tf.group(*diag_ops)

    def _no_diagnose():
        """step >= DIAG_STEPS 时不执行任何诊断操作"""
        return tf.no_op()

    # 仅在前 DIAG_STEPS 个 step 执行诊断，之后完全无开销
    diag_op = tf.cond(
        tf.less(worker_global_step, DIAG_STEPS),
        _diagnose_photo_sid_labels,
        _no_diagnose
    )
    print_ops.append(diag_op)
    ...
    user_sid_prob = tf.cast(tf.reshape(user_sid_prob, [-1, tf.shape(user_sid_prob)[1]]), tf.float32) # [batch, beam] → float32 for UniPredict
