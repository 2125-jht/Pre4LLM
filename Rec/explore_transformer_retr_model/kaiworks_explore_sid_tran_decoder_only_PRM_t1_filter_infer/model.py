...
class MultiInterestModel(object):
    ...
    def __init__(self, feature_emb_dict, feature_emb_size_dict, dim=256, vocab_sizes=[8192, 8192, 8192], print_ops=None, is_training=True):
        """
        初始化多兴趣模型
        ...
        # 训练时 float32 存储（兼容热启加载），推理时 float16（配合 FuseMioVariableCast 加速）
        infer_dtype = tf.float16 if not is_training else tf.float32
    ...
    def model(self, photo_sid, label, photo_semantic_id_int):
        ...
        # ---- FP16 诊断（训练时监控推理 FP16 路径关键节点）----
        # 推理时 encoder_input 会被 cast 到 float16，提前检查其数值范围
        '''self._print_ops.append(tf.print("[FP16-DIAG train] user_static_emb mean=", tf.reduce_mean(tf.cast(user_static_emb, tf.float32)),
                                         " abs_mean=", tf.reduce_mean(tf.abs(tf.cast(user_static_emb, tf.float32))),
                                         output_stream=sys.stdout))
        self._print_ops.append(tf.print("[FP16-DIAG train] user_click_emb mean=", tf.reduce_mean(tf.cast(user_click_emb, tf.float32)),
                                         " abs_mean=", tf.reduce_mean(tf.abs(tf.cast(user_click_emb, tf.float32))),
                                         output_stream=sys.stdout))
        self._print_ops.append(tf.print("[FP16-DIAG train] encoder_input mean=", tf.reduce_mean(encoder_input),
                                         " abs_mean=", tf.reduce_mean(tf.abs(encoder_input)),
                                         " has_nan=", tf.reduce_any(tf.is_nan(encoder_input)),
                                         " has_inf=", tf.reduce_any(tf.is_inf(encoder_input)),
                                         " fp16_safe=", tf.reduce_all(tf.abs(encoder_input) < 65504.0),
                                         output_stream=sys.stdout))'''
        # ⚠️ 如果 encoder_input 均值 < 1e-3，推理 FP16 精度不足
        # ⚠️ 如果 |encoder_input| > 65504，推理 FP16 会溢出为 Inf
        ...
        # 训练时 _vocab_embedding 已是 float32，无需额外 cast
        # 保留 cast 以确保 decoder 和 proj 层的 dense 变量为 float32（kaiworks push_dense_init 不支持 float16）
        decoder_input = tf.cast(decoder_input, tf.float32)

        # ---- FP16 诊断：vocab embedding lookup 结果（推理时为 float16）----
        # 训练时是 float32，但推理时 _vocab_embedding 为 float16，embedding_lookup 结果也是 float16
        '''self._print_ops.append(tf.print("[FP16-DIAG train] decoder_input(vocab_emb) mean=", tf.reduce_mean(decoder_input),
                                         " abs_mean=", tf.reduce_mean(tf.abs(decoder_input)),
                                         " has_nan=", tf.reduce_any(tf.is_nan(decoder_input)),
                                         " has_inf=", tf.reduce_any(tf.is_inf(decoder_input)),
                                         output_stream=sys.stdout))'''
        # ⚠️ 如果 decoder_input 均值 < 1e-3，推理 FP16 精度不足

        # === 5. Transformer编码器 ===
        # encoder_output = encoder_input
        # 先看看没有encoder结构的效果？如果可以就不用加encoder了
        #encoder_output = layer_norm(tf.cast(encoder_input, tf.float16), scope="enc_ln")
        encoder_output = layer_norm(encoder_input, scope="enc_ln")
        
        # ---- FP16 诊断：encoder_output（推理时直接为 float16）----
        '''self._print_ops.append(tf.print("[FP16-DIAG train] encoder_output mean=", tf.reduce_mean(encoder_output),
                                         " abs_mean=", tf.reduce_mean(tf.abs(encoder_output)),
                                         " has_nan=", tf.reduce_any(tf.is_nan(encoder_output)),
                                         " has_inf=", tf.reduce_any(tf.is_inf(encoder_output)),
                                         output_stream=sys.stdout))'''
        # ⚠️ 推理时 enc_out_base = layer_norm(cast(encoder_input, fp16))，值域与训练不同
        # 如果 encoder_output 均值 < 1e-3，FP16 可能导致精度损失
        ...
        # ---- FP16 诊断：decoder output（推理时为 float16）----
        '''self._print_ops.append(tf.print("[FP16-DIAG train] decoder_output mean=", tf.reduce_mean(decoder_output),
                                         " abs_mean=", tf.reduce_mean(tf.abs(decoder_output)),
                                         " has_nan=", tf.reduce_any(tf.is_nan(decoder_output)),
                                         " has_inf=", tf.reduce_any(tf.is_inf(decoder_output)),
                                         output_stream=sys.stdout))'''
        ...
        # 关键：训练图去掉 build_variables()，由 tf.layers.dense 按输入 dtype(float32) 延迟创建变量
        ...
        use_logq_correction = False  # 推理时不使用纠偏
        prm_losses = []
        for step in range(len(self._vocab_sizes)):
            ...
            # ---- FP16 诊断：PRM target embedding（推理时为 float16 sum-pool）----
            '''self._print_ops.append(tf.print("[FP16-DIAG train] step", step, "prm_target_emb mean=", tf.reduce_mean(target_embedding),
                                             " abs_mean=", tf.reduce_mean(tf.abs(target_embedding)),
                                             " has_nan=", tf.reduce_any(tf.is_nan(target_embedding)),
                                             " has_inf=", tf.reduce_any(tf.is_inf(target_embedding)),
                                             output_stream=sys.stdout))'''
            ...
            # ---- FP16 诊断：PRM logits（推理时为 float16）----
            '''self._print_ops.append(tf.print("[FP16-DIAG train] step", step, "prm_logits mean=", tf.reduce_mean(prm_logits),
                                             " abs_mean=", tf.reduce_mean(tf.abs(prm_logits)),
                                             " has_nan=", tf.reduce_any(tf.is_nan(prm_logits)),
                                             " has_inf=", tf.reduce_any(tf.is_inf(prm_logits)),
                                             output_stream=sys.stdout))'''
            # ⚠️ 如果 PRM logits 均值 < 1e-3，推理 FP16 精度不足，建议放大输出值
        ...
        for step in range(len(self._vocab_sizes)):
            with tf.variable_scope('proj_%d' % step):
                ...
                pred_logit = tf.cast(
                    tf.layers.dense(decoder_output[:, step, :], self._vocab_sizes[step], name='pred'),
                    tf.float32)  # [batch_size, vocab_size]
                print_tensor("logits/pred_logit_%d" % step, pred_logit)

                # ---- FP16 诊断：proj logits（推理时 dense 层输入为 float16）----
                '''self._print_ops.append(tf.print("[FP16-DIAG train] step", step, "proj_logits mean=", tf.reduce_mean(pred_logit),
                                                 " abs_mean=", tf.reduce_mean(tf.abs(pred_logit)),
                                                 " has_nan=", tf.reduce_any(tf.is_nan(pred_logit)),
                                                 " has_inf=", tf.reduce_any(tf.is_inf(pred_logit)),
                                                 output_stream=sys.stdout))'''
                # ⚠️ 如果 proj logits 均值 < 1e-3，推理 FP16 精度不足
                ...
                # 转换标签为one-hot编码
                one_hot_labels = tf.one_hot(label[:, step], self._vocab_sizes[step], dtype=tf.float32)
    ...
    def beam_search_fast(self, beam_size=512, temperature=1):
        ...
        * PRM 重新打分后保留 beam_size 条进入下一步。
        ...
        # ---- FP16 诊断：MLP 输出（FP16 路径关键节点）----
        '''user_static_emb_diag = tf.cast(user_static_emb, tf.float32)
        user_click_emb_diag = tf.cast(user_click_emb, tf.float32)
        encoder_input_diag = tf.cast(encoder_input, tf.float32)
        self._print_ops.append(tf.print("[FP16-DIAG] user_static_emb mean=", tf.reduce_mean(user_static_emb_diag),
                                         " abs_mean=", tf.reduce_mean(tf.abs(user_static_emb_diag)),
                                         output_stream=sys.stdout))
        self._print_ops.append(tf.print("[FP16-DIAG] user_click_emb mean=", tf.reduce_mean(user_click_emb_diag),
                                         " abs_mean=", tf.reduce_mean(tf.abs(user_click_emb_diag)),
                                         output_stream=sys.stdout))
        self._print_ops.append(tf.print("[FP16-DIAG] encoder_input mean=", tf.reduce_mean(encoder_input_diag),
                                         " abs_mean=", tf.reduce_mean(tf.abs(encoder_input_diag)),
                                         " has_nan=", tf.reduce_any(tf.is_nan(encoder_input_diag)),
                                         " has_inf=", tf.reduce_any(tf.is_inf(encoder_input_diag)),
                                         output_stream=sys.stdout))'''
        # ⚠️ 如果 encoder_input 的均值 < 1e-3，FP16 精度可能不足，建议放大输出值
        ...
        #enc_out_base = layer_norm(tf.cast(encoder_input, tf.float16), scope="enc_ln")     # [B, L_enc, C]
        enc_out_base = layer_norm(tf.cast(encoder_input, tf.float16), scope="enc_ln") 

        # ---- FP16 诊断：encoder output ----
        '''enc_out_fp32 = tf.cast(enc_out_base, tf.float32)
        self._print_ops.append(tf.print("[FP16-DIAG] enc_out_base mean=", tf.reduce_mean(enc_out_fp32),
                                         " abs_mean=", tf.reduce_mean(tf.abs(enc_out_fp32)),
                                         " has_nan=", tf.reduce_any(tf.is_nan(enc_out_fp32)),
                                         " has_inf=", tf.reduce_any(tf.is_inf(enc_out_fp32)),
                                         output_stream=sys.stdout))
        self._print_ops.append(tf.assert_none_nan(enc_out_fp32, message="[FP16-DIAG] enc_out_base has NaN!"))
        self._print_ops.append(tf.assert_none_inf(enc_out_fp32, message="[FP16-DIAG] enc_out_base has Inf!"))'''
        ...
        # 预计算 batch 索引（循环外只算一次，减少冗余 graph node）
        ...
        # 增量维护 PRM path embedding sum-pool，避免每步全量 recompute embedding_lookup + reduce_sum
        prm_path_emb_sums = None   # step 0 后初始化为 emb(sid0)

        # PRM encoder cache 在 step==0 时预计算，后续层通过 forward_with_encoder_cache 复用
        ...
        for step, V in enumerate(self._vocab_sizes):
            # 只 embed 当前 token
            dec_in = tf.nn.embedding_lookup(self._vocab_embedding, seqs[:, :, -1])  # [B,beam,1,C] float16 in inference

            # ---- FP16 诊断：vocab embedding lookup ----
            '''dec_in_diag = tf.cast(dec_in, tf.float32)
            self._print_ops.append(tf.print("[FP16-DIAG] step", step, "dec_in mean=", tf.reduce_mean(dec_in_diag),
                                             " abs_mean=", tf.reduce_mean(tf.abs(dec_in_diag)),
                                             " has_nan=", tf.reduce_any(tf.is_nan(dec_in_diag)),
                                             " has_inf=", tf.reduce_any(tf.is_inf(dec_in_diag)),
                                             output_stream=sys.stdout))'''
            ...
            # ---- FP16 诊断：decoder step output ----
            '''dec_out_diag = tf.cast(dec_out, tf.float32)
            self._print_ops.append(tf.print("[FP16-DIAG] step", step, "dec_out mean=", tf.reduce_mean(dec_out_diag),
                                             " abs_mean=", tf.reduce_mean(tf.abs(dec_out_diag)),
                                             " has_nan=", tf.reduce_any(tf.is_nan(dec_out_diag)),
                                             " has_inf=", tf.reduce_any(tf.is_inf(dec_out_diag)),
                                             output_stream=sys.stdout))'''
            ...
            # ---- FP16 诊断：proj logits ----
            '''logits_diag = tf.cast(logits, tf.float32)
            self._print_ops.append(tf.print("[FP16-DIAG] step", step, "proj_logits mean=", tf.reduce_mean(logits_diag),
                                             " abs_mean=", tf.reduce_mean(tf.abs(logits_diag)),
                                             " has_nan=", tf.reduce_any(tf.is_nan(logits_diag)),
                                             " has_inf=", tf.reduce_any(tf.is_inf(logits_diag)),
                                             output_stream=sys.stdout))'''
            ...
            if step == 0:
                ...
                # 增量维护：step 0 prefix = [sid0]，sum-pool = emb(sid0)
                prm_path_emb_sums = tf.nn.embedding_lookup(self._vocab_embedding, next_tok_glb)  # [B, beam_size, dim] float16 in inference

                # ---- FP16 诊断：step 0 PRM path embedding ----
                '''prm_emb_diag = tf.cast(prm_path_emb_sums, tf.float32)
                self._print_ops.append(tf.print("[FP16-DIAG] step 0 prm_path_emb mean=", tf.reduce_mean(prm_emb_diag),
                                                 " abs_mean=", tf.reduce_mean(tf.abs(prm_emb_diag)),
                                                 " has_nan=", tf.reduce_any(tf.is_nan(prm_emb_diag)),
                                                 " has_inf=", tf.reduce_any(tf.is_inf(prm_emb_diag)),
                                                 output_stream=sys.stdout))'''
            ...
            # ==============================================================
                ...
                # --- 第二阶段：PRM 打分 + 剪枝 ---
                # 增量维护 PRM target embedding：parent_prefix_sum + emb(new_token)
                # 比全量 recompute (embedding_lookup 全部 prefix tokens + reduce_sum) 节省 embedding lookup 计算量
                parent_prm_path_emb_sum = tf.gather_nd(prm_path_emb_sums, gather_parent)  # [B, cand, dim]
                next_tok_embedding = tf.nn.embedding_lookup(self._vocab_embedding, next_tok_glb)  # [B, cand, dim] float16 in inference
                prm_target_emb = parent_prm_path_emb_sum + next_tok_embedding          # [B, cand, dim]
                prm_target_embedding = tf.reshape(
                    prm_target_emb,
                    [B * prm_candidate_size, 1, self._dim]
                )

                # 使用 encoder KV cache 避免重复计算 encoder 侧的 K/V 投影
                # 也无需构建 prm_encoder_output / prm_src_mask 两个大张量，节省显存
                ...
                # ---- FP16 诊断：PRM logits ----
                '''prm_logits_diag = tf.cast(prm_logits, tf.float32)
                self._print_ops.append(tf.print("[FP16-DIAG] step", step, "prm_logits mean=", tf.reduce_mean(prm_logits_diag),
                                                 " abs_mean=", tf.reduce_mean(tf.abs(prm_logits_diag)),
                                                 " has_nan=", tf.reduce_any(tf.is_nan(prm_logits_diag)),
                                                 " has_inf=", tf.reduce_any(tf.is_inf(prm_logits_diag)),
                                                 output_stream=sys.stdout))'''
                # 如果 PRM logits 均值过小 (<1e-3)，FP16 精度可能不足
                ...
                if step < len(self._vocab_sizes) - 1:
                    # cache 按 PRM 最终留下的父 beam 重排，供下一层解码使用
                    ...
                    # 增量维护：更新 surviving beams 的 PRM target embedding sum，供下一层使用
                    prm_path_emb_sums = tf.gather_nd(prm_target_emb, gather_prm)  # [B, beam, dim]
        ...
        # ---- FP16 诊断：最终输出概率 ----
        '''probs_diag = tf.cast(probs, tf.float32)
        self._print_ops.append(tf.print("[FP16-DIAG] final_probs mean=", tf.reduce_mean(probs_diag),
                                         " abs_mean=", tf.reduce_mean(tf.abs(probs_diag)),
                                         " has_nan=", tf.reduce_any(tf.is_nan(probs_diag)),
                                         " has_inf=", tf.reduce_any(tf.is_inf(probs_diag)),
                                         output_stream=sys.stdout))'''
    ...
    def beam_search_fast_no_prm(self, beam_size=512, temperature=1):
        ...
        user_click_emb_diag = tf.cast(user_click_emb, tf.float32)
        encoder_input_diag = tf.cast(encoder_input, tf.float32)
        self._print_ops.append(tf.print("[FP16-DIAG no_prm] user_static_emb mean=", tf.reduce_mean(user_static_emb_diag),
                                         " abs_mean=", tf.reduce_mean(tf.abs(user_static_emb_diag)),
                                         output_stream=sys.stdout))
        self._print_ops.append(tf.print("[FP16-DIAG no_prm] user_click_emb mean=", tf.reduce_mean(user_click_emb_diag),
                                         " abs_mean=", tf.reduce_mean(tf.abs(user_click_emb_diag)),
                                         output_stream=sys.stdout))
        self._print_ops.append(tf.print("[FP16-DIAG no_prm] encoder_input mean=", tf.reduce_mean(encoder_input_diag),
                                         " abs_mean=", tf.reduce_mean(tf.abs(encoder_input_diag)),
                                         " has_nan=", tf.reduce_any(tf.is_nan(encoder_input_diag)),
                                         " has_inf=", tf.reduce_any(tf.is_inf(encoder_input_diag)),
                                         output_stream=sys.stdout))'''
        ...
        # ---- FP16 诊断：encoder output ----
        '''enc_out_fp32 = tf.cast(enc_out_base, tf.float32)
        self._print_ops.append(tf.print("[FP16-DIAG no_prm] enc_out_base mean=", tf.reduce_mean(enc_out_fp32),
                                         " abs_mean=", tf.reduce_mean(tf.abs(enc_out_fp32)),
                                         " has_nan=", tf.reduce_any(tf.is_nan(enc_out_fp32)),
                                         " has_inf=", tf.reduce_any(tf.is_inf(enc_out_fp32)),
                                         output_stream=sys.stdout))
        self._print_ops.append(tf.assert_none_nan(enc_out_fp32, message="[FP16-DIAG no_prm] enc_out_base has NaN!"))
        self._print_ops.append(tf.assert_none_inf(enc_out_fp32, message="[FP16-DIAG no_prm] enc_out_base has Inf!"))'''
        ...
        for step, V in enumerate(self._vocab_sizes):
            ...
            # ---- FP16 诊断：vocab embedding lookup ----
            '''dec_in_diag = tf.cast(dec_in, tf.float32)
            self._print_ops.append(tf.print("[FP16-DIAG no_prm] step", step, "dec_in mean=", tf.reduce_mean(dec_in_diag),
                                             " abs_mean=", tf.reduce_mean(tf.abs(dec_in_diag)),
                                             " has_nan=", tf.reduce_any(tf.is_nan(dec_in_diag)),
                                             " has_inf=", tf.reduce_any(tf.is_inf(dec_in_diag)),
                                             output_stream=sys.stdout))'''
            ...
            # ---- FP16 诊断：decoder step output ----
            '''dec_out_diag = tf.cast(dec_out, tf.float32)
            self._print_ops.append(tf.print("[FP16-DIAG no_prm] step", step, "dec_out mean=", tf.reduce_mean(dec_out_diag),
                                             " abs_mean=", tf.reduce_mean(tf.abs(dec_out_diag)),
                                             " has_nan=", tf.reduce_any(tf.is_nan(dec_out_diag)),
                                             " has_inf=", tf.reduce_any(tf.is_inf(dec_out_diag)),
                                             output_stream=sys.stdout))'''
            ...
            # ---- FP16 诊断：proj logits ----
            '''logits_diag = tf.cast(logits, tf.float32)
            self._print_ops.append(tf.print("[FP16-DIAG no_prm] step", step, "proj_logits mean=", tf.reduce_mean(logits_diag),
                                             " abs_mean=", tf.reduce_mean(tf.abs(logits_diag)),
                                             " has_nan=", tf.reduce_any(tf.is_nan(logits_diag)),
                                             " has_inf=", tf.reduce_any(tf.is_inf(logits_diag)),
                                             output_stream=sys.stdout))'''
            ...
            # ==============================================================
            #  第一层（step==0）：cur_beam=1，topk 结果已是全局最优，跳过一次等价 top_k
            # ==============================================================
            if step == 0:
                best_idx = tf.tile(
                ...
                best_scores = tf.squeeze(topk_logp, axis=1)                    # [B, beam_size]

                # 直接构建 seqs：tile <START> + 拼上选中的 token
                start_tiled  = tf.tile(seqs, [1, beam_size, 1])                # [B, beam_size, 1]
                next_tok_glb = tf.squeeze(topk_tok, axis=1) + offsets[0]       # [B, beam_size]
                seqs = tf.concat(
                    [start_tiled, tf.expand_dims(next_tok_glb, -1)], axis=-1)  # [B, beam_size, 2]
            ...
            # ==============================================================
            #  后续层（step>=1）：decoder 选全局 top-beam_size
            # ==============================================================
            else:
                # 累积得分
                ...
                if step < len(self._vocab_sizes) - 1:
                    # cache 按 decoder 最终留下的父 beam 重排，供下一层解码使用
        ...
        # 去掉 <START>
        seqs  = seqs[:, :, 1:]
        # 返回 decoder 累积概率（scores = 累积 log 概率）
        probs = tf.exp(scores)

        # ---- FP16 诊断：最终输出概率 ----
        '''probs_diag = tf.cast(probs, tf.float32)
        self._print_ops.append(tf.print("[FP16-DIAG no_prm] final_probs mean=", tf.reduce_mean(probs_diag),
                                         " abs_mean=", tf.reduce_mean(tf.abs(probs_diag)),
                                         " has_nan=", tf.reduce_any(tf.is_nan(probs_diag)),
                                         " has_inf=", tf.reduce_any(tf.is_inf(probs_diag)),
                                         output_stream=sys.stdout))'''
