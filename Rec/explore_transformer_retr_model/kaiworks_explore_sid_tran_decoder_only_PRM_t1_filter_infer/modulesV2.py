...
def layer_norm(x, scope, eps=1e-6):
    ...
    with tf.variable_scope(f"{scope}/layer_norm", reuse=tf.AUTO_REUSE):
        gamma = tf.get_variable("gamma", [x.get_shape()[-1]], dtype=in_dtype, initializer=tf.ones_initializer())
        beta = tf.get_variable("beta", [x.get_shape()[-1]], dtype=in_dtype, initializer=tf.zeros_initializer())
...
class PRMModel():
    ...
    @staticmethod
    def _fp16_custom_getter(getter, *args, **kwargs):
        """变量以 float32 存储（checkpoint 兼容），但返回 float16 供计算使用。"""
        requested_dtype = kwargs.get('dtype', None)
        # 强制变量以 float32 创建/存储
        kwargs['dtype'] = tf.float32
        var = getter(*args, **kwargs)
        # 如果调用方请求 float16，返回 float16 cast 供计算
    ...
    def build_variables(self):
        def dense_vars(scope, input_dim, output_dim, use_bias=True):
            with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
                tf.get_variable("kernel", [input_dim, output_dim], dtype=tf.float32, initializer=tf.glorot_uniform_initializer())
                if use_bias:
                    tf.get_variable("bias", [output_dim], dtype=tf.float32, initializer=tf.zeros_initializer())
    ...
    def forward(self, target_embedding, hidden_states, src_mask, training):
        # 统一输入 dtype：训练用 float32 保证精度，推理用 float16 加速
        compute_dtype = tf.float32 if training else tf.float16
        target_embedding = tf.cast(target_embedding, compute_dtype)
        hidden_states = tf.cast(hidden_states, compute_dtype)
        if not training:
            scope = tf.variable_scope("prm_model", reuse=tf.AUTO_REUSE, custom_getter=self._fp16_custom_getter)
        else:
            scope = tf.variable_scope("prm_model", reuse=tf.AUTO_REUSE)
        with scope:
        ...
        #return tf.cast(tf.squeeze(score, axis=[1, 2]), tf.float32)

    def build_encoder_cache(self, hidden_states):
        # 推理专用：输入 cast 到 float16，配合 custom_getter 实现全 fp16 计算
        ...
        with tf.variable_scope("prm_model", reuse=tf.AUTO_REUSE, custom_getter=self._fp16_custom_getter):
            with tf.variable_scope("multi_head_attention", reuse=tf.AUTO_REUSE):
    ...
    def forward_with_encoder_cache(self, target_embedding, k_enc_cache, v_enc_cache, src_mask, cur_beam, training):
        # 推理专用：输入 cast 到 float16，配合 custom_getter 实现全 fp16 计算
        target_embedding = tf.cast(target_embedding, tf.float16)
        def split_heads(x, num_heads):
            batch_size = tf.shape(x)[0]
            T_x = tf.shape(x)[1]
            depth = x.get_shape().as_list()[-1] // num_heads
