...
model_queue_prefix="kaiworks_explore_sid_tran_multi_interest_decoder_only_Qformer_query10_light_more_copy"
model_dir = "predict/conf_gpu"
param_json = "parameter_config.json"
PXTRS = ['user_sid_origin', "user_sid_prob", "query_indices"]
sign_format = "mio"
shard_offset = 0
queue_shard_num = 1
EMBEDDING_SHARDS = 1
...
class ModelRetrFlow(LeafFlow, MioApiMixin, OfflineApiMixin, UniPredictApiMixin, ExploreModelApiMixin, RetrievalApiMixin, GsuApiMixin, EmbeddingApiMixin):
            ...
            self.uni_predict_fused(
              ...
              ## embedding 拉取相关配置，对应 fetch_mio_embedding 部分
               embedding_fetchers = [dict(
                ...
                colossusdb_embd_table_name="gr_multi_query10_light_more_gpu",
  ...
  # .enrich_attr_by_py(
    ...
    def retrieve(self):
      ...
      .enrich_attr_by_lua(
        import_common_attr = ["user_sid_int", "user_sid_prob", "query_indices"],
        import_item_attr = ["index"],
        function_for_item = "calculate",
        export_item_attr = ["query_sid_list", "query_sid", "prob1", "prob2", "prob3", "query_index"],
        lua_script = """
          function calculate()
            ...
            -- 获取当前 index 对应的 query 索引（0 代表第一个 query）
            query_index = math.floor(query_indices[index+1])
            return query_sid_list, query_int, prob1, prob2, prob3, query_index
          end
        """
      ) \
       .pack_item_attr(
    ...
    def post_process(self):
      return self.filter_by_attr(
        attr_name = "query_index",
        remove_if = "!=",
        compare_to = "{{query}}",
...
service = LeafService(
  ...
  common_attrs_from_request = [
    "user",
    "query",  # 新增：从请求中接收 query 参数，用于过滤指定 query 的结果
  ],
