...
class MultiInterestModel(object):
    ...
    def model(self, photo_sid, label, photo_semantic_id_int):
        ...
        #计算正交loss
        gram_matrix = tf.matmul(coarse_norm, coarse_norm, transpose_b=True)  # [B, Q, Q]
        identity = tf.eye(self._query_token_numb, batch_shape=[batch_size])  # [B, Q, Q]
        orth_loss_per_sample = tf.reduce_sum(tf.square(gram_matrix - identity), axis=[1, 2]) # [B]
        ...
        orth_loss_masked = tf.reduce_sum(orth_loss_per_sample * loss_mask) / (tf.reduce_sum(loss_mask) + 1e-9)  
        print_tensor('loss/orth_loss', orth_loss_masked)
        orth_weight = 0.1
        loss = main_loss + orth_weight * orth_loss_masked
