...
class MultiInterestModel(object):
    ...
    def model(self, photo_sid, label, photo_semantic_id_int):
        ...
        preference_embedding = tf.reduce_sum(sid_embedding ,axis=1) #[b,dim]
        ...
        coarse_interest = coarse_interest_model.forward(coarse_interest_input, encoder_output, src_mask, training=True) # [b, 1, dim]
        


        fine_item_input = tf.concat([tf.expand_dims(preference_embedding,axis=1), sid_embedding[:, :-1, :]], axis=1)  # [b, code_size, dim]
        
        fine_item_input_v2 = tf.concat([coarse_interest, sid_embedding[:, :-1, :]], axis=1)  # [b, code_size, dim]


        with tf.variable_scope("fine_item_decoder", reuse=tf.AUTO_REUSE):

            fine_item_model = DecoderModel(num_layers=1, dim=self._dim, num_heads=8, dropout_rate=0.1, hidden_dim=self._dim*2, training=True)
            decoder_output = fine_item_model.forward(fine_item_input, encoder_output, src_mask, training=True)  # [b, code_size, dim]
            
            decoder_output_v2 = fine_item_model.forward(fine_item_input_v2, encoder_output, src_mask, training=True)  # [b, code_size, dim]


        # === 7. InfoNCE Loss between coarse_interest and preference_embedding ===
        coarse_interest_squeezed = tf.squeeze(coarse_interest, axis=1)  # [b, dim]
        
        # 归一化
        coarse_norm = tf.nn.l2_normalize(coarse_interest_squeezed, axis=-1)  # [b, dim]
        pref_norm = tf.nn.l2_normalize(preference_embedding, axis=-1)  # [b, dim]
        
        # 计算相似度矩阵
        temperature_nce = 0.05
        logits_matrix = tf.matmul(coarse_norm, pref_norm, transpose_b=True) / temperature_nce  # [b, b]
        
        # InfoNCE loss: 对角线为正样本，其他为负样本
        labels_nce = tf.range(batch_size)
        info_nce_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
            labels=labels_nce, 
            logits=logits_matrix
        )
        info_nce_loss = tf.reduce_mean(info_nce_loss)
        
        print_tensor("loss/info_nce_loss", info_nce_loss)
        
        # 计算平均余弦相似度用于监控
        cos_sim = tf.reduce_sum(coarse_norm * pref_norm, axis=-1)  # [b]
        print_tensor("interest_pref_sim", tf.reduce_mean(cos_sim))
        ...
        # 计算加权平均损失
        reconstruction_loss = tf.reduce_sum((losses[0] + losses[1] + losses[2]) * loss_mask) / (tf.reduce_sum(loss_mask) + 1e-9)
        
        # === 计算 v2 的 loss (不回传梯度) ===
        losses_v2 = []
        for step in range(len(self._vocab_sizes)):
            with tf.variable_scope('proj_%d' % step, reuse=True):
                pred_logit_v2 = tf.layers.dense(decoder_output_v2[:, step, :], self._vocab_sizes[step], name='pred')
                print_tensor("logits_true/pred_logit_v2_%d" % step, pred_logit_v2)
                
                
                # 1. 求 softmax 概率
                pred_prob_v2 = tf.nn.softmax(pred_logit_v2/temperature, axis=-1)  # [B, V]

                # print_tensor("probs/pred_prob_%d" % step, pred_prob)

                # 2. 取出正确 label 的概率
                #    先拼出索引对 (batch_idx, label_id)
                batch_idx_v2 = tf.range(tf.shape(pred_prob_v2)[0], dtype=tf.int32)
                indices_v2   = tf.stack([batch_idx_v2, label[:, step]], axis=1)  # [B, 2]
                correct_p_v2 = tf.gather_nd(pred_prob_v2, indices_v2)               # [B]
                # 3. 打印
                print_tensor("probs_true/correct_token_prob_%d" % step, tf.reduce_sum(correct_p_v2 * loss_mask) / (tf.reduce_sum(loss_mask) + 1e-9))

                greater_v2 = tf.cast(pred_prob_v2 > tf.expand_dims(correct_p_v2, 1),tf.float32)  # [B,V]
                correct_token_rank_v2 = 1 + tf.reduce_sum(greater_v2, axis=1)      # [B], 1=top1
                trim_rank_v2 = masked_trimmed_mean(correct_token_rank_v2, loss_mask, trim_ratio=0.05,name="trim_rank_%d" % step)
                print_tensor("probs_true/correct_token_rank_%d" % step,trim_rank_v2)
                
                max_probs_v2, _ = tf.nn.top_k(pred_prob_v2, k=1)
                print_tensor("probs_true/max_token_prob_%d" % step, tf.reduce_sum(tf.squeeze(max_probs_v2, -1) * loss_mask) / (tf.reduce_sum(loss_mask) + 1e-9))
                

                # 转换标签为one-hot编码
                one_hot_labels = tf.one_hot(label[:, step], self._vocab_sizes[step])
                # 计算交叉熵损失，使用温度缩放(temperature=2.0)
                loss_i_v2 = tf.nn.softmax_cross_entropy_with_logits(labels=one_hot_labels, logits=pred_logit_v2/temperature)
                losses_v2.append(loss_i_v2)
            
                # 打印每个层次的损失
                print_tensor("loss_true/loss_%d" % step, tf.reduce_sum(loss_i_v2 * loss_mask) / (tf.reduce_sum(loss_mask) + 1e-9))
                # 计算各种recall指标
                recall_at_k(pred_logit_v2, label[:, step], loss_mask, self._print_ops, top_k=[1, 16, 128, 256], name="true_predict_recall_%d" % step)
        
        true_ntp_loss = tf.reduce_sum((losses_v2[0] + losses_v2[1] + losses_v2[2]) * loss_mask) / (tf.reduce_sum(loss_mask) + 1e-9)
        print_tensor("loss/true_ntp_loss", true_ntp_loss)

        # 添加InfoNCE loss，权重系数可调
        info_nce_weight = 1
        total_loss = reconstruction_loss + info_nce_weight * info_nce_loss + true_ntp_loss
        
        print_tensor("loss/reconstruction_loss", reconstruction_loss)
