...
from model import MultiInterestModel, LazyMultiInterestModel   # ← 把文件名替换成你定义 MultiInterestModel 的源码文件名
...
def main_lazy():
    """测试 LazyMultiInterestModel 的参数量 & FLOPs，与 main() 对比"""
    dim = 256
    max_len = 200
    vocab_sizes = (8192, 8192, 8192)
    batch_size = 1
    ...
    with g_train.as_default():
        ...
        trainable_params, trainable_detail = count_params(trainable_only=True)
        all_params, _ = count_params(trainable_only=False)

        # 统计 cross-attn 参数差异
        cross_wk_params = sum(n for name, n, _ in trainable_detail
                             if 'decoder_layer_' in name and 'multi_head_attention/w_k' in name)
        cross_wv_params = sum(n for name, n, _ in trainable_detail
                             if 'decoder_layer_' in name and 'multi_head_attention/w_v' in name)
        lazy_wq_params = sum(n for name, n, _ in trainable_detail
                            if 'multi_head_attention/w_q' in name)
        lazy_wo_params = sum(n for name, n, _ in trainable_detail
                            if 'multi_head_attention/w_o' in name)
        context_k_ln_params = sum(n for name, n, _ in trainable_detail
                                 if 'context_k_ln' in name)
        ...
        with tf.Session(graph=g_train, config=cfg) as sess:
            ...
            fwd_flops, _ = profile_flops(g_train, sess, fetches=[total_loss],
                                         feed_dict=fd, note="Lazy Forward(loss)")
            train_flops, _ = profile_flops(g_train, sess,
                                           fetches=[train_op, total_lb_loss, total_loss],
                                           feed_dict=fd, note="Lazy Train step")

            print("\n===== Lazy Decoder Params =====")
            print(f"Trainable params      : {human(trainable_params)}")
            print(f"All variables         : {human(all_params)}")
            print(f"cross-attn w_k params : {human(cross_wk_params)}  (应为 0)")
            print(f"cross-attn w_v params : {human(cross_wv_params)}  (应为 0)")
            print(f"lazy_cross_attn w_q   : {human(lazy_wq_params)}")
            print(f"lazy_cross_attn w_o   : {human(lazy_wo_params)}")
            print(f"context_k_ln params   : {human(context_k_ln_params)}")
            print("===== Lazy Decoder FLOPs (per step, batch_size=%d) =====" % batch_size)
            print(f"Forward-only (loss) : {fwd_flops/1e9:.4f} GFLOPs")
            print(f"Train step (F+B+Opt): {train_flops/1e9:.4f} GFLOPs")
    ...
    with g_infer.as_default():
        ...
        gen_loc_lazy, probs_lazy = model.beam_search_lazy(beam_size=beam_size, temperature=1.0)
        ...
        with tf.Session(graph=g_infer, config=cfg) as sess:
            ...
            infer_fwd_flops, _ = profile_flops(g_infer, sess,
                                               fetches=[total_loss], feed_dict=fd,
                                               note="Lazy Infer forward(loss)")
            beam_flops, _ = profile_flops(g_infer, sess,
                                          fetches=[gen_loc_lazy, probs_lazy], feed_dict=fd,
                                          note=f"Lazy BeamSearch (beam={beam_size})")

            print("===== Lazy Inference FLOPs (per run, batch_size=%d) =====" % batch_size)
            print(f"Lazy Infer forward(loss) : {infer_fwd_flops/1e9:.4f} GFLOPs")
            print(f"Lazy BeamSearch         : {beam_flops/1e9:.4f} GFLOPs")

if __name__ == "__main__":
    # 默认运行原有模型
    main()
    # 如需对比 Lazy Decoder，取消下行注释：
    # main_lazy()