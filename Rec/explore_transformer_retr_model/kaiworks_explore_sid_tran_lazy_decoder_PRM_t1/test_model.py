...
def build_dummy_inputs(batch_size=4, max_len=200, dim=512,
                       vocab_sizes=(8192, 8192, 8192),
                       d_user_each=512, d_click_pid=512, d_click_aid=512):
    """构建占位符与随机伪数据"""
...
def main_lazy():
    """测试 LazyMultiInterestModel（OneRec-V2 Lazy Decoder）"""
    dim = 512
    max_len = 200
    vocab_sizes = (8192, 8192, 8192)
    batch_size = 1
    ...
    with g.as_default():
        ...
        # 注意：PRM 模型也有 multi_head_attention/w_k, w_v，需要按 scope 前缀区分
        # decoder 的 w_k/w_v 在 decoder_layer_*/multi_head_attention/ 下
        # lazy decoder 没有这些，只有 multi_head_attention/w_q, w_o
        decoder_cross_wk = any("decoder_layer_" in v.name and "multi_head_attention/w_k" in v.name
                               for v in all_vars)
        decoder_cross_wv = any("decoder_layer_" in v.name and "multi_head_attention/w_v" in v.name
                               for v in all_vars)
        lazy_cross_wq = any("multi_head_attention/w_q" in v.name for v in all_vars)
        lazy_cross_wo = any("multi_head_attention/w_o" in v.name for v in all_vars)
        context_k_ln = any("context_k_ln" in v.name for v in all_vars)

        # 6) Beam Search Lazy
        gen_loc_lazy, probs_lazy = model.beam_search_lazy(beam_size=512, temperature=1.0)
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            print(f"decoder cross-attn w_k 存在: {decoder_cross_wk}  (应为 False)")
            print(f"decoder cross-attn w_v 存在: {decoder_cross_wv}  (应为 False)")
            print(f"lazy_cross_attn w_q 存在: {lazy_cross_wq}  (应为 True)")
            print(f"lazy_cross_attn w_o 存在: {lazy_cross_wo}  (应为 True)")
            print(f"context_k_ln 存在: {context_k_ln}  (应为 True)")
            print("=" * 45 + "\n")

            for t in range(1, num_steps + 1):
                _, total_v, gs = sess.run(
                    [train_op, total_loss, global_step],
                    feed_dict=fd
                )
                if t % log_every == 0:
                    print("[Lazy Train] step=%d  total=%.6f" % (gs, total_v))

                if t % eval_every == 0:
                    gl_lazy, pr_lazy = sess.run([gen_loc_lazy, probs_lazy], feed_dict=fd)
                    print("[Lazy Eval] beam gen_part_loc[0,0]:", gl_lazy[0, 0])

            # 最终评估
            gl_lazy, pr_lazy = sess.run([gen_loc_lazy, probs_lazy], feed_dict=fd)
            print("[Lazy Final Eval] gen_part_loc[0,0]:", gl_lazy[0, 0])
            print("[Lazy Final Eval] gen_part_loc shape:", gl_lazy.shape)
            print("[Lazy Final Eval] probs shape:", pr_lazy.shape)
        

if __name__ == "__main__":
    # 默认运行原有模型测试
    main()
    # 如需测试 Lazy Decoder，取消下面的注释：
    # main_lazy()
