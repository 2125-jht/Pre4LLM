...
def main_lazy():
    """测试 LazyMultiInterestModel 的训练/推理耗时，与 main() 对比"""
    dim = 512
    max_len = 200
    vocab_sizes = (8192, 8192, 8192)
    batch_size = 1
    ...
    with g.as_default():
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            for t in range(1, num_steps + 1):
                ...
                if t % log_every == 0:
                    print("[Lazy Train] step=%d  lb=%.6f  total=%.6f  time=%.2f ms" %
                          (gs, lb_v, total_v, train_dur * 1000.0))

                # === 推理计时（beam search lazy） ===
                if t % eval_every == 0:
                    t1 = perf_counter()
                    ...
                    print("[Lazy Eval]  step=%d  gen_part_loc[0,0]: %s  time=%.2f ms" %
                          (gs, str(gl_lazy[0,0]), infer_dur * 1000.0))
            ...
            final_infer_dur = perf_counter() - t2
            infer_times.append(final_infer_dur)
            print("[Lazy Final Eval] gen_part_loc[0,0]: %s  time=%.2f ms" %
                  (str(gl_lazy[0,0]), final_infer_dur * 1000.0))

            # === 汇总统计 ===
            def _fmt_stats(ts):
                if not ts:
                    return "N/A"
                ts_ms = np.asarray(ts) * 1000.0
                return "avg=%.2f ms  p95=%.2f ms  max=%.2f ms  (N=%d)" % (
                    ts_ms.mean(), np.percentile(ts_ms, 95), ts_ms.max(), ts_ms.size
                )

            print("[Lazy Summary] Train: %s" % _fmt_stats(train_times))
            print("[Lazy Summary] Infer : %s" % _fmt_stats(infer_times))
