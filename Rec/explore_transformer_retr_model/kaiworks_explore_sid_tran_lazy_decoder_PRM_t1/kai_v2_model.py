...
from model import MultiInterestModel, LazyMultiInterestModel
...
parser.add_argument('--use_lazy_decoder', default=None, nargs='?', const=True)  # 切换 Lazy Decoder；不传时由环境变量 USE_LAZY_DECODER 决定
parser.add_argument('--with_kai_v2', default=True) #False True 
args = parser.parse_known_args()[0]
# Lazy Decoder 开关：命令行 > 环境变量 > 默认值(True)
# 云上不能改命令时，设置环境变量 USE_LAZY_DECODER=False 即可回退到传统 Decoder
if args.use_lazy_decoder is None:
    args.use_lazy_decoder = os.environ.get('USE_LAZY_DECODER', 'True').lower() not in ('false', '0', 'no')
    ...
    if len(weight) < dense_variable_nums: # 不存在base模型里，但存在新模型里，即【新增参数】
        for var_name, var in tf_weight.items():
            if var_name not in weight: # tf_weight 是新模型的所有w，而weight是前面一项项放入的。这里表示新模型有而旧模型没有。
                # ---- 特殊处理：用 enc_ln 的值初始化 context_k_ln ----
                # context_k_ln 和 enc_ln 都对 encoder_input 做 LayerNorm，
                # 用已学习的 enc_ln gamma/beta 比随机初始化效果更好
                if var_name == "context_k_ln/layer_norm/gamma" and "enc_ln/layer_norm/gamma" in weight:
                    weight[var_name] = weight["enc_ln/layer_norm/gamma"].copy()
                    print("新增参数 {} 用 enc_ln/layer_norm/gamma 的值初始化".format(var_name))
                elif var_name == "context_k_ln/layer_norm/beta" and "enc_ln/layer_norm/beta" in weight:
                    weight[var_name] = weight["enc_ln/layer_norm/beta"].copy()
                    print("新增参数 {} 用 enc_ln/layer_norm/beta 的值初始化".format(var_name))
                else:
                    weight[var_name] = var # 表示：将新增的var_name 的数值arr赋予给 weight。 此处可改为自定义初始化后的arr
                                        # 或者使用 numpy 自己控制数值如何初始化
                    print("加载的 dense variable({}) 是新增参数，使用 TF 初始化值".format(var_name))
    ...
    if len(extra) < dense_variable_nums: # 不存在base模型里，但存在新模型里，即【新增extra参数】
        for var_name, var in ps_extra.items():
            if var_name not in extra: # ps_extra是新模型的所有extra，而extra是前面一项项放入的
                # ---- 特殊处理：用 enc_ln 的 extra 初始化 context_k_ln 的 extra ----
                if var_name == "context_k_ln/layer_norm/gamma" and "enc_ln/layer_norm/gamma" in extra:
                    extra[var_name] = extra["enc_ln/layer_norm/gamma"].copy()
                    print("新增 extra 参数 {} 用 enc_ln/layer_norm/gamma 的 extra 值初始化".format(var_name))
                elif var_name == "context_k_ln/layer_norm/beta" and "enc_ln/layer_norm/beta" in extra:
                    extra[var_name] = extra["enc_ln/layer_norm/beta"].copy()
                    print("新增 extra 参数 {} 用 enc_ln/layer_norm/beta 的 extra 值初始化".format(var_name))
                else:
...
# 创建多兴趣推荐模型实例（根据 --use_lazy_decoder 切换 Lazy / 传统版本）
...
if is_training:
    ...
    if args.use_lazy_decoder:
        user_sid, user_sid_prob = model.beam_search_lazy(beam_size=512, temperature=1)
    else:
        user_sid, user_sid_prob = model.beam_search_fast(beam_size=512, temperature=1)
