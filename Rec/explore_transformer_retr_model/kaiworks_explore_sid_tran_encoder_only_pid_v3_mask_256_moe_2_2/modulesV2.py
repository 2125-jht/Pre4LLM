...
def moe_swiglu_feed_forward_network(dim,
                                    ...
                                    training=False):
    """
    2/2 Dense-MoE（两个专家，两个都激活）+ SwiGLU + Switch-style LB loss（带 token_mask）
    x: [B, L, D] -> out: [B, L, D]
    """
    assert num_experts == 2 and top_k == 2, "本 dense 版本固定为 2/2"
    ...
    def ffn(x, training=training, token_mask=None):
        ...
        with tf.variable_scope("moe_ffn", reuse=tf.AUTO_REUSE):
            ...
            # -------- Dense 前向：两个 expert 都算，然后按 gate 概率线性混合 --------
            x_flat = tf.reshape(x, [N, D])                                   # [N,D]
            out_e0 = expert_forward(x_flat, expert_id=0)                      # [N,D]
            out_e1 = expert_forward(x_flat, expert_id=1)                      # [N,D]

            out_e0 = tf.reshape(out_e0, [B, L, D])                            # [B,L,D]
            out_e1 = tf.reshape(out_e1, [B, L, D])                            # [B,L,D]

            g0 = gate_probs[..., 0:1]                                         # [B,L,1]
            g1 = gate_probs[..., 1:2]                                         # [B,L,1]
            out = g0 * out_e0 + g1 * out_e1                                   # [B,L,D]
