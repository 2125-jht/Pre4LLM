...
# global param
predict_server_name = "grpc_exploreGRRetrModeDeboost"
model_queue_prefix="kaiworks_explore_sid_trans_yzc_v3_256_moe_2_2"
model_dir = "predict/conf_gpu_deboostNeg"
param_json = "parameter_config.json"
PXTRS = ['user_sid_origin', "user_sid_prob", "origin_logp", "change_logp", "unClickSid"]
sign_format = "mio"
shard_offset = 0
queue_shard_num = 1
EMBEDDING_SHARDS = 1
...
class ModelRetrFlow(LeafFlow, MioApiMixin, OfflineApiMixin, UniPredictApiMixin, ExploreModelApiMixin, RetrievalApiMixin, GsuApiMixin, EmbeddingApiMixin):
            ...
            self.uni_predict_fused(
              ...
              ## embedding 拉取相关配置，对应 fetch_mio_embedding 部分
               embedding_fetchers = [dict(
                ...
                colossusdb_embd_table_name="gr_moe_2_2",
    ...
    def get_unclick_pid(self):
      return self.fetch_remote_embedding(
        ...
        input_attr_name = "uStandardExploreRealshowPhotoIdList",
        id_converter = {"type_name": "mioEmbeddingIdConverter"},
        output_attr_name = "realShowPhotoSidList",
        client_side_shard = True,
        size = 3,
      ) \
      .enrich_attr_by_lua(
        import_common_attr = ["realShowPhotoSidList"],
        function_for_common = "calculate",
        # 将 calculate 函数的返回值依次存入 export_item_attr 指定的 3 个 item_attr 中
        export_common_attr = ["realShowPhotoSidOneList", "realShowPhotoSidOneListLength"],
        lua_script = """
          function calculate()
            local unClickPhotoIdListSidOne = {}
            for i = 1, #unClickPhotoIdListSidFloatList, 3 do
                -- 检查当前组的第一个元素是否非零
                if unClickPhotoIdListSidFloatList[i] ~= 0 then
                    table.insert(unClickPhotoIdListSidOne, math.floor(unClickPhotoIdListSidFloatList[i] * 1000 + 0.5))
                end
            end
            return unClickPhotoIdListSidOne, #unClickPhotoIdListSidOne
          end
        """
      ) \
      .enrich_attr_by_lua(
        import_common_attr = ["realShowPhotoSidOneList", "realShowPhotoSidOneListLength"],
        function_for_common = "calculate",
        export_common_attr = ["originSidListLength", "top1SidLength", "top2SidLength", "top3SidLength"],
        lua_script = """
          function calculate()
            local countMap = {}
      ...
      .perflog_attr_value(
        common_attrs = [
          "originSidListLength", "top1SidLength", "top2SidLength", "top3SidLength"
        ],
        check_point="length",
      ) \
      .set_attr_value(
      ...
      .cast_attr_type(
        attr_type_cast_configs=[
          {
            "to_type": "double",
            "from_common_attr": "deboostSidList",
            "to_common_attr": "recentUnclickSidZero"
          }
        ]
      )
