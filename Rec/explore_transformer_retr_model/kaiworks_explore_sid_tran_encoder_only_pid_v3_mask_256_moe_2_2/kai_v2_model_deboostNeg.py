...
if is_training:
    ...
    recentUnclickSidZero = config.get_dense_fea("recentUnclickSidZero", dim=20, dtype=tf.int64)
    recentUnclickSidZero = tf.cast(recentUnclickSidZero, dtype=tf.int32)
    user_sid, user_sid_prob, origin_logp, change_logp, unClickSid  = model.beam_search_fast_deboostNeg(recentUnclickSidZero, beam_size=512, temperature=1)
    ...
    user_targets = [
        ...
        ("origin_logp", origin_logp),
        ("change_logp", change_logp),
        ("unClickSid", unClickSid),
    ]
    q_names, preds = zip(*user_targets)
    print("====> q_name: ", q_names)
    config.dump_predict_config('./uni_retr_server_local_ann/predict/conf_gpu_deboostNeg/', user_targets, input_type=3, extra_preds=q_names, dump_mode="user_predict")
