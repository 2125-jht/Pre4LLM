...
class MultiInterestModel(object):
    ...
    def model(self, photo_sid, label, photo_semantic_id_int):
        ...
        decoder_model = DecoderModel(num_layers=4, dim=self._dim, num_heads=8, dropout_rate=0.1, hidden_dim=int((8/3) * self._dim), training=True)
        ...
        LOAD_BALANCE_LOSS_COEF = 1e-2
        
        lb_losses = tf.get_collection("lb_loss")
        total_lb_loss = tf.add_n(lb_losses) if lb_losses else tf.constant(0.0, tf.float32)
        
        print_tensor("lb_loss", total_lb_loss)
        
        # 计算加权平均NTPloss
        total_ntp_loss = tf.reduce_sum((losses[0] + losses[1] + losses[2]) * loss_mask) / (tf.reduce_sum(loss_mask) + 1e-9)
        
        print_tensor("ntp_loss", total_ntp_loss)
        
        final_loss = total_ntp_loss + LOAD_BALANCE_LOSS_COEF * total_lb_loss
        
        return final_loss, result_dict
    ...
    def beam_search_fast(self, beam_size=512, temperature=1):
        ...
        * step>0 时保持固定 beam_size。
        ...
        decoder_model = DecoderModel(num_layers=4, dim=self._dim, num_heads=8,
                                    dropout_rate=0.1, hidden_dim=int((8/3) * self._dim))
    ...
    def beam_search_fast_more(self, beam_size=512, last_size=2048, temperature=1):
        """
        O(batch·beam·logV) 近似复杂度的束搜索（显存与 beam_size 线性）
        ...
        * step>0 时保持固定 beam_size。
        ...
        # ---------- ③ 逐层解码 ----------
        for step, V in enumerate(self._vocab_sizes):
            beam_size = last_size if step == 2 else beam_size
            # 只 embed 当前 token
            dec_in = tf.nn.embedding_lookup(self._embedding, seqs[:, :, -1])  # [B,beam,1,C]
            dec_in = tf.reshape(dec_in, [B*cur_beam, 1, self._dim])
    ...
    def beam_search_fast_deboostNeg(self, recentUnclickSidZero, beam_size=512, temperature=1):
        """
        O(batch·beam·logV) 近似复杂度的束搜索（显存与 beam_size 线性）
        ...
        * step>0 时保持固定 beam_size。
        ...
        origin_logp = tf.zeros([B, 1], dtype=tf.float32)  
        change_logp = tf.zeros([B, 1], dtype=tf.float32)  
        # ---------- ③ 逐层解码 ----------
        for step, V in enumerate(self._vocab_sizes):
            # 只 embed 当前 token
            dec_in = tf.nn.embedding_lookup(self._embedding, seqs[:, :, -1])  # [B,beam,1,C]
            ...
            def deboost_unClickSid(logp, unClickSid):
                bs, length = tf.shape(logp)[0], tf.shape(logp)[1]
                k = tf.shape(unClickSid)[1]

                indices = tf.stack([tf.tile(tf.expand_dims(tf.range(bs), 1), [1, k]), unClickSid], axis=-1)

                # 提取出需要修改的值
                values_to_modify = tf.gather_nd(logp, indices)

                # 将提取出的值乘以 0.
                modified_values = values_to_modify * 1.1

                # 创建更新的张量
                updates = tf.scatter_nd(indices, modified_values, tf.shape(logp))

                # 将更新的值应用到原始张量 a 上
                logp_updated = logp * tf.cast(tf.equal(updates, 0), logp.dtype) + updates

                return logp_updated

            logp = tf.nn.log_softmax(logits / temperature)                     # [B, cur_beam, V]
            if step == 0:
                origin_logp = tf.reshape(logp, [B, -1])
                logp = deboost_unClickSid(origin_logp, recentUnclickSidZero)
                change_logp = tf.reshape(logp, [B, -1])
                logp = tf.reshape(logp, [B, -1, self._vocab_sizes[step]])
        ...
        return gen_part_loc, probs, origin_logp, change_logp, recentUnclickSidZero