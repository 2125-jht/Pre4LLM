...
class SIDRecModel(object):
    ...
    def model(self, colossus_semantic_id_list, photo_sid, label, photo_semantic_id_int, label_click):
        """
        单次前向 + 分步mask：
        ...
        # === 常量 ===
        K_tail = 64     # 第一级短期窗口
        K_sel  = 50     # 二/三级 colossus 选择上限 便于padding
        temperature = 1.0
        eps = 1e-9
        ...
        def masked_avg(x, m):
            ...
            denom = tf.reduce_sum(m_f, axis=1, keepdims=True) + eps
        ...
        valid_cnt = tf.reduce_sum(tf.cast(step_mask, tf.int32))
        pos_cnt = tf.reduce_sum(tf.cast(pos_mask * step_mask, tf.int32))
        neg_cnt = tf.reduce_sum(tf.cast(neg_mask * step_mask, tf.int32))
        print_tensor("label_count/valid_total", valid_cnt)
        print_tensor("label_count/poslabel", pos_cnt)
        print_tensor("label_count/neglabel", neg_cnt)
        ...
        def ce_loss(logits_i, y_i, w_i):
            ...
            denom = tf.reduce_sum(w_i) + eps
            return tf.reduce_sum(loss_i * w_i) / denom

        # 对每个语义层级分别计算损失
        pos_den = tf.reduce_sum(pos_mask * step_mask) + eps    # 正样本分母（按有效样本计）
        neg_den = tf.reduce_sum(neg_mask * step_mask) + eps    # 负样本分母（按有效样本计）
        
        loss_pos0 = ce_loss(logits[0], label[:, 0], pos_mask * step_mask)
        loss_pos1 = ce_loss(logits[1], label[:, 1], pos_mask * step_mask)
        loss_pos2 = ce_loss(logits[2], label[:, 2], pos_mask * step_mask)
        loss_pos = loss_pos0 + loss_pos1 + loss_pos2
        
        # 负样本：仅第 0 级使用 -log(1-p)，其他级 0
        # 计算第一级
        prob = tf.nn.softmax(logits[0], axis=-1)
        batch_idx = tf.range(tf.shape(prob)[0], dtype=tf.int32)
        idx = tf.stack([batch_idx, label[:, 0]], axis=1)
        true_p = tf.gather_nd(prob, idx)
        loss_neg0 = -tf.math.log(tf.clip_by_value(1.0 - true_p, eps, 1.0))
        loss_neg0 = tf.reduce_sum(loss_neg0 * neg_mask * step_mask) / neg_den
        # 后面两级置0
        loss_neg1 = tf.zeros_like(loss_neg0)
        loss_neg2 = tf.zeros_like(loss_neg0)
        
        # 混合正/负样本 loss
        loss = loss_pos + alpha * loss_neg0
        
        # debug
        print_tensor("loss_pos/loss", loss_pos)
        ...
        def log_probs_and_recall(logits_i, y_i, w_i, name):
            ...
            print_tensor(f"probs/true_token_prob_{name}",
                        tf.reduce_sum(true_p * w_i) / (tf.reduce_sum(w_i) + eps))
            print_tensor(f"probs_pos/true_token_prob_{name}",
                        tf.reduce_sum(true_p * pos_mask * w_i) / pos_den)
            print_tensor(f"probs_neg/true_token_prob_{name}",
                        tf.reduce_sum(true_p * neg_mask * w_i) / neg_den)

            # 打印max probs
            max_probs, _ = tf.nn.top_k(prob, k=1)
            print_tensor(f"probs/max_token_prob_{name}",
                        tf.reduce_sum(tf.squeeze(max_probs, -1) * w_i) / (tf.reduce_sum(w_i) + eps))
            print_tensor("probs_pos/max_token_prob_{name}",
                        tf.reduce_sum(tf.squeeze(max_probs, -1) * pos_mask * w_i) / pos_den)
            print_tensor("probs_neg/max_token_prob_{name}",
                        tf.reduce_sum(tf.squeeze(max_probs, -1) * neg_mask * w_i) / neg_den)
            
            # === recall：整体 + 正样本 + 负样本 ===
            recall_at_k(logits_i, y_i, w_i, self._print_ops,
                            top_k=[1, 16, 128], name=f"predict_recall_{name}")
            recall_at_k(logits_i, y_i, pos_mask * w_i, self._print_ops,
                            top_k=[1, 16, 128], name=f"predict_recall_pos_{name}")
            recall_at_k(logits_i, y_i, neg_mask * w_i, self._print_ops,
                            top_k=[1, 16, 128], name=f"predict_recall_neg_{name}")

        with tf.variable_scope('step_0'):
            print_tensor("loss_pos/loss_0", loss_pos0)
            print_tensor("loss_neg/loss_0", loss_neg0)
            log_probs_and_recall(logits[0], label[:, 0], step_mask, "0")
        with tf.variable_scope('step_1'):
            print_tensor("loss_pos/loss_1", loss_pos1)
            print_tensor("loss_neg/loss_1", loss_neg1)
            log_probs_and_recall(logits[1], label[:, 1], step_mask, "1")
        with tf.variable_scope('step_2'):
            print_tensor("loss_pos/loss_2", loss_pos2)
            print_tensor("loss_neg/loss_2", loss_neg2)
            log_probs_and_recall(logits[2], label[:, 2], step_mask, "2")

        return loss

    def beam_search_fast(self, colossus_semantic_id_list,
