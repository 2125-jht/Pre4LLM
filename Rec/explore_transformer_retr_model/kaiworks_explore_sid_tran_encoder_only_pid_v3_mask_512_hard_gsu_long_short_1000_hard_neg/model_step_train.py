...
class SIDRecModel(object):
    ...
    def model(self, click_semantic_id_list, colossus_semantic_id_list, photo_sid, label, photo_semantic_id_int):
        """
        训练前向（token-by-token + KV cache）：
        step0: enc=user_static+click(200)，但跨注意力 mask 为“长(200)/短(64)”逐样本二选一
        step1/2: enc=user_static+colossus_sel(≤200，与 label[0] 同类 & 最近)
        """
        # === 常量/工具 ===
        num_heads = 8
        ...
        def expand_src_mask(mask_BxS):
            return tf.reshape(mask_BxS, [tf.shape(mask_BxS)[0], 1, 1, tf.shape(mask_BxS)[1]])
        ...
        # click 有效位
        click_mask_left = tf.sequence_mask(click_used_len, maxlen=click_max_len, dtype=tf.int8)  # 左对齐
        click_mask = tf.reverse(click_mask_left, axis=[1])                                       # 与 emb 对齐

        # === 2.1 为 step0 准备“长/短”两种 mask，并按相似度逐样本选择 ===
        Lc = tf.shape(user_click_sid)[1]
        pos = tf.expand_dims(tf.range(Lc, dtype=tf.int32), 0)                 # [1,Lc]
        tail64 = tf.cast(pos >= (Lc - K_tail), tf.int8)                       # [1,Lc]
        click_mask_short = tf.bitwise.bitwise_and(click_mask, tail64)         # [B,Lc]
        ...
        pool_long  = masked_avg(click_sid_emb, click_mask)
        pool_short = masked_avg(click_sid_emb, click_mask_short)

        label_lvl1 = tf.cast(label[:, 0], tf.int32)
        label_emb0 = tf.nn.embedding_lookup(self._vocab_embedding, label_lvl1)     # [B,C]

        def cos(a, b):
            return tf.reduce_sum(tf.nn.l2_normalize(a, -1) * tf.nn.l2_normalize(b, -1), axis=-1)

        bias = 0.0  # 调整偏向于short的程度 例如设置为0.02
        sim_long  = cos(label_emb0, pool_long)                 # [B]
        sim_short = cos(label_emb0, pool_short) + bias         # [B]
        ...
        cond = tf.broadcast_to(choose_short[:, None, None], tf.shape(pos0_long))
        pos0_click = tf.where(cond, pos0_short, pos0_long)        # [B,1,1+Lc]

        # debug
        with tf.variable_scope('choose_sim'):
            print_tensor("long", sim_long)
            print_tensor("short", sim_short)
        ...
        # === 3) Decoder 输入（teacher forcing：只用到 BOS, y0, y1） ===
        start_tok = tf.fill([B, 1], self._total_vocab_size)            # [B,1]
        dec_in_ids = tf.concat([start_tok, photo_sid[:, :2]], axis=1)  # [B,3]
        dec_in_emb = tf.nn.embedding_lookup(self._vocab_embedding, dec_in_ids)  # [B,3,C]
        ...
        # === 5) STEP 0：enc=user_static+click(200)，但 mask=pos0_click(长/短二选一) ===
        enc0 = tf.concat([user_static_emb, user_click_emb], axis=1)       # [B,1+Lc,C]
        src_mask0 = expand_src_mask(pos0_click[:, 0, :])                  # [B,1,1,1+Lc]
        
        x0 = dec_in_emb[:, 0:1, :]                                        # BOS
        y0, cache = decoder_model.step(x0, 1, enc0, src_mask0, cache)     # 保留 self-KV
        h0 = y0[:, 0, :]
        with tf.variable_scope('proj_0'):
            logits0 = tf.layers.dense(h0, self._vocab_sizes[0], name='pred')  # [B,V0]

        # === 6) 准备 colossus（仅用于 step1/2）===
        ...
        idx = tf.cast(tf.range(Lcol)[None, :], tf.float32)  # 越大越新
        scores_sel = tf.cast(cond_mask, tf.float32) * idx + (1.0 - tf.cast(cond_mask, tf.float32)) * (-1e9)
        top_vals, top_idx = tf.nn.top_k(scores_sel, k=K_sel)                # [B,K_sel]

        sel_fea = tf.batch_gather(user_colossus_fea, top_idx)               # [B,K_sel,Fea]
        ...
        enc1 = tf.concat([user_static_emb, sel_emb], axis=1)                # [B,1+K_sel,C]
        src_mask1 = expand_src_mask(tf.concat([user_tok_mask, sel_present], axis=1))  # [B,1,1,1+K_sel]

        # 预计算 enc-KV（对齐 DecoderLayer 命名）
        def split_heads(x, h):
            depth = x.get_shape().as_list()[-1]
            Dh_ = depth // h
            reshaped = tf.reshape(x, [B, -1, h, Dh_])   # [B,L,H,Dh]
        ...
        for i in range(decoder_model.num_layers):
            with tf.variable_scope(f"decoder_layer_{i}/multi_head_attention", reuse=tf.AUTO_REUSE):
                k_enc_lin = tf.layers.dense(enc1, self._dim, use_bias=False, name="w_k")
                v_enc_lin = tf.layers.dense(enc1, self._dim, use_bias=False, name="w_v")
            cache[f"k_enc_{i}"] = split_heads(k_enc_lin, num_heads)[:, None, ...]  # [B,1,H,L,Dh]
            cache[f"v_enc_{i}"] = split_heads(v_enc_lin, num_heads)[:, None, ...]  # [B,1,H,L,Dh]

        # === 7) step1 / step2（复用 step0 的 self-KV，替换 enc-KV）===
        x1 = dec_in_emb[:, 1:2, :]                               # 输入 y0（teacher forcing）
        y1, cache = decoder_model.step(x1, 1, enc1*0.0, src_mask1, cache)
        h1 = y1[:, 0, :]
        with tf.variable_scope('proj_1'):
            logits1 = tf.layers.dense(h1, self._vocab_sizes[1], name='pred')

        x2 = dec_in_emb[:, 2:3, :]                               # 输入 y1（teacher forcing）
        y2, cache = decoder_model.step(x2, 1, enc1*0.0, src_mask1, cache)
        h2 = y2[:, 0, :]
        with tf.variable_scope('proj_2'):
            logits2 = tf.layers.dense(h2, self._vocab_sizes[2], name='pred')

        # === 8) 损失（逐 step 掩码）===
        # step_mask = tf.cast(photo_semantic_id_int > 0, tf.float32)   # [B]
        step_mask = tf.where(
        ...
        def ce_loss(logits, y, w):
            oh = tf.one_hot(y, tf.shape(logits)[-1])
            loss_i = tf.nn.softmax_cross_entropy_with_logits(labels=oh, logits=logits/temperature)
            num = tf.reduce_sum(w) + 1e-9
            return tf.reduce_sum(loss_i * w) / num

        loss0 = ce_loss(logits0, label[:, 0], step_mask)
        loss1 = ce_loss(logits1, label[:, 1], step_mask)
        loss2 = ce_loss(logits2, label[:, 2], step_mask)
        loss = loss0 + loss1 + loss2

        # 调试
        # 计算enc的余弦相似度
        enc0_sim = tf.reshape(enc0, [B, -1])
        enc1_sim = tf.reshape(enc1, [B, -1])
        print_tensor("enc_sim/enc0", calc_sim_cos(enc0_sim))
        print_tensor("enc_sim/enc1", calc_sim_cos(enc1_sim))
        # 计算dec out的余弦相似度
        print_tensor("dec_sim/dec_out0", calc_sim_cos(h0))
        print_tensor("dec_sim/dec_out1", calc_sim_cos(h1))
        print_tensor("dec_sim/dec_out2", calc_sim_cos(h2))

        # 监控
        def log_probs_and_recall(logits, y, w, name):
            prob = tf.nn.softmax(logits, axis=-1)
            batch_idx = tf.range(tf.shape(prob)[0], dtype=tf.int32)
            idx = tf.stack([batch_idx, y], axis=1)
            true_p = tf.gather_nd(prob, idx)
            print_tensor(f"probs/true_token_prob_{name}",
                        tf.reduce_sum(true_p * w) / (tf.reduce_sum(w) + 1e-9))
            max_probs, _ = tf.nn.top_k(prob, k=1)
            print_tensor(f"probs/max_token_prob_{name}",
                        tf.reduce_sum(tf.squeeze(max_probs, -1) * w) / (tf.reduce_sum(w) + 1e-9))
            recall_at_k(logits, y, w, self._print_ops, top_k=[1, 16, 128], name=f"predict_recall_{name}")

        with tf.variable_scope('step_0'):
            print_tensor("loss/loss_0", loss0)
            log_probs_and_recall(logits0, label[:, 0], step_mask, "0")
        with tf.variable_scope('step_1'):
            print_tensor("loss/loss_1", loss1)
            log_probs_and_recall(logits1, label[:, 1], step_mask, "1")
        with tf.variable_scope('step_2'):
            print_tensor("loss/loss_2", loss2)
            log_probs_and_recall(logits2, label[:, 2], step_mask, "2")
    ...
    def beam_search_fast(self, click_semantic_id_list, colossus_semantic_id_list,
        ...
        # click（最新->最旧）反转为（最旧->最新）
        user_click_fea = tf.concat([self._feature_emb_dict[fea] for fea in user_click_fea_names], axis=2)  # [B,Lc,F]
        click_max_len = 200
        click_raw_len = tf.cast(self._feature_emb_size_dict['user_profile_v1_click_pid_list'], tf.int32)
        ...
        mask_long  = tf.concat([user_tok_mask, click_mask],       axis=1)    # [B,1+Lc]
