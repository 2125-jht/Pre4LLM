...
if args.with_kai_v2:
    ...
    def filter_mask_wrapper(dataset):
        ...
        dataset.add_feature('context_info__click', dataset.DENSE, tf.int64, 1)
        ...
        def mask_fn(batch):
            ...
            label_click             = tf.cast(batch['context_info__click'], tf.float32)
            ...
            pos_signal = label_play_over_3s*(label_play_over_7s+label_finish+label_like+label_follow+label_comment+label_collect+label_download+label_profile_enter)
            pos_flag = tf.cast(pos_signal > 0.0, tf.float32)
            
            # 负样本：未点击
            neg_flag = tf.where(tf.less(label_click, 1.0), tf.ones_like(label_click), tf.zeros_like(label_click))
            
            # 采样负样本           
            keep_p = tf.convert_to_tensor(0.1, tf.float32)    # 固定保留概率
            shape = tf.shape(neg_flag)
            rnd = tf.random.uniform(shape, dtype=tf.float32)
            neg_keep_mask  = tf.cast(rnd < keep_p, tf.float32)
            neg_sampled    = neg_flag * neg_keep_mask                       # 仅保留一部分负样本

            # === 形成最终“被选中参与训练”的标记 ===
            # 规则：正样本必选；负样本按采样结果；其余不选
            selected = tf.maximum(pos_flag, neg_sampled)                    # ∈{0,1}

            mask = tf.less(selected, 1.0)
...
if is_training:
    ...
    # === 获取label的标记 ===
    label_click = config.get_dense_fea("context_info__click", dim=1, dtype=tf.int64)

    # === 获取语义ID相关特征 ===
    colossus_photo_semantic_id_list = config.get_dense_fea("colossus_photo_semantic_id_list", dim=1000, dtype=tf.int64) # [B, 1000]
    ...
    # === 模型前向传播 ===
    print("=== 模型前向传播 ===")
    loss = model.model(colossus_photo_semantic_id_list, photo_semantic_id, label, photo_semantic_id_int, label_click)  # 计算训练损失
    ...
    colossus_semantic_id_list = config.get_dense_fea("colossus_photo_semantic_id_list", dim=3000, dtype=tf.float32) * 1000 # [B, 1000]
    colossus_semantic_id_list = tf.reshape(colossus_semantic_id_list, [-1, 3, 1000])[:, 0, ]
    colossus_semantic_id_list = tf.cast(colossus_semantic_id_list, dtype=tf.int32)

    user_sid, user_sid_prob = model.beam_search_fast(colossus_semantic_id_list, beam_size=512, temperature=1)
