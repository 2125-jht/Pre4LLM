...
user_click_fea_names = [
    "user_colossus_pid_list",  # 用户点击的视频ID列表
...
class MultiInterestModel(object):
    ...
    def __init__(self, feature_emb_dict, feature_emb_size_dict, dim=128, selected_size=200, vocab_sizes=[8192, 8192, 8192], print_ops=None):
        """
        初始化多兴趣模型
    ...
    def model(self, photo_sid, label, photo_semantic_id_int):
        ...
        raw_len   = tf.cast(
            self._feature_emb_size_dict['user_colossus_pid_list'],
        ...
        def crop_tail_right_pad(seq, valid_len, max_len, pad_value=-0.0, name="crop_tail_right_pad"):
            """
            从每条序列 seq[b, :, :] 中取最后 min(valid_len[b], max_len) 个位置；
            输出右对齐长度 max_len，不足左补 pad_value。

            Args:
                seq:        [B, L, D] float32 (或其它) tensor
                valid_len:  [B] 或 [B,1] int32 tensor, 每条序列真实有效长度 (<= L)
                max_len:    int python 标量，要保留的最大最近长度
                pad_value:  float 常量，用于左侧 padding

            Returns:
                out:       [B, max_len, D] tensor
                used_len:  [B] int32，每条序列保留的真实长度 (已截断到 max_len)
            """
            with tf.name_scope(name):
                ...
                used_len  = tf.minimum(valid_len, max_len)                      # [B]
                B         = tf.shape(valid_len)[0]
                L         = tf.shape(seq)[1]

                # j: [max_len]
                j = tf.range(max_len, dtype=tf.int32)[tf.newaxis, :]            # [1, max_len]

                # src_idx_raw[b,j] = valid_len[b] + j - max_len
                src_idx_raw = valid_len[:, tf.newaxis] + j - max_len            # [B, max_len]

                # clip 到 [0, L-1]，避免 gather 越界；被 clip 的 <0 位置稍后会被 pad 掉
                src_idx = tf.clip_by_value(src_idx_raw, 0, L - 1)

                # 构造 batch 维索引
                batch_ids = tf.range(B, dtype=tf.int32)[:, tf.newaxis]          # [B,1]
                batch_ids = tf.tile(batch_ids, [1, max_len])                    # [B, max_len]

                gather_idx = tf.stack([batch_ids, src_idx], axis=-1)            # [B, max_len, 2]
                gathered = tf.gather_nd(seq, gather_idx)                        # [B, max_len, D]

                # keep_mask: True 表示该位置对应有效 token
                # 等价条件：src_idx_raw >= 0 且 j >= (max_len - used_len)
                # 直接用 src_idx_raw >= 0 即可；因为 src_idx_raw < 0 的都应 pad。
                keep_mask = src_idx_raw >= 0                                    # [B, max_len]
                keep_mask = tf.cast(keep_mask, seq.dtype)                       # float

                pad_tensor = tf.fill([B, max_len, D], tf.cast(pad_value, seq.dtype))
                out = keep_mask[:, :, tf.newaxis] * gathered + (1.0 - keep_mask[:, :, tf.newaxis]) * pad_tensor

                return out, used_len
        
        # === 2.2 仅保留“有效序列的最后 selected_size 条”，不足左补 -1 ===
        user_click_fea, used_len = crop_tail_right_pad(
            user_click_fea,
            valid_len=valid_len,
            max_len=max_len,
            pad_value=-0.0,
            name="user_click_recent_vec"
        )   # [B, max_len, dim], used_len:[B]
        # 通过MLP处理点击特征
        user_click_emb = mlp('user_click_emb', user_click_fea, [4*self._dim], self._dim, activation=tf.nn.leaky_relu)
        # user_click_emb = user_click_emb[:, :max_len, :]
