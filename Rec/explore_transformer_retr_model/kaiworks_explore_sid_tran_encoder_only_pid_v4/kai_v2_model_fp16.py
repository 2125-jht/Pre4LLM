    ...
    for attr in all_features:
        ...
        if not is_training:
            ...
            if not attr.expand:
                attr.expand = 1
            if attr.is_common:
                embed, size_var = config.new_embedding(attr.attr_name, dim=attr.dim, slots=attr.slots, expand=attr.expand, compress_group="USER", sized=True, dtype=tf.float16, trainable=False)
            else:
            ...
            feature_emb_dict[attr.attr_name] = config.new_embedding(attr.attr_name, dim=attr.dim, slots=attr.slots, expand=attr.expand, dtype=tf.float16, trainable=False)
...
if is_training:
    ...
    user_sid, user_sid_prob = model.beam_search_fast(beam_size=128, temperature=1)
    ...
    user_sid_prob = tf.reshape(user_sid_prob, [-1, tf.shape(user_sid_prob)[1] * tf.shape(user_sid_prob)[2]], tf.float32)
