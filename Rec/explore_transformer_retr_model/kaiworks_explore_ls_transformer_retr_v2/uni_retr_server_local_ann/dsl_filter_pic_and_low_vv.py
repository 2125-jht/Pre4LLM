...
import base64
import collections
from ann_quality_merge import *
current_dir = os.path.dirname(__file__)
sys.path.append('/home/liyunhao/data/ks/dragon/tools/pypi/')

# global param
predict_server_name = "grpc_KaiWorksExploreLSTFilterLowQualityAndDay"
...
class ModelRetrFlow(LeafFlow, MioApiMixin, OfflineApiMixin, UniPredictApiMixin, ExploreModelApiMixin, RetrievalApiMixin, GsuApiMixin):
    ...
    def filter_by_video_quality_score(self):
      ...
      .get_local_ann_embedding(
        src_data_type = "photo_cover",
        dim = 1,
        embedding_item_attr = "video_cover_score_list"
      ) \
      ...
      .enrich_attr_by_lua(
        import_item_attr = ["video_quality_score_list", "video_cover_score_list"],
        function_for_item = "calculate",
        export_item_attr = ["video_quality_score", "video_cover_score"],
        lua_script = """
          function calculate()
            return video_quality_score_list[1], video_cover_score_list[1]
          end
        """
      ) \
      .log_debug_info(
...
service.build(output_file=os.path.join(model_dir, "dynamic_json_config_flow_video_low_vv.json"))
