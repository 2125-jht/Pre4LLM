...
def gen_custom_label():
    ...
    # 质量分label处理逻辑
    label_dislike           = config.get_dense_fea("context_info__dislike", dim=1, dtype=tf.int64)
    sense_photo             = config.get_dense_fea("photo_info__content_safety_level_with_namespace__level_hot_online", dim=1, dtype=tf.int64)
    sense_photo_second      = config.get_dense_fea("photo_info__audit_b_second_tag", dim=1, dtype=tf.int64)
    tab                     = tf.cast(config.get_dense_fea("tab", dim=1, dtype=tf.int64), dtype=tf.float32)
    tab_mask                = tf.where(tf.equal(tab, 0.0), tf.ones_like(tab), tf.zeros_like(tab))
    # 观感优 or 观感高热优
    pos_condition           = tf.logical_or(tf.equal(sense_photo, 4), tf.equal(sense_photo, 5))

    label_sense_photo       = tf.where(pos_condition, tf.ones_like(sense_photo), tf.zeros_like(sense_photo))
    label_sense_photo       = tf.cast(label_sense_photo, dtype=tf.float32)

    cover_level             = config.get_dense_fea("photo_info__audit_hot_cover_level", dim=1, dtype=tf.int64)
    cover_pos               = tf.equal(cover_level, 2023740)
    cover_neg_1             = tf.logical_or(tf.equal(cover_level, 2023742), tf.equal(cover_level, 2023743))
    cover_neg_2             = tf.logical_or(tf.equal(cover_level, 2023744), tf.equal(cover_level, 2023745))
    cover_neg_3             = tf.logical_or(tf.equal(cover_level, 2023746), tf.equal(cover_level, 2231037))
    cover_neg               = tf.logical_or(tf.logical_or(cover_neg_1, cover_neg_2), cover_neg_3)
    label_cover             = tf.where(cover_pos, tf.ones_like(cover_level, dtype=tf.float32), 0.5 * tf.ones_like(cover_level, dtype=tf.float32))
    label_cover             = tf.where(cover_neg, tf.zeros_like(cover_level, dtype=tf.float32), label_cover)
    label_cover             = tf.cast(label_cover, dtype=tf.float32)
    tf.summary.histogram("quality/label_cover", label_cover)

    return label_click, label_sense_photo, label_cover, tab_mask
...
if is_training:
    ...
    print(f"====> train, gen...")
    # [batch_size, history_size, embedding_dim]
    ...
    total_num = tf.reduce_sum(tf.cast(feature_emb_size_dict["user_colossus_pid_list"], dtype=tf.float32))
    log_tensor("feature/colossus_pid_length", feature_emb_size_dict["user_colossus_pid_list"])
    log_tensor("feature/colossus_channel_length", feature_emb_size_dict["user_colossus_aid_list"])
    log_tensor("feature/colossus_channel_length", feature_emb_size_dict["user_colossus_channel_list"])
    channel_dense = tf.cast(config.get_dense_fea("colossus_channel_list", dim=1000, dtype=tf.int64), dtype=tf.float32)
    playtime_dense = tf.cast(config.get_dense_fea("colossus_play_time_list", dim=1000, dtype=tf.int64), dtype=tf.float32)
    duration_dense = tf.cast(config.get_dense_fea("colossus_duration_list", dim=1000, dtype=tf.int64), dtype=tf.float32)
    label_dense = tf.cast(config.get_dense_fea("colossus_label_list", dim=1000, dtype=tf.int64), dtype=tf.float32)
    print_tensor("feature/channel", channel_dense)
    print_tensor("feature/playtime", playtime_dense)
    print_tensor("feature/duration", duration_dense)
    print_tensor("feature/label", label_dense)
    tf.summary.scalar("feature/channel_without_zero", tf.reduce_mean(channel_dense) * batch_size * 1000.0 / total_num)
    tf.summary.scalar("feature/playtime_without_zero", tf.reduce_mean(playtime_dense) * batch_size * 1000.0 / total_num)
    tf.summary.scalar("feature/duration_without_zero", tf.reduce_mean(duration_dense) * batch_size * 1000 / total_num)
    tf.summary.scalar("feature/label_without_zero", tf.reduce_mean(label_dense) * batch_size * 1000 / total_num)

    all_param_dict["photo_mmu_embedding"] = config.get_dense_fea("photo_mmu_embedding", dim=64, dtype=tf.float32)
    
    user_top_embedding, photo_top_embedding = model._multi_interest_encode_v2(photo_semantic_id, label, photo_semantic_id_int)  # 计算训练损失
    ...
    with tf.control_dependencies(print_ops):
        targets = []
        label, *_ = gen_custom_label() 

        output = sigmoid_layer("ctr_click", user_top_embedding, photo_top_embedding)
        targets.append(("click", output, label, tf.ones_like(label), "auc"))
        
        # print(tf.shape(user_top_embedding), tf.shape(photo_top_embedding))
        
        loss_main, cos_mat = sampled_softmax_loss(label, user_top_embedding, photo_top_embedding, logQ=None, name="click")
    ...
    with tf.control_dependencies(print_ops):
        targets = []
        label, *_ = gen_custom_label()  # 生成自定义标签
        ...
        # ###
        # label, label_sense_photo, label_cover, tab_mask = gen_custom_label()
        # if args.with_kai_v2:
        #     config.set_feature_score_attr("explore_click_label", data_source_name="train")

        # print_tensor("label", label)
        # targets = []
        # losses = {}
        # #################################### logQ 热度消偏 ####################################
    ...
    if args.dryrun:
        ...
        interest_embedding, readout, photo_emb, _, indices = model._multi_interest_encode_v2()
        all_param_dict["photo_mmu_embedding"] = tf.zeros_like(photo_emb)
        photo_quality_score = model._photo_score_module()
        print("outside model order:")
        user_emb = tf.reshape(interest_embedding, [-1, interest_embedding.shape[1] * interest_embedding.shape[2]])
        user_targets = [("user_emb", user_emb)]
        q_names, preds = zip(*user_targets)
        print("====> q_name: ", q_names)
        photo_emb = tf.tile(photo_emb, [1, model._num_interest])
        photo_targets = [("photo_top_layer", photo_emb)]
        config.dump_predict_config('./uni_retr_server_local_ann/predict/conf', user_targets, input_type=3, extra_preds=q_names, dump_mode="user_predict")
        config.dump_predict_config('./uni_retr_server_local_ann/predict_photo/conf', photo_targets, input_type=3,
                                   extra_preds=q_names, dump_mode="photo_predict")
