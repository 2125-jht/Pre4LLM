    ...
    def filter_mask_wrapper(dataset):
        ...
        def mask_fn(batch):
            ...
            label_finish = tf.where(
                tf.greater_equal(playing_time, duration_ms),
                tf.ones_like(playing_time),
                tf.zeros_like(playing_time)
            )

            # 是否播放超过3秒
            label_play_over_3s = tf.where(
                tf.greater_equal(playing_time, 3000),
                tf.ones_like(playing_time),
                tf.zeros_like(playing_time)
            )
            ...
            label_play_over_7s = tf.where(
                tf.greater_equal(playing_time, 7000),
                tf.ones_like(playing_time),
                tf.zeros_like(playing_time)
            )
...
if is_training:
    ...
    user_play_semantic_id_list_int = config.get_dense_fea("colossus_photo_semantic_id_list", dim=1000, dtype=tf.int64)
    ...
    loss = model.model(user_play_semantic_id_list_int_last, photo_semantic_id, label, photo_semantic_id_int)
    ...
    user_sid, user_sid_prob = model.beam_search_v2()
    user_sid = user_sid[:, :, 1:]
    user_sid_prob = user_sid_prob[:, :, 1:]
