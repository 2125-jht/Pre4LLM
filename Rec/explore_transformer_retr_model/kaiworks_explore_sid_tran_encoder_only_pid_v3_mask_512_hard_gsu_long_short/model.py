...
class SIDRecModel(object):
    ...
    def model(self, click_semantic_id_list, colossus_semantic_id_list, photo_sid, label, photo_semantic_id_int):
        ...
        # === 2.1 第一级 长/短 200/64 二选一（按与 label[0] 的相似度） ===
        ...
        # # debug
        # print_tensor("valid_colossus_sid", tf.reduce_sum(tf.cast(col_valid, tf.int32), axis=1))
        # print_tensor("select_colossus", tf.reduce_sum(tf.cast(cond_mask, tf.int32), axis=1))
