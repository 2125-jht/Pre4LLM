...
# 训练时用最近多少个点击样本扩展 sid0 set-label，同时从 user click sequence 头部裁掉这些样本防止泄漏
NUMB_TEST_PHOTO = 10
...
if is_training:
    ...
    print("=== sid0 set-list numb_test_photo: {} ===".format(numb_test_photo))
    # === 获取基础特征 ===
    ...
    sid0_set_labels, sid0_set_label_mask = buildSid0SetLabels(
        photo_semantic_id_int,
        user_profile_v1_click_semantic_id_list_int,
        numb_test_photo
    )
    ...
    loss, result_dict = model.model(
        ...
        label,
        photo_semantic_id_int,
        sid0_set_labels=sid0_set_labels,
        sid0_set_label_mask=sid0_set_label_mask,
        numb_test_photo=numb_test_photo
    )  # 计算训练损失
    ...
    user_sid, user_sid_prob = model.beam_search_fast_share_head(beam_size=500, temperature=1)
