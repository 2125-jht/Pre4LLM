...
def recall_at_k_multi_label(predict, multi_hot_label, label_count, loss_mask,
                            print_ops, top_k=[16, 128, 256], name="default",
                            min_top_k_upper_bound=1):
    """
    多 label / set-list 场景的 Recall@K。

    Args:
        predict: [B, V] logits
        multi_hot_label: [B, V]，去重后的 multi-hot 标签
        label_count: [B]，每条样本去重后的有效 label 数
        loss_mask: [B]，有效样本 mask
        top_k: 固定需要统计的 K
        name: TensorBoard 名称前缀
        min_top_k_upper_bound: “最小 label 数” recall 可能取到的上界，用于一次性取够 top-k
    """
    predict = tf.convert_to_tensor(predict)
    multi_hot_label = tf.cast(multi_hot_label, tf.float32)
    label_count = tf.cast(label_count, tf.float32)
    loss_mask = tf.cast(loss_mask, tf.float32)

    max_eval_k = max(top_k + [max(1, int(min_top_k_upper_bound))])
    top_k_indices = tf.nn.top_k(predict, k=max_eval_k).indices  # [B, max_eval_k]

    batch_size = tf.shape(top_k_indices)[0]
    batch_idx = tf.tile(tf.expand_dims(tf.range(batch_size), 1), [1, max_eval_k])
    gather_idx = tf.stack([batch_idx, top_k_indices], axis=2)
    hit_matrix = tf.gather_nd(multi_hot_label, gather_idx)  # [B, max_eval_k]

    denom = tf.maximum(label_count, 1.0)

    valid_label_count = tf.boolean_mask(tf.cast(label_count, tf.int32), loss_mask > 0)
    min_label_count = tf.cond(
        tf.size(valid_label_count) > 0,
        lambda: tf.maximum(tf.reduce_min(valid_label_count), 1),
        lambda: tf.constant(1, dtype=tf.int32)
    )
    mean_label_count = tf.cond(
        tf.size(valid_label_count) > 0,
        lambda: tf.reduce_mean(tf.cast(valid_label_count, tf.float32)),
        lambda: tf.constant(1.0, dtype=tf.float32)
    )
    tf.summary.scalar("top_k/{}_min_label_count".format(name), mean_label_count)
    tf.summary.scalar("top_k/{}_batch_min_label_count".format(name), tf.cast(min_label_count, tf.float32))

    dynamic_k = tf.maximum(tf.cast(label_count, tf.int32), 1)
    dynamic_k = tf.minimum(dynamic_k, max_eval_k)
    min_k_mask = tf.sequence_mask(dynamic_k, maxlen=max_eval_k, dtype=tf.float32)
    min_hit = tf.reduce_sum(hit_matrix * min_k_mask, axis=1)
    min_recall = min_hit / denom
    min_recall_value = tf.reduce_sum(min_recall * loss_mask) / (tf.reduce_sum(loss_mask) + 0.001)
    tf.summary.scalar("top_k/{}_min".format(name), min_recall_value)

    for k in top_k:
        hit_at_k = tf.reduce_sum(hit_matrix[:, :k], axis=1)
        recall_at_k_value = tf.reduce_sum((hit_at_k / denom) * loss_mask) / (tf.reduce_sum(loss_mask) + 0.001)
        tf.summary.scalar("top_k/{}_{}".format(name, k), recall_at_k_value)
