...
def processListLabel(input_tensor):
    a = tf.bitwise.right_shift(input_tensor, 30)
    a = tf.bitwise.bitwise_and(a, 0x7FFF)
...
def buildSid0SetLabels(photo_semantic_id_int, click_semantic_id_list_int, numb_test_photo):
    """构造 sid0 的 set-label 监督，同时返回有效 label mask。"""
    numb_test_photo = max(0, int(numb_test_photo))

    target_sid0 = processLabel(photo_semantic_id_int)[:, :1]
    target_mask = tf.cast(photo_semantic_id_int > 0, dtype=tf.float32)

    if numb_test_photo == 0:
        return target_sid0, target_mask

    extra_click_int = click_semantic_id_list_int[:, :numb_test_photo]
    extra_sid0 = processListLabel(extra_click_int)
    extra_mask = tf.cast(extra_click_int > 0, dtype=tf.float32)

    sid0_set_labels = tf.concat([target_sid0, extra_sid0], axis=1)
    sid0_set_label_mask = tf.concat([target_mask, extra_mask], axis=1)
    return sid0_set_labels, sid0_set_label_mask


def trimRecentClicks(click_tensor, valid_len, numb_test_photo):
    """去掉最前面的最近点击，返回截断后的序列和对应有效长度。"""
    numb_test_photo = max(0, int(numb_test_photo))
    if numb_test_photo == 0:
        return click_tensor, valid_len

    trimmed_click_tensor = click_tensor[:, numb_test_photo:, :]
    trimmed_valid_len = tf.maximum(valid_len - numb_test_photo, 0)
    return trimmed_click_tensor, trimmed_valid_len
