...
from util import trimRecentClicks
...
class MultiInterestModel(object):
    ...
    def __init__(self, feature_emb_dict, feature_emb_size_dict, dim=256, vocab_sizes=[8192, 8192, 8192], print_ops=None):
        ...
        self._query_token_numb=32
        self._stage_one_dim=dim
        self._stage_two_dim=dim
        self._dim = dim
        self._start_token_id = self._total_vocab_size + self._query_token_numb
    ...
    def model(self, photo_sid, label, photo_semantic_id_int,
              sid0_set_labels=None, sid0_set_label_mask=None, numb_test_photo=0):
        """
        主训练模型前向传播
        ...
        Args:
            ...
            sid0_set_labels: sid0 的 set-list 标签，shape=[batch_size, numb_test_photo+1]
            sid0_set_label_mask: sid0 的 set-list 有效mask，shape=[batch_size, numb_test_photo+1]
            numb_test_photo: 训练时从点击序列头部截掉的样本数，用于防止数据泄漏
        ...
        user_click_fea, used_len = trimRecentClicks(user_click_fea, used_len, numb_test_photo)
        max_len = tf.shape(user_click_fea)[1]
        ...
        # === 4. 构建编码器输入：query token 前置 ===
        encoder_input = tf.concat([query_token_emb, user_static_emb, user_click_emb], axis=1)

        encoder_input_sim = tf.reshape(encoder_input[:,self._query_token_numb:,:], [batch_size, -1])
        print_tensor("encoder_input_sim", calc_sim_cos(encoder_input_sim))

        # === 4-A. 构建 Encoder 的 padding mask =============================
        # 整个序列长度 = query_token_numb + 1（user token）+ max_len（点击序列）
        total_len  = self._query_token_numb + 1 + max_len
        ...
        seq_mask   = tf.concat([query_tok, user_tok, click_mask], axis=1)  # [B, total_len]
        ...
        encoder_output_sim = tf.reshape(encoder_output[:,self._query_token_numb:,:], [batch_size, -1])
        ...
        interest_embed = tf.layers.dense(coarse_interest, self._stage_two_dim, name='project', activation=tf.nn.leaky_relu) # [b, 1, dim]从兴趣提取语义到sid生成
        ...
        fine_item_input=tf.concat([interest_embed, input_sid_embed], axis=1)


        fine_item_model = DecoderModel(num_layers=4, dim=self._stage_two_dim, num_heads=8, dropout_rate=0.1, hidden_dim=self._stage_two_dim*2, training=True, query_token_numb=self._query_token_numb)
        decoder_output = fine_item_model.forward(fine_item_input, training=True)  # [b, code_size, dim]

        
        # 计算解码器各步输出的余弦相似度（用于调试）
        for i in range(len(self._vocab_sizes)):
            similarity = calc_sim_cos(decoder_output[:, self._query_token_numb + i, :])
        ...
        if sid0_set_labels is None:
            sid0_set_labels = label[:, :1]
        if sid0_set_label_mask is None:
            sid0_set_label_mask = tf.ones_like(sid0_set_labels, dtype=tf.float32)
        sid0_set_labels = tf.cast(sid0_set_labels, tf.int32)
        sid0_set_label_mask = tf.cast(sid0_set_label_mask, tf.float32)
        sid0_label_one_hot = tf.one_hot(sid0_set_labels, self._vocab_sizes[0], dtype=tf.float32)
        sid0_label_one_hot = sid0_label_one_hot * tf.expand_dims(sid0_set_label_mask, axis=-1)
        sid0_multi_hot = tf.reduce_max(sid0_label_one_hot, axis=1)
        sid0_unique_prev = tf.cumsum(sid0_label_one_hot, axis=1, exclusive=True)
        sid0_unique_position_mask = sid0_set_label_mask * tf.cast(
            tf.equal(tf.reduce_sum(sid0_unique_prev * sid0_label_one_hot, axis=-1), 0.0),
            tf.float32
        )
        sid0_label_count = tf.reduce_sum(sid0_multi_hot, axis=1)
        sid0_list_k = sid0_set_labels.get_shape().as_list()[1]
        sid0_list_k = sid0_list_k if sid0_list_k is not None else 1
        print_tensor("sid0_list_label_count", sid0_label_count)
        ...
        for step in range(len(self._vocab_sizes)):
            with tf.variable_scope('proj_%d' % step):
                ...
                pred_logit = tf.layers.dense(decoder_output[:, self._query_token_numb+step, :], self._vocab_sizes[step], name='pred') # [batch_size, vocab_size]
                ...
                if step == 0:
                    sid0_position_prob = tf.reduce_sum(
                        sid0_label_one_hot * tf.expand_dims(pred_prob, axis=1),
                        axis=-1
                    )
                    sid0_correct_prob_list = tf.reduce_sum(
                        sid0_position_prob * sid0_unique_position_mask,
                        axis=1
                    ) / (sid0_label_count + 1e-9)
                    print_tensor(
                        "probs/correct_token_prob_list_%d" % step,
                        tf.reduce_sum(sid0_correct_prob_list * loss_mask) / (tf.reduce_sum(loss_mask) + 1e-9)
                    )
                    result_dict['truth%d_list_probs' % step] = sid0_correct_prob_list

                    sid0_position_rank = 1 + tf.reduce_sum(
                        tf.cast(
                            tf.expand_dims(pred_prob, axis=1) > tf.expand_dims(sid0_position_prob, axis=2),
                            tf.float32
                        ),
                        axis=2
                    )
                    sid0_correct_rank_list = tf.reduce_sum(
                        sid0_position_rank * sid0_unique_position_mask,
                        axis=1
                    ) / (sid0_label_count + 1e-9)
                    sid0_trim_rank_list = masked_trimmed_mean(
                        sid0_correct_rank_list, loss_mask, trim_ratio=0.05, name="trim_rank_list_%d" % step)
                    print_tensor("probs/correct_token_rank_list_%d" % step, sid0_trim_rank_list)

                    sid0_top_list_probs, _ = tf.nn.top_k(pred_prob, k=sid0_list_k, sorted=True)
                    sid0_top_list_mask = tf.sequence_mask(
                        tf.cast(sid0_label_count, tf.int32), maxlen=sid0_list_k, dtype=tf.float32)
                    sid0_max_prob_list = tf.reduce_sum(
                        sid0_top_list_probs * sid0_top_list_mask,
                        axis=1
                    ) / (sid0_label_count + 1e-9)
                    print_tensor(
                        "probs/max_token_prob_list_%d" % step,
                        tf.reduce_sum(sid0_max_prob_list * loss_mask) / (tf.reduce_sum(loss_mask) + 1e-9)
                    )

                    log_prob = tf.nn.log_softmax(pred_logit/temperature, axis=-1)
                    loss_i = -tf.reduce_sum(log_prob * sid0_multi_hot, axis=1)
                else:
                    # 转换标签为one-hot编码
                ...
                if step == 0:
                    recall_at_k_multi_label(
                        ...
                        self._print_ops,
                        top_k=[16, 128, 256],
                        name="predict_recall_list_%d" % step,
                        min_top_k_upper_bound=sid0_list_k
                    )
    ...
    def beam_search_fast(self, beam_size=512, temperature=1):
        ...
        * step>0 时保持固定 beam_size。
        ...
        start_tok = tf.fill([B, 1], self._total_vocab_size)   # global id of <START> [b,1]
        ...
        for i in range(self._query_token_numb):
            ...
            for step, V in enumerate(self._vocab_sizes):
                ...
                with tf.variable_scope(scope_name):
                    fine_item_model = DecoderModel(num_layers=1, dim=self._dim, num_heads=8, dropout_rate=0.1, hidden_dim=self._dim*2)
    ...
    def beam_search_fast_share_head(self, beam_size=512, temperature=1):
        """
        O(batch·beam·logV) 近似复杂度的束搜索（显存与 beam_size 线性）
        ...
        * step>0 时保持固定 beam_size。
        ...
        # === 5 构建query token ===
        query_token_id = tf.constant(self._total_vocab_size, dtype=tf.int32)     # 标量
        query_token_indice = tf.fill([batch_size, 1], query_token_id)           # [b,1]
        coarse_interest_input = tf.nn.embedding_lookup(self._embedding, query_token_indice)  # [b,1,dim]
                 
        # === 6. 粗粒度解码器 ===
        # 使用4层Transformer解码器生成序列表示
        coarse_interest_model = QFormer(num_layers=4, dim=self._dim, num_heads=8, dropout_rate=0.1, hidden_dim=self._dim*2, training=True)
        coarse_interest = coarse_interest_model.forward(coarse_interest_input, enc_out_base, src_mask, training=True) # [b, 1, dim]


        # ========== 0) 把 decoder 维折叠进 batch：B*Q ==========
        query_token_embed_all = tf.reshape(coarse_interest, [B * self._query_token_numb, -1]) # [B*q, dim],实现并行推理
        ## enc_out_base: [B, Lenc, D] -> [B*Q, Lenc, D]（每个 decoder复用同一个 enc_out）
        Lenc = tf.shape(enc_out_base)[1]
        enc_out_all = tf.reshape(tf.tile(tf.expand_dims(enc_out_base, 1), [1, self._query_token_numb, 1, 1]),[B * self._query_token_numb, Lenc, -1])
        src_mask_all = tf.reshape(tf.tile(tf.expand_dims(src_mask, 1), [1, self._query_token_numb, 1, 1, 1]),[B * self._query_token_numb, 1, 1, -1])

        # ---------- ② Beam 状态初始化 ----------
        BQ = B * self._query_token_numb
        ...
        for step, V in enumerate(self._vocab_sizes):
            ...
            batch_idx = tf.tile(tf.expand_dims(tf.range(BQ), 1), [1, beam_size]) #batch中的每条取beam size个
        ...
        # 合并 head*beam
        all_seqs   = tf.reshape(gen_part_loc, [B, self._query_token_numb * beam_size, -1])   # [B, Q*beam, Seq]
        all_probs  = tf.reshape(probs,        [B, self._query_token_numb * beam_size, -1])   # [B, Q*beam, Seq]
        all_scores = tf.reshape(scores,       [B, self._query_token_numb * beam_size])       # [B, Q*beam]
