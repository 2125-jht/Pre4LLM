...
class MultiInterestModel(object):
    ...
    def model(self, photo_sid, label, photo_semantic_id_int, playing_time=None, duration_ms=None):
        """
        主训练模型前向传播
        ...
        Args:
            ...
            duration_ms: 视频时长(毫秒)，仅用于监控统计，shape=[batch_size, 1]
        ...
        # 打印 batch 中视频时长分布（分母为有效样本数）
        if duration_ms is not None:
            duration_ms_flat = tf.reshape(duration_ms, [-1])
            is_short = tf.cast(tf.less_equal(duration_ms_flat, 17000), tf.float32)
            is_long = tf.cast(tf.greater(duration_ms_flat, 17000), tf.float32)
            valid_count = tf.reduce_sum(valid) + 1e-9
            # 短/长视频比例（两者之和应≈1.0，用于交叉验证）
            short_ratio = tf.reduce_sum(is_short * valid) / valid_count
            long_ratio = tf.reduce_sum(is_long * valid) / valid_count
            print_tensor("duration_short_ratio", short_ratio)
            print_tensor("duration_long_ratio", long_ratio)
            print_tensor("duration_ratio_sum", short_ratio + long_ratio)
            # 有效样本的平均视频时长（毫秒）
            avg_duration_ms = tf.reduce_sum(duration_ms_flat * valid) / valid_count
            print_tensor("avg_duration_ms", avg_duration_ms)

        # === 7-A. PRM loss ===
        prm_model = PRMModel(input_dim=self._dim, prm_dim=self._dim // 2, num_heads=4, dropout_rate=0.1, training=True)
    ...
    def beam_search_fast(self, beam_size=512, temperature=1):
        ...
        * 最终输出天然按 decoder 累积概率降序，无需末尾重排。
        ...
        prm_model = PRMModel(input_dim=self._dim, prm_dim=self._dim // 2, num_heads=4, dropout_rate=0.1, training=False)
