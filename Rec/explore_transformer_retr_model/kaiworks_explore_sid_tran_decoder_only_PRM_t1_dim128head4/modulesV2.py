...
class PRMModel():
    """Path Reranking Model — 轻量化 cross-attention 评分模块。

    与主 decoder 共享 input_dim (256) 的嵌入空间，但内部以更小的
    prm_dim (128) + 更少的 num_heads (4) 运算，计算量约 1/4。

    关键设计：
    - input_proj: [input_dim → prm_dim] 统一降维，encoder 输出和
      target embedding 共享同一投影（因二者处于同一向量空间）
    - 残差连接在 prm_dim 空间内闭合，无需回映到 input_dim
    """
    def __init__(self, input_dim, prm_dim, num_heads, dropout_rate, training=False):
        super(PRMModel, self).__init__()
        self.input_dim = input_dim    # 主模型维度 (256)
        self.dim = prm_dim            # PRM 内部维度 (128)
        self.num_heads = num_heads    # PRM 注意力头数 (4)
        self.head_dim = prm_dim // num_heads  # 32
        self.score_hidden_dim = max(prm_dim // 2, 1)  # 64
        self.dropout_rate = dropout_rate
        self.cross_attention = multi_head_attention
    ...
    def build_variables(self):
        ...
        with tf.variable_scope("prm_model", reuse=tf.AUTO_REUSE):
            # Input projection: main model dim → PRM dim (256→128)
            # encoder 输出和 target embedding 共享同一投影
            dense_vars("input_proj", self.input_dim, self.dim, use_bias=True)
            ...
            with tf.variable_scope("cross_attn_ln/layer_norm", reuse=tf.AUTO_REUSE):
    ...
    def project_kv(self, hidden_states):
        ...
        hidden_states: [B, L_enc, input_dim]
        Returns:
            K: [B, H, L_enc, Dh]
            V: [B, H, L_enc, Dh]
        """
        ...
        with tf.variable_scope("prm_model", reuse=tf.AUTO_REUSE):
            # 先降维 input_dim → prm_dim，再做 attention 内部投影
            hidden_states_proj = tf.layers.dense(hidden_states, self.dim, name="input_proj")  # [B, L_enc, prm_dim]
            with tf.variable_scope("multi_head_attention", reuse=tf.AUTO_REUSE):
                K = tf.layers.dense(hidden_states_proj, self.dim, use_bias=False, name="w_k")
                V = tf.layers.dense(hidden_states_proj, self.dim, use_bias=False, name="w_v")
    ...
    def forward_with_kv(self, target_embedding, K_pre, V_pre, src_mask, training):
        ...
        target_embedding: [B*cand, 1, input_dim]
        K_pre, V_pre:     [B*cand, H, L_enc, Dh]  (already split heads)
        """
        with tf.variable_scope("prm_model", reuse=tf.AUTO_REUSE):
            # 降维 input_dim → prm_dim，残差在 prm_dim 空间闭合
            target_proj = tf.layers.dense(target_embedding, self.dim, name="input_proj")  # [B*cand, 1, prm_dim]
            ...
            # Residual + LayerNorm (both in prm_dim space)
            target_attn = layer_norm(target_proj + target_attn, scope="cross_attn_ln")
            score = mlp(
                "target_score",
                target_attn,
    ...
    def forward(self, target_embedding, hidden_states, src_mask, training):
        with tf.variable_scope("prm_model", reuse=tf.AUTO_REUSE):
            # 降维 input_dim → prm_dim
            target_proj = tf.layers.dense(target_embedding, self.dim, name="input_proj")
            hidden_proj = tf.layers.dense(hidden_states, self.dim, name="input_proj")
            ...
            # Residual connection + LayerNorm (in prm_dim space)
            target_attn = layer_norm(target_proj + target_attn, scope="cross_attn_ln")
            score = mlp(
                "target_score",
                target_attn,
