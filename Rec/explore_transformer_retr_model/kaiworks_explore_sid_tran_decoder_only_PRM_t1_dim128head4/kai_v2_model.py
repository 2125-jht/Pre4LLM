...
if args.with_kai_v2:
    ...
    def filter_mask_wrapper(dataset):
        # 1. 声明所需的样本过滤特征字段
        # 新正样本定义：按 duration 分档混合阈值（绝对+比例），样本播放时长 >= 对应档位阈值即为正样本
        #   Part 2 已合并进分档逻辑（5-20s完播即通过），不再需要单独补充
        ...
        def mask_fn(batch):
            ...
            # ===== Part 1: 按 duration 分档设置播放时长阈值 =====
            # 不同时长视频的"有效消费"标准不同，混合绝对阈值和比例阈值
            #   duration < 5s         → playing_time >= 5s  (固定绝对阈值；<5s视频放着易自动放完但未必真看，故用>duration的5s卡掉)
            #   5s <= dur < 20s       → playing_time >= 1.0 * duration  (完播，短视频最严格)
            #   20s <= dur < 40s      → playing_time >= 0.9 * duration
            #   40s <= dur < 80s      → playing_time >= 0.8 * duration
            #   80s <= dur < 120s     → playing_time >= 0.7 * duration
            #   duration >= 120s      → playing_time >= 85s  (长视频绝对阈值，不再按比例)
            threshold = tf.where(duration_ms < 5000.0,    tf.ones_like(duration_ms) * 5000.0,           # 绝对 5s
                       tf.where(duration_ms < 20000.0,   duration_ms * 1.0,                              # 100% 完播
                       tf.where(duration_ms < 40000.0,   duration_ms * 0.9,                              # 90%
                       tf.where(duration_ms < 80000.0,   duration_ms * 0.8,                              # 80%
                       tf.where(duration_ms < 120000.0,  duration_ms * 0.7,                              # 70%
                                                  tf.ones_like(duration_ms) * 85000.0)))))  # 绝对 85s
            label_play_over_threshold = tf.where(tf.greater_equal(playing_time, threshold), tf.ones_like(playing_time), tf.zeros_like(playing_time))
            action_cnt = label_play_over_threshold
...
if is_training:
    ...
    # === 模型前向传播 ===
    print("=== 模型前向传播 ===")
    # 获取视频时长和播放时长用于 loss 加权
    duration_ms = tf.cast(config.get_dense_fea("photo_info__duration_ms", dim=1, dtype=tf.int64), tf.float32)
    playing_time = tf.cast(config.get_dense_fea("context_info__playing_time", dim=1, dtype=tf.int64), tf.float32)
    # 加权逻辑已从 duration_ms 改为 playing_time：长播放样本权重更高
    # duration_ms 仍传入用于监控统计（短/长视频比例、平均时长等）
    loss, result_dict = model.model(photo_semantic_id, label, photo_semantic_id_int, playing_time=playing_time, duration_ms=duration_ms)  # 计算训练损失
