...
def rms_norm(x, scope, eps=1e-6):
    ...
    with tf.variable_scope(f"{scope}/rms_norm", reuse=tf.AUTO_REUSE):
        ...
        x_fp32 = tf.cast(x, tf.float32)
        gamma_fp32 = tf.cast(gamma, tf.float32)
        mean_square = tf.reduce_mean(tf.square(x_fp32), axis=-1, keepdims=True)
        normalized = x_fp32 / tf.sqrt(mean_square + eps)

        output = gamma_fp32 * normalized
    return tf.cast(output, in_dtype)
...
class QformerLayer:
    ...
    def forward(self, x, enc_output, src_mask, training=False):
        with tf.variable_scope(f"{self.name}", reuse=tf.AUTO_REUSE):
            ...
            cross_attn_output = self.mha(x_norm_1, enc_output, enc_output, self.num_heads, src_mask, self.dropout_rate, training=training)
            out1 = x + cross_attn_output


            x_norm_3 = rms_norm(out1, scope="ffn_ln")
            ffn_output = self.ffn(x_norm_3, training=training)
            out3 = out1 + ffn_output
...
class DecoderLayer:
    ...
    def forward(self, x, enc_output, src_mask, training=False):
        with tf.variable_scope(f"{self.name}", reuse=tf.AUTO_REUSE):
            x_norm_1 = rms_norm(x, scope="cross_atten_ln")
            cross_attn_output = self.cross_attention(x_norm_1, enc_output, enc_output, self.num_heads, src_mask, self.dropout_rate, training=training)
            out1 = x + cross_attn_output

            x_norm_2 = rms_norm(out1, scope="self_atten_ln")
            attn_output = self.self_attention(x_norm_2, x_norm_2, x_norm_2, self.num_heads, self.dropout_rate, training=training)
    ...
    def step(self, x_t, cur_beam,         # [B,1,D] 当步输入
        ...
        with tf.variable_scope(f"{self.name}", reuse=tf.AUTO_REUSE):
            ...
            with tf.variable_scope("multi_head_attention", reuse=tf.AUTO_REUSE):
                q_cur = tf.layers.dense(x_norm_1, self.dim, use_bias=False, name="w_q",trainable=False)
            ...
            with tf.variable_scope("multi_head_attention_causality", reuse=tf.AUTO_REUSE):
                ...
                q_t = tf.layers.dense(x_norm_2, depth, use_bias=False, name="w_q",trainable=False)
                k_t = tf.layers.dense(x_norm_2, depth, use_bias=False, name="w_k",trainable=False)
                v_t = tf.layers.dense(x_norm_2, depth, use_bias=False, name="w_v",trainable=False)
