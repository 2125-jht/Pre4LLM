...
# === 创建模型实例 ===
print("=== 创建sid推荐模型 ===")
model = SIDRecModel(all_param_dict, feature_emb_size_dict, print_ops=print_ops)
...
if is_training:
    ...
    user_colossus_photo_semantic_id_list = config.get_dense_fea("colossus_photo_semantic_id_list", dim=1000, dtype=tf.int64) # [B, 1000]
    user_colossus_play_time_list = config.get_dense_fea("colossus_play_time_list", dim=1000, dtype=tf.int64) # [B, 1000]
    user_colossus_label_list = config.get_dense_fea("colossus_label_list", dim=1000, dtype=tf.int64) # [B, 1000]
    
    # 确保数据类型正确，避免bitwise操作时的类型错误
    user_colossus_label_list = tf.cast(user_colossus_label_list, tf.int64)
    
    user_colossus_action_list = getActionList(user_colossus_label_list)
    ...
    # user_play_semantic_id_list_int_last = getLast(user_play_semantic_id_list_int, 64)
    
    # print_ops.append(tf.print("user_effective_play_semantic_id_list_int_last", user_effective_play_semantic_id_list_int_last, summarize=-1, output_stream=sys.stdout))
    # user_play_semantic_id_list_int_last = processInput(user_play_semantic_id_list_int_last)
    # photo_semantic_id = processInput(photo_semantic_id_int)
    # label = processLabel(photo_semantic_id_int)

    loss = model.model(user_colossus_photo_semantic_id_list, user_colossus_play_time_list, user_colossus_action_list, photo_semantic_id_int)
        
    # _,_ = model.beam_search_fast(user_colossus_photo_semantic_id_list, user_colossus_play_time_list, user_colossus_action_list, beam_size=10, temperature=1)
    ...
    user_colossus_photo_semantic_id_list = tf.cast(user_colossus_photo_semantic_id_list, tf.int64)
    user_colossus_play_time_list = tf.cast(user_colossus_play_time_list, tf.int64)
    user_colossus_label_list = tf.cast(user_colossus_label_list, tf.int64)
    
    # print(user_colossus_photo_semantic_id_list.dtype)
    # print(user_colossus_play_time_list.dtype)
    # print(user_colossus_label_list.dtype)
    # tf.bitwise.bitwise_and(user_colossus_label_list, 1)
    
    user_colossus_action_list = getActionList(user_colossus_label_list)
    
    user_sid, user_sid_prob = model.beam_search_fast(user_colossus_photo_semantic_id_list, user_colossus_play_time_list, user_colossus_action_list, beam_size=512, temperature=1)
