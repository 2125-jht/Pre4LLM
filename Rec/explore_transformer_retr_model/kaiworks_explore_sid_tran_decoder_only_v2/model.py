...
class SIDRecModel(object):
    ...
    def __init__(self, feature_emb_dict, feature_emb_size_dict, dim=256, select_size=64, 
    ...
    def _select_recent(self, photo_ids, play_time, action):
        """
        Args
        ----
        photo_ids : int64 [B, L]   0 表示无效
        play_time : int64 [B, L]
        action    : int64 [B, L]   0/1
        Returns
        -------
        ...
        mask &= (tf.greater_equal(play_time, 7) |
                (tf.greater_equal(play_time, 3) & tf.equal(action, 1)))
    ...
    # --------------------------- 前向 & loss ----------------------------
    def model(self, user_sid, user_time, user_act, tgt_sid):            # shapes: [B,1000] / [B,1]
        
        B = tf.shape(tgt_sid)[0]

        # 1) 拼接当前目标
        photo_ids = tf.concat([user_sid, tgt_sid], 1)       # [B,1001]
        play_time = tf.concat([user_time,
                               tf.fill([B, 1], tf.cast(7, user_time.dtype))], 1)
        action    = tf.concat([user_act, tf.ones([B, 1], user_act.dtype)], 1)

        # 2) 最近 k 条
        photo_sel, valid_len = self._select_recent(photo_ids, play_time, action)   # [B,k] [B,1]
    ...
    def beam_search_fast(self, user_sid, user_time, user_act, beam_size=64, temperature=1.0):
        """
        ...
        photo_sel, valid_len = self._select_recent(user_sid, user_time, user_act)   # [B,k], [B,1]
