...
class MultiInterestModel(object):
    ...
    def beam_search_fast(self, beam_size=512, temperature=1):
        ...
        * 最终输出天然按 decoder 累积概率降序，无需末尾重排。
        ...
        # PRM K/V 延迟构建：step==0 时 build_encoder_cache 一次
        ...
        for step, V in enumerate(self._vocab_sizes):
            ...
            if step == 0:
                prm_model.build_variables()
                # 对 enc_out_base [B, L_enc, dim] 投影一次，得到 [B, 1, H, L_enc, Dh] 缓存
            ...
            logp = tf.nn.log_softmax(tf.cast(logits, tf.float32) / temperature)  # [B, cur_beam, V]
            ...
            # ==============================================================
                ...
                beam_batch_idx = tf.tile(tf.expand_dims(tf.range(B), 1), [1, prm_candidate_size])
                # PRM 剪枝后 gather 用的是 beam_size 维度，需要单独建
                beam_batch_idx_beam = tf.tile(tf.expand_dims(tf.range(B), 1), [1, beam_size])
                ...
                # 使用 broadcast-based KV cache：K/V 不物化大张量，惰性 broadcast_to 到 cur_beam
                # prm_k_enc_cache / prm_v_enc_cache 已在 step==0 时构建完毕
                ...
                beam_batch_idx_diag = tf.tile(tf.expand_dims(tf.range(B), 1), [1, beam_size])
                prm_selected_parents = tf.gather_nd(
                    parent_beam,
                    tf.stack([beam_batch_idx_diag, prm_best_idx_tmp], axis=2)
                )  # [B, beam_size]
                ...
                prm_logits_scaled = tf.cast(prm_logits, tf.float32) / prm_temperature
                _, prm_best_idx = tf.nn.top_k(prm_logits_scaled, k=beam_size)
                # tf.sort 不存在于 TF 1.x，用 top_k + reverse 实现升序排列
                _, sorted_desc = tf.nn.top_k(prm_best_idx, k=beam_size)
                prm_best_idx = tf.reverse(sorted_desc, axis=[-1])               # [B, beam] 升序 = cand_seqs 原序
                gather_prm = tf.stack([beam_batch_idx_beam, prm_best_idx], axis=2)  # [B, beam, 2]
                ...
                gather_parent_after_prm = tf.stack([beam_batch_idx_beam, selected_parent_beam], axis=2)
