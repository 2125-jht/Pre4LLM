...
def layer_norm(x, scope, eps=1e-6):
    ...
    with tf.variable_scope(f"{scope}/layer_norm", reuse=tf.AUTO_REUSE):
        gamma = tf.get_variable("gamma", [x.get_shape()[-1]], dtype=in_dtype, initializer=tf.ones_initializer(), trainable=False)
        beta = tf.get_variable("beta", [x.get_shape()[-1]], dtype=in_dtype, initializer=tf.zeros_initializer(), trainable=False)

        x_fp32 = tf.cast(x, tf.float32)
        gamma_fp32 = tf.cast(gamma, tf.float32)
        beta_fp32 = tf.cast(beta, tf.float32)
        mean, variance = tf.nn.moments(x_fp32, axes=[-1], keep_dims=True)
        normalized = (x_fp32 - mean) / tf.sqrt(variance + eps)
        output = gamma_fp32 * normalized + beta_fp32
    return tf.cast(output, in_dtype)
...
def multi_head_attention(queries, keys, values, num_heads, src_mask, dropout_rate, training=False):
    ...
    def scaled_dot_product_attention(Q, K, V, src_mask):
        ...
        V_fp32 = tf.cast(V, tf.float32)

        matmul_qk = tf.matmul(Q_fp32, K_fp32, transpose_b=True)
        dk = tf.cast(tf.shape(K_fp32)[-1], tf.float32)
        ...
        output = tf.matmul(attention_weights, V_fp32)
        # ---- FP32 island end ----
        return tf.cast(output, out_dtype), attention_weights
    
    with tf.variable_scope("multi_head_attention", reuse=tf.AUTO_REUSE):
        depth = queries.get_shape().as_list()[-1]
        Q = tf.layers.dense(queries, depth, use_bias=False, name="w_q",trainable=False)
        K = tf.layers.dense(keys,    depth, use_bias=False, name="w_k",trainable=False)
        V = tf.layers.dense(values,  depth, use_bias=False, name="w_v",trainable=False)
        ...
        output = tf.layers.dense(concat_attention, depth, name="w_o",trainable=False)
...
def feed_forward_network(dim, hidden_dim, dropout_rate, training=False):
    def ffn(x, training=training):
        ...
        with tf.variable_scope(f"feed_forward_network", reuse=tf.AUTO_REUSE):
            x = tf.layers.dense(x, hidden_dim, activation=tf.nn.relu, name="w_up",trainable=False)
            x = tf.layers.dropout(x, rate=dropout_rate,
                                    training=training, name="ffn_dropout")
            x = tf.layers.dense(x, dim, name="w_down",trainable=False)
            return x
    return ffn
...
def scaled_attention(q,            # [B*beam, H, T_q, Dh]  (单步)
    ...
    B = tf.shape(q_fp32)[0] // cur_beam
    H = tf.shape(q_fp32)[1]
    T_q = tf.shape(q_fp32)[2]
    T_k = tf.shape(k_fp32)[2]

    # ---------- 1. 点积并缩放 ----------
    attn_scores = tf.matmul(q_fp32, k_fp32, transpose_b=True)        # [B*beam,H,Tq,Tk]
    dk = tf.cast(tf.shape(k_fp32)[-1], tf.float32)
    ...
    if src_mask is not None:       # Padding mask / Cross-attn mask
        ...
        src_mask = tf.cast(src_mask, tf.float32)
    ...
    # ---------- 4. 加权求和 ----------
    context = tf.matmul(attn_weights, v_fp32)        # [B*beam,H,Tq,Dh]
    ...
    return tf.cast(context, out_dtype)          # 返回给上层 (仍是 3D，后续可接线性层)
    ...
    def step(self, x_t, cur_beam,         # [B,1,D] 当步输入
        ...
        with tf.variable_scope(f"{self.name}", reuse=tf.AUTO_REUSE):
            with tf.variable_scope("multi_head_attention_causality", reuse=tf.AUTO_REUSE):
                ...
                # ---------- ① 当前 token 的 q/k/v ----------
                depth = x_t.shape[-1]
                q_t = tf.layers.dense(x_t, depth, use_bias=False, name="w_q",trainable=False)
                k_t = tf.layers.dense(x_t, depth, use_bias=False, name="w_k",trainable=False)
                v_t = tf.layers.dense(x_t, depth, use_bias=False, name="w_v",trainable=False)
                ...
                self_out = tf.layers.dense(self_out, depth, name="w_o",trainable=False)  # [B*beam,1,D]
            
            out1 = layer_norm(x_t + self_out, scope="self_atten_ln")
                        
            with tf.variable_scope("multi_head_attention", reuse=tf.AUTO_REUSE):
                # ---------- ② Cross-Attn ----------
                q_cur = tf.layers.dense(out1, self.dim, use_bias=False, name="w_q",trainable=False)
                ...
                if f"k_enc_{layer_id}" not in cache:
                    k_enc = split_heads(tf.layers.dense(enc_output, self.dim, use_bias=False,
                                                        name="w_k",trainable=False), self.num_heads)
                    v_enc = split_heads(tf.layers.dense(enc_output, self.dim, use_bias=False,
                                                        name="w_v",trainable=False), self.num_heads)
                ...
                cross_out = tf.layers.dense(cross_out, self.dim, name="w_o",trainable=False) # [B,L_dec,D]
...
class PRMModel():
    ...
    def build_variables(self):
        def dense_vars(scope, input_dim, output_dim, use_bias=True):
            with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
                tf.get_variable("kernel", [input_dim, output_dim], dtype=tf.float16, initializer=tf.glorot_uniform_initializer(), trainable=False)
                if use_bias:
                    tf.get_variable("bias", [output_dim], dtype=tf.float16, initializer=tf.zeros_initializer(), trainable=False)

        def ln_vars(scope, dim):
            with tf.variable_scope(f"{scope}/layer_norm", reuse=tf.AUTO_REUSE):
                tf.get_variable("gamma", [dim], dtype=tf.float16, initializer=tf.ones_initializer(), trainable=False)
                tf.get_variable("beta",  [dim], dtype=tf.float16, initializer=tf.zeros_initializer(), trainable=False)
    ...
    def project_kv(self, hidden_states):
        ...
        with tf.variable_scope("prm_model", reuse=tf.AUTO_REUSE):
            with tf.variable_scope("multi_head_attention", reuse=tf.AUTO_REUSE):
                K = tf.layers.dense(hidden_states, self.dim, use_bias=False, name="w_k", trainable=False)
                V = tf.layers.dense(hidden_states, self.dim, use_bias=False, name="w_v", trainable=False)
    ...
    def forward_with_kv(self, target_embedding, K_pre, V_pre, src_mask, training):
        ...
        Used by training model() — K/V are pre-tiled to [B*cand, H, L_enc, Dh] for in-batch negatives.
        target_embedding: [B*cand, 1, dim]
        K_pre, V_pre:     [B*cand, H, L_enc, Dh]  (already split heads, already tiled)
        """
        with tf.variable_scope("prm_model", reuse=tf.AUTO_REUSE):
        ...
        with tf.variable_scope("prm_model", reuse=tf.AUTO_REUSE):
            with tf.variable_scope("multi_head_attention", reuse=tf.AUTO_REUSE):
                ...
                target_attn = scaled_attention(q_cur, k_enc, v_enc,
                                               ...
                                               dropout_rate=self.dropout_rate,
                                               training=training)
                target_attn = tf.layers.dense(target_attn, self.dim,
                                              name="w_o", trainable=False)
            # Residual + LayerNorm (PostNorm)
            target_attn = layer_norm(target_embedding + target_attn, scope="cross_attn_ln")
