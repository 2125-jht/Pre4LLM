...
model_queue_prefix="kaiworks_explore_sid_tran_decoder_only_PRMv2_filter_t_1"
...
class ModelRetrFlow(LeafFlow, MioApiMixin, OfflineApiMixin, UniPredictApiMixin, ExploreModelApiMixin, RetrievalApiMixin, GsuApiMixin, EmbeddingApiMixin):
            ...
            self.uni_predict_fused(
              ...
              ## embedding 拉取相关配置，对应 fetch_mio_embedding 部分
               embedding_fetchers = [dict(
                ...
                colossusdb_embd_table_name="PRMv2_filter_t1",
              ...
              executor_config = dict(
                context_per_device=40,  				# 每张卡上设置的 context 个数，
                memory_size_per_context=1024, # 每个 context 占用的显存，单位 MB
              ),
            )
            .log_debug_info( # 埋日志，方便 dry‑run/线上 Debug
              common_attrs = [config["input_name"] for config in slots_config if config["common"]],  # 打印哪些全局属性
              for_debug_request_only = True,  # True 则仅调试请求才输出
              respect_sample_logging = False,  # 是否遵守全局按用户维度的打印采样，若为 False 则每条 debug info 必打
              log_tag = "uni_predict"
            )
        )
  ...
  # .enrich_attr_by_py(
    ...
    def retrieve(self):
      ...
      .retrieve_by_remote_colossusdb_index( # 从异地部署的统一存储（ colossusdb ）版本倒排索引通过 Query 语句进行 item 召回
        client_kconf = "colossus.inverted_index_kconf_client.explore_sid_index_client",
        reason = 1,  # 召回原因码，可用于后续分析
        default_search_num = 10,
        querys = [{ # querys: [list] 查询条件列表，多个条件之间取并集进行结果返回
      ...
      .enrich_attr_by_lua(
        ...
        export_item_attr = ["query_sid_list", "query_sid", "prob"],
        lua_script = """
          function calculate()
            ...
            local prob = user_sid_prob[index+1]
            return query_sid_list, query_int, prob
          end
        """
      ) \
       .pack_item_attr(
    ...
    def post_process(self):
      return self.deduplicate() \
        ...
        .gen_common_attr_by_lua( # 根据已有的 common_attr 利用 lua 表达式生成新的 common_attr 用于后续使用
          attr_map={
...
service.build(output_file=os.path.join(model_dir, "dynamic_json_config_gpu_l20.json"))
