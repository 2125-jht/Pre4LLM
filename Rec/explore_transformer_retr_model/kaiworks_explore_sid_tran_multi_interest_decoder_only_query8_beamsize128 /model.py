...
class MultiInterestModel(object):
    ...
    def model(self, photo_sid, label, photo_semantic_id_int):
        ...
        coarse_interest_model = DecoderModel(num_layers=4, dim=self._dim, num_heads=8, dropout_rate=0.1, hidden_dim=self._dim*2, training=True)
    ...
    def beam_search_fast(self, beam_size=512, temperature=1):
        ...
        * step>0 时保持固定 beam_size。
        ...
        coarse_interest_model = DecoderModel(num_layers=4, dim=self._dim, num_heads=8, dropout_rate=0.1, hidden_dim=self._dim*2)
        ...
        # === 6. 粗粒度解码器 ===
        # 使用4层Transformer解码器生成序列表示
        coarse_interest_model = DecoderModel(num_layers=5, dim=self._dim, num_heads=8, dropout_rate=0.1, hidden_dim=self._dim*2, training=True)
