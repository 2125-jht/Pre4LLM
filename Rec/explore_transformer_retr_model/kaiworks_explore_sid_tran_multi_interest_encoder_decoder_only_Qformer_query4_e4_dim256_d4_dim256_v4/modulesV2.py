...
class EncoderLayer:
    ...
    def forward(self, x, src_mask, training):
        with tf.variable_scope(f"{self.name}", reuse=tf.AUTO_REUSE):
            x_norm_1 = rms_norm(x, scope="self_atten_ln")
            attn_output = self.mha(x_norm_1, x_norm_1, x_norm_1, self.num_heads, src_mask, self.dropout_rate, training=training)
            out1 = x + attn_output

            x_norm_2 = rms_norm(out1, scope="ffn_ln")
            ffn_output = self.ffn(x_norm_2, training=training)
