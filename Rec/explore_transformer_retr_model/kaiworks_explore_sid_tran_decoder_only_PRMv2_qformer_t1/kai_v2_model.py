"""
kai_v2_model.py — 基于 Kai v2.0 框架的多兴趣推荐模型配置脚本

本脚本负责：
1. 解析命令行参数，选择运行模式（训练 / 推理）和框架后端（Kai v2 / Kai v1 / MIO）
2. 配置特征嵌入（embedding），包括共享嵌入和特征复制
3. 定义样本过滤逻辑（基于用户行为信号过滤低质量样本）
4. 构建训练 / 推理计算图，配置优化器和评估目标
5. 支持稠密参数自定义加载（增量训练场景下的参数热启动）

运行方式：
    python kai_v2_model.py --mode train  --with_kai_v2 True   # Kai v2.0 训练
    python kai_v2_model.py --mode predict --with_kai_v2 True   # Kai v2.0 推理
    python kai_v2_model.py --mode train  --with_kai_v2 False   # MIO 框架训练
"""

from __future__ import print_function
MODEL_TRANS_ORIGIN='cpp'  # 全局常量：指定 Transformer 模型的来源
...
parser.add_argument('--dryrun', dest='dryrun', const=True, default=False, nargs='?') # 不执行实际训练/推理
parser.add_argument('--with_kai', default=False)
parser.add_argument('--text', default=False) # 是否启用文本特征
parser.add_argument('--tower', choices=None, dest='tower', default='False') # 是否使用 Tower 推理模式
...
output_var_list = []  # output_var_list: 存储 kai_output_embedding 中注册的需要输出 embedding 的 TF 变量
custom_grad_dict = {} # custom_grad_dict: 存储自定义梯度映射（变量名 → 输出 embedding tensor）
...
if args.with_kai_v2:
    ...
    default_param_attr = config.nn.ParamAttr(initializer=config.nn.UniformInitializer(0.0001),  # 稠密参数初始化
    access_method=config.nn.ProbabilityAccess(100.0),  # 访问策略（没懂）
    recycle_method=config.nn.UnseendaysRecycle(delete_after_unseen_days=30, delete_threshold=1.0, allow_dynamic_delete=True)) # 参数回收策略
    config.nn.set_default_param_attr(default_param_attr) # 设置默认参数属性（初始化器、访问策略、回收策略）
    ...
    class DumpTensorHook(config.training.RunHookBase):
        def __init__(self, table_name, dump_tensors_dict):
            ...
            Args:
                table_name(string): 导出目标 HDFS 表名
                dump_tensors_dict: 需要导出的 tensor 字典
                    key (str): tensor 名称，用于标识和查找
                    value (tf.Tensor): tensor 操作节点，用于获取计算结果
            """
            assert isinstance(dump_tensors_dict, dict)
            worker_id: int = kai.current_rank()  # 获取当前 worker 的 rank（分布式训练中的编号）
            model_path: str = kai.Config().save_option.model_path
            # 创建数据表对象，用于将数据写入 HDFS
        ...
        def before_step_run(self, step_run_context):
            """
                在每步运行前，将 self._dump_tensors_dict 中的tensor注入fetches中
                后续step run图时会自动跑出来对应Tensor的数值

            Args:
                step_run_context (_type_): _description_ 训练步骤的运行上下文对象

            Returns:
                StepRunArgs: 包含待 fetch 的 tensor 字典
            """
            return kai.training.StepRunArgs(fetches=self._dump_tensors_dict)

        def after_step_run(self, step_run_context, step_run_values):
            """
            在每步运行后，获取 tensor 计算结果并写入数据表

            Args:
                step_run_context: 训练步骤的运行上下文对象
                step_run_values: 运行结果，包含 fetches 中请求的所有 tensor 值
            """
            sink_data = {} # 存储待写入表的数据
            for name, op in self._dump_tensors_dict.items():
                # 1. 从 step_run_values 中取出每个 tensor 的值
                value = step_run_values.result[name]
                batch_size = value.shape[0]
                # 2. 将值 reshape 为 [batch_size, -1] 形状
                sink_data[name] = value.reshape(batch_size, -1)
            # 3. 附加 step_id 和 pass_id 元信息
    ...
    #################################### 通过样本自带数据进行过滤 ####################################
    # ==== 正样本播放时长阈值表（秒级分桶，单位 ms，共 401 个元素，索引 0..400）====
    # 生成规则（v2 第二版）：
    #   [  0,  13) s : 固定 20s        — 短视频强卡，暂不允许短视频进入正样本
    #   [ 13,  30) s : 大盘 p90 分位数  — 短视频严格
    #   [ 30,  58) s : 大盘 p85 分位数  — 中短视频较严格
    #   [ 58, 140) s : 大盘 p80 分位数  — 中长视频正常
    #   [140, 150) s : 固定 85s        — 长视频梯度过渡
    #   [150, 160) s : 固定 90s        — 长视频梯度过渡
    #   [160, 170) s : 固定 95s        — 长视频梯度过渡
    #   [170, 400] s : 固定 100s       — 长视频宽松，固定阈值规避高 duration 桶的分位数噪声
    # 精度：分位数阈值取自 Excel 并四舍五入到 0.1s（100ms）
    # 数据来源：计算大盘分位数_Snippet 1_32938564.xlsx，按秒级 duration_bucket 取对应分位数
    # duration > 400s 也 clip 到 400（对应 100s 固定阈值）
    PLAYING_TIME_THRESHOLD_MS = [
        20000, 20000, 20000, 20000, 20000, 20000, 20000, 20000, 20000, 20000,
        20000, 20000, 20000, 20500, 20400, 21900, 22800, 22500, 23900, 23800,
        25400, 27700, 27500, 28000, 28400, 29300, 29500, 30300, 31800, 32300,
        30000, 32400, 33300, 34600, 35300, 36400, 37200, 38500, 39100, 40500,
        41300, 42000, 43600, 44500, 45300, 46100, 47000, 47900, 49000, 49800,
        50800, 51900, 53000, 53600, 54700, 55800, 56600, 57500, 53000, 48100,
        50900, 55000, 57400, 58800, 57700, 59300, 58800, 58400, 58600, 60800,
        64800, 59500, 60100, 61400, 59600, 57300, 56300, 65000, 60400, 60600,
        65500, 63500, 64500, 67700, 65300, 67800, 69600, 68900, 64900, 69300,
        69800, 75400, 72500, 75500, 72800, 73400, 73300, 74400, 70100, 81300,
        73400, 73800, 76100, 70300, 82400, 83000, 73800, 79000, 72500, 74900,
        81300, 81800, 86600, 78700, 91300, 82500, 76700, 81700, 82800, 80300,
        79700, 93900, 76200, 92900, 80300, 84800, 85100, 86300, 89600, 94700,
        90200, 92400, 89200, 77600, 103400, 86700, 80300, 85500, 77900, 66200,
        85000, 85000, 85000, 85000, 85000, 85000, 85000, 85000, 85000, 85000,
        90000, 90000, 90000, 90000, 90000, 90000, 90000, 90000, 90000, 90000,
        95000, 95000, 95000, 95000, 95000, 95000, 95000, 95000, 95000, 95000,
    ...
    def filter_mask_wrapper(dataset):
        ...
        def mask_fn(batch):
            ...
            # ===== 查表计算播放时长阈值（O(1)，秒级分桶，替代嵌套 tf.where）=====
            # duration_sec: 取整到秒，clip 到 [0, 400] 对应阈值表索引范围
            threshold_table = tf.constant(PLAYING_TIME_THRESHOLD_MS, dtype=tf.float32)  # [401]
            duration_sec = tf.cast(tf.floor(duration_ms / 1000.0), tf.int32)
            duration_sec = tf.clip_by_value(duration_sec, 0, 400)
            threshold = tf.gather(threshold_table, duration_sec)  # [B, 1]，单位 ms

            # 正样本判定：playing_time >= threshold
            # passed: 1.0 = 正样本（保留），0.0 = 负样本（过滤）
            passed = tf.cast(tf.greater_equal(playing_time, threshold), tf.float32)

            # ===== 监控：各 duration 段正样本通过率 & 训练样本占比（过滤前 batch 统计）=====
            dur_flat  = tf.reshape(duration_ms, [-1])   # [B]，去掉最后维便于 reduce
            pass_flat = tf.reshape(passed, [-1])         # [B]
            batch_size_f = tf.cast(tf.shape(dur_flat)[0], tf.float32)
            # 8 个 duration 区间：短 / 中短 / 中 / 中长 / 长梯度
            seg_configs = [
                ('filter/seg_0_13s',       0.0,      13000.0),  # 固定 20s
                ('filter/seg_13_30s',   13000.0,     30000.0),  # p90
                ('filter/seg_30_58s',   30000.0,     58000.0),  # p85
                ('filter/seg_58_140s',  58000.0,    140000.0),  # p80
                ('filter/seg_140_150s', 140000.0,   150000.0),  # 固定 85s
                ('filter/seg_150_160s', 150000.0,   160000.0),  # 固定 90s
                ('filter/seg_160_170s', 160000.0,   170000.0),  # 固定 95s
                ('filter/seg_170s_plus', 170000.0,      None),  # 固定 100s
            ]
            for seg_name, lo_ms, hi_ms in seg_configs:
                if hi_ms is not None:
                    in_seg = tf.cast(tf.logical_and(
                        tf.greater_equal(dur_flat, lo_ms),
                        tf.less(dur_flat, hi_ms)
                    ), tf.float32)
                else:
                    in_seg = tf.cast(tf.greater_equal(dur_flat, lo_ms), tf.float32)
                seg_count  = tf.reduce_sum(in_seg)
                safe_count = tf.maximum(seg_count, 1.0)
                # 正样本通过率：该段中 playing_time >= threshold 的样本占比
                tf.summary.scalar(seg_name + '/pass_rate',
                                  tf.reduce_sum(in_seg * pass_flat) / safe_count)
                # 训练样本占比：该段样本数 / batch 总数（过滤前）
                tf.summary.scalar(seg_name + '/sample_prop',
                                  seg_count / batch_size_f)

            # mask=True 表示过滤掉（丢弃），mask=False 表示保留（正样本）
            mask = tf.less(passed, 1.0)
            return mask
    ...
    # ---- 非 Kai v2.0 框架：使用 MIO 框架 ----
    import tensorflow as tf
    from mio_tensorflow.config import MioConfig
    if not args.dryrun and not args.with_kai:
        import mio_tensorflow.patch as mio_tensorflow_patch
        # 应用 MIO 框架的 patch，替换部分 TF 原生操作
        mio_tensorflow_patch.apply()
    
    logging.basicConfig()
    # 加载基础 YAML 配置
    base_config: str = os.path.join(os.path.dirname(os.path.realpath(__file__)), './base.yaml')
    # 构建 MIO 配置对象

    config = MioConfig.from_base_yaml(
        base_config,
        clear_embeddings=True,  # clear_embeddings/clear_params: 是否在加载时清除嵌入/参数
        clear_params=True,
        dryrun=args.dryrun,
        label_with_kv=True,  # label_with_kv: 标签是否使用 KV 格式
        grad_no_scale=False,  # grad_no_scale: 是否不对梯度做缩放
...
def my_load_dense_func(warmup_weight: dict, warmup_extra: dict, ps_weight: dict, ps_extra: dict, tf_weight: dict, load_option):
    ...
    处理逻辑（对照 base 模型与新模型的参数差异）：
    1. 【删除参数】base 模型存在但新模型不存在 → 从 warmup 中删除
    2. 【修改参数】维度不匹配 → 随机初始化（均匀分布 [-1e-4, 1e-4]）
    3. 【新增参数】新模型存在但 base 模型不存在 → 使用 TF 初始化值
    4. 对 extra（optimizer 依赖参数，如动量）做同样的对齐处理
    '''
    weight = None
    extra = None
    ...
    if warmup_weight is not None and len(warmup_weight) > 0:
        for var_name in list(warmup_weight):
            ...
            if var_name not in tf_weight: # 表示参数存在base模型，但新模型没有。即【删除参数】
                ...
                del warmup_weight[var_name]
                del warmup_extra[var_name]
                warmup_weight[var_name] = np.random.uniform(-1e-4, 1e-4,size=tf_weight[var_name].shape).astype(np.float32) # kai2 必须使用np.float32，numpy默认是float64
    ...
    # 校验：最终 weight 和 extra 的参数数量必须与新模型一致
...
def mark_common_attr():
    """
    在推理服务器的 DNN 模型配置 YAML 中，标记通用（common）特征属性

    功能：读取 dnn_model.yaml，遍历 embedding 的 slots_config，
    将属于 common 特征的 slot 标记为 is_common: True，
    用于 Tower 推理模式区分 common / non-common 特征
    """
    common_embeddings = []
...
def get_param_dict():
    ...
    根据运行模式（训练/推理）采用不同的嵌入创建方式：
    - 推理模式：使用 sized=True 获取 size 变量，区分 common / non-common 特征
    - 训练模式：直接创建嵌入，不区分 common 属性

    Kai v2.0 额外处理：
    - declare_reallocate_slots: 配置共享嵌入（share）和特征复制（copy）
    - get_sparse_fea: 获取稀疏特征的 offset，计算变长特征的实际长度
    """
    if args.with_kai_v2:
        # 共享嵌入：将 share_input_slots 映射到 share_output_slots（remap+inplace）
        config.declare_reallocate_slots(share_input_slots, share_output_slots, remap=True, inplace=True)
        # 特征复制：将 copy_input_slots 复制到 copy_output_slots（remap+非inplace）
    ...
    for attr in all_features:
        ...
        if not is_training:
            ...
            if attr.is_common:
                ...
                # 通用特征：标记 common=True，推理时使用 batching 模式
        ...
        if attr.expand is not None and attr.expand > 1: # 如果是序列特征且 expand > 1，将 embedding reshape 为 [batch, expand, dim]
            feature_emb_dict[attr.attr_name] = tf.reshape(
                feature_emb_dict[attr.attr_name], [-1, attr.expand, attr.dim]
            )

        # 获取变长特征的实际长度
        if args.with_kai_v2:
            # Kai v2.0：通过 get_sparse_fea 获取稀疏特征的 offset
            sparse_feature = config.get_sparse_fea(name=str(attr.slots[0]))
            offset = sparse_feature[1]
            size_var = offset[1:] - offset[0:-1]  # offset 的差分即为每个样本中该特征的元素个数
            feature_emb_size_dict[attr.attr_name] = size_var
            ...
            # Kai v1.0：通过 get_signs 获取稀疏特征的 offset
...
def kai_output_embedding(feature, output_emb):
    """
    将输出 embedding 注册到 Kai 框架，使其在训练过程中被正确优化

    Kai v2.0 模式下：
    - 将 output_emb 注册到 custom_grad_dict（变量名 → 输出 embedding）
    - 将 feature 添加到 output_var_list

    非 Kai v2.0 模式下：
    - 使用 config.custom_gradients 自定义梯度
    - 使用 AssignAdd 优化器（w = decay_rate * w + add_rate * g）

    Args:
        feature: 需要输出 embedding 的 TF 变量
        output_emb: 输出的 embedding tensor
    """
    if args.with_kai_v2:
...
worker_global_step = config.get_step()  # 获取全局训练步数
...
if is_training:
    ...
    # === 数据预处理 ===
    photo_semantic_id = processInput(photo_semantic_id_int)  # 处理输入序列，将整数编码的语义ID拆分为三层（a, b, c），并映射到统一词表空间，[batch_size, 3 * seq_len]
    label = processLabel(photo_semantic_id_int)  # 处理标签，将整数编码的语义ID拆分为三层标签（不映射词表偏移），[batch_size, 3 * seq_len]


    # === 模型前向传播 ===
    print("=== 模型前向传播 ===")
    # 获取播放时长用于 loss 加权：播放时间越长权重越高，使模型偏好深度消费的样本
    playing_time = tf.cast(config.get_dense_fea("context_info__playing_time", dim=1, dtype=tf.int64), tf.float32)
    loss, result_dict = model.model(photo_semantic_id, label, photo_semantic_id_int, playing_time=playing_time)  # 计算训练损失
    ...
    user_sid_origin = tf.cast(tf.reshape(user_sid, [-1, tf.shape(user_sid)[1] * tf.shape(user_sid)[2]]), tf.float32)    # [batch, beam * 3] -> float32
    user_sid_prob = tf.reshape(user_sid_prob, [-1, tf.shape(user_sid_prob)[1]]) # [batch, beam]
    ...
    # 定义推理输出目标
    user_targets = [
        ("user_sid_origin", user_sid_origin),   # 语义ID序列
        ("user_sid_prob", user_sid_prob)         # 语义ID概率
    ]
    q_names, preds = zip(*user_targets)
    print("====> q_name: ", q_names)
    # 导出推理配置
    # input_type=3: 表示输入类型为特定格式
    # dump_mode="user_predict": 用户侧推理模式
