...
class MultiInterestModel(object):
    ...
    def __init__(self, feature_emb_dict, feature_emb_size_dict, dim=256, vocab_sizes=[8192, 8192, 8192], print_ops=None):
        ...
        self._qformer_query_num = 1   # K=1，单 query 压缩行为序列为 1 个画像 token（与静态 1 拼 = 2）
    ...
    def model(self, photo_sid, label, photo_semantic_id_int, playing_time=None):
        ...
        portrait_l2 = tf.sqrt(tf.reduce_mean(tf.square(click_portrait[:, 0, :]), axis=-1))   # [B]
        ...
        portrait_vec = click_portrait[:, 0, :]                                       # [B, D]
        ...
        # click_portrait: [B, K, D] (K=1), QFormer 抽取的用户兴趣画像
        ...
        portrait_main = click_portrait[:, 0, :]                                          # [B, D]
