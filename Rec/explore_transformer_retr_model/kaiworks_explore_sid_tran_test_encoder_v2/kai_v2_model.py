...
if is_training:
    ...
    user_top_embedding, photo_top_embedding = model.model(photo_semantic_id, label, photo_semantic_id_int)
    ...
    if args.dryrun:
        ...
        user_sid = model.beam_search()
        user_sid = user_sid[:, :, 1:]
        # user_sid = tf.reshape(user_sid, [-1, tf.shape(user_sid)[1] * tf.shape(user_sid)[2]])
        user_sid_int = tf.cast(processOutputV2(user_sid), tf.float32)
