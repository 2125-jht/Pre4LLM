...
# global param
predict_server_name = "grpc_exploreGRRetr"
model_queue_prefix="kaiworks_explore_sid_trans_v4"
            ...
            self.uni_predict_fused(
              ...
              ## embedding 拉取相关配置，对应 fetch_mio_embedding 部分
               embedding_fetchers = [dict(
                ...
                colossusdb_embd_table_name="sid_v4",
              ...
              executor_config = dict(
                ...
                memory_size_per_context=10240, # 每个 context 占用的显存，单位 MB
              ),
            )
            .log_debug_info(
      ...
      return self.enrich_attr_by_lua(
        ...
        lua_script = """
          function calculate()
            ...
            for i = 1, N do
              local index = (i - 1) * 3 + 1
              local sum = math.floor(user_sid_origin[index]) * 8192 * 8192 + math.floor(user_sid_origin[index + 1]) * 8192 + math.floor(user_sid_origin[index + 2])
