...
# global param
predict_server_name = "grpc_exploreSidCountRetr"
...
class ModelRetrFlow(LeafFlow, MioApiMixin, OfflineApiMixin, UniPredictApiMixin, ExploreModelApiMixin, RetrievalApiMixin, GsuApiMixin, EmbeddingApiMixin):
            ...
            self.uni_predict_fused(
              ...
              ## embedding 拉取相关配置，对应 fetch_mio_embedding 部分
               embedding_fetchers = [dict(
                ...
                colossusdb_embd_model_name="kaiworks_fountain_long_sequence_transformer",
                colossusdb_embd_table_name="fountain_lst_v1_1",
    ...
    def count_sid(self):
      return self.retrieve_by_common_attr(attr="user_effective_play_semantic_id_list_int", reason=9000) \
        .copy_item_meta_info(
          save_item_id_to_attr="item_id",
        ) \
        .count_reco_result(save_count_to = "origin_sid_length") \
        .attr_statistic(
          statistic_config=[{
            "attr_name": "item_id",
            "aggregator": "count",
            "save_values_to": "sid_list",
            "save_results_to": "sid_count_list",
          }],
        ) \
        .limit(0) \
        .retrieve_by_common_attr(attr='sid_list', reason=9001) \
        .count_reco_result(
          save_count_to = "sid_num",
          target_reason = 9001,
        ) \
        .copy_item_meta_info(
          save_item_id_to_attr="item_id",
        ...
        .dispatch_common_attr(
          from_common_attr = "sid_count_list",
          to_item_attr = "sid_count",
        ) \
        .sort(score_from_attr = "sid_count") \
        .log_debug_info(
          for_debug_request_only = True,
        ...
        .pack_item_attr(
          ...
          mappings = [
            ...
            "to_common_attr": "sid_list_sorted",
            },
            {
              "from_item_attr": "sid_count",
              "to_common_attr": "sid_count_list_sorted"
            }
          ],
        ) \
        .enrich_attr_by_py(
          function_set = FunctionSet,
          py_function = FunctionSet.extract_sid_list_func
        ) \
            .perflog_attr_value(
          check_point = "sid_count",
          common_attrs = [
            "sid_num",
            "extract_sid_list_length"
          ],
          item_attrs = [
            "sid_count"
          ]
    ...
    def prepare(self):
      return self.copy_user_meta_info(
        ...
        .fetch_remote_embedding(
          ...
          input_attr_name = "user_effective_play_pid_list",
  ...
  # .enrich_attr_by_py(
    ...
    def retrieve(self):
      return self.retrieve_by_remote_index(
        name = "test",
...
service.build(output_file=os.path.join(model_dir, "dynamic_json_config_count.json"))
