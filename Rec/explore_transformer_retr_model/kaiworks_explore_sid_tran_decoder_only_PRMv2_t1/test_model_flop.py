...
def dump_prm_variable_order(graph, tag=""):
    """打印图中所有 prm_model/* 变量的名字、shape 和顺序，用于训练/推理图对比。"""
    all_vars = graph.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES)
    prm_vars = [v for v in all_vars if "prm_model" in v.name]
    print(f"\n===== PRM Variable Order [{tag}] =====")
    for i, v in enumerate(prm_vars):
        print(f"  [{i:2d}] {v.name:60s} shape={v.shape.as_list()}")
    print(f"===== Total prm_model vars: {len(prm_vars)} =====\n")
...
def main():
    ...
    with g_train.as_default():
        ...
        with tf.Session(graph=g_train, config=cfg) as sess:
            ...
            # 打印 PRM 变量顺序（用于训练/推理对比）
            dump_prm_variable_order(g_train, tag="train")
    ...
    with g_infer.as_default():
        ...
        with tf.Session(graph=g_infer, config=cfg) as sess:
            ...
            # 打印 PRM 变量顺序（用于训练/推理对比）
            dump_prm_variable_order(g_infer, tag="infer")
