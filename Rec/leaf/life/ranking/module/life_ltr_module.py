...
class LifeLtrModule(CommonModule):
    ...
    def process(self) -> None:
        self.flow \
            .if_("enable_life_ltr_predict == 1") \
                .delegate_enrich(
                    kess_service="{{life_ltr_predict_service}}",
                    recv_item_attrs=[
                        {"name": "life_click", "as": "life_ltr_pctr"}
                    ],
                    timeout_ms=100,
                    ...
                    request_type="life_ltr_predict",
                    partition_size="{{life_ltr_partition_size}}",
                ) \
            .end_()
