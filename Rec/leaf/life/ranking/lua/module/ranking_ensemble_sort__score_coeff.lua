...
function score_coeff_calculate_stage2()
    ...
    return score_consume_time_ltr, score_pctr_x_pcltr, score_pctr_x_pepstr, score_pctr_x_pptr, score_pctr_x_pdtr, score_pctr_x_pftr, score_pctr_x_fr_score2, score_pctr_x_awesome_wtd
end

function get_life_pctr_pcoc_score()
    -- 获取河图一级类目
    local hetu_level_one_array = hetu_tag_level_info__hetu_level_one or {}
    local hetu_level_one_attr = hetu_level_one_array[1] or 0
    -- 获取用户活跃度
    local user_active_degree = math.max(uNebulaXlifeVisitDays30dKV or 0, uNebulaDoubleFindVisitDays30dKV or 0)
    -- 根据用户活跃度和河图类目从配置中获取对应的PCOC值
    local active_hetu_pctr_pcoc_score = 1.0
    if (user_active_degree >= 0 and user_active_degree <= 3) then
        active_hetu_pctr_pcoc_score = shallow_consumer_user_pcoc[hetu_level_one_attr] 
    elseif (user_active_degree >= 4 and user_active_degree <= 7) then 
        active_hetu_pctr_pcoc_score = secondary_shallow_consumer_user_pcoc[hetu_level_one_attr]
    elseif (user_active_degree >= 8 and user_active_degree <= 14) then
        active_hetu_pctr_pcoc_score = moderate_user_pcoc[hetu_level_one_attr]
    elseif (user_active_degree >= 15 and user_active_degree <= 21) then 
        active_hetu_pctr_pcoc_score = sub_deep_user_pcoc[hetu_level_one_attr]
    elseif (user_active_degree >= 22 and user_active_degree <= 30) then 
        active_hetu_pctr_pcoc_score = sub_deep_user_pcoc[hetu_level_one_attr]
    else
        active_hetu_pctr_pcoc_score = 1.0
    end
    if (active_hetu_pctr_pcoc_score == nil) then
        active_hetu_pctr_pcoc_score = 1.0
    end
    return active_hetu_pctr_pcoc_score
end
...
function get_cutoff_rate()
    local vd_cnt = explore_reco_leaf_rank_model_vd_input_count or 0
    local pic_cnt = explore_reco_leaf_rank_model_pic_input_count or 0
    local pic_max_thred = pic_cutoff_rate_max_thred or 0.15
    local out_num = life_fr_s2_out_num or 320

    local vd_cnt_out = vd_cnt
    local pic_cnt_out = pic_cnt

    local pic_rate = pic_cnt / (vd_cnt + pic_cnt)
    pic_cnt_out = math.floor(out_num * math.min(pic_rate, pic_max_thred))
    vd_cnt_out = out_num - pic_cnt_out
    return vd_cnt_out, pic_cnt_out
end
