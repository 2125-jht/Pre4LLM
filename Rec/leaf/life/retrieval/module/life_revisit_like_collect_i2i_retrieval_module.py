...
class LifeRevisitLikeCollectI2iRetrievalModule(RetrievalModule):
  ...
  def process(self) -> None:
    self.flow \
      .gen_common_attr_by_lua(
        attr_map = {
          "user_redis_key" : "'' .. tostring(_USER_ID_)"
        }
      ) \
      .get_common_attr_from_redis(
        cluster_name = "recoMissPhoto",
        redis_params = [
          {
            "redis_key": "{{user_redis_key}}",
            "output_attr_name": "revisit_list"
          }
        ]
      ) \
      .split_string(
        input_common_attr = "revisit_list",
        output_common_attr = "revisit_pid_list",
        delimiters=",",
        parse_to_int = True
      ) \
      .get_remote_embedding(
        ...
        item_list_from_attr = "revisit_pid_list",
