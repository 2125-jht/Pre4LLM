...
class WhiteAuthorQueueParitioner(ChannelSortQueuePartitioner):
  def __init__(self, name, flow, config):
    super().__init__(name, flow, config)

  def _calculate_flag(self):
    self.flow \
      .if_("enable_use_is_support_author == 1") \
        .copy_attr(
          attrs=[{
            "from_item": "is_white_author",
            "to_item": self._flag_attr,
          }]
        ) \
      .end_()

class WhiteAuthorQueuePrerankScorer(ChannelSortQueueScorer):
  def __init__(self, name, flow, config):
    super().__init__(name, flow, config)
