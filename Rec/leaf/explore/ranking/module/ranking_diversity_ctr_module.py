...
class RankingDiversityCtrModule(CommonModule):
    def __init__(self, name: str) -> None:
        super().__init__(name)

    def process(self) -> None:
        self.flow \
        .if_("enable_explore_diversity_ctr == 1") \
          .explore_custom_trim_user_info(
            user_info_attr = "userInfo",
            save_trimed_user_info_to_attr = "explore_diversity_ctr_trimmed_user_info",
            trim_user_info = [
              "id",
              "device_id",
              "gender",
          ...
          .delegate_enrich(
            kess_service = "{{explore_diversity_ctr_infer_name}}",
            recv_item_attrs = [
              {"name": "diversity_ctr", "as": "explore_diversity_ctr_score"}
            ],
            timeout_ms = 100,
            send_common_attrs = [
              "explore_diversity_ctr_trimmed_user_info",
            ...
            partition_size = "{{explore_diversity_ctr_partition_size}}"
          ) \
        .end_()