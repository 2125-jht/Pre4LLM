...
class CascadingPicFcLtrPredictModule(CommonModule):
  ...
  def process(self) -> None:
    self.flow \
        ...
        .end_() \
        .if_("enable_pic_cascade_ltr_infer == 1") \
          .delegate_enrich(
            kess_service="{{explore_mc_pic_ltr_infer_service}}",
            recv_item_attrs=[
              {"name": "weighted_ctr", "as": "pic_mc_ltr_ctr"},
              {"name": "fvtr", "as": "pic_mc_ltr_fvtr"},
              {"name": "view_cnt", "as": "pic_mc_ltr_wtd"},
            ],
            timeout_ms=50,
            send_item_attrs=self.pic_ltr_photo_features,
            send_common_attrs=self.pic_ltr_user_features,
            request_type="default",
            partition_size="{{explore_mc_pic_ltr_infer_partition_size}}",
            target_item={
              "is_picture": 1,
            }
          ) \
