...
function full_rank_score_cal()
    ...
    local ctr_power = explore_rerank_ctr_power_adjust_weight or 1.0
    local explore_rerank_fetr_feff_power = explore_rerank_fetr_feff_power or 1.0
    local pctr_adjust = pctr^ctr_power
    ...
    local pltr_score = (no_pctr_multiply > 0 and pltr * score_factor or pltr * score_factor * pctr_adjust) * (1.0 + (pltr_duration_debias_coffe or 0.0))
    local fr_score1_score = no_pctr_multiply > 0 and fr_score1 * score_factor or fr_score1 * score_factor * pctr_adjust
    local fr_score2_score = no_pctr_multiply > 0 and fr_score2 * score_factor or fr_score2 * score_factor * pctr_adjust
    local l2r_score_score = l2r_score * score_factor
    local pftr_score = (no_pctr_multiply > 0 and pftr * score_factor or pftr * score_factor * pctr_adjust) * (1.0 + (pftr_duration_debias_coffe or 0.0))
    local duration_gt_58s_score = duration_gt_58s * score_factor
    local pptr_score = pptr * score_factor
    local plvtr_score = (no_pctr_multiply > 0 and plvtr * score_factor * coeff or plvtr * score_factor * pctr_adjust * coeff) * (1.0 + (plvtr_duration_debias_coffe or 0.0))
    local pepstr_score = no_pctr_multiply > 0 and pepstr * score_factor or pepstr * score_factor * pctr_adjust
    local explore_fr_ensemble_score_score = explore_fr_ensemble_score
    --预留score 实验看是否可以去掉
    local pcltr_score = (no_pctr_multiply > 0 and pcltr * score_factor or pcltr * score_factor * pctr_adjust) * (1.0 + (pcltr_duration_debias_coffe or 0.0))
    local fetr_score = no_pctr_multiply > 0 and fetr * score_factor or fetr^explore_rerank_fetr_feff_power * score_factor * pctr_adjust
    local fountain_eff_score = no_pctr_multiply > 0 and fountain_eff * score_factor or fountain_eff^explore_rerank_fetr_feff_power * score_factor * pctr_adjust
    --预留diversity 要修改c++代码后续加
    local pcmtr_score = (no_pctr_multiply > 0 and pcmtr * score_factor or pcmtr * score_factor * pctr_adjust) * (1.0 + (pcmtr_duration_debias_coffe or 0.0))
    local pcmef_score = no_pctr_multiply > 0 and pcmef * score_factor or pcmef * score_factor * pctr_adjust
