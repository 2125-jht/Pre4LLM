...
# 从不同的 来源处获取 author list, 最后用统一的倒排索引做 a2i 召回
class CommonRemoteIndexRetrievalModule(RetrievalModule):
  ...
  def process(self):
    ...
    self.flow \
      .if_("enable_shuffle_retr_result ~= nil and enable_shuffle_retr_result == 1", to_be_delete = "date=2024-05-29;committer=liubaoan") \
      ...
      .sort(score_from_attr = "index_score") \
  ...
  def search_num(self) -> int:
    return self.config.get("search_num", 10)
