...
if args.with_kai_v2:
    ...
    def filter_mask_wrapper(dataset):
        ...
        def mask_fn(batch):
            ...
            # ===== Part 1: 按 duration 分档设置播放时长阈值（均为绝对阈值） =====
            # 不同时长视频的"有效消费"标准不同，全部使用绝对播放时长阈值
            #   duration < 40s        → playing_time >= 27s
            #   40s <= dur < 80s      → playing_time >= 50s
            #   80s <= dur < 120s     → playing_time >= 60s
            #   duration >= 120s      → playing_time >= 70s
            threshold = tf.where(duration_ms < 40000.0,    tf.ones_like(duration_ms) * 27000.0,
                       tf.where(duration_ms < 80000.0,    tf.ones_like(duration_ms) * 50000.0,
                       tf.where(duration_ms < 120000.0,   tf.ones_like(duration_ms) * 60000.0,
                                                  tf.ones_like(duration_ms) * 70000.0)))
