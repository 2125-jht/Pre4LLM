...
base_config = '/home/renyuxin/trinkle/kuiba_to_kai/model_pairwise/base.yaml'
...
if (args.mode == 'train'):
  ...
  for loss_name ,model in loss_point_dict.items():
    if "next" in loss_name:
      ...
      loss_point = tf.losses.log_loss(labels=labels_point, predictions=preds_point, weights = weights_point, reduction=tf.losses.Reduction.SUM)
      # sum_loss += loss_point
      wt_reshape = tf.reshape(weights_point, [-1, 6])
      pred_reshape = tf.reshape(preds_point, [-1, 6])
      logit_reshape = tf.reshape(logit_point, [-1, 6])
      label_reshape = tf.reshape(labels_point,[-1,6])

      # y_pred_sorted, indices_pred = tf.nn.top_k(pred_shape, k=wt_shape.shape[-1], sorted=True)
      # y_true_sorted, _ = tf.nn.top_k(wt_shape, k=wt_shape.shape[-1], sorted=True)
      # true_sorted_by_preds = tf.batch_gather(logit_shape, indices_pred)

      true_diffs = wt_reshape[:, :, None] - wt_reshape[:, None, :]
      padded_pairs_mask = tf.greater(true_diffs, 0)

      # pos_idxs = tf.range(1,wt_shape.shape[-1]+ 1)
      # D = (1.0/tf.cast(pos_idxs, tf.float32))[tf.newaxis, :]
      scores_diffs = tf.expand_dims(pred_reshape, -1) - tf.expand_dims(pred_reshape, 1)
      scores_diffs = tf.clip_by_value(scores_diffs, -1e8, 1e8)
      # weights = tf.abs(D[:, :, tf.newaxis]-D[:, tf.newaxis, :])
      # weights *= tf.abs(true_sorted_by_preds[:, :, tf.newaxis] - true_sorted_by_preds[:, tf.newaxis, :])

      weights = tf.sigmoid(scores_diffs)
      loss_pair = tf.reduce_sum(-tf.log(weights)*tf.cast(padded_pairs_mask, dtype=tf.float32))
      targets.append(("pos" + str(0), tf.reshape(tf.nn.sigmoid(logit_reshape[:,0]),[-1,1]), tf.reshape(label_reshape[:,0],[-1,1]), sample_flag, "auc"))
      targets.append(("wpos" + str(0), tf.reshape(tf.nn.sigmoid(logit_reshape[:,0]),[-1,1]), tf.reshape(wt_reshape[:,0],[-1,1]), sample_flag, "linear_regression"))
      # loss_tensor_dict["pair_loss"] = loss_pair
      loss_tensor_dict["point_loss"] = loss_point
