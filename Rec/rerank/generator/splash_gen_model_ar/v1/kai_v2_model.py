...
if args.with_kai_v2:
    ...
    def filter_mask_wrapper(dataset):
        ...
        def mask_fn(batch):
            ...
            #         # tf.math.less(realshow, 2),
            #         tf.math.less(fountain_click_sum, 1),
            #     ),
            #     is_short_request,
            # )
            return mask
            # return tf.math.less(realshow, 1)
        # 3.返回mask_fn
        return mask_fn
    # 注册过滤条件
...
def get_label_dict(list_dim=30):
    label_value_dict = {}
...
# print_ops.append(tf.print("========> context_pctr: ", dense_value_dict["context_pctr"], summarize=-1, output_stream=sys.stdout))
# print_ops.append(tf.print("========> context_pltr: ", dense_value_dict["context_pltr"], summarize=-1, output_stream=sys.stdout))
# print_ops.append(tf.print("========> context_pwtd: ", dense_value_dict["context_pwtd"], summarize=-1, output_stream=sys.stdout))
# print_ops.append(tf.print("========> context_pvtr: ", dense_value_dict["context_pvtr"], summarize=-1, output_stream=sys.stdout))
# print_ops.append(tf.print("========> context_pwtr: ", dense_value_dict["context_pwtr"], summarize=-1, output_stream=sys.stdout))
...
if is_training:
    ...
    _, gen_loss, _, pos_output, predict = model_class.model()
    ...
    with tf.control_dependencies(print_ops):
        logits = tf.reduce_sum(pos_output, axis=-1)
        ...
        for i in range(3):
            targets.append(('ltr_{}'.format(i), tf.expand_dims(pos_output[:,i], -1), tf.expand_dims(true_label[:, i], -1), tf.ones_like(tf.expand_dims(true_label[:, i], -1)), 'auc'))
    ...
    '''
    logits: [batch_size, beam_size, vocab_size, seq_length]
    ...
    for i in range(1): # beam size
        ...
        print(f"selected_indices_i shape: {selected_indices_i.shape}")
        tensor_zeros = tf.zeros(vocab_size, dtype=tf.int32)  # [vocab_size]
        print(f"tensor_zeros shape: {tensor_zeros.shape}")

        output_tensor = tf.tensor_scatter_nd_update(
            tensor_zeros, 
            tf.expand_dims(selected_indices_i, 1), 
            tf.range(selected_indices_i.shape[0], 0, -1)
        ) # 相当于给个分数
