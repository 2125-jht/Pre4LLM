...
class StackedTransformerModel():
    def __init__(self, num_layers, dim, num_heads, dk, dropout_rate, k, training=False):
        ...
        self.decoder_layers = [DecoderLayer(f"decoder_layer_{i}", dim, num_heads, dk, dropout_rate) for i in range(num_layers)]
...
class FountainDeepLtrMultiTaskModel:
    ...
    def choose_item(self, decoder_emb, vocab_emb, method=0, use_gumbel_softmax=False, tau=1.0, hard=True):
        ...
        with tf.variable_scope("choose_item", reuse=tf.AUTO_REUSE):
            if not self._training:
    ...
    def model(self, training=True, decode_method="beam_search"):
        ...
        with tf.variable_scope("prepare", reuse=tf.AUTO_REUSE):
            ...
            if self._training:
                ...
                like_label = tf.reshape(label_dicts['context_info__like_list'], [-1, self.candidates_size])
                like_label = like_label[:,:self.list_size] # (?,list_size)，截断为 list_size 个
                follow_label = tf.reshape(label_dicts['context_info__follow_list'], [-1, self.candidates_size])
                follow_label = follow_label[:,:self.list_size]
                forward_label = tf.reshape(label_dicts['context_info__forward_list'], [-1, self.candidates_size])
                forward_label = forward_label[:,:self.list_size]
                comment_label = tf.reshape(label_dicts['context_info__comment_list'], [-1, self.candidates_size])
                comment_label = comment_label[:,:self.list_size]
                next_label = tf.reshape(label_dicts['context_info__fountain_slide_to_next_list'],  [-1, self.candidates_size])
        ...
        with tf.variable_scope("hidden_state_encoder", reuse=tf.AUTO_REUSE):
            concat_emb = tf.concat([user_embs[:,0,:], source_embs[:,0,:]], axis=-1)
            hidden_states = tf.layers.dense(concat_emb, 128, activation=tf.nn.relu)
            hidden_states = tf.layers.dense(hidden_states, 64, activation=tf.nn.relu) # (?,64)
            encoder_output = tf.expand_dims(hidden_states, axis=1) # (?,1,64)
            print("encoder output shape ",encoder_output.shape)
        with tf.variable_scope("generator", reuse=tf.AUTO_REUSE):
            # 初始化transformer模型
            model = StackedTransformerModel(num_layers=1, dim=64, num_heads=4, dk=64, dropout_rate=0.1, k=6)
            self.candidate_emb = model.forward(common_embs, training=True)
            self.vocab_embs = tf.concat([pad_embedding, sos_embedding, self.candidate_emb, eos_embedding], axis=1) # (?,candidates_size+3,32)
            if self._training:
                item_embeddings = tf.gather_nd(self.vocab_embs, gather_indices) # (?,candidates_size+3,32) 中查找对应 list idx 的 emb
                print("item_embeddings shape", item_embeddings.shape) # (?,list_size+1,32)

                item_embedding = model.forward_decoder(encoder_output, item_embeddings, training=True) # (?,list_size+1,32)
                print("item_embedding shape ", item_embedding.shape)

                # nce loss
                nce_loss = self.nce_loss(item_embedding[:, :-1, :], self.candidate_emb)

                # 从候选集选取 item, 0: nn, 1: cosine; 是否进行采样
                predict = self.choose_item(item_embedding, self.vocab_embs, method=0, use_gumbel_softmax=False, tau=0.1, hard=True) # (?,list_size+1,candidates_size+3)
                ...
                # advantage_reward = play_time + relate_like * 5.0 + relate_follow * 20.0 + relate_commnet * 10.0
                ...
                position_weight = tf.cast(tf.tile(tf.expand_dims(tf.constant([1.5, 1.2, 1]), axis=0), [batch_size, 1]), dtype=tf.float32) # (?, 3)
                ...
                # self.print_ops.append(tf.print("seq_weight ", seq_weight, summarize=8, output_stream=sys.stdout))
                ...
                # gen_loss = tf.reduce_sum(valid_pos_output * advantage * seq_advantage, axis=-1)
                gen_loss = tf.reduce_sum(valid_pos_output * advantage, axis=-1)
                print("gen_loss shape", gen_loss.shape)
                gen_loss = tf.reduce_mean(gen_loss)
                return predict, nce_loss, gen_loss
            
            else:
                def beam_search(model, encoder_output, sos_token, eos_token, pad_token, beam_size=3, max_length=6):
                    batch_size, vocab_size, vocab_dim = tf.shape(self.vocab_embs)[0], tf.shape(self.vocab_embs)[1], self.vocab_embs.shape[-1]
                    ...
                    candidate_emb = tf.tile(tf.expand_dims(self.candidate_emb, axis=1), [1, beam_size, 1, 1])  # [batch_size, beam_size, vocab_size, dim])
                    vocab_embs = tf.tile(tf.expand_dims(self.vocab_embs, axis=1), [1, beam_size, 1, 1])  # [batch_size, beam_size, vocab_size, dim])
                    probs = []
                    # 开始 Beam Search
                    for step in range(max_length):
                        ...
                        decoder_input = tf.gather_nd(vocab_embs, gather_indices) # (?, beam_size,candidates_size+3,32) 中查找对应 list idx 的 emb
                        ...
                        # # nce loss 不实际计算
                        nce_loss = self.nce_loss(decoder_output[:, 0, :-1, :], candidate_emb[:, 0, :, :])
                        # 计算 logits
                        # logits = tf.matmul(self.vocab_embs, tf.transpose(decoder_output, perm=[0, 1, 3, 2])) # [batch_size, beam_size, vocab_size, seq_length]
                        logits = self.choose_item(decoder_output, vocab_embs, method=0, use_gumbel_softmax=False, tau=0.1, hard=True) # [batch_size, beam_size, seq_length, vocab_size]
