...
class FountainDeepLtrMultiTaskModel:
    def __init__(self, parameters_dict, label_value_dict, dense_value_dict, print_ops, list_size, candidates_size, dim=32, extra_param_dict= None, training=True):
        ...
        self.evtr_embedding = tf.get_variable("evtr_embedding", shape=[1, 64], initializer=tf.random_uniform_initializer())
        self.lvtr_embedding = tf.get_variable("lvtr_embedding", shape=[1, 64], initializer=tf.random_uniform_initializer())
        self.ltr_embedding = tf.get_variable("ltr_embedding", shape=[1, 64], initializer=tf.random_uniform_initializer())
        self.wtr_embedding = tf.get_variable("wtr_embedding", shape=[1, 64], initializer=tf.random_uniform_initializer())
        self.cmtr_embedding = tf.get_variable("cmtr_embedding", shape=[1, 64], initializer=tf.random_uniform_initializer())
        self.wtd_embedding = tf.get_variable("wtd_embedding", shape=[1, 64], initializer=tf.random_uniform_initializer())
        self.finish_embedding = tf.get_variable("finish_embedding", shape=[1, 64], initializer=tf.random_uniform_initializer())
    ...
    def choose_item(self, decoder_emb, vocab_emb, method=0, use_gumbel_softmax=False, tau=1.0, hard=True):
        ...
        if not self._training:
            ...
            beam_size = tf.shape(decoder_emb)[1]
            list_size = tf.shape(decoder_emb)[2]
            vocab_size = tf.shape(vocab_emb)[2]
            dim = vocab_emb.shape[-1]
            decoder_emb = tf.reshape(decoder_emb, [batch_size * beam_size, list_size, dim])
            vocab_emb = tf.reshape(vocab_emb, [batch_size * beam_size, vocab_size, dim])
        if method == 0:
        ...
        if not self._training:
            predict = tf.reshape(predict, [batch_size, beam_size, list_size, vocab_size])
        return predict
    ...
    def mha_layer_4d(self, name, query, key, dim_in=64, num_heads=4, dropout_rate=0.0, training=False, causal_mask=False):
        ...
        '''
        with tf.variable_scope("mha_layer_4d", reuse=tf.AUTO_REUSE):
    ...
    def model(self, training=True, decode_method="beam_search"):
        ...
        with tf.variable_scope("prepare", reuse=tf.AUTO_REUSE):
            ...
            if self._training:
                ...
                like_label = tf.cast(tf.reshape(label_dicts['context_info__like_list'], [-1, self.candidates_size]), tf.float32)
                ...
                playtime = tf.cast(tf.clip_by_value(playtime / 1000, 0, 600), dtype=tf.float32)
                svtr_label = tf.cast(playtime <= 3, dtype=tf.float32) # (?,list_size)
        ...
        with tf.variable_scope("generator", reuse=tf.AUTO_REUSE):
            ...
            evtr_embedding = tf.tile(tf.expand_dims(self.evtr_embedding, axis=0), [batch_size, self.list_size, 1]) # (?,list_size,64)
            ltr_embedding = tf.tile(tf.expand_dims(self.ltr_embedding, axis=0), [batch_size, self.list_size, 1]) # (?,list_size,64)
            wtr_embedding = tf.tile(tf.expand_dims(self.wtr_embedding, axis=0), [batch_size, self.list_size, 1]) # (?,list_size,64)
            cmtr_embedding = tf.tile(tf.expand_dims(self.cmtr_embedding, axis=0), [batch_size, self.list_size, 1]) # (?,list_size,64)
            wtd_embedding = tf.tile(tf.expand_dims(self.wtd_embedding, axis=0), [batch_size, self.list_size, 1]) # (?,list_size,64)
            finish_embedding = tf.tile(tf.expand_dims(self.finish_embedding, axis=0), [batch_size, self.list_size, 1]) # (?,list_size,64)
            if self._training:
                evtr_embedding = evtr_embedding * tf.expand_dims(click_label, axis=-1) # (?,list_size,64)
                ltr_embedding = ltr_embedding * tf.expand_dims(like_label, axis=-1) # (?,list_size,64)
                wtr_embedding = wtr_embedding * tf.expand_dims(follow_label, axis=-1) # (?,list_size,64)
                cmtr_embedding = cmtr_embedding * tf.expand_dims(comment_label, axis=-1) # (?,list_size,64)
                wtd_embedding = wtd_embedding * tf.expand_dims(wtd_label, axis=-1) # (?,list_size,64)
                finish_embedding = finish_embedding * tf.expand_dims(playtime / 30.0, axis=-1) # (?,list_size,64)
            else:
                evtr_embedding = tf.reshape(evtr_embedding, [1, -1, evtr_embedding.shape[-1]]) * 1.0
                ltr_embedding = tf.reshape(ltr_embedding, [1, -1, ltr_embedding.shape[-1]]) * 0.1
                wtr_embedding = tf.reshape(wtr_embedding, [1, -1, wtr_embedding.shape[-1]]) * 0.1
                cmtr_embedding = tf.reshape(cmtr_embedding, [1, -1, cmtr_embedding.shape[-1]]) * 0.1
                wtd_embedding = tf.reshape(wtd_embedding, [1, -1, wtd_embedding.shape[-1]]) * 1.0
                finish_embedding = tf.reshape(finish_embedding, [1, -1, finish_embedding.shape[-1]]) * 0.5
            action_emb = tf.concat([evtr_embedding, ltr_embedding, wtr_embedding, cmtr_embedding, wtd_embedding, finish_embedding], axis=-1)
            action_emb = tf.layers.dense(action_emb, 64, activation=tf.nn.relu, name="action_fc") # (?,list_size,64)
            action_emb = tf.concat([action_emb, tf.zeros([batch_size, 1, 64], dtype=tf.float32)], axis=1) # (?,list_size+1,64)
            ...
            if self._training:
                ...
                item_embeddings = item_embeddings + action_emb
                ...
                # advantage
                advantage_reward = playtime # 得到真实时长
                like_weight = tf.where(playtime >= 7.0, like_label * 20.0, tf.zeros_like(like_label))
                like_weight = tf.where(playtime >= 20.0, like_label * 100.0, like_weight)
                follow_weight = tf.where(playtime >= 7.0, follow_label * 50.0, tf.zeros_like(follow_label))
                follow_weight = tf.where(playtime >= 20.0, follow_label * 200.0, follow_weight)
                comment_weight = tf.where(playtime >= 7.0, comment_label * 50.0, tf.zeros_like(comment_label))
                comment_weight = tf.where(playtime >= 20.0, comment_label * 200.0, comment_weight)
                next_weight = tf.where(next_label >= 1.0, next_label * 10.0, tf.ones_like(next_label) * (-100.0))
                svtr_weight = tf.where(svtr_label < 1.0, svtr_label * 0.0, tf.ones_like(svtr_label) * (-100.0))
                advantage_reward = playtime + next_weight + svtr_weight + like_weight + follow_weight + comment_weight
                advantage_reward = tf.where(advantage_reward > 0.0, advantage_reward, tf.zeros_like(advantage_reward, dtype=tf.float32))
                # advantage_reward = playtime + next_label * 4.0 + like_label * 10.0 + follow_label * 50.0 + comment_label * 50.0 + forward_label * 20.0
                ...
                for i in range(self.list_size):
                    ...
                    bpr_loss2 = self.bpr_loss(ith_prediction, advantage_reward, tf.expand_dims(bpr_threshold, axis=-1), realshow_label > 0)
                ...
                mask = tf.concat([realshow_label, zeros], axis=1) # (?, list_size)
                mask = tf.cast(mask,dtype=tf.float32)
                valid_pos_output = -tf.log(pos_output+1e-9)*mask
                ...
                seq_weight = tf.reduce_sum(item_weight * mask) / valid_counts # (?,)
                ...
                # self.print_ops.append(tf.print("advantage ", advantage[2], summarize=8, output_stream=sys.stdout))
                # self.print_ops.append(tf.print("seq_advantage ", seq_advantage, summarize=8, output_stream=sys.stdout))
                seq_advantage = tf.expand_dims(seq_advantage, axis=1)
                weight = 0.9 * advantage + 0.1 * seq_advantage
                weight = tf.where(tf.concat([svtr_label, tf.zeros([batch_size, 1], dtype=tf.float32)], axis=-1) > 0, tf.zeros_like(weight), weight) # (?, list_size + 1)
                gen_loss = tf.reduce_sum(valid_pos_output * weight, axis=-1)
                print("gen_loss shape", gen_loss.shape)
                gen_loss = tf.reduce_mean(gen_loss)
                return predict, nce_loss, gen_loss, bpr_loss
                ...
                def beam_search(model, encoder_output, sos_token, eos_token, pad_token, beam_size=3, max_length=6):
                    ...
                    for step in range(max_length):
                        ...
                        # decoder forward
                        decoder_output = model.forward_decoder(tf.reshape(encoder_output, [batch_size * beam_size, tf.shape(encoder_output)[-2], vocab_dim]),
