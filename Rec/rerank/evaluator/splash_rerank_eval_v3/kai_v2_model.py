...
def get_play_labels(duration, play_time):
    ...
    # 有效播、长播
    # thresholds = [[7.0, 12.0], [9.0, 14.0], [11.0, 18.0], [13.0, 24.0], [12.0, 27.0], [7.0, 21.0]]
...
if is_training:
    ...
    # # print_ops.append(tf.print("click_label ", click_label[8], summarize=10, output_stream=sys.stdout))
    # print_ops.append(tf.print("wtd_label ", wtd_label[8], summarize=10, output_stream=sys.stdout))
    # print_ops.append(tf.print("vtr_label ", vtr_label[8], summarize=10, output_stream=sys.stdout))
    # print_ops.append(tf.print("play_time_s ", play_time_s[8], summarize=10, output_stream=sys.stdout))
    # print_ops.append(tf.print("pvtr_wt ", pvtr_wt[8], summarize=10, output_stream=sys.stdout))
    # print_ops.append(tf.print("pwtd_wt ", pwtd_wt[8], summarize=10, output_stream=sys.stdout))
    # print_ops.append(tf.print("context_pwtd ", context_pwtd[8], summarize=10, output_stream=sys.stdout))
    # print_ops.append(tf.print("evtr_weight ", evtr_weight[8], summarize=10, output_stream=sys.stdout))
    ...
    with tf.control_dependencies(print_ops):
        ...
        for loss_name in point_wise_output_dict:
            ...
            if loss_name == "vtr":
                ...
                loss = tf.losses.log_loss(labels=lvtr_label, predictions=output, weights=weight_with_mask, reduction=tf.losses.Reduction.SUM)
                #loss = loss / 100.0
                ...
                evtr_weight = tf.where(click_label > 0.0, evtr_weight, tf.ones_like(evtr_weight))
                evtr_weight = tf.where(tf.logical_and(click_label > 0.0, duration_s < 7.0), evtr_weight * 0.7, tf.ones_like(evtr_weight))
                loss = tf.losses.log_loss(labels=click_label, predictions=output, weights=weight_with_mask * evtr_weight, reduction=tf.losses.Reduction.SUM)
