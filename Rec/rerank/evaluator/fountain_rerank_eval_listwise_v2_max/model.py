...
# 暂停训练另外两种 List 时长参数化；保留实现，后续只需打开该开关。
ENABLE_AUXILIARY_WATCH_TIME_METHODS = False
...
class EvaluatorModel:
    def __init__(self, parameters_dict, print_ops, list_size, candidates_size, list_num,
                 point_wise_tasks, max_interaction_value, segment_num, training=True):
        ...
        # fountain_gen_time 的时间段位置记忆。它只服务于 item 级生成式
        # 时长 Head，不与 List Value 分支的位置编码或参数共享。
        self.segment_embeddings = tf.get_variable(
            name='gen_time_segment_embeddings',
            shape=[1, self._segment_num, 64],
            initializer=tf.random_normal_initializer(),
        )
    ...
    def _build_list_value_outputs(
        ...
        with tf.variable_scope("list_value_branch", reuse=tf.AUTO_REUSE):
            ...
            # 和累计互动价值。各任务共享 value_hidden，但保留独立输出头。
            ...
            # 启用的 WT Head 共享显式 Prefix 累计 duration 特征，避免只在
            # 标签编码/秒数解码阶段使用时长。log1p 压缩时长长尾后再投影。
            if list_duration_s is None:
                prefix_duration_log = tf.zeros(
                    [batch_size, self._list_num, self._list_size, 1],
                    dtype=tf.float32,
                )
            else:
                prefix_duration_log = tf.expand_dims(
                    tf.math.log1p(
                        tf.cumsum(
                            tf.maximum(list_duration_s, 0.0),
                            axis=-1,
                        )
                    ),
                    axis=-1,
                )
            duration_hidden = tf.layers.dense(
                prefix_duration_log,
                16,
                activation=tf.nn.leaky_relu,
                name="prefix_duration_hidden",
            )
            watch_time_hidden = tf.layers.dense(
                tf.concat([value_hidden, duration_hidden], axis=-1),
                64,
                activation=tf.nn.leaky_relu,
                name="prefix_watch_time_hidden",
            )

            # 时长方式一：直接在 log1p 空间回归 Prefix WT。
            # softplus 保证解码前非负；累计六个位置的标签理论上最多为
            # 6 * 36000 秒，因此只做与标签上界一致的数值截断。
            ...
            max_watch_time_log = tf.math.log(
                tf.constant(
                    36000.0 * self._list_size + 1.0,
                    dtype=tf.float32,
                )
            )
            prefix_watch_time_log_space = tf.math.expm1(
                tf.minimum(prefix_watch_time_log, max_watch_time_log)
            )

            if ENABLE_AUXILIARY_WATCH_TIME_METHODS:
                # 时长方式二：预测 Prefix WT 相对 Prefix 累计 duration 的比例。
                prefix_watch_time_duration_normalized_ratio = tf.nn.sigmoid(
                    tf.squeeze(
                        tf.layers.dense(
                            watch_time_hidden,
                            1,
                            activation=None,
                            name="prefix_watch_time_duration_normalized_logit",
                        ),
                        axis=-1,
                    ),
                    name="prefix_watch_time_duration_normalized_ratio",
                )

                # 时长方式三：预测 Prefix WT 在 duration 条件分桶中的连续位置。
                prefix_watch_time_duration_bucketed_position = tf.nn.sigmoid(
                    tf.squeeze(
                        tf.layers.dense(
                            watch_time_hidden,
                            1,
                            activation=None,
                            name="prefix_watch_time_duration_bucketed_logit",
                        ),
                        axis=-1,
                    ),
                    name="prefix_watch_time_duration_bucketed_position",
                )
            ...
            # 在 log1p 空间建模，压缩深度互动的较大业务权重，并避免少量高价值
            # 样本主导 raw-space 回归。
            ...
            # log-space 方式直接在这里按 length_probs 聚合；辅助方式重新开启后，
            # 在 kai_v2_model.py 完成秒数解码再使用相同公式聚合。
            expected_list_watch_time_legacy = tf.reduce_sum(
                length_probs * prefix_watch_time_log_space,
                axis=-1,
                name="expected_watch_time",
            )
            expected_list_watch_time_log_space = tf.identity(
                expected_list_watch_time_legacy,
                name="expected_list_watch_time_log_space",
            )
            ...
            outputs = {
                ...
                # 保留原 key 作为 log-space 秒数输出的兼容别名。
                ...
                # 保留原 key 作为 log-space 最终结果的兼容别名。
                "expected_list_watch_time": expected_list_watch_time_legacy,
    ...
    def model(self, list_index, list_duration_s=None) -> tuple:
        with tf.variable_scope("evaluator", reuse=tf.AUTO_REUSE):
                    ...
                    for loss_name, output in output_fea_dict.items():
                        with tf.variable_scope(f"output_mlp_{loss_name}", reuse=tf.AUTO_REUSE):
                            if loss_name == "gen_time":
                                # 忠实沿用原 fountain_gen_time：每个 item 的 task
                                # feature 先展开成 N 个时间段 token，再沿时间段轴做
                                # causal decoder；这里的 causal 轴不是 List item 轴。
                                gen_input = tf.layers.dense(
                                    output,
                                    64 * self._segment_num,
                                    activation=tf.nn.leaky_relu,
                                    name="segment_token_projection",
                                )
                                gen_input = tf.reshape(
                                    gen_input,
                                    [-1, self._segment_num, 64],
                                )  # (batch*list_num*list_size, segment_num, 64)
                                decoder = DecoderLayer(
                                    name="gen_time_decoder",
                                    dim=64,
                                    num_heads=2,
                                    dim_in=64,
                                    dropout_rate=0.0,
                                ...
                                gen_time_output = decoder.forward(
                                    gen_input,
                                    self.segment_embeddings,
                                    training=self._training,
                                )
                                gen_time_output = tf.layers.dense(
                                    gen_time_output,
                                    32,
                                    activation=tf.nn.leaky_relu,
                                    name="segment_hidden",
                                ...
                                output_dict[loss_name] = tf.reshape(
                                    gen_time_output,
                                    [
                                        batch_size,
                                        self._list_num * self._list_size,
                                        self._segment_num,
                                    ],
                                )
                            else:
            ...
            # -------- 原有 point-wise 基线分支 --------
            # 虽然监督和输出粒度都是 item，但这里先对 [CLS, item_1,...,item_6]
            # 做不带 causal mask 的 List self-attention：
            #
            #   h_i^list = SelfAttention([CLS, x_1,...,x_6])_i
            #
            # 因此每个位置的 ltr/vtr/click/wtd 都能读取整条候选 List 的内容，
            # 严格说是“List-aware item prediction”，而不是相互独立的单点模型。
            ...
            # PLE 为 point-wise 任务提供共享/独立专家，任务间再做一次 output
            # attention。ltr/vtr/click/wtd 输出单个 sigmoid；gen_time 输出
            # segment_num 个按时间段 causal 建模的 sigmoid。
            # 各任务的标签、权重、loss 和时长解码公式见 kai_v2_model.py。
