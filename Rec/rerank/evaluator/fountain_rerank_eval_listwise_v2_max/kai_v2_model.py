...
LIST_NUM = 30
LIST_SIZE = 6
GEN_TIME_SEGMENT_NUM = 20
GEN_TIME_MIN_DURATION_S = 20.0
GEN_TIME_MAX_DURATION_S = 400.0

# 本轮只验证 point-wise fountain_gen_time。List Value 实现暂时保留，
# 但不进入训练总 loss、不注册训练 metrics，也不导出推理结果。
ENABLE_LIST_VALUE_BRANCH = False
...
def gen_time_label_encode(
    play_time,
    duration,
    num_buckets=GEN_TIME_SEGMENT_NUM,
    min_dur=GEN_TIME_MIN_DURATION_S,
    max_dur=GEN_TIME_MAX_DURATION_S,
):
    """沿用 fountain_gen_time，将 item 播放时长编码成累计分段进度。

    输入和输出形状分别为 [batch, candidate] 与
    [batch, candidate, num_buckets]。前序完整分段为 1，当前分段为
    桶内连续进度，后续分段为 0。
    """
    batch_size = tf.shape(duration)[0]
    candidate_size = duration.shape[1]
    play_time = tf.reshape(play_time, [batch_size * candidate_size, 1])
    duration = tf.reshape(duration, [batch_size * candidate_size, 1])

    duration_clip = tf.clip_by_value(duration, min_dur, max_dur)
    play_time = tf.clip_by_value(play_time, 0.0, duration_clip)
    segment_index = tf.cast(
        tf.range(1, num_buckets + 1),
        tf.float32,
    )[None, :]
    segment_start = duration_clip * (
        (segment_index - 1.0) / float(num_buckets)
    )
    segment_end = duration_clip * (
        segment_index / float(num_buckets)
    )
    segment_length = tf.maximum(segment_end - segment_start, 1e-6)
    segment_done = tf.clip_by_value(
        (play_time - segment_start) / segment_length,
        0.0,
        1.0,
    )
    segment_mask = tf.clip_by_value(
        (duration - segment_start) / segment_length,
        0.0,
        1.0,
    )
    return (
        tf.reshape(
            segment_done,
            [batch_size, candidate_size, num_buckets],
        ),
        tf.reshape(
            segment_mask,
            [batch_size, candidate_size, num_buckets],
...
def gen_time_label_decode(
    ...
    """沿用原 fountain_gen_time 公式，将累计分段预测解码为秒数。"""
    batch_size = tf.shape(duration)[0]
    candidate_size = duration.shape[1]
    ...
    bucket_length = duration_clip / float(num_buckets)
    completed_bucket_count = prediction[:, 0:1] * tf.reduce_sum(
        prediction * segment_mask,
        axis=-1,
        keepdims=True,
    )
    watch_time = completed_bucket_count * bucket_length
    return tf.reshape(watch_time, [batch_size, candidate_size])
...
# 四个原有标量 item 级头，加一个20段生成式时长 item 级头；完整标签、
# 权重与 loss 公式写在下方对应 loss 分支旁。
# item 表示已融合候选 List 上下文；没有独立 point-wise pEVV 或互动行为头，
# 新增互动预估位于共享消费长度分布的 List Value 分支。
...
model_class = EvaluatorModel(
    ...
    segment_num=GEN_TIME_SEGMENT_NUM,
)
...
if is_training:
    ...
    # 启用的 WT Head 显式读取候选 Prefix 累计 duration。标签字典已经按
    ...
    gen_segment_label, gen_segment_mask = gen_time_label_encode(
        play_time=play_time_s,
        duration=duration_s,
    )
    ...
    gen_segment_zeros = tf.zeros(
        [batch_size, 1, GEN_TIME_SEGMENT_NUM],
        dtype=tf.float32,
    )
    list_gen_segment_label = tf.gather(
        tf.concat([gen_segment_zeros, gen_segment_label], axis=1),
        rerank_list_item_idx_flat_list,
        axis=1,
        batch_dims=1,
    )  # (?, list_num, list_size, segment_num)
    list_gen_segment_mask = tf.gather(
        tf.concat([gen_segment_zeros, gen_segment_mask], axis=1),
        rerank_list_item_idx_flat_list,
        axis=1,
        batch_dims=1,
    )  # (?, list_num, list_size, segment_num)
    ...
    with tf.control_dependencies(print_ops):
        ...
        # 四个标量 head 均由 sigmoid 输出 p_i∈[0,1]；gen_time 为每个
        # item 输出 GEN_TIME_SEGMENT_NUM 个 sigmoid。下文统一使用：
        ...
        for loss_name in point_wise_output_dict:
            ...
            if loss_name == "gen_time":
                output = tf.reshape(
                    output,
                    [
                        batch_size,
                        LIST_NUM * LIST_SIZE,
                        GEN_TIME_SEGMENT_NUM,
                    ],
                )
            else:
                output = tf.reshape(
            ...
            if loss_name == "ltr":
                ...
                # 原 fountain_gen_time：逐 item 预测20段累计完成进度，使用
                # 分段 BCE、相邻段单调约束和解码秒数的 log-Huber 联合训练。
                flat_gen_segment_label = tf.reshape(
                    list_gen_segment_label,
                    [
                        batch_size,
                        LIST_NUM * LIST_SIZE,
                        GEN_TIME_SEGMENT_NUM,
                ...
                flat_gen_segment_mask = tf.reshape(
                    list_gen_segment_mask,
                    [
                        batch_size,
                        LIST_NUM * LIST_SIZE,
                        GEN_TIME_SEGMENT_NUM,
                ...
                gen_weight = (
                    ...
                    * flat_gen_segment_mask
                )
                segment_ce_loss = tf.losses.log_loss(
                    labels=flat_gen_segment_label,
                    predictions=output,
                    weights=gen_weight,
                )
                tf.summary.scalar(
                    "fountain_gen_time_ce_loss",
                    segment_ce_loss,
                )

                # 原方法约束前一段完成概率不小于后一段。适配 List 张量后
                # 按有效 item×相邻段数归一化，避免 loss 随有效 List 数波动。
                order_diff = output[:, :, :-1] - output[:, :, 1:]
                order_violation = tf.nn.relu(-order_diff) \
                    * tf.expand_dims(mask, axis=-1)
                valid_item_count = tf.reduce_sum(
                    tf.cast(tf.greater(mask, 0.0), tf.float32)
                )
                order_loss = tf.reduce_sum(order_violation) / (
                    valid_item_count * (GEN_TIME_SEGMENT_NUM - 1) + 1e-6
                )
                tf.summary.scalar(
                    "fountain_gen_time_order_loss",
                    order_loss,
                )

                gen_time_wt = gen_time_label_decode(
                    prediction=output,
                    duration=list_duration_s,
                )
                gen_time_wt_log = tf.math.log1p(gen_time_wt)
                play_time_log = tf.math.log1p(
                    tf.clip_by_value(
                        tf.reshape(
                            list_play_time_s,
                            [batch_size, LIST_NUM * LIST_SIZE],
                        ...
                        0.0,
                        GEN_TIME_MAX_DURATION_S,
                    )
                )
                gen_time_huber_loss = tf.losses.huber_loss(
                    labels=play_time_log,
                    predictions=gen_time_wt_log,
                    weights=mask,
                    delta=0.2,
                )
                tf.summary.scalar(
                    "fountain_gen_time_huber_loss",
                    gen_time_huber_loss,
                )
                loss = (
                    segment_ce_loss
                    + 2.0 * gen_time_huber_loss
                    + 0.1 * order_loss
                )
                targets.append((
                    "gen_time",
                    output,
                    flat_gen_segment_label,
                    gen_weight,
                    "linear_regression",
                ))
                targets.append((
                    "fountain_gen_time_wt",
                    gen_time_wt,
                    tf.reshape(
                        list_play_time_s,
                        [batch_size, LIST_NUM * LIST_SIZE],
        ...
        # - expected_list_watch_time：log-space Prefix WT 按 P(K) 聚合，
        #   单位为秒；
        ...
        if not ENABLE_LIST_VALUE_BRANCH:
            # List Value 张量仍保留在源码中，便于后续一键恢复；本轮不把它们
            # 注册成训练 metrics，避免训练任务继续 fetch 该分支。
            list_value_metric_names = {
                "continuation",
                "expected_consume_length",
                "expected_list_watch_time",
                "expected_list_watch_time_log_space",
            ...
            targets = [
                ...
                if target[0] not in list_value_metric_names
                and not target[0].startswith("prefix_")
                and not target[0].startswith("list_wt_from_")
            ]
    ...
    pgen_time_segments = tf.reshape(
        point_wise_output_dict["gen_time"],
        [-1, LIST_NUM * LIST_SIZE, GEN_TIME_SEGMENT_NUM],
    )
    ...
    duration_s_flat = tf.reshape(
        list_duration_s,
        [-1, LIST_NUM * LIST_SIZE],
    )
    duration_s_flat_int = tf.cast(duration_s_flat, tf.int32)
    pvtr_wt = get_watch_time_from_vtr(pvtr, duration_s_flat_int) # (? , CANDIDATES_SIZE)
    pwtd_wt = wtd_decode(pwtd, duration_s_flat_int, duration_bucket=wtd_buckets, play_time_buckets_ragged=wtd_configs) # (? , CANDIDATES_SIZE)
    pgen_time = gen_time_label_decode(
        prediction=pgen_time_segments,
        duration=duration_s_flat,
    )
    ...
    targets.append((f"pgen_time", pgen_time))
    ...
    # List Value 分支本轮关闭，不导出 expected_list_* / expected_consume_length。
    if ENABLE_LIST_VALUE_BRANCH:
