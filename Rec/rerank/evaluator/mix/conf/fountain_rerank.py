...
def slide_model(parameters_dict, loss_name):
  ...
  n_s = 5
  with tf.variable_scope("slide_model", reuse=tf.AUTO_REUSE):
    atten = user_tower("ctr_user", user, atten, query_dim = 320, nh = 1, dim = 80)
    query = tf.concat([user, atten], 1)
    for i in range(n):
      p = parameters[i * batch : (i+1) * batch]
      ctr_labels.append(p[-2])
      pxtr3 = tf.concat(p[0:2], 1)
      pxtr1 = tf.concat(p[2:12], 1)
      pxtr2 = tf.concat(p[12:17], 1)
      data0 = tf.concat(p[17:-3], 1)
      #labels.append(tf.cast(p[-1], tf.int32))
      labels.append(p[-1])
      weights.append(p[-3])
      
      input = simple_dense_network(tf.concat([data0, query, pxtr3], 1), [256, 128], name + '_item_dnn', name + '_item_dnn{}_param')
      data1 = gate_tower(name + '_vtr_dnn1', input, [64], pxtr1)
      data2 = gate_tower(name + '_vtr_dnn2', input, [64], pxtr2)
      gate = gate_layer(name + '_vtr_gate' + str(i), query, 64)
