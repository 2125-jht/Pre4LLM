...
model_name = "rerank_v1"
predict_use_batching = False
predict_use_gpu = False
predict_limit = 1200
predict_server_name = "grpc_fountainKaiMixRerankPredictServerV2";
...
PXTRS = ["pos" + str(i) for i in range(6)]
PXTRS_ATTRS_MAPPING = {
}
for key in PXTRS:
...
def LoadSlotMappingRename():
  ...
  for name, slot_id in slots_mapping.items():
    if name.find('_weight') != -1 or name.find('_label') != -1:
      continue
    if name.find('_idx') != -1:
...
def load_mio_tf_model():
  ...
  for slot in slots_config:
    ...
    if input_name.find('_weight') != -1 or input_name.find('_label') != -1:
      continue
