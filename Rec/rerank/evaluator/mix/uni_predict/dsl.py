...
predict_limit = 1200
predict_server_name = "grpc_fountainKaiSplashRerankUniInferServer"
model_queue_prefix="produce_tower_xtr_model"
embedding_shard_num=6
embedding_kess_name = "grpc_fountainKaiMixRerankEmbeddingServerV2"
...
class PredictServerFlow(LeafFlow, KuibaApiMixin, MioApiMixin, OfflineApiMixin, UniPredictApiMixin):
  PXTRS = ["splash_pos" + str(i) for i in range(2)]
  ...
  def predict_with_mio_model(self, **kwargs):
    ...
    pipeline_final = prepare_pipeline\
      ...
      .uni_predict_fused(
        ...
        outputs = [dict(attr_name=attr_name,tensor_name=tensor_name) for attr_name, tensor_name in model_config.outputs if attr_name in self.PXTRS],
        ## batching 相关配置
      ...
      .perflog_attr_value(check_point="follow.photo.fullrank",item_attrs=self.PXTRS,)
...
predict_flow = PredictServerFlow(name = "default").predict_with_mio_model(
   ...
   embedding_kess_name="grpc_fountainKaiMixRerankEmbeddingServerV2",
   shards=6,
   queue_prefix="produce_tower_xtr_model",
...
service = LeafService(kess_name="grpc_fountainKaiSplashRerankUniInferServer",
