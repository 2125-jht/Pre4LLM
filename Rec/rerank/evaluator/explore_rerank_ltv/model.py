...
class RevisitModel:
    ...
    def model(self):
        ...
        with tf.variable_scope("mlp", reuse=tf.AUTO_REUSE):
            ...
            is_next = tf.layers.dense(h3_ext, 1, activation=tf.nn.sigmoid, name="isnext", reuse=tf.AUTO_REUSE)
            s_h3 = tf.reshape(tf.concat([h3_ext, ltr, is_next],axis=-1), [-1, 500])
            ltr = tf.reshape(ltr, [-1, 10])
            is_next = tf.reshape(is_next, [-1, 10])
        ...
        return ltr, is_next, pred_revisit, print_ops
