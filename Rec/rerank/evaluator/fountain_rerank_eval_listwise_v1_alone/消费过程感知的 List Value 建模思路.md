...
```plain/text
realshow_num             实际 real_show=1 的个数 K
...
实际训练以 `real_show=1` 的个数作为消费长度 K，并要求候选 List 的前 K 个 item 与真实曝光顺序逐项一致。已确认中间的 `real_show=0` 是后续样本受到端排影响后的顺序打乱，不能当作快速划过。若有多个 Prefix 匹配候选，选择其中旧 evaluator 分数最高的一个；没有匹配候选的请求不进入 List Value loss 或 matched 离线指标。历史排查曾观察到约三成请求不匹配，当前训练继续通过 TensorBoard 记录实际丢弃比例。Prefix 价值只监督这条匹配 List 的前 K 个位置。

为了区分模型变化与评估人口变化，训练 loss 只使用上述 Prefix-matched mask；离线评估同时输出 matched 口径和 `_legacy_max_score` 历史口径。后者复刻旧版每请求选择旧分最高 List 的 mask，仅用于纵向对照，不保证事实 Prefix 对齐。
