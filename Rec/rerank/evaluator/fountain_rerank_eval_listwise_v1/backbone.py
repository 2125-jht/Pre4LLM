        ...
        return self \
            ...
            .get_abtest_params(
              ...
              ab_params = [
                ...
                # List Value 分支与 standalone 使用同一套 AB 参数名；weight=0 即关闭。
              ...
              .enrich_attr_by_lua(
                ...
                lua_script="""
                  ...
                  function calculate_list_scores()
                    ...
                    -- Step3: List Value 分支仍在最后叠加，保证它的 AB 权重在所有模式生效
                    if use_order_es == 1 then
                      ...
                      point_scores = calc_order_es_scores(list_num, list_score_weight_pairs)
                    else
                      for i = 1, list_num do
                        local start_idx = (i - 1) * rerank_list_item_size
                        local es_i = 0.0
                        if i <= rerank_list_true_num then
                          for j = 1, rerank_list_item_size do
                            local item_idx = start_idx + j
                            local element_score = 0.0
                            if use_multiply == 1 then
                              -- 每项 weight=0 即关闭；所有注册过 AB 的单点/外部分数均在此生效。
                              local multiply_score = 1.0
                              if pvtr_weight > 0 then multiply_score = multiply_score * ((1.0 + pvtr[item_idx] * 0.1) ^ pvtr_weight) end
                              if pwtd_weight > 0 then multiply_score = multiply_score * ((1.0 + pwtd[item_idx] * 0.1) ^ pwtd_weight) end
                              if pltr_weight > 0 then multiply_score = multiply_score * ((1.0 + pltr[item_idx] * 3.0) ^ pltr_weight) end
                              if pctr_weight > 0 then multiply_score = multiply_score * ((1.0 + pctr[item_idx] * 4.0) ^ pctr_weight) end
                              if context_pwtd_weight > 0 then multiply_score = multiply_score * ((1.0 + context_pwtd[item_idx] * 0.1) ^ context_pwtd_weight) end
                              if context_pctr_weight > 0 then multiply_score = multiply_score * ((1.0 + context_pctr[item_idx] * 4.0) ^ context_pctr_weight) end
                              if context_plvtr_weight > 0 then multiply_score = multiply_score * ((1.0 + context_plvtr[item_idx] * 4.0) ^ context_plvtr_weight) end
                              if context_psvtr_weight > 0 then multiply_score = multiply_score * ((1.0 + (1.0 - context_psvtr[item_idx]) * 4.0) ^ context_psvtr_weight) end
                              if context_pltr_weight > 0 then multiply_score = multiply_score * ((1.0 + context_pltr[item_idx] * 4.0) ^ context_pltr_weight) end
                              if context_pcmtr_weight > 0 then multiply_score = multiply_score * ((1.0 + context_pcmtr[item_idx] * 2.0) ^ context_pcmtr_weight) end
                              if context_pwtr_weight > 0 then multiply_score = multiply_score * ((1.0 + context_pwtr[item_idx] * 4.0) ^ context_pwtr_weight) end
                              if context_pftr_weight > 0 then multiply_score = multiply_score * ((1.0 + context_pftr[item_idx] * 4.0) ^ context_pftr_weight) end
                              element_score = multiply_score
                            else
                              element_score =
                                context_pwtd_weight * context_pwtd[item_idx]
                                + context_pctr_weight * context_pctr[item_idx] * 10.0
                                + context_plvtr_weight * context_plvtr[item_idx] * 10.0
                                + context_psvtr_weight * (1.0 - context_psvtr[item_idx]) * 10.0
                                + context_pltr_weight * context_pltr[item_idx] * 100.0
                                + context_pcmtr_weight * context_pcmtr[item_idx] * 100.0
                                + context_pwtr_weight * context_pwtr[item_idx] * 100.0
                                + context_pftr_weight * context_pftr[item_idx] * 100.0
                                + pvtr_weight * pvtr[item_idx]
                                + pwtd_weight * pwtd[item_idx]
                                + pltr_weight * pltr[item_idx] * 10.0
                                + pctr_weight * pctr[item_idx] * 10.0
                            end
                            if enable_position_decay == 1 then
                    ...
                    -- 拆开记录三层分数：
                    -- original_eval_list_scores = 单点模型 + 外部传入分及其 List 聚合；
                    -- list_value_scores = 本模型 List Value 三个 head 的加权贡献；
                    -- eval_list_scores = 两者之和。
                    local original_scores = {}
                    local list_value_scores = {}
                    local scores = {}
                    for i = 1, list_num do
                      if i <= rerank_list_true_num then
                        original_scores[i] = (point_scores[i] or 0.0)
                          + expected_ltr_weight * (expected_ltr[i] or 0.0)
                          + expected_pctr_weight * (expected_pctr[i] or 0.0)
                          + expected_comment_weight * (expected_comment[i] or 0.0) * 100.0
                          + expected_like_weight * (expected_like[i] or 0.0) * 50.0
                          + expected_follow_weight * (expected_follow[i] or 0.0) * 100.0
                        list_value_scores[i] =
                          expected_list_watch_time_weight * (expected_list_watch_time[i] or 0.0)
                          + expected_list_effective_vv_weight * (expected_list_effective_vv[i] or 0.0)
                          + expected_consume_length_weight * (expected_consume_length[i] or 0.0)
                        scores[i] = original_scores[i] + list_value_scores[i]
                      else
                        original_scores[i] = 0.0
                        list_value_scores[i] = 0.0
                        scores[i] = 0.0
                      end
                    end
                    return original_scores, list_value_scores, scores
                  end
                """
              ) \
            .end_() \
            .perflog_attr_value(
              ...
              common_attrs=[
                ...
                "context_pwtr_weight",
                "context_plvtr_weight",
                "context_psvtr_weight",
                "context_pftr_weight",
