...
if is_training:
    ...
    # 与原版 fountain_rerank_flash_eval 保持一致：每个请求只选择旧分最高
    # List，不再额外按是否存在真实曝光过滤请求。Prefix 匹配仅用于数据诊断。
    ...
    # Y_l 使用旧 evaluator 选中的事实曝光 List；只利用其已曝光 Prefix 的反馈。
    ...
    observed_feedback_count = tf.cast(realshow_num, tf.float32)
    ...
    preference_pair_before_match_mask = tf.cast(
        tf.logical_and(
            tf.logical_and(
    ...
    # 合成偏好必须确认 Y_l 确实产生了当前反馈，因此比原 factual loss 多一层
    # Top1 Prefix 匹配过滤；不匹配请求不会进入 preference loss。
    preference_pair_mask = preference_pair_before_match_mask * tf.cast(
        max_score_prefix_match,
        tf.float32,
    )
    ...
    # 训练时在 30 条候选后追加 Y_w/Y_l，一次前向共享 list_backbone。
    # 标准 factual 监督仍只读取前 LIST_NUM 条；合成 pair 不获得回归标签。
    preference_list_index = tf.stack(
        [personalized_list_index, logged_list_index],
        axis=1,
    )
    ...
    # 因而不会拿 K 之后的反事实 item 做 Prefix 监督。
    ...
    with tf.control_dependencies(print_ops):
        ...
        # 再按参与训练的最高分 List 数量求均值，使每个请求保持相同样本权重。
        ...
        #   -log sigmoid((V(Y_w) - V(Y_l)) / temperature)
        ...
        # 第 0 条为 Y_w，第 1 条为 Y_l。保留各分量 margin 便于判断综合
        ...
        preference_loss_per_request = tf.nn.softplus(
            -preference_margin / PREFERENCE_TEMPERATURE
        )
        preference_loss = tf.reduce_sum(
            preference_loss_per_request * preference_pair_mask
        ...
        weighted_preference_loss = PREFERENCE_LOSS_WEIGHT * preference_loss
        ...
        # 两个比例都只在参与训练的最高分 List 上统计，口径与 length loss 一致。
        ...
        tf.summary.scalar(
            "list_value/preference/pair_rate_before_top1_match",
            tf.reduce_sum(preference_pair_before_match_mask)
            / (comparable_request_count + 1e-8),
        )
        tf.summary.scalar(
            "list_value/preference/pair_rate_after_top1_match",
            preference_pair_count / (comparable_request_count + 1e-8),
        )
        tf.summary.scalar(
            "list_value/preference/top1_match_retention_rate",
            preference_pair_count
            / (tf.reduce_sum(preference_pair_before_match_mask) + 1e-8),
        )
        tf.summary.scalar(
