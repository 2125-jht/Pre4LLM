...
point_wise_tasks = ["ltr", "vtr", "click", "wtd", "continue"]
# list_wise_tasks = ["listwise_wtd"]
list_wise_tasks = []
model_class = EvaluatorModel(all_param_dict, feature_emb_size_dict, label_value_dict, print_ops, list_size=LIST_SIZE, candidates_size=CANDIDATES_SIZE, list_num=LIST_NUM,
...
if is_training:
    ...
    linear_wt_weight = tf.clip_by_value(play_time_s, 0, 120) * 0.035# scale_watch_time
    effective_action = tf.cast(tf.greater((1 - svtr_label) + like_label + follow_label + forward_label + comment_label, 0.0), tf.float32)  # [B, 1]
    is_miss = effective_action * tf.cast(tf.equal(play_time_s, 0.0), tf.float32)  # [B, 1]
    normal_value = tf.minimum(play_time_s / 3.0, 100.0)
    ...
    # advantage_reward = tf.clip_by_value(play_time_s, 0, 240) * 0.4 + finish_label * 2.0
    ...
    # continue 目标：item i 之后下一个 item 是否继续曝光，即 slide_label[i] == 1
    # sel_slide_label shape: (?, list_num, list_size)，sel_vtr_wt shape: (?, list_num, list_size)
    sel_slide_label = list_slide_label  # 1=继续滑，0=不继续（退出/跳走）

    # list 级真实时长（用于 expected_vtr_wt 可比 AUC）
    list_play_time_s_reduced = tf.reduce_sum(list_play_time_s, axis=-1, keepdims=True)  # (?, list_num, 1)
    ...
    with tf.control_dependencies(print_ops):
        ...
        for loss_name in point_wise_output_dict: # point-wise 有三维，压缩为两维方便处理
            ...
            if loss_name == "ltr":
                ...
                targets.append((loss_name, output, label, mask, "auc"))
            elif loss_name == "continue":
                # continue: 预测第 i 个 item 后用户是否继续下滑（slide_label[i]）
                # loss 用全部 LIST_SIZE 个位置（含最后一个，其标签为"滑出 list"是否成立）
                # 全概率公式只用前 LIST_SIZE-1 个 continue 作为 p_reach 的来源
                output_3d = tf.reshape(output, [batch_size, LIST_NUM, LIST_SIZE])
                cont_mask_3d = tf.reshape(list_show_label, [batch_size, LIST_NUM, LIST_SIZE]) * tf.expand_dims(rerank_list_mask, axis=-1)  # (?, list_num, list_size)
                cont_pred_flat = tf.reshape(output_3d, [batch_size, LIST_NUM * LIST_SIZE])
                cont_labels_flat = tf.reshape(sel_slide_label, [batch_size, LIST_NUM * LIST_SIZE])
                cont_mask_flat = tf.reshape(cont_mask_3d, [batch_size, LIST_NUM * LIST_SIZE])
                loss = focal_loss_with_neg_upsampling(labels=cont_labels_flat, predictions=cont_pred_flat, weights=cont_mask_flat, gamma=2.0, neg_weight=5.0)
                targets.append((loss_name, cont_pred_flat, cont_labels_flat, cont_mask_flat, "auc"))
                # 全概率公式：expected_vtr_wt = sum_i(p_reach_i * vtr_wt_i)
                # p_reach[:,:,0]=1, p_reach[:,:,i]=cont[0]*...*cont[i-1]，只用前 LIST_SIZE-1 个
                vtr_wt_3d = tf.reshape(list_vtr_wt, [batch_size, LIST_NUM, LIST_SIZE])
                ones_prefix = tf.ones([batch_size, LIST_NUM, 1], dtype=tf.float32)
                p_reach = tf.concat([ones_prefix, tf.math.cumprod(output_3d[:, :, :-1], axis=-1, exclusive=False)], axis=-1)  # (?, list_num, list_size)
                expected_vtr_wt = tf.reduce_sum(p_reach * vtr_wt_3d, axis=-1)  # (?, list_num)
                targets.append(("expected_vtr_wt", expected_vtr_wt, tf.reshape(list_play_time_s_reduced, [batch_size, LIST_NUM]), rerank_list_mask, "linear_regression"))
                targets.append(("expected_vtr_wt_origin", tf.reduce_sum(vtr_wt_3d, axis=-1), tf.reshape(list_play_time_s_reduced, [batch_size, LIST_NUM]), rerank_list_mask, "linear_regression"))
        ...
        #             loss = tf.losses.huber_loss(labels=listwise_wtd_label, predictions=output, weights=rerank_list_mask, reduction=tf.losses.Reduction.SUM, delta=0.05)
        #             loss = loss * 150.0
    ...
    # continue 预测 & expected_vtr_wt（全概率公式）
    pvtr_wt_item = tf.reshape(pvtr_wt, [-1, LIST_NUM, LIST_SIZE])  # (1, list_num, list_size)
    cont_raw = tf.reshape(point_wise_output_dict["continue"], [-1, LIST_NUM, LIST_SIZE])  # (1, list_num, list_size)
    ones_prefix = tf.ones([tf.shape(cont_raw)[0], LIST_NUM, 1], dtype=tf.float32)
    cont_prefix = cont_raw[:, :, :-1]  # exclusive prod 的依据
    p_reach = tf.concat([ones_prefix, tf.math.cumprod(cont_prefix, axis=-1, exclusive=False)], axis=-1)  # (1, list_num, list_size)
    # expected_vtr_wt = tf.reduce_sum(p_reach * pvtr_wt_item, axis=-1)  # (1, list_num)
    expected_vtr_wt = tf.reduce_sum(pvtr_wt_item, axis=-1)  # (1, list_num)
