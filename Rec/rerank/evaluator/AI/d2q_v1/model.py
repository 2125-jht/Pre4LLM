...
class EvaluatorModel:
    def __init__(self, parameters_dict, label_value_dict, segment_num, print_ops, list_size, candidates_size, list_num,
        ...
        # D2Q: duration bucket bias embedding (9 buckets x 16 dim)
        _n_d2q_buckets = len(wtd_config["buckets"]) + 1  # 9 buckets
        self.d2q_duration_bucket_emb = tf.get_variable(
            name='d2q_duration_bucket_emb',
            shape=[_n_d2q_buckets, 16],
            initializer=tf.random_normal_initializer()
        )
        self._segment_num = segment_num
        self.segment_embeddings = tf.get_variable(name='segment_embeddings', shape=[1, segment_num, 64], initializer=tf.random_normal_initializer())
    ...
    def _get_shared_features(self, input_dicts) -> tuple:
        with tf.variable_scope("share_bottom", reuse=tf.AUTO_REUSE):
            ...
            _duration_s = self._label_value_dict["photo_info__duration_ms_list"] / 1000.0
            _photo_duration_s  = input_dicts["photo_duration_s"]
            context_pwtd  = input_dicts["context_pwtd"]
            if not self._training:
                ...
                _duration_s = tf.reshape(_duration_s, [1, -1])
                _photo_duration_s = tf.reshape(_photo_duration_s, [1, -1, _photo_duration_s.shape[-1]])
                context_pwtd = tf.reshape(context_pwtd, [1, -1, context_pwtd.shape[-1]])
            ...
            # D2Q: compute duration bucket emb per candidate item
            _d2q_boundaries = tf.constant(wtd_config["buckets"], dtype=tf.float32)
            _d2q_boundaries_tiled = tf.tile(tf.expand_dims(_d2q_boundaries, 0), [tf.shape(_duration_s)[0], 1])
            _d2q_bucket_id = tf.searchsorted(_d2q_boundaries_tiled, tf.cast(_duration_s, tf.float32), out_type=tf.int32)  # (?, CANDIDATES_SIZE)
            _d2q_bucket_emb = tf.nn.embedding_lookup(self.d2q_duration_bucket_emb, _d2q_bucket_id)  # (?, CANDIDATES_SIZE, 16)
            print("d2q_bucket_emb shape: ", _d2q_bucket_emb, " photo_duration_s shape: ", _photo_duration_s)
            _d2q_bias_emb = tf.concat([_d2q_bucket_emb, _photo_duration_s, context_pwtd], axis=-1)
            return common_embs, _d2q_bias_emb
        ...
        with tf.variable_scope("evaluator", reuse=tf.AUTO_REUSE):
            def multi_task_module(name, point_wise_input, loss_names, shared_key, cand_size):
                with tf.variable_scope(f"task_{name}", reuse=tf.AUTO_REUSE):
                    ...
                    input_feature_dict = {x: point_wise_input for x in loss_names}  # (?*list_num, list_size, dim)
                    ...
                    for loss_name, output in output_fea_dict.items():
                        with tf.variable_scope(f"output_mlp_{loss_name}", reuse=tf.AUTO_REUSE):
                            output  = tf.layers.dense(output, 64, activation=tf.nn.leaky_relu)  # (?*list_num, list_size, 64)
                            if loss_name in ["gen_time"]:
                                print("gen_time branch output: ", output)
                                gen_input = tf.layers.dense(output, 64 * self._segment_num, activation=tf.nn.leaky_relu)  # (?, list_size, seg_num * d)
                                gen_input = tf.reshape(gen_input, [-1, self._segment_num, 64]) # (?, seg_num, d)
                                decoder = DecoderLayer(name=loss_name + "_decoder", dim=64, num_heads=2, dim_in=64, dropout_rate=0.0, training=self._training)
                                gen_time_output = decoder.forward(gen_input, self.segment_embeddings, training=self._training) # (?, seg_num, d)
                                gen_time_output = tf.layers.dense(gen_time_output, 32, activation=tf.nn.leaky_relu)
                                gen_time_output = tf.reshape(tf.layers.dense(gen_time_output, 1, activation=tf.nn.sigmoid), [batch_size, self._list_num * self._list_size, self._segment_num])
                                output_dict[loss_name] = gen_time_output # (?, m*n, seg_num)
                            else:
                                output = tf.layers.dense(output, 32, activation=tf.nn.leaky_relu)
                                if loss_name in ["wtd"]:
                                    bias_tower = tf.layers.dense(_list_d2q_bias_emb_flat, 32, activation=tf.nn.leaky_relu)  # (?*list_num,list_size,32)
                                    bias_tower = tf.layers.dense(bias_tower, 32, activation=tf.nn.leaky_relu)  # (?*list_num,list_size,32)
                                    output = output + bias_tower
            ...
            com_embs, _d2q_bias_emb = self._get_shared_features(input_dicts) # (?, cand_size, 512)
            print("com_embs:", com_embs)
            print("_d2q_bias_emb:", _d2q_bias_emb)
            ...
            _list_d2q_bias_emb = tf.gather_nd(tf.concat([tf.zeros(shape=[batch_size, 1, _d2q_bias_emb.shape[-1]], dtype=tf.float32), _d2q_bias_emb], axis=1), indices) # (?, list_num, list_size, dim)
            _list_d2q_bias_emb_flat = tf.reshape(_list_d2q_bias_emb, [batch_size * self._list_num, self._list_size, _list_d2q_bias_emb.shape[-1]])  # (?*list_num,list_size,16)
            print("_list_d2q_bucket_emb ", _list_d2q_bias_emb)
            # cls token
