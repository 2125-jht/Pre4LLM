...
def wtd_encode(duration, play_time, duration_bucket, play_time_buckets_ragged, bucket_weights=None):
    '''
      duration: (?,cand_size)
      play_time: (?,cand_size)
      2-D wtd
    ...
    weight = tf.gather(bucket_weights, bucket_idx)  # (?, cand_size)
    return ratio, weight
...
def gen_time_label_encode(playtime, duration, num_buckets=10, min_dur=20.0, max_dur=400.0):
    batch_size = tf.shape(duration)[0]
    cand_size  = duration.shape[1]
    playtime = tf.reshape(playtime, [batch_size * cand_size, 1])  # [B*C, 1]
    duration = tf.reshape(duration, [batch_size * cand_size, 1])  # [B*C, 1]

    duration_clip = tf.clip_by_value(duration, min_dur, max_dur)
    playtime = tf.clip_by_value(playtime, 0.0, duration_clip)

    idx = tf.cast(tf.range(1, num_buckets + 1), tf.float32)[None, :]  # [1, N]
    bucket_start_clip = duration_clip * ((idx - 1.0) / float(num_buckets))  # [B*C, N]
    bucket_end_clip   = duration_clip * (idx / float(num_buckets))           # [B*C, N]
    bucket_len  = tf.maximum(bucket_end_clip - bucket_start_clip, 1e-6)
    bucket_done = tf.clip_by_value((playtime - bucket_start_clip) / bucket_len, 0.0, 1.0)
    bucket_mask = tf.clip_by_value((duration  - bucket_start_clip) / bucket_len, 0.0, 1.0)

    bucket_done = tf.reshape(bucket_done, [batch_size, cand_size, num_buckets])  # [B, C, N]
    bucket_mask = tf.reshape(bucket_mask, [batch_size, cand_size, num_buckets])  # [B, C, N]
    return bucket_done, bucket_mask


def gen_time_label_decode(predict, duration, num_buckets=10, min_dur=20.0, max_dur=400.0):
    batch_size = tf.shape(duration)[0]
    list_num  = duration.shape[1]
    list_size  = duration.shape[2]
    predict  = tf.reshape(predict,  [batch_size * list_num * list_size, num_buckets])  # ✅ [B*C, N]
    duration = tf.reshape(duration, [batch_size * list_num * list_size, 1])            # [B*C, 1]

    duration_clip = tf.clip_by_value(duration, min_dur, max_dur)
    predict = tf.clip_by_value(predict, 0.0, 1.0)

    idx = tf.cast(tf.range(1, num_buckets + 1), tf.float32)[None, :]
    bucket_end_clip   = duration_clip * (idx / float(num_buckets))
    bucket_start_clip = duration_clip * ((idx - 1.0) / float(num_buckets))
    bucket_len  = tf.maximum(bucket_end_clip - bucket_start_clip, 1e-6)
    bucket_mask = tf.clip_by_value((duration - bucket_start_clip) / bucket_len, 0.0, 1.0)

    bucket_len = duration_clip / float(num_buckets)
    k_hat  = predict[:, 0:1] * tf.reduce_sum(predict * bucket_mask, axis=-1, keepdims=True)  # [B*C, 1]
    result = k_hat * bucket_len                                                               # [B*C, 1]

    result = tf.reshape(result, [batch_size, list_num * list_size])  # [B, C]
    return result
...
wtd_bucket_weights = tf.constant(wtd_config["bucket_weights"], dtype=tf.float32)  # D2Q: per-bucket loss weight
...
point_wise_tasks = ["ltr", "vtr", "click", "wtd", "gen_time"]
# list_wise_tasks = ["listwise_wtd"]
list_wise_tasks = []
segment_num = 20
model_class = EvaluatorModel(all_param_dict, label_value_dict, segment_num, print_ops, list_size=LIST_SIZE, candidates_size=CANDIDATES_SIZE, list_num=LIST_NUM,
...
if is_training:
    ...
    # print_ops.append(tf.print('rerank_list_item_idx_flat_list', rerank_list_item_idx_flat_list[8],
    #                  summarize = 10, output_stream=sys.stdout))
    # print_ops.append(tf.print('real_show_rerank_indices', real_show_rerank_indices[8], summarize=10, output_stream=sys.stdout))

    model_class._training=True
    point_wise_output_dict, list_wise_output_dict = model_class.model(list_index = rerank_list_item_idx_flat_list)
    print(f"====> train, gen...")
    ...
    wtd_label, wtd_weight = wtd_encode(duration=duration_s, play_time=play_time_s, duration_bucket=wtd_buckets, play_time_buckets_ragged=wtd_configs, bucket_weights=wtd_bucket_weights)  # wtd_weight: (?, CAND_SIZE)
    gen_segment_label, gen_segment_mask = gen_time_label_encode(play_time_s, duration_s, num_buckets=segment_num, min_dur=20.0, max_dur=400.0)
    # wtdv2_label = wtd_encode(duration=duration_s, play_time=play_time_s, duration_bucket=wtd_buckets_fountain, play_time_buckets_ragged=wtd_configs_fountain)
    # vtrv2_label = vtr_encode(playtime_s=play_time_s, duration_s=duration_s)
    ...
    click_label2 = tf.where(tf.logical_and(tf.equal(duration_s, 0.0), tf.greater_equal(play_time_s, 4.0)), tf.ones_like(click_label), click_label)
    ...
    evtr_weight = 1.0 + like_label * 20.0 + follow_label * 100.0 + comment_label * 100.0
    # evtr_weight = 1.0 + finish_label * 4.0 + like_label * 200.0 + follow_label * 200.0 + comment_label * 200.0 + forward_label * 50.0
    # evtr_weight = 1.0 + finish_label * 4.0 + wt_weight * 0.1
    # lvtr_weight = 1.0 + like_label * 30.0 + follow_label * 200.0 + comment_label * 100.0 + forward_label * 200.0 \
    #     + slide_label * 10.0 + finish_label * 10.0
    # base_ltr_weight = 1.0 + base_ltr_label * 10.0 + like_label * 20.0 + follow_label * 200.0 + comment_label * 100.0 + forward_label * 100.0 \
    #     + slide_label * 10.0 + base_finish_label * 10.0 + base_lvtr_label * 0.0 + linear_wt_weight
    ...
    # reward_weight = tf.where(tf.logical_and(tf.greater(evtr_label, 0.0), tf.greater(duration_s, 0.0)), tf.ones_like(reward_weight) * 2.0, reward_weight)
    # reward_weight = tf.where(tf.logical_and(tf.greater(base_ltr_label, 0.0), tf.greater(duration_s, 0.0)), tf.ones_like(reward_weight) * 2.0, reward_weight)
    # reward_weight = tf.where(tf.logical_and(tf.greater(base_finish_label, 0.0), tf.greater(duration_s, 0.0)), tf.ones_like(reward_weight) * 10.0, reward_weight)
    # reward_weight = tf.where(tf.logical_and(action_once, tf.greater(duration_s, 10.0)), tf.ones_like(reward_weight) * 5.0, reward_weight)
    # reward_weight = tf.where(tf.logical_and(tf.logical_and(tf.greater(evtr_label, 0.0), action_once), tf.greater(duration_s, 10.0)), tf.ones_like(reward_weight) * 50.0, reward_weight)
    # reward_weight = tf.where(tf.logical_and(tf.greater(base_ltr_label, 0.0), action_once), tf.ones_like(reward_weight) * 100.0, reward_weight)
    # reward_weight = tf.where(tf.logical_and(tf.greater(base_finish_label, 0.0), action_once), tf.ones_like(reward_weight) * 150.0, reward_weight)

    advantage_reward = play_time_s + slide_label * 5.0 + like_label * 40.0 + follow_label * 100.0 + comment_label * 100.0 + forward_label * 100
    ...
    listwise_play_time_s = tf.reduce_sum(list_play_time_s, axis=-1, keepdims=False) # (?, list_num)
    listwise_duration_s = tf.reduce_sum(list_duration_s, axis=-1, keepdims=False) # (?, list_num)
    print("listwise_play_time_s", listwise_play_time_s)
    print("listwise_duration_s", listwise_duration_s)
    # listwise_wtd_label = wtd_encode(duration=listwise_duration_s, play_time=listwise_play_time_s,
    #                                 duration_bucket=list_wtd_buckets_fountain, play_time_buckets_ragged=list_wtd_configs_fountain)
    ...
    # list_vtrv2_label = tf.gather(tf.concat([zeros, vtrv2_label], axis=-1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1) # (?, list_num, list_size)
    ...
    gen_zeros = tf.zeros([tf.shape(zeros)[0], 1, segment_num], dtype=tf.float32)
    list_gen_segment_label = tf.gather(tf.concat([gen_zeros, gen_segment_label], axis=1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1) # (?, list_num, list_size, segment_num)
    list_gen_segment_mask = tf.gather(tf.concat([gen_zeros, gen_segment_mask], axis=1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1) # (?, list_num, list_size, segment_num)
    # D2Q: per-bucket sample weight, from wtd_encode (same bucket_idx as wtd_label)
    list_wtd_weight = tf.gather(tf.concat([tf.ones([tf.shape(zeros)[0], 1]), wtd_weight], axis=-1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1)  # (?, list_num, list_size)
    # list_wtdv2_label = tf.gather(tf.concat([zeros, wtdv2_label], axis=-1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1) # (?, list_num, list_size)
    ...
    list_click_label2 = tf.gather(tf.concat([zeros, click_label2], axis=-1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1) # (?, list_num, list_size)
    # list_like_label = tf.gather(tf.concat([zeros, like_label], axis=-1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1) # (?, list_num, list_size)
    # list_follow_label = tf.gather(tf.concat([zeros, follow_label], axis=-1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1) # (?, list_num, list_size)
    # list_comment_label = tf.gather(tf.concat([zeros, comment_label], axis=-1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1) # (?, list_num, list_size)
    # list_forward_label = tf.gather(tf.concat([zeros, forward_label], axis=-1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1) # (?, list_num, list_size)
    # list_ltr_label = tf.gather(tf.concat([zeros, ltr_label], axis=-1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1) # (?, list_num, list_size)
    ...
    # list_lvtr_label = tf.gather(tf.concat([zeros, lvtr_label], axis=-1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1) # (?, list_num, list_size)
    ...
    # list_lvtr_weight = tf.gather(tf.concat([zeros, lvtr_weight], axis=-1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1) # (?, list_num, list_size)
    list_base_ltr_label = tf.gather(tf.concat([zeros, base_ltr_label], axis=-1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1) # (?, list_num, list_size)
    list_base_ltr_weight = tf.gather(tf.concat([zeros, base_ltr_weight], axis=-1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1) # (?, list_num, list_size)
    # list_base_next_weight = tf.gather(tf.concat([zeros, base_next_weight], axis=-1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1) # (?, list_num, list_size)
    list_reward_label = tf.gather(tf.concat([zeros, reward_label], axis=-1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1) # (?, list_num, list_size)
    list_reward_weight = tf.gather(tf.concat([zeros, reward_weight], axis=-1), rerank_list_item_idx_flat_list, axis=1, batch_dims=1) # (?, list_num, list_size)
    ...
    list_wtd_wt = wtd_decode(tf.reshape(point_wise_output_dict["wtd"], [batch_size, LIST_NUM * LIST_SIZE]),
                             tf.reshape(tf.cast(list_duration_s, dtype=tf.int32), [batch_size, LIST_NUM * LIST_SIZE]),
                             duration_bucket=wtd_buckets, play_time_buckets_ragged=wtd_configs) # (? , list_num * list_size)
    # list_wtdv2_wt = wtd_decode(tf.reshape(point_wise_output_dict["wtdv2"], [batch_size, LIST_NUM * LIST_SIZE]),
    #                            tf.reshape(tf.cast(list_duration_s, dtype=tf.int32), [batch_size, LIST_NUM * LIST_SIZE]),
    #                            duration_bucket=wtd_buckets_fountain, play_time_buckets_ragged=wtd_configs_fountain) # (? , list_num * list_size)
    # listwise_wtd_wt = wtd_decode(list_wise_output_dict["listwise_wtd"], listwise_duration_s,
    #                              duration_bucket=list_wtd_buckets_fountain, play_time_buckets_ragged=list_wtd_configs_fountain) # (? , list_num)

    mask = tf.reshape(list_show_label * tf.expand_dims(rerank_list_mask, axis=-1), [batch_size, LIST_NUM * LIST_SIZE])


    # print_ops.append(tf.print("show_label ", show_label[8], summarize=10, output_stream=sys.stdout))

    with tf.control_dependencies(print_ops):
        # adn loss calc
        targets = []
        sum_loss = 0.0
        # eval_duration_buckets = [0,9,13,20,38,71,118,195,1000000000]
        # eval_buckets_name = ["0_9s","9_13s","13_20s","20_38s","38_71s","71_118s","118_195s","195s+"]
        for loss_name in point_wise_output_dict: # point-wise 有三维，压缩为两维方便处理
            output = point_wise_output_dict[loss_name] # (? * list_num, list_size)
            output = tf.reshape(output, [batch_size, LIST_NUM * LIST_SIZE]) if loss_name != "gen_time" else output
            print(loss_name, output)

            if loss_name == "ltr":
                list_evtr_weight = tf.reshape(list_evtr_weight, [batch_size, LIST_NUM * LIST_SIZE])
                # label = tf.reshape(list_evtr_label, [batch_size, LIST_NUM * LIST_SIZE])
                label = tf.reshape(list_click_label2, [batch_size, LIST_NUM * LIST_SIZE])
                loss = tf.losses.log_loss(labels=label, predictions=output, weights=mask * list_evtr_weight) * 0.0  # default: weighted mean
                targets.append((loss_name, output, label, mask, "auc"))
            elif loss_name == "vtr":
                list_vtr_label = tf.reshape(list_vtr_label, [batch_size, LIST_NUM * LIST_SIZE])
                loss = tf.losses.huber_loss(labels=list_vtr_label, predictions=output, weights=mask, delta=0.05)  # default: weighted mean
                loss = loss * 100.0
                ...
                list_wtd_weight_flat = tf.reshape(list_wtd_weight, [batch_size, LIST_NUM * LIST_SIZE])
                # loss = tf.losses.huber_loss(labels=list_wtd_label, predictions=output, weights=mask * list_wtd_weight_flat, reduction=tf.losses.Reduction.SUM, delta=0.05) * 150.0
                loss = tf.losses.log_loss(labels=list_wtd_label, predictions=output, weights=mask * list_wtd_weight_flat)  # default: weighted mean
                targets.append((loss_name, output, list_wtd_label, mask, "linear_regression"))
                targets.append(("list_wtd_wt", list_wtd_wt, tf.reshape(list_play_time_s, [batch_size, LIST_NUM * LIST_SIZE]), mask, "linear_regression"))
            elif loss_name == "gen_time":
                list_gen_segment_mask = tf.reshape(list_gen_segment_mask, [batch_size, LIST_NUM * LIST_SIZE, segment_num])
                list_gen_segment_label = tf.reshape(list_gen_segment_label, [batch_size, LIST_NUM * LIST_SIZE, segment_num])
                output = tf.reshape(output, [batch_size, LIST_NUM * LIST_SIZE, segment_num])
                weight_with_mask = tf.expand_dims(mask, axis=-1) * list_gen_segment_mask
                loss = tf.losses.log_loss(labels=list_gen_segment_label, predictions=output, weights=weight_with_mask)  # default: weighted mean
                targets.append((loss_name, output, list_gen_segment_label, weight_with_mask, "auc"))
                tf.summary.scalar('fountain_gen_time_ce_loss', loss)
                # order loss
                output_4d = tf.reshape(output, [batch_size, LIST_NUM, LIST_SIZE, segment_num])
                mask_4d = tf.reshape(mask, [batch_size, LIST_NUM, LIST_SIZE, 1])
                order_diff = (output_4d[:,:,:,:-1]- output_4d[:,:,:,1:]) * mask_4d
                order_diff = tf.reshape(order_diff, [batch_size, -1])
                valid_cnt = tf.reduce_sum(tf.cast(tf.greater(tf.reshape(mask, [-1]), 0.0), tf.float32))
                order_loss = tf.reduce_sum(tf.nn.relu(-order_diff)) / (valid_cnt * (segment_num - 1) + 1e-6)
                tf.summary.scalar('fountain_gen_time_order_loss_', order_loss)
                # huber loss
                gen_wt = gen_time_label_decode(output, list_duration_s, num_buckets=segment_num, min_dur=20.0, max_dur=400.0) # (?, list_num * list_size)
                gen_wt_log = tf.log(gen_wt + 1.0)
                wt_log = tf.log(tf.clip_by_value(tf.reshape(list_play_time_s, [batch_size, LIST_NUM * LIST_SIZE]), 0.0, 400) + 1.0)
                huber_loss = tf.losses.huber_loss(labels=wt_log, predictions=gen_wt_log, weights=mask, delta=0.2)  # default: weighted mean
                tf.summary.scalar('fountain_gen_time_huber_loss', huber_loss)
                loss = loss + huber_loss * 2.0 + order_loss * 0.1
                targets.append(("fountain_gen_time_wt", gen_wt, tf.reshape(list_play_time_s, [batch_size, LIST_NUM * LIST_SIZE]), mask, "linear_regression"))
            elif loss_name == "click":
                weight = tf.reshape(list_advantage, [batch_size, LIST_NUM * LIST_SIZE])
                label = tf.reshape(list_click_label2, [batch_size, LIST_NUM * LIST_SIZE])
                loss = tf.losses.log_loss(labels=label, predictions=output, weights=mask * weight)  # default: weighted mean
                targets.append((loss_name, output, label, mask, "auc"))
        ...
        # if len(list_wise_output_dict) > 0:
        #     for loss_name in list_wise_output_dict:
        #         output = list_wise_output_dict[loss_name] # # (?,list_num)
        #         print(loss_name, output)
        #         if loss_name == "listwise_wtd":
        #             loss = tf.losses.huber_loss(labels=listwise_wtd_label, predictions=output, weights=rerank_list_mask, delta=0.1)
        #             loss = loss * 150.0
        #             targets.append((loss_name, output, listwise_wtd_label, rerank_list_mask, "linear_regression"))
        #             # targets.append(("listwise_wtd_wt", listwise_wtd_wt, listwise_play_time_s, rerank_list_mask, "linear_regression"))
        #         sum_loss += loss
        #         tf.summary.scalar('loss_' + loss_name, loss)
