...
负 List `Y_l` 使用旧分 Top1 的完整 6-item List；正 List `Y_w` 只在其前 K 个
真实曝光位置内按用户反馈重排，K 之后的候选内容和相对顺序保持不变。只有当
...
才计算 preference loss。训练图在原 30 条候选后追加 `Y_w/Y_l`，共享同一次
`list_backbone` 前向；合成的 `Y_w/Y_l` 只参与 pairwise loss，不接受事实回归标签。
...
`L_preference` 在总 loss 中的权重为 `0.4`。按上一版 TB 的 loss 结构估算，
其加权贡献约占总 loss 的 14%；实际比例由
`loss_contribution/preference_ratio` 持续监控。`Y_w` 仍不解释为具有真实
反事实回报的曝光样本。
