...
base_config = '/home/xuwei09/kuiba_test/kuiba_to_kai_ltr/model/fountain_set2list_model/conf/base.yaml'
...
common_embeddings = set([])
if "sign_feature_is_common" in config_from_kuiba:
  common_dict = config_from_kuiba["sign_feature_is_common"]
  for key in common_dict.keys():
    if common_dict[key] == True:
       common_embeddings.add("KAI_" + key)

def new_sized_embedding(name, dim, expand, slots, common):
    if args.with_kai:
        return config.new_embedding(name, dim=dim, expand=expand, slots=slots, sized=True)
   # uni predict,batching
    if args.mode == 'predict':
      if name in common_embeddings:
        return config.new_embedding(name, dim = dim, expand = expand, slots =slots, sized = True, compress_group="USER")
      else:
        return config.new_embedding(name, dim=dim, expand=expand, slots=slots, sized=True, common=common) # cmt:batching
...
def ppnet(loss_name, gate_fea, input_fea, hidden_units, activation = tf.nn.relu, last_unit=None):
   with tf.variable_scope("{}_ppnet".format(loss_name), reuse=tf.AUTO_REUSE): 
      for i, hidden_unit in enumerate(hidden_units) : 
          input_fea = tf.layers.dense(input_fea, hidden_unit, activation=activation)
          # input_fea = norm(input_fea, loss_name + 'ln'+ str(i))
          input_weight = gate_layer(loss_name + str(i), tf.stop_gradient(gate_fea), hidden_unit)
          input_fea = input_fea * input_weight
      
      if last_unit == 1: 
        input_fea = tf.layers.dense(input_fea, last_unit, activation=tf.nn.sigmoid) 
          #net = tf.clip_by_value(net, eps, 1 - eps)
          #net = tf.layers.batch_normalization(net)
   return input_fea
...
def cal_list_ce_loss(labels, weights, output_l2r_scores):
   real_rewards = tf.concat(labels, axis = 1) * tf.concat(weights, axis = 1)
   real_rewars = tf.nn.sigmoid(real_rewards) / tf.reduce_sum(tf.nn.sigmoid(real_rewards), axis=-1, keepdims=True)
   pred_scores = tf.nn.sigmoid(output_l2r_scores) / tf.reduce_sum(tf.nn.sigmoid(output_l2r_scores), axis=-1, keepdims=True)
   loss = tf.losses.log_loss(labels=real_rewars, predictions=pred_scores, reduction=tf.losses.Reduction.SUM)
   return loss, real_rewards, pred_scores

def DTRN(loss_name, input_fea, list_num, input_dim, head_num):
  with tf.variable_scope(loss_name+"DTRN", reuse=tf.AUTO_REUSE):
    hidden_unit = tf.layers.dense(input_fea, head_num * input_dim, activation = tf.nn.relu)
    hidden_unit = tf.reshape(hidden_unit, [-1, list_num, head_num, input_dim])
    hidden_unit = tf.layers.dense(hidden_unit, input_dim, activation = tf.nn.sigmoid)
    refine_score = tf.reduce_mean(hidden_unit, axis=-2)
    out_score = refine_score * input_fea
  return out_score
...
def poslabel_model(parameters_dict, loss_name):
  ...
  user = tf.concat(parameters[:user_fea_num], axis = 1)
  user_id = tf.concat(parameters[:6], axis = 1)
  ...
  for i in range(n): 
    ...
    weights.append(p[-1]) 
    pos_emb = get_pos_embedding('ctr_10' + 'pos_', i, rown, dim=64)
  ...
  photo_div_emb = tf.concat([ltr_pid_emb, hetu_emb], axis = -1)
  ...
  user_ids = tf.tile(tf.expand_dims(user_id, 1), [1, n, 1])
  short_net_query = tf.concat([user_ids, ltr_pid_emb, hetu_emb], axis=2)
  ...
  view_net_output = short_net(loss_name +"ltr_view_list",
                              ...
                              action_item_size = 64,
                              att_emb_size = 64)
  gate_fea =  tf.concat([view_net_output, user_ems, datas, interact_pxtrs, watchtime_pxtrs], axis=-1)
  data = ppnet(loss_name +'l2r_dnn', tf.concat([gate_fea, short_net_query], axis = 2), gate_fea, [256, 128, 64])
  ...
  output_l2r_scores = []
  for i in range(n) : 
    output_hidden = fc_layer(loss_name + 'ltr_proj', atten_outputs[i], [64, 32])
    output_hidden = fc_layer(loss_name + 'ltr_out_none', output_hidden, [32], activation = None)
    output_pos = fc_layer(loss_name + 'ltr_out_pos', output_hidden, [1], activation = tf.nn.sigmoid)
    contexts.append(tf.stop_gradient(output_hidden))
    output_l2r_scores.append(output_pos)
  return output_l2r_scores, labels, weights, contexts, context_query_fea, context_other_emb, labels_list, weights_list, photo_div_emb

def poslabel_ctr_model(parameters_dict, loss_name):
  output_l2r_scores, labels, weights, contexts, context_query_fea, context_other_emb, labels_list, weights_list, _ = poslabel_model(parameters_dict, loss_name)
  n = 10
  losses = []
  output_dict = {}
  ...
  # l2r 单点模型
  for i in range(n):
    losses.append(tf.losses.log_loss(labels=labels[i], predictions=output_l2r_scores[i], reduction=tf.losses.Reduction.SUM))
  output_dict['loss'] = losses
  output_dict['preds'] = [output_l2r_scores[i] for i in range(n)]
  output_dict['labels'] = [labels[i] for i in range(n)]
  output_dict['weights'] = weights
  output_dict['q_names'] = ["ctr" + '_idx' + str(i) for i in range(n)]
  return output_dict

def poslabel_ltr_model(parameters_dict, loss_name):
  output_l2r_scores, labels, weights, contexts, context_query_fea, context_other_emb, labels_list, weights_list, photo_div_emb = poslabel_model(parameters_dict, loss_name)
  losses = []
  output_dict = {}
  n = 10
  ...
  output = ppnet(loss_name+"context_ltr_out", context_input, context_input, [256, 128, 64], last_unit=1)
  scores_list = [output]
  # l2r 单点模型
  for i in range(n):
    losses.append(tf.losses.log_loss(labels=labels[i], predictions=output_l2r_scores[i], weights = weights[i], reduction=tf.losses.Reduction.SUM))
  for i in range(1):
    losses.append(tf.losses.log_loss(labels=labels_list[i], predictions=output, weights = weights_list[i], reduction=tf.losses.Reduction.SUM))
  # sim_loss = 0.01 * cal_similarity_score(photo_embs)
  output_dict['loss'] = losses
  output_dict['preds'] = [output_l2r_scores[i] for i in range(n)] + [output]
  output_dict['labels'] = [labels[i] for i in range(n)] + labels_list
  output_dict['weights'] = [weights[i] for i in range(n)] + weights_list
  output_dict['q_names'] = ["l2r" + '_idx' + str(i) for i in range(n)] + ["list_ltr0"]
  if (args.mode == 'predict'):
    for i in range(n+1):
      if i < 10:
        output = output_l2r_scores[i]
        out_name = 'l2r_pos'
...
def gen_point_model(parameters_dict, loss_name):
  output_dict = {}
  parameters_name = get_parameter_names_by_loss_name(loss_name) 
  parameters = [parameters_dict[name] for name in parameters_name] 
  user_fea_num = 22
  ...
  cl_net_sv = tf.reshape(parameters_dict["l2r_user_view_list_sv"], (rown, n, 4))
  cl_net_lv = tf.reshape(parameters_dict["l2r_user_view_list_lv"], (rown, n, 4))
  cl_input = tf.concat([cl_net_pid, cl_net_aid, cl_net_hetu1, cl_net_hetu2, cl_net_ev, cl_net_sv, cl_net_lv], axis = -1)
  cl_input = tf.stop_gradient(cl_input)
  name = "gen_pos_model"
  batch = photo_fea_num + 2
  n = 10
  if args.mode == 'predict':
    n = 1
  loss = 0
  atten = user_tower("ctr_user", user, cl_input, query_dim = 332, nh = 1, dim = 92)
  ...
  for i in range(n): 
    ...
    watchtime_pxtr = tf.concat(p[13:20], 1)
    ltr_pid = tf.concat(p[20:22], -1)
    ltr_hetu = tf.concat(p[44:46], -1)
    data0 = tf.concat(p[20:-2], 1)
    weight = p[-1]
    pos_emb = get_pos_embedding('ctr_10' + 'pos_', i, rown, dim=64)  # 用list模型的pos_emb
    gate_fea = tf.concat([user, atten, interact_pxtr, data0, watchtime_pxtr], axis = 1)
    short_net_query = tf.concat([user, ltr_pid, ltr_pid], axis=1)
    short_net_query = tf.stop_gradient(short_net_query)
    query_dim = 64
    short_net_query = fc_layer(name + 'ltr_user_short', short_net_query, [query_dim])
    gate_fea = tf.stop_gradient(gate_fea)
    data = ppnet(name+'l2r_dnn', tf.concat([gate_fea, user_id, ltr_pid, ltr_hetu], axis=-1), gate_fea, [256, 128, 64])
    data1 = gate_tower(name+'vtr_dnn1', interact_pxtr, [64], short_net_query)
    data2 = gate_tower(name+'vtr_dnn2', watchtime_pxtr, [64], short_net_query)
    gate = gate_layer2(name+'vtr_gate', short_net_query, 64)
    data_pxtr = data1 * gate + data2 * (1 - gate)
    pred = ppnet(name + 'l2r_dnn_out', tf.concat([gate_fea, user_id, ltr_pid, ltr_hetu], axis=-1), tf.concat([data, data_pxtr, pos_emb],axis=-1), [64, 64], last_unit=1)
    loss += tf.losses.log_loss(labels=label, predictions=pred, weights = weight, reduction=tf.losses.Reduction.SUM)
  output_dict['loss'] = loss
  output_dict['preds'] = pred
  output_dict['labels'] = label
  output_dict['weights'] = weight
  output_dict['q_names'] = "gen_pos_l2r"
  if args.mode == 'predict':
    output = pred
    out_name = 'gen_l2r_pos'
    output = tf.identity(output, "{}".format(out_name))
    output_dict[out_name] = output
  return output_dict

def cal_similarity_score(photo_emb, smooth = 5.0):
  '''
    input photo_emb: [batch_size, len, photo_emb]
    output sim_score: [batch_size, len]
  '''
  photo_emb = tf.stop_gradient(photo_emb)
  sim_matrix = tf.matmul(photo_emb, tf.transpose(photo_emb, perm = [0,2,1]))
  diag_part = tf.matrix_diag_part(sim_matrix)
  sim_score = (tf.reduce_sum(sim_matrix, [1,2]) - tf.reduce_sum(diag_part, [1]))/2
  div_score = tf.exp(-1 * sim_score / smooth)
  return sim_score, div_score

loss_model_dict = { 
  # "ctr_10" : ctr_label_model, 
  "l2r_10" : poslabel_ltr_model, 
  # "ctr_10" : poslabel_ctr_model
...
if (args.mode == 'train'):
  ...
  parameters_dict, join_parameters_size_dict, label_dict, label_value_dict, weight, sample_flag = get_kuiba_loss_relative("l2r_10", True, True, True)
  gen_out_dict = gen_point_model(parameters_dict, "l2r_10")
  gen_pred = gen_out_dict['preds']
  gen_label = gen_out_dict['labels']
  gen_loss = gen_out_dict['loss']
  gen_q_name = gen_out_dict['q_names']
  gen_weight = gen_out_dict['weights']
  targets.append((gen_q_name+"_label", gen_pred, gen_label, sample_flag, 'auc'))
  targets.append((gen_q_name+"_reward", gen_pred, gen_weight, sample_flag, 'linear_regression'))
  loss_tensor_dict[gen_q_name] = gen_loss
  dense_loss_tensor_dict[gen_q_name] = gen_loss
  for loss_name, model in loss_model_dict.items():
    ...
    if loss_name == "ctr_10":
      ...
      for i, pred in enumerate(preds):
        ...
        targets.append((q_names[i]+"_label", pred, labels[i], sample_flag, auc))
        targets.append((q_names[i]+'_reward', pred, weights[i], sample_flag, 'linear_regression'))
...
if args.mode == 'predict':
  targets = []
  parameters_dict, join_parameters_size_dict, label_dict, label_value_dict, weight, sample_flag = get_kuiba_loss_relative("l2r_10", False, False, False)
  xtr_output = gen_point_model(parameters_dict, "l2r_10")
  for (k, v) in xtr_output.items():
    if k.find('gen_l2r_pos') != -1:
     targets.append((k, v))
  for loss_name, model in loss_model_dict.items():
