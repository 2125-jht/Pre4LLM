...
class EvaluatorModel:
    def __init__(self, parameters_dict, print_ops, list_size, candidates_size, list_num,
                 point_wise_tasks, max_interaction_value, training=True):
    ...
    def _build_list_value_outputs(
        ...
        with tf.variable_scope("list_value_branch", reuse=tf.AUTO_REUSE):
            ...
            # 三种 WT Head 共享显式 Prefix 累计 duration 特征，避免只在
    ...
    def model(self, list_index, list_duration_s=None) -> tuple:
        with tf.variable_scope("evaluator", reuse=tf.AUTO_REUSE):
            ...
            # PLE 为四个任务提供共享/独立专家，任务间再做一次 output attention；
            # 每个输出最终为 p_{task,i}=sigmoid(MLP_task(h_i))，范围 [0,1]。
            ...
            # -------- 梯度隔离的因果 List Value 分支 --------
            #
            # 所有启用的 List WT Head 共用该隔离入口：List Value loss 只能更新
            # list_value_input_adapter / list_value_branch，不能反向修改共享底层；
            # point-wise loss 仍可正常更新 share_bottom / com_embs。
            # 推理阶段 stop_gradient 等价于 identity。
            list_value_source = tf.stop_gradient(
                hidden_states,
                name="list_value_detached_shared_features",
            )
            list_value_zeros = tf.zeros(
                shape=[batch_size, 1, list_value_source.shape[-1]],
                dtype=tf.float32,
            )
            list_value_item_emb = tf.gather_nd(
                tf.concat([list_value_zeros, list_value_source], axis=1),
                indices,
            )  # (?, list_num, list_size, dim)
            with tf.variable_scope("list_value_input_adapter", reuse=tf.AUTO_REUSE):
                list_value_item_emb = tf.layers.dense(
                    list_value_item_emb,
                    64,
                    activation=tf.nn.leaky_relu,
                    name="projection",
                )
            print("list_value_item_emb ", list_value_item_emb)
