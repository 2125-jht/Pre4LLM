...
def pairwise_bpr_loss_v2(output, score, threshold, weight, mask):
    ...
    pairwise_labels = tf.cast(tf.greater_equal(score_diff, threshold), tf.float32)
    pairwise_weight = tf.where(tf.greater(weight_diff, 0.0), weight_diff + 1.0, tf.ones_like(weight_diff))
    ...
    # 计算BPR损失
    bpr_loss = -tf.log(logit_diff) * pairwise_labels * pairwise_weight
...
def get_play_labels(duration, play_time):
    ...
    # 有效播、长播
    thresholds = [[8.0, 12.0], [10.0, 14.0], [11.0, 18.0], [12.0, 20.0], [14.0, 28.0], [13.0, 27.0], [9.0, 23.0]]
...
# dense feature
label_value_dict["pctr"] = tf.cast(tf.reshape(config.get_label("context_info__pctr_list", dim=CANDIDATES_SIZE), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
label_value_dict["plvtr"] = tf.cast(tf.reshape(config.get_label("context_info__plvtr_list", dim=CANDIDATES_SIZE), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
label_value_dict["pvtr"] = tf.cast(tf.reshape(config.get_label("context_info__pvtr_list", dim=CANDIDATES_SIZE), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
...
label_value_dict["pltr"] = tf.cast(tf.reshape(config.get_label("context_info__pltr_list", dim=CANDIDATES_SIZE), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
label_value_dict["pcmtr"] = tf.cast(tf.reshape(config.get_label("context_info__pcmtr_list", dim=CANDIDATES_SIZE), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
label_value_dict["pwtr"] = tf.cast(tf.reshape(config.get_label("context_info__pwtr_list", dim=CANDIDATES_SIZE), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
label_value_dict["photo_info__explore_stat__click_count_list"] = tf.cast(tf.reshape(config.get_dense_fea("photo_info__explore_stat__click_count_list", dim=CANDIDATES_SIZE, dtype=tf.int64), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
label_value_dict["photo_info__explore_stat__real_show_count_list"] = tf.cast(tf.reshape(config.get_dense_fea("photo_info__explore_stat__real_show_count_list", dim=CANDIDATES_SIZE, dtype=tf.int64), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
label_value_dict["photo_info__explore_stat__like_count_list"] = tf.cast(tf.reshape(config.get_dense_fea("photo_info__explore_stat__like_count_list", dim=CANDIDATES_SIZE, dtype=tf.int64), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
label_value_dict["photo_info__explore_stat__long_play_count_list"] = tf.cast(tf.reshape(config.get_dense_fea("photo_info__explore_stat__long_play_count_list", dim=CANDIDATES_SIZE, dtype=tf.int64), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
label_value_dict["photo_info__explore_stat__short_play_count_list"] = tf.cast(tf.reshape(config.get_dense_fea("photo_info__explore_stat__short_play_count_list", dim=CANDIDATES_SIZE, dtype=tf.int64), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
label_value_dict["photo_info__explore_stat__follow_count_list"] = tf.cast(tf.reshape(config.get_dense_fea("photo_info__explore_stat__follow_count_list", dim=CANDIDATES_SIZE, dtype=tf.int64), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
label_value_dict["photo_info__explore_stat__view_length_sum_list"] = tf.cast(tf.reshape(config.get_dense_fea("photo_info__explore_stat__view_length_sum_list", dim=CANDIDATES_SIZE, dtype=tf.int64), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
label_value_dict["photo_info__fountain_stats__real_show_count_list"] = tf.cast(tf.reshape(config.get_dense_fea("photo_info__fountain_stats__real_show_count_list", dim=CANDIDATES_SIZE, dtype=tf.int64), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
label_value_dict["photo_info__fountain_stats__like_count_list"] = tf.cast(tf.reshape(config.get_dense_fea("photo_info__fountain_stats__like_count_list", dim=CANDIDATES_SIZE, dtype=tf.int64), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
label_value_dict["photo_info__fountain_stats__follow_count_list"] = tf.cast(tf.reshape(config.get_dense_fea("photo_info__fountain_stats__follow_count_list", dim=CANDIDATES_SIZE, dtype=tf.int64), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
label_value_dict["photo_info__fountain_stats__long_play_count_list"] = tf.cast(tf.reshape(config.get_dense_fea("photo_info__fountain_stats__long_play_count_list", dim=CANDIDATES_SIZE, dtype=tf.int64), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
label_value_dict["photo_info__fountain_stats__short_play_count_list"] = tf.cast(tf.reshape(config.get_dense_fea("photo_info__fountain_stats__short_play_count_list", dim=CANDIDATES_SIZE, dtype=tf.int64), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
label_value_dict["photo_info__fountain_stats__view_length_sum_list"] = tf.cast(tf.reshape(config.get_dense_fea("photo_info__fountain_stats__view_length_sum_list", dim=CANDIDATES_SIZE, dtype=tf.int64), [-1, CANDIDATES_SIZE]), dtype=tf.float32)
...
# print_ops.append(tf.print("photo_emp_explore_ctr ", label_value_dict["photo_emp_explore_ctr"][8], summarize=10, output_stream=sys.stdout))
# print_ops.append(tf.print("photo_emp_explore_ltr ", label_value_dict["photo_emp_explore_ltr"][8], summarize=10, output_stream=sys.stdout))
# print_ops.append(tf.print("photo_emp_explore_avg_time ", label_value_dict["photo_emp_explore_avg_time"][8], summarize=10, output_stream=sys.stdout))
# print_ops.append(tf.print("photo_emp_fountain_lvtr ", label_value_dict["photo_emp_fountain_lvtr"][8], summarize=10, output_stream=sys.stdout))
# print_ops.append(tf.print("photo_emp_fountain_svtr ", label_value_dict["photo_emp_fountain_svtr"][8], summarize=10, output_stream=sys.stdout))
# print_ops.append(tf.print("photo_emp_fountain_ltr ", label_value_dict["photo_emp_fountain_ltr"][8], summarize=10, output_stream=sys.stdout))
# print_ops.append(tf.print("photo_emp_fountain_wtr ", label_value_dict["photo_emp_fountain_wtr"][8], summarize=10, output_stream=sys.stdout))
# print_ops.append(tf.print("photo_emp_fountain_avg_time ", label_value_dict["photo_emp_fountain_avg_time"][8], summarize=10, output_stream=sys.stdout))
# print_ops.append(tf.print("photo_emp_fountain_avg_fintr ", label_value_dict["photo_emp_fountain_avg_fintr"][8], summarize=10, output_stream=sys.stdout))
...
if is_training:
    ...
    # rerank_list_item_idx_flat_list = tf.clip_by_value(rerank_list_item_idx_flat_list, 0, CANDIDATES_SIZE)
    ...
    evtr_weight = 1.0 + lvtr_label + finish_label * 5.0 + like_label * 30.0 + follow_label * 200.0 + comment_label * 200.0 + forward_label * 200.0
    lvtr_weight = 1.0 + like_label * 30.0 + follow_label * 200.0 + comment_label * 100.0 + forward_label * 150.0 \
    ...
    reward_label = tf.where(tf.greater(click_label, 0.0), tf.ones_like(reward_label) * 2.0, reward_label)
    ...
    reward_label = tf.where(tf.logical_and(tf.greater(click_label, 0.0), action_once), tf.ones_like(reward_label) * 5.0, reward_label)
    ...
    reward_weight = tf.where(tf.greater(base_ltr_label, 0.0), tf.ones_like(reward_weight) * 3.0, reward_weight)
    reward_weight = tf.where(tf.greater(base_finish_label, 0.0), tf.ones_like(reward_weight) * 4.0, reward_weight)
    reward_weight = tf.where(action_once, tf.ones_like(reward_weight) * 5.0, reward_weight)
    reward_weight = tf.where(tf.logical_and(tf.greater(click_label, 0.0), action_once), tf.ones_like(reward_weight) * 30.0, reward_weight)
    reward_weight = tf.where(tf.logical_and(tf.greater(base_ltr_label, 0.0), action_once), tf.ones_like(reward_weight) * 50.0, reward_weight)
    reward_weight = tf.where(tf.logical_and(tf.greater(base_finish_label, 0.0), action_once), tf.ones_like(reward_weight) * 80.0, reward_weight)

    # advantage_reward = play_time_s + slide_label * 2.0 + like_label * 10.0 + follow_label * 50.0 + comment_label * 50.0 + forward_label * 50
    advantage_reward = play_time_s + slide_label
    ...
    # print_ops.append(tf.print("list_show_label ", list_show_label[8], summarize=10, output_stream=sys.stdout))
    ...
    with tf.control_dependencies(print_ops):
        ...
        for loss_name in point_wise_output_dict: # point-wise 有三维，压缩为两维方便处理
            ...
            if loss_name == "ltr":
                # label = tf.reshape(list_reward_label, [batch_size, LIST_NUM * LIST_SIZE])
                # weight = tf.reshape(list_reward_weight, [batch_size, LIST_NUM * LIST_SIZE])
                # loss = pairwise_bpr_loss_v2(output, label, tf.zeros_like(label), weight, mask)
                # loss = tf.reduce_sum(loss)
                # ltr_label = tf.reshape(list_base_ltr_label, [batch_size, LIST_NUM * LIST_SIZE])
                # targets.append((loss_name, output, ltr_label, mask, "auc"))
                ...
                loss = tf.losses.log_loss(labels=label, predictions=output, weights=mask * weight, reduction=tf.losses.Reduction.SUM)
                targets.append((loss_name, output, label, mask, "auc"))
            elif loss_name == "vtr":
                list_vtr_label = tf.reshape(list_vtr_label, [batch_size, LIST_NUM * LIST_SIZE])
                loss = tf.losses.huber_loss(labels=list_vtr_label, predictions=output, weights=mask, reduction=tf.losses.Reduction.SUM, delta=0.05)
                loss = loss * 150.0
                ...
                loss = tf.losses.log_loss(labels=list_wtd_label, predictions=output, weights=mask, reduction=tf.losses.Reduction.SUM)
                ...
                list_slide_label = tf.reshape(list_slide_label, [batch_size, LIST_NUM * LIST_SIZE])
                # TODO: fix here
                loss = tf.losses.log_loss(labels=list_slide_label, predictions=output, weights=tf.zeros_like(mask), reduction=tf.losses.Reduction.SUM)
                targets.append((loss_name, output, list_slide_label, mask, "auc"))
            elif loss_name == "click":
                list_evtr_weight = tf.reshape(list_evtr_weight, [batch_size, LIST_NUM * LIST_SIZE])
                list_evtr_label = tf.reshape(list_evtr_label, [batch_size, LIST_NUM * LIST_SIZE])
                list_duration_s = tf.reshape(list_duration_s, [batch_size, LIST_NUM * LIST_SIZE])
                list_evtr_weight = tf.where(tf.greater(list_evtr_label, 0.0), list_evtr_weight, tf.ones_like(list_evtr_weight))
                list_evtr_weight = tf.where(tf.logical_and(tf.greater(list_evtr_label, 0.0), tf.equal(list_duration_s, 0.0)), list_evtr_weight * 0.3, tf.ones_like(list_evtr_weight))
                loss = tf.losses.log_loss(labels=list_evtr_label, predictions=output, weights=mask * list_evtr_weight, reduction=tf.losses.Reduction.SUM)
                # loss = tf.losses.log_loss(labels=list_evtr_label, predictions=output, weights=mask, reduction=tf.losses.Reduction.SUM)
                targets.append((loss_name, output, list_evtr_label, mask, "auc"))
