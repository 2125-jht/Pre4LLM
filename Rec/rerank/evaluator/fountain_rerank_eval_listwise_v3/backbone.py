...
all_model_preds = [
  ...
  "pltr",
  "expected_vtr_wt",
  # v3 三个独立实验输出。relative_list_watch_time 仅用于校准排查，
  # 线上实际加分使用 relative_list_delta。
  "reach_pointwise_wt",
  "reach_incremental_wt",
  "relative_list_delta",
        ...
        return self \
            ...
            .get_abtest_params(
              ...
              ab_params = [
                ...
                # v3 默认全部关闭，保证生成配置后线上打分与 v2 完全一致。
                # 每次 AB 只打开一个权重，避免实验收益无法归因。
                ("fountain_rerank_eval_list_reach_pointwise_wt_weight", 0.0, "reach_pointwise_wt_weight"),
                ("fountain_rerank_eval_list_reach_incremental_wt_weight", 0.0, "reach_incremental_wt_weight"),
                ("fountain_rerank_eval_list_relative_delta_weight", 0.0, "relative_delta_weight"),
            ...
            .if_("item_num >= 60") \
              ...
              .enrich_attr_by_lua(
                ...
                import_common_attr=["pctr", "pwtd", "pvtr", "pltr", "expected_vtr_wt", "reach_pointwise_wt", "reach_incremental_wt", "relative_list_delta", "expected_ltr", "expected_pctr", "expected_like", "expected_comment", "expected_follow", "context_pctr", "context_pwtd", "context_pltr", "context_pcmtr", "context_pwtr", "context_plvtr", "context_psvtr", "context_pftr", "pctr_weight", "pwtd_weight", "pvtr_weight", "pltr_weight", "pslide_weight", "expected_wt_weight", "reach_pointwise_wt_weight", "reach_incremental_wt_weight", "relative_delta_weight", "expected_ltr_weight", "expected_pctr_weight", "expected_like_weight", "expected_comment_weight", "expected_follow_weight", "context_pctr_weight", "context_pwtd_weight", "context_pltr_weight", "context_pwtr_weight", "context_pcmtr_weight", "context_plvtr_weight", "context_psvtr_weight", "context_pftr_weight", "rerank_list_item_idx_flat_list_max_size", "rerank_list_item_size", "rerank_list_true_num", "enable_position_decay", "use_multiply", "use_order_es"],
                ...
                lua_script="""
                  ...
                  function calculate_list_scores()
                    ...
                    for i = 1, list_num do
                      ...
                      local es_i = 0.0
                      local list_es_i = 0.0
                      local expected_wt_reward = expected_wt_weight * expected_vtr_wt[i] * 0.1
                      -- v3 三个 List 级分数彼此独立。两个 WT 沿用 0.1 缩放；
                      -- relative delta 位于 log1p 空间，先截断到 [-2, 2] 防止异常值。
                      local reach_pointwise_wt_reward = reach_pointwise_wt_weight * reach_pointwise_wt[i] * 0.1
                      local reach_incremental_wt_reward = reach_incremental_wt_weight * reach_incremental_wt[i] * 0.1
                      local clipped_relative_delta = math.max(-2.0, math.min(2.0, relative_list_delta[i] or 0.0))
                      local relative_delta_reward = relative_delta_weight * clipped_relative_delta
                      ...
                      if use_multiply == 1 then
                        ...
                        if expected_wt_weight > 0.0 then
                          list_multiply_score = list_multiply_score * ((1.0 + expected_vtr_wt[i] * 0.1) ^ expected_wt_weight)
                        end
                        if reach_pointwise_wt_weight > 0.0 then
                          list_multiply_score = list_multiply_score * ((1.0 + reach_pointwise_wt[i] * 0.1) ^ reach_pointwise_wt_weight)
                        end
                        if reach_incremental_wt_weight > 0.0 then
                          list_multiply_score = list_multiply_score * ((1.0 + reach_incremental_wt[i] * 0.1) ^ reach_incremental_wt_weight)
                        end
                        if relative_delta_weight > 0.0 then
                          list_multiply_score = list_multiply_score * math.exp(clipped_relative_delta * relative_delta_weight)
                        end
                        ...
                        list_es_i = expected_wt_reward + reach_pointwise_wt_reward + reach_incremental_wt_reward + relative_delta_reward + expected_ltr_reward + expected_pctr_reward + expected_comment_reward + expected_like_reward + expected_follow_reward
                      end
            ...
            .log_debug_info(
              ...
              common_attrs = [
                ...
                "expected_follow",
                "expected_wt_weight",
                "reach_pointwise_wt_weight",
                "reach_incremental_wt_weight",
                "relative_delta_weight",
                "expected_ltr_weight",
