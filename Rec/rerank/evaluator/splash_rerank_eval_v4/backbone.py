...
kess_name = "grpc_fountain_splash_rerank_model_flash_evaluator_server"
all_model_preds = [
  "pctr",
  "pwtd",
  "pvtr",
    ...
    def predict_with_mio_model(self, **kwargs):
        ...
        return self \
            ...
            .get_abtest_params(
              ...
              device_id = "{{device_id}}",
              ab_params = [
                ("fountain_splash_rerank_eval_list_enable_position_decay", True, "enable_position_decay"),
                ("fountain_splash_rerank_eval_list_enable_mul_es", False, "enable_mul_es"),
                ...
                ("fountain_splash_rerank_eval_list_context_pftr_weight", 0.1, "context_pftr_weight"),
                ("fountain_splash_rerank_eval_list_context_plvtr_weight", 0.0, "context_plvtr_weight"),
              ]
            ) \
            ...
            .if_("item_num >= 30 or rerank_list_item_idx_flat_list == nil") \
              ...
              .enrich_attr_by_lua(
                ...
                import_common_attr=["pctr", "pwtd", "pvtr", "pltr", "pslide", "context_pctr", "context_pwtd", "context_pltr", "context_pcmtr", "context_pwtr", "context_plvtr", "context_psvtr", "context_pftr", "pctr_weight", "pwtd_weight", "pvtr_weight", "pltr_weight", "pslide_weight", "context_pctr_weight", "context_pwtd_weight", "context_pltr_weight", "context_pwtr_weight", "context_pcmtr_weight", "context_plvtr_weight", "context_psvtr_weight", "context_pftr_weight", "rerank_list_item_idx_flat_list_max_size", "rerank_list_item_size", "rerank_list_true_num", "enable_position_decay", "enable_mul_es"],
                    ...
                    for i = 1, list_num do
                      ...
                      if i < rerank_list_true_num + 1 then
                        for j = 1, rerank_list_item_size do
                          ...
                          if enable_mul_es == 1 then
                            local context_pltr_reward = context_pltr_weight * (context_pltr[start_idx + j] * 200.0 + context_pcmtr[start_idx + j] * 100.0 + context_pwtr[start_idx + j] * 150.0)
                            element_score = (1.0 + context_pwtd_weight * context_pwtd[start_idx + j]) * (1.0 + context_pctr_weight * context_pctr[start_idx + j]) * (1.0 + context_pltr_weight * context_pltr[start_idx + j])
                          else
                            ...
                            local context_pltr_reward = context_pltr_weight * context_pltr[start_idx + j] * 120.0
                            local context_pcmtr_reward = context_pcmtr_weight * context_pcmtr[start_idx + j] * 80.0
                            local context_pwtr_reward = context_pwtr_weight * context_pwtr[start_idx + j] * 105.0
                            ...
                            local pltr_reward = pltr_weight * pltr[start_idx + j] * 11.0
                            local pctr_reward = pctr_weight * pctr[start_idx + j] * 10.0
                            element_score = context_pwtd_reward + context_pctr_reward + context_plvtr_reward + context_pltr_reward + context_pcmtr_reward + context_pwtr_reward + context_pftr_reward + pvtr_reward + pltr_reward + pctr_reward + pwtd_reward
                          end
