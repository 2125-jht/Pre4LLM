...
if is_training:
    ...
    with tf.control_dependencies(print_ops):
        ...
        for loss_name in point_wise_output_dict:
            output = point_wise_output_dict[loss_name] # (?, cand_size)
            ...
            if loss_name == "vtr":
                ...
                weight_with_mask = show_label * is_first_screen # only realshow sample
                ...
                weight_with_mask = show_label # only realshow sample
                loss = tf.losses.log_loss(labels=ltr_label, predictions=output, weights=weight_with_mask, reduction=tf.losses.Reduction.SUM)
                #loss = loss / 100.0
                ...
                loss = tf.losses.log_loss(labels=click_label, predictions=output, weights=weight_with_mask, reduction=tf.losses.Reduction.SUM)
    ...
    pctr = tf.gather(tf.concat([zeros, pctr], axis=-1), label_value_dict['rerank_list_item_idx_flat_list'], axis=1, batch_dims=1) # (?, list_num, list_size)
    pctr = tf.reshape(pctr, [-1, LIST_NUM * LIST_SIZE])
    # pwtd = tf.gather(tf.concat([zeros, pwtd], axis=-1), label_value_dict['rerank_list_item_idx_flat_list'], axis=1, batch_dims=1) # (?, list_num, list_size)
    pvtr = tf.gather(tf.concat([zeros, pvtr], axis=-1), label_value_dict['rerank_list_item_idx_flat_list'], axis=1, batch_dims=1) # (?, list_num, list_size)
    pvtr = tf.reshape(pvtr, [-1, LIST_NUM * LIST_SIZE])
    pltr = tf.gather(tf.concat([zeros, pltr], axis=-1), label_value_dict['rerank_list_item_idx_flat_list'], axis=1, batch_dims=1) # (?, list_num, list_size)
    pltr = tf.reshape(pltr, [-1, LIST_NUM * LIST_SIZE])
    print("pltr ", pltr)
    pvtr_wt = tf.gather(tf.concat([zeros, pvtr_wt], axis=-1), label_value_dict['rerank_list_item_idx_flat_list'], axis=1, batch_dims=1) # (?, list_num, list_size)
    pvtr_wt = tf.reshape(pvtr_wt, [-1, LIST_NUM * LIST_SIZE])
    rerank_list_item_idx_flat_list_print = tf.reshape(label_value_dict['rerank_list_item_idx_flat_list'], [-1, LIST_NUM * LIST_SIZE])
