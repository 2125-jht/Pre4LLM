    ...
    def model(self, training=True, decode_method="beam_search", beam_size=1, max_length=10):
        ...
        with tf.variable_scope("generator", reuse=tf.AUTO_REUSE):
            ...
            if self._training:
                ...
                like_weight = tf.where(playtime >= 7.0, like_label * 50.0, tf.zeros_like(like_label))
                # like_weight = tf.where(playtime >= 16.0, like_label * 100.0, like_weight)
                follow_weight = tf.where(playtime >= 7.0, follow_label * 150.0, tf.zeros_like(follow_label))
                # follow_weight = tf.where(playtime >= 16.0, follow_label * 800.0, follow_weight)
                comment_weight = tf.where(playtime >= 7.0, comment_label * 150.0, tf.zeros_like(comment_label))
                # comment_weight = tf.where(playtime >= 16.0, comment_label * 800.0, comment_weight)
                next_weight = tf.where(next_label >= 1.0, next_label * 5.0, tf.zeros_like(next_label))
                wtd_weight = tf.where(tf.logical_and(wtd_label >= 0.6, playtime > 6.0), wtd_label * 10.0, tf.zeros_like(wtd_label))
                next_lvtr_weight = tf.where(tf.math.logical_and(next_label >= 1.0, playtime > 15.0), next_label, tf.zeros_like(next_label))
                ...
                # mask 掉非有效播放会掉vv
                # valid_pos_output = -tf.log(pos_output+1e-9) * tf.cast(tf.concat([click_label, tf.cast(zeros, dtype=tf.float32)], axis=1), dtype=tf.float32)
                # 子序列 loss
