...
duration_time_buckets = [0,9,13,20,38,71,118,195]
play_time_buckets = [[0,8, 12, 16, 19, 25, 29, 33, 39, 45, 52, 58, 65, 71, 77, 84, 92, 101, 111, 122, 135, 153, 173, 196, 223, 256, 297, 349, 420, 527, 730, 100000000],
[0,9, 11, 13, 15, 17, 20, 23, 27, 32, 36, 39, 43, 47, 52, 56, 60, 64, 68, 72, 75, 78, 81, 84, 87, 90, 93, 97, 101, 104, 109, 113, 118, 123, 128, 132, 136, 140, 145, 149, 153, 158, 163, 169, 175, 181, 189, 198, 209, 221, 234, 249, 267, 291, 321, 340, 363, 392, 430, 483, 570, 757, 100000000],
[0,9, 11, 13, 17, 21, 27, 31, 36, 40, 44, 49, 55, 61, 67, 73, 80, 87, 93, 98, 102, 106, 109, 113, 116, 120, 124, 128, 132, 135, 138, 142, 146, 151, 156, 161, 167, 173, 179, 186, 192, 199, 206, 213, 221, 230, 240, 251, 264, 283, 300, 315, 337, 362, 394, 443, 519, 686, 100000000],
[0,9, 11, 12, 14, 16, 19, 22, 26, 30, 35, 39, 44, 49, 55, 62, 70, 76, 82, 88, 95, 101, 109, 116, 123, 130, 136, 141, 147, 152, 157, 162, 167, 172, 178, 184, 191, 197, 204, 212, 219, 225, 232, 239, 247, 256, 265, 275, 284, 295, 306, 317, 331, 346, 365, 379, 395, 415, 443, 481, 536, 581, 645, 759, 1006, 100000000],
[0,9, 11, 13, 16, 21, 26, 31, 35, 39, 44, 50, 57, 65, 75, 86, 95, 104, 113, 124, 135, 147, 161, 174, 187, 199, 210, 218, 225, 232, 238, 245, 251, 258, 265, 272, 279, 285, 292, 299, 305, 312, 319, 326, 334, 342, 349, 357, 366, 373, 380, 386, 393, 401, 413, 429, 448, 472, 494, 521, 555, 597, 630, 675, 719, 777, 825, 897, 1002, 1167, 1556, 100000000],
[0,9, 11, 14, 17, 22, 28, 33, 37, 42, 48, 54, 62, 71, 83, 96, 108, 120, 132, 146, 162, 178, 196, 215, 234, 256, 278, 301, 325, 350, 373, 393, 407, 419, 430, 442, 453, 465, 477, 489, 502, 515, 528, 542, 555, 569, 580, 590, 600, 610, 619, 629, 638, 648, 658, 670, 681, 694, 707, 717, 730, 755, 801, 873, 996, 1241, 100000000],
[0,9, 13, 16, 21, 28, 34, 42, 47, 54, 62, 71, 82, 96, 112, 131, 147, 163, 181, 200, 220, 242, 266, 285, 302, 321, 339, 358, 378, 398, 418, 439, 461, 483, 506, 529, 553, 578, 603, 629, 655, 681, 705, 725, 744, 761, 778, 795, 812, 831, 849, 869, 888, 906, 926, 948, 971, 999, 1020, 1041, 1063, 1087, 1113, 1147, 1165, 1185, 1212, 1238, 1280, 1343, 1443, 1584, 1798, 2212, 100000000],
[0,13, 23, 33, 34, 42, 53, 61, 70, 80, 93, 108, 125, 143, 159, 176, 195, 216, 239, 264, 290, 319, 350, 380, 404, 428, 452, 477, 503, 531, 560, 590, 621, 654, 688, 724, 762, 802, 844, 888, 935, 982, 1031, 1081, 1130, 1174, 1220, 1260, 1291, 1323, 1357, 1393, 1430, 1470, 1514, 1560, 1610, 1664, 1726, 1794, 1869, 1944, 2029, 2167, 2465, 3051, 100000000],
[0,13, 23, 33, 34, 42, 53, 61, 70, 83, 96, 106, 115, 126, 139, 154, 170, 188, 207, 229, 253, 279, 308, 340, 374, 411, 451, 494, 528, 561, 596, 632, 672, 713, 758, 806, 855, 908, 966, 1020, 1077, 1129, 1184, 1243, 1304, 1369, 1438, 1512, 1589, 1670, 1757, 1845, 1930, 1988, 2031, 2073, 2130, 2190, 2255, 2327, 2409, 2496, 2599, 2711, 2830, 2951, 3050, 3203, 3387, 3575, 3736, 3965, 4227, 4636, 5353, 100000000]]
  
buck_len = len(play_time_buckets)
# view_list_dim = [32, 32, 8, 8, 4, 4, 4]
view_list_dim = [8,8]
ltr_view_sg_list = [False, False, False, False, False, False, False]
view_list_length = 50
def getDurationId(duration) :
  duration_index = 0
  for i, val in enumerate(duration_time_buckets):
    duration_index = i
    if tf.less_equal(duration, val):
      break
  play_list_len = len(duration_time_buckets)
  if duration >  duration_time_buckets[play_list_len-1] :
    duration_index = play_list_len
  return duration_index

def getPlayBuckids(play_time_list, pred_play_times):
  play_time_len = len(play_time_list)
  play_index = 0
  buck_id = pred_play_times * (play_time_len+1)
  buck_upper = math.ceil(buck_id)
  buck_floor = math.floor(buck_id)
  if buck_upper > play_time_len-1:
    return play_time_list[play_time_len-1]
  if buck_upper == 0:
    return 0.0
  bucket = (buck_id - buck_floor)/(buck_upper - buck_floor)
  play_upper = play_time_list[buck_upper]
  play_floor = play_time_list[buck_floor]
  play = bucket * (play_upper - play_floor) + play_floor
  return play
...
def self_attention_lora(loss, query, data, n, nh, dim, mask):
    scope = loss + "_self_attention"
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
      batch_size = tf.shape(data)[0]
...
def short_net(loss, query, data_list, list_len, list_dim, query_dim, nh = 1,
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        query = tf.reshape(query, [batch_size, 6, query_dim])
        #output = self_attention(scope, query, data, 1, nh, dim) # [b,  nh * dim]
        output = short_transformer(loss, query, data, 6, nh = 1, dim = query_dim) # [b,  nh * dim]
        #output = tf.layers.dense(output, dim, activation=tf.nn.relu)
    return output
...
def poslabel_model(parameters_dict, loss_name):
  ...
  mid_play_time = [mid_play_ev_pids, mid_play_lv_pids, mid_play_sv_pids]

  # cl_net_pid = tf.reshape(parameters_dict["uMidPlayViewPids"], (rown, n, 32))
  # cl_net_aid = tf.reshape(parameters_dict["uMidPlayViewAids"], (rown, n, 32))
  cl_net_hetu1 =  tf.reshape(parameters_dict["uMidPlayViewHetu1"], (rown, n, 8))
  cl_net_hetu2 = tf.reshape(parameters_dict["uMidPlayViewHetu2"], (rown, n, 8))
  # cl_net_ev = tf.reshape(parameters_dict["uMidPlayViewEvLabels"], (rown, n, 4))
  # cl_net_lv = tf.reshape(parameters_dict["uMidPlayViewLvLabels"], (rown, n, 4))
  # cl_net_sv = tf.reshape(parameters_dict["uMidPlayViewSvLabels"], (rown, n, 4))
  # cl_input = [cl_net_pid, cl_net_aid, cl_net_hetu1, cl_net_hetu2, cl_net_ev, cl_net_lv, cl_net_sv]
  ...
  for i in range(n): 
    p = parameters[i * batch : (i+1) * batch] 
    ltr_pid = tf.concat(p[18:20], -1)
    ltr_pids.append(ltr_pid)
    durations.append(p[20])
    l2r_weights.append(p[-6]) 
    l2r_labels.append(p[-5])
    play_weights.append(p[-4])
    play_rate.append(p[-3])
    next_weights.append(p[-2]) 
    next_labels.append(p[-1])
    pvtrs.append(p[0])
    interact_pxtr = tf.concat(p[1:11], 1)
    watchtime_pxtr = tf.concat(p[11:19], 1)
    interact_pxtrs.append(interact_pxtr)
    watchtime_pxtrs.append(watchtime_pxtr)
    data0.append(tf.concat(p[19:-6], 1))
    hetus.append(tf.concat(p[43:45], 1))
    if (args.mode == 'train'):
        pos_emb = get_pos_embedding(loss_name + 'pos', i, rown, dim=64)
    elif (args.mode == 'predict'):
        pos_emb = get_pos_embedding(loss_name + 'pos', 0, rown, dim=64)
  ...
  #用户list处理
  real_act_slot, mid_act_slot, foutain_act_slot  = tf.split(fc_layer(loss_name + 'user_act_slot_dnn', user, [32, 3]),[1,1,1], 1)
  ...
  user_ev_slot, user_lv_slot, user_sv_slot, fountain_lv_slot,  fountain_ev_slot= tf.split(fc_layer(loss_name + 'user_play_slot_dnn', user, [32, 5]),[1,1,1,1,1], 1)
  ...
  pid_att = self_attention(loss_name +"pid_attention", ltr_pid_emb, ltr_pid_emb, 6, 1, 64, mask = None)
  short_net_query = tf.concat([user_ems, ltr_pid_emb, hetu_emb, pid_att], axis=2)
  query_dim = 64
  short_net_query = fc_layer(loss_name + 'ltr_user_short', short_net_query, [query_dim])
  short_net_query2 = fc_layer(loss_name + 'ltr_user_short2', short_net_query, [32])
  view_net_output = short_net(loss_name +"ltr_view_list",
                              ...
                              action_item_size = 64,
                              att_emb_size = 64)
  pos_output = short_net(loss_name +"pos_list",
                              short_net_query2,
                              [user_act_embs],
                              11,
                              [32],
                              ...
                              action_item_size = 32,
                              att_emb_size = 32)
  play_output = short_net(loss_name +"play_list",
                            short_net_query2,
                            [mid_play_emb],
                            5,
                            [32],
                            ...
                            action_item_size = 32,
                            att_emb_size = 32)
  neg_output = short_net(loss_name +"neg_list",
                            short_net_query2,
                            [real_neg_pids],
                            1,
                            [32],
                            ...
                            action_item_size = 32,
                            att_emb_size = 32)
  gate_fea =  tf.concat([user_ems, view_net_output, pos_output, neg_output, play_output, datas, interact_pxtrs, watchtime_pxtrs], axis=-1)
  ...
  data = fc_layer(loss_name + 'l2r_dnn', gate_fea, [256, 128, 64])
  data1 = gate_tower(loss_name +'vtr_dnn1', interact_pxtrs, [64], short_net_query)
  data2 = gate_tower(loss_name +'vtr_dnn2', watchtime_pxtrs, [64], short_net_query)
  gate = gate_layer2(loss_name +'vtr_gate', short_net_query, 64)
  data_pxtr = data1 * gate + data2 * (1 - gate)
  pxtr_wt = fc_layer('pxtr_wt', tf.reshape(data_pxtr, [-1, 6*64]), [256, 128, 64, 6])
  avg_data_pxtr = tf.reshape(tf.reduce_mean(tf.reshape(pxtr_wt, [-1, 6, 1]) * data_pxtr, axis=1), [-1, 1, 64])
  data_pxtr = tf.concat([data_pxtr,avg_data_pxtr], axis=1)
  
  data = tf.add(data, pos_embeddings)
  data_wt = fc_layer('data_wt', tf.reshape(data, [-1, 6*64]), [256, 128, 64, 6])
  list_emb = tf.reshape(tf.reduce_mean(tf.reshape(data_wt, [-1, 6, 1]) * gate_tower(loss_name +'list_emb', data, [64], user_ems),axis=1),[-1,1,64])
  data = tf.concat([data, list_emb],axis=1)
  global_output = transformer(loss_name + "ltr_trf_1", data, data, n+1, nh=1, dim=64, mask=None)
  # atten_output = SRGA_transformer(loss_name + "ltr_srga", data, data, data, n, nh = 1, dim = 64, mask_GSA = [-1, 0], mask_LSA = [2, 2]) # [batch, 4, 128]
  atten_output = tf.reshape(tf.concat([global_output, data_pxtr],axis=-1), (rown, (n+1) * 128)) 
  atten_outputs = tf.split(atten_output, [128] * (n+1), 1)
  ...
  for i in range(n) : 
    ...
    output_list = self_attention_lora("output_cross_attention", output_list, output_list, 3, 1, 128, mask = None)
    output_list = tf.split(output_list, 3, 1)
    for j in range(3):
      ...
      if key == "wtd":
        ...
        # output_pvtr = fc_layer(key + 'pvtrs_proj', pvtrs[i], [1])
        # output_merge = fc_layer(key + 'pvtr_out_pos', tf.concat([output_hidden,output_pvtr],axis=-1), [32])
        output_pos = fc_layer(key + 'ltr_out_pos', output_hidden, [1], activation = tf.nn.sigmoid)
        output_play_scores.append(output_pos)
     
# list model
  output_list_play_score = fc_layer('list_play', atten_outputs[n], [64, 32])
  ...
  # next 单点模型
  for i in range(n):
    losses.append(tf.losses.log_loss(labels=next_labels[i], predictions=output_next_scores[i], weights = next_weights[i], reduction=tf.losses.Reduction.SUM))

  # l2r 单点模型
  for i in range(n):
    losses.append(tf.losses.log_loss(labels=l2r_labels[i], predictions=output_l2r_scores[i], weights = l2r_weights[i], reduction=tf.losses.Reduction.SUM))
  
  # play 结构
  avg_play_weight = sum(play_weights)/6
  for i in range(n):
    losses.append(tf.losses.log_loss(labels=play_rate[i], predictions=output_play_scores[i], reduction=tf.losses.Reduction.SUM))
  # pred_play_times = []
  # for i in range(n):
  #   play_time_buckets = play_buckets[i]
  #   buck_len = len(play_time_buckets)
  #   preds = tf.split(output_play_scores[i], num_or_size_splits=buck_len, axis=-1)
  #   pred_play_time = 0
  #   squeueloss = 0
  #   logloss = 0
  #   for j, val in enumerate(play_time_buckets):
  #     # 多个二分类任务，loss由logloss+huber+序列loss组成
  #     # 对播放时长分桶, 播放时长>分桶左阈值的，label都设置为1，当前桶>0.5*(左阈值+右边阈值)为1，否则为0，其他桶的label均为0
  #     # 桶内观看时长为bucket_size * p的和
  #     label = tf.where(tf.greater(play_weights[i], val/1000), tf.ones_like(play_weights[i]), tf.zeros_like(play_weights[i]))
  #     pred = tf.nn.sigmoid(preds[j])
  #     logloss += tf.losses.log_loss(labels=label, predictions=pred, reduction=tf.losses.Reduction.SUM)
  #     if j < buck_len-1:
  #       predj1 = tf.nn.sigmoid(preds[j+1])
  #       pred_play_time += (play_time_buckets[j+1] - play_time_buckets[j])*pred/1000
  #       squeueloss += tf.where(tf.greater(predj1, pred), predj1-pred, tf.zeros_like(pred))
  #   pred_play_times.append(pred_play_time)
  #   mseloss = tf.losses.mean_squared_error(play_weights[i], pred_play_time)
  #   losses.append(logloss)
  
  # list loss
  avg_play = tf.reshape(tf.reduce_mean(tf.stack(play_weights, axis = 1), axis=1), [-1,1])
  losses.append(tf.losses.huber_loss(labels=avg_play, predictions = output_list_play_score, reduction=tf.losses.Reduction.SUM, delta = 1.0))

  # list 模型
  output_dict['loss'] = losses
  output_dict['preds'] = [output_next_scores[i] for i in range(n)] + [output_l2r_scores[i] for i in range(n)] + [output_play_scores[i] for i in range(n)] +[output_list_play_score]
  output_dict['labels'] = [next_labels[i] for i in range(n)] + [l2r_labels[i] for i in range(n)] + [play_rate[i] for i in range(n)] + [avg_play]
  output_dict['q_names'] = ["next" + '_idx' + str(i) for i in range(n)] + ["l2r" + '_idx' + str(i) for i in range(n)] + ["play" + '_idx' + str(i) for i in range(n)] + ["list_play"]
  ...
  if (args.mode == 'predict'):
    for i in range(3*n+1):
      if i < 6:
        ...
        output = output_l2r_scores[i-6]
        out_name = 'pos'
        output = tf.identity(output, "{}".format(out_name + str(i-6)))
        output_dict[out_name + str(i-6)] = output
      elif i <18:
        output = output_play_scores[i-12]
        out_name = 'play'
        output = tf.identity(output, "{}".format(out_name + str(i-12)))
        output_dict[out_name + str(i-12)] = output
      else :
        output = output_list_play_score
        out_name = 'list'
        output = tf.identity(output, "{}".format(out_name + str(i-18)))
        output_dict[out_name + str(i-18)] = output
  return output_dict

loss_model_dict = { 
  # "ctr_10" : ctr_label_model, 
  "slide_l2r_6" : poslabel_model, 
  ...
  for loss_name, model in loss_model_dict.items():
    ...
    for i, pred in enumerate(preds):
      if i < 12:
        ...
        targets.append((q_names[i], pred, labels[i], sample_flag, "linear_regression"))
