...
# List Value loss 权重。三种 WT 建模方式同时训练并平分原有 WT loss
# 预算，避免新增 Head 后将时长梯度机械放大为三倍。
LENGTH_LOSS_WEIGHT = 1.0
WATCH_TIME_METHOD_COUNT = 3.0
...
# duration-normalized Huber 的标签和预测均落在 [0, 1]，其原始数值通常
# 小于 log-space Huber / bucket BCE。先用固定系数对齐初始贡献，并通过
# TensorBoard 分方法记录真实加权贡献，后续可按训练统计校准。
...
# duration-bucketed Prefix WT 使用已有的 List 级统计配置。配置按候选 List
# 累计 duration 选择播放时长边界；代码侧使用可读的语义命名，不沿用 item
# Head 的缩写。
...
# item 表示已融合候选 List 上下文；当前没有独立 pEVV 或互动行为预测头。
...
if is_training:
    ...
    # -------- 三种并行 Prefix WT 建模 --------
    # 1. log-space：原有 softplus(log1p-WT) 回归；
    # 2. duration-normalized：预测 PrefixWT / PrefixDuration；
    # 3. duration-bucketed：预测 duration 条件分桶中的连续位置。
    # 三者复用同一 causal Prefix 表征、length_probs、标签 mask 和最终聚合。
    ...
    with tf.control_dependencies(print_ops):
        ...
        # 外部模型组的 item 级 context_pwtd，使用相同候选布局和 padding mask。
        list_context_pwtd_3d = list_context_pwtd * candidate_item_mask

        # 三种聚合方式使用完全相同的 item WT，只改变位置权重：
        ...
        # VTR/WTD 各保留三组；外部 context_pwtd 补充直接求和对照。
        ...
        # 前缀价值损失直接约束每一个已观测 Prefix。三种 WT Head 同时训练：
        # 1. log-space：log1p-WT Huber；
        # 2. duration-normalized：PrefixWT/PrefixDuration Huber；
        # 3. duration-bucketed：连续桶位置的 soft-label BCE。
        ...
        # 三种 List 总 WT 均在相同 log1p 秒数空间校准，保证最终输出的
        # loss 口径一致，只让 Prefix 参数化和原生 Prefix loss 不同。
        ...
        # 累计价值随前缀变长不应下降。三种 WT 统一在 log1p 秒数空间
        # 约束，避免 raw-second 单调误差因量纲过大压过 EVV。
        ...
        monotonic_loss = wt_monotonic_loss + evv_monotonic_loss
        ...
        for method_name, method_prediction in [
        ...
        # 三种 WT 方法分别记录真实加权贡献，便于验证 loss 数值是否均衡。
        ...
        # -------- item WT 聚合方式的离线对照 --------
        # 以下输出只用于日志评估，不参与训练 loss：
        # - from_vtr/from_wtd：item WT 分别由 point-wise VTR/WTD 解码；
    ...
    # 训练与推理统一使用 P(K=k) × PrefixValue[1:k]。duration 条件的
    # 两种方式在图外层使用真实候选 duration 完成秒数解码。
