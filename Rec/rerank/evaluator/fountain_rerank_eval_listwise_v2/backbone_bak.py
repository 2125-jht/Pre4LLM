...
class PredictServerFlow(LeafFlow, KuibaApiMixin, MioApiMixin, OfflineApiMixin, GsuApiMixin, CofeaApiMixin, UniPredictApiMixin, CommonApiMixin, ExploreModelApiMixin):
    def predict_with_mio_model(self, **kwargs):
        ...
        return self \
            ...
            .pack_item_attr_to_item_attr(
              from_item_attrs=["context_info__pwtd"],
            ...
            .if_("item_num >= 60") \
              ...
              .enrich_attr_by_lua(
                ...
                import_common_attr=["pctr", "pwtd", "pvtr", "pltr", "pctr_weight", "pwtd_weight", "pvtr_weight", "pltr_weight", "rerank_list_item_idx_flat_list_max_size", "rerank_list_item_size", "rerank_list_true_num"],
                ...
                lua_script="""
                  function calculate_list_scores()
                    ...
                    function get_rank_map(pxtr)
                      local pxtr_arr = {}
                      for j = 1, #pxtr do table.insert(pxtr_arr, pxtr[j]) end
                      local wrapped_arr = {}
                        for orig_idx, v in ipairs(pxtr_arr) do
                            table.insert(wrapped_arr, {
                                orig_idx = orig_idx,  -- 保留原始索引（Lua 数组默认从 1 开始）
                                pxtr = v
                            })
                        end
                        table.sort(wrapped_arr, function(a, b)
                            return a.pxtr > b.pxtr  -- 降序排序，pxtr 大的在前
                        end)
                        local pxtr_rank_map = {}
                        for rank, item in ipairs(wrapped_arr) do
                            pxtr_rank_map[pxtr[item.orig_idx]] = rank  -- key=原value，value=排名
                        end
                        return pxtr_rank_map
                    end

                    pctr_rank_map = get_rank_map(pctr)
                    pvtr_rank_map = get_rank_map(pvtr)
                    pwtd_rank_map = get_rank_map(pwtd)
                    pltr_rank_map = get_rank_map(pltr)
                    for i = 1, list_num do
                      local start_idx = (i - 1) * rerank_list_item_size
                      local es_i = 0.0
                      if i < rerank_list_true_num + 1 then
                        for j = 1, rerank_list_item_size do
                          local adj_pctr = pctr_weight / (pctr_rank_map[pctr[start_idx + j]] + 10.0)
                          local adj_pvtr = pvtr_weight / (pvtr_rank_map[pvtr[start_idx + j]] + 10.0)
                          local adj_pwtd = pwtd_weight / (pwtd_rank_map[pwtd[start_idx + j]] + 10.0)
                          local adj_pltr = pltr_weight / (pltr_rank_map[pltr[start_idx + j]] + 10.0)
                          local element_score = adj_pctr + adj_pvtr + adj_pwtd + adj_pltr
