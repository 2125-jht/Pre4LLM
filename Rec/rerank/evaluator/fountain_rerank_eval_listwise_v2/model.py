...
class EvaluatorModel:
    ...
    def _build_list_value_outputs(
        ...
        with tf.variable_scope("list_value_branch", reuse=tf.AUTO_REUSE):
            ...
            # 时长方式二：预测 Prefix WT 相对 Prefix 累计 duration 的归一化比例。
            # duration 本身在 kai_v2_model.py 中参与标签编码和秒数解码；
            # 此处只产生 [0, 1] 的可学习比例。
            ...
            # 时长方式三：预测 Prefix WT 在 duration 条件分桶中的连续桶位置。
            # kai_v2_model.py 使用 List 级分桶配置将该位置插值还原为秒数。
            ...
            # 三种时长方式复用同一 length_probs。duration 条件的两个方式要在
            # kai_v2_model.py 完成秒数解码后再按相同公式聚合。
