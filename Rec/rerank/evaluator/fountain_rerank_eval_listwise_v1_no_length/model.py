...
class EvaluatorModel:
    ...
    def _build_list_value_outputs(self, list_item_emb, batch_size):
        """构建不预测消费长度的因果 List Value 分支。

        输入 list_item_emb 的形状为 [batch, list_num, list_size, 64]。
        第 k 个位置只允许读取前 k 个 item，用于预测各前缀的累计观看时长、
        累计有效播放数；固定最后一个物理位置已经读取完整 List，额外直接
        预测整条 List 的最终 WT/EVV，线上不依赖不可知的 real_show K。
        """
        with tf.variable_scope("list_value_branch", reuse=tf.AUTO_REUSE):
            ...
            # 完整 List 总价值头：固定取最后一个物理位置。因果注意力下该位置
            # 已读取全部 LIST_SIZE 个 item，训练和线上都无需知道真实 K。
            full_list_hidden = prefix_hidden[:, :, -1, :]
            total_value_hidden = tf.layers.dense(
                full_list_hidden,
                64,
                activation=tf.nn.leaky_relu,
                name="total_value_hidden",
            )
            expected_watch_time_log = tf.nn.softplus(
                tf.squeeze(
                    tf.layers.dense(
                        total_value_hidden,
                        1,
                        activation=None,
                        name="expected_watch_time_logit",
                    ),
                    axis=-1,
                ),
                name="expected_watch_time_log",
            )
            expected_watch_time = tf.math.expm1(
                tf.minimum(expected_watch_time_log, max_watch_time_log),
                name="expected_watch_time",
            )
            expected_effective_vv = tf.multiply(
                tf.nn.sigmoid(
                    tf.squeeze(
                        tf.layers.dense(
                            total_value_hidden,
                            1,
                            activation=None,
                            name="expected_effective_vv_logit",
                        ),
                        axis=-1,
                    )
                ),
                tf.cast(self._list_size, tf.float32),
            ...
            return {
                ...
                "expected_watch_time_log": expected_watch_time_log,
                "expected_watch_time": expected_watch_time,
                "expected_effective_vv": expected_effective_vv,
            }
