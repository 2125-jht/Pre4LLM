...
> `fountain_rerank_eval_listwise_v1_nlength` 是独立的无消费长度版本：不再预测
> `P(K)`，也不再使用 `P(K) × PrefixValue(K)` 聚合。训练时仍以最后一个
> `real_show` 的物理位置确定 K，并监督 `1...K` 的全部物理前缀；线上最终
> WT/EVV 则由固定完整 List 表征直接预测，因此不依赖线上不可知的 K。

## nlength 版实现口径

- causal prefix encoder 和 prefix WT/EVV head 保留，作为过程辅助监督；内部
  `real_show=0` 的 gap 不压缩，价值增量为 0。
- 删除消费长度分类 head、长度 loss、长度指标及 `expected_consume_length` 输出。
- 固定取 causal encoder 最后一个物理位置的 hidden；它已经读取完整 6-item
  List，再通过独立总量 head 直接预测 `expected_list_watch_time` 和
  `expected_list_effective_vv`。
- 完整 List WT 使用 `log1p` 空间的 Huber loss，EVV 使用原空间 Huber loss；
  线上沿用原有两个 AB 权重名，便于与 v1 做单变量对照。

下面保留原 v1 概率分解方案和样本诊断记录，作为实验背景与对照。
