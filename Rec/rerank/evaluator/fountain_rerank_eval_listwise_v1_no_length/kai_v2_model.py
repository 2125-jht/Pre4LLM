...
# nlength 版 List Value loss 权重。保留 point-wise 主任务与前缀价值辅助监督，
# 完整 List 的 WT/EVV 由固定全 List 表征直接预测，不训练消费长度分布。
...
if is_training:
    ...
    # -------- 新增 List Value 标签 --------
    # 按物理位置累加，构造 Prefix[1:k] 的累计 WT/EVV。内部 real_show=0
    # 位置保留为零增量；prefix_label_mask 同时限制“旧分最高 List”和
    # “位置不晚于最后一个 real_show”两个条件，不拿 K 之后的位置做监督。
    ...
    # P0：List 级事实标签只累计真实曝光 item，与参考版本保持一致。
    # 预测侧仍使用完整 List，不引入 show oracle。
    list_play_time_s_reduced_raw = tf.reduce_sum(
        list_play_time_s,
        axis=-1,
    )
    ...
    p0_wt_label_abs_delta_mean = tf.reduce_sum(
        tf.abs(list_play_time_s_reduced_raw - list_play_time_s_reduced)
        * listwise_match_mask
    ...
    # 将原单点 VTR/WTD 解码出的 item WT 在每条候选 List 的完整 6 个位置上求和。
    # padding 位置必须显式置零，否则 duration=0 仍可能被解码出非零时长。
    valid_list_item_mask = tf.cast(
        tf.greater(rerank_list_item_idx_flat_list, 0),
        tf.float32,
    )
    ...
    with tf.control_dependencies(print_ops):
        ...
        expected_watch_time_log = list_value_output_dict["expected_watch_time_log"]
        ...
        # 固定完整 List 表征直接预测事实总价值。WT 直接复用 head 的 log 空间
        # 输出训练，避免再做 log1p(expm1(...))；线上输出仍为原始秒数。
        list_wt_loss = tf.losses.huber_loss(
            labels=tf.math.log1p(list_play_time_s_reduced),
            predictions=expected_watch_time_log,
        ...
        list_value_loss = weighted_prefix_wt_loss \
    ...
    # nlength 版训练与推理统一使用固定完整 List 表征的直接总价值预测。
