...
#kafka_topic="fountain_rank_sample_log"
kafka_topic="reco_hot_context_rank_joint_log"
#hdfs_path="viewfs:///home/reco_5/mpi/panshangkao/fountain_ltr_v3/sample_batch_data/2021-08-23/17"  
hdfs_path=""
args = parser.parse_args()

kai_kuiba_config_filename = args.from_kai_kuiba_conf
kuiba_config_filename = args.from_kuiba_conf
# 一些老数据 label_value 全是 0, 如果需要处理成存在即为1, 这里需要设置为 True, 否则设置为 False
use_compatible_labels = True

# 这个最好 kml 做渲染
reader_name="mio_fountain_rerank_mix"

# 这个 attr 在 dragonfly 里面是必须的, 用来给 item 一个 id, 根据业务自己需要配置
item_id_attr_name=""
# 用于特征算分里面，表示哪个算click
label_atttr_name="slide_ctr_6"
...
for loss_name in loss_functions:
  ...
  for loss_label in loss_labels:
    loss_label_obj = loss_labels[loss_label]
    label_key = "LABEL_ATTR_" + loss_label.split(",")[-1];
    # labels_value_dict
    labels_value_dict[label_key] = 0
    if "expired_output" in loss_label_obj:
      expired_outputs = loss_label_obj["expired_output"]
      assert(len(expired_outputs) == 1)
      labels_value_dict[label_key] = expired_outputs[0]
    labels_mapping[loss_label] = label_key
    # labels
    if use_compatible_labels:
      compatible_labels.append(loss_label)

  for label_key in labels_value_dict:
    if label_key not in labels_item_attr:
      # labels_item_attr
      labels_item_attr.append(label_key)
      label_lua_script = label_lua_script \
        + "  local " + label_key + " = " + label_key + " or 0 \n"
  label_lua_script = label_lua_script \
    + "  local " + loss_name + "_sample_flag = 0 \n"
  label_lua_script = label_lua_script \
    + "  local " + loss_name + "_label = 0 \n"
  label_lua_script = label_lua_script \
    + "  local " + loss_name + "_label_value = 0 \n"
  for label_key in labels_value_dict:
    label_lua_script = label_lua_script \
      + "  if " + label_key + " > 0 then \n" \
      + "    " + loss_name + "_sample_flag = 1 \n"
    if labels_value_dict[label_key] == 1:
      label_lua_script = label_lua_script \
        + "    " + loss_name + "_label = 1 \n" \
        + "    " + loss_name + "_label_value = 1 \n"
    label_lua_script = label_lua_script \
      + "  end \n"
label_lua_script = label_lua_script + "  return"
for i, loss_attr in enumerate(loss_attrs):
  if i == 0:
    label_lua_script = label_lua_script + " " + loss_attr
  else:
    label_lua_script = label_lua_script + ", " + loss_attr
label_lua_script = label_lua_script + "\nend"
print("--------------------------------------------------")
print(label_lua_script)
print("--------------------------------------------------")
...
logical_flow \
  ...
  .retrieve_from_raw_sample_package(
    ...
    #labels=[
    #  dict(label="China,push_mix_all,show", attr="show"),
    #  dict(label="China,push_mix_all,click", attr="click"),
    #],
    save_locale_to="",
    save_channel_to="",
  ...
  .enrich_attr_by_lua(
    #import_item_attr = ["show", "click"],
    import_item_attr = labels_item_attr,
    function_for_item = "calculate",
    #export_item_attr = [
    #  "ctr_sample_flag", "ctr_label", "ctr_label_value"
    #],
    export_item_attr = loss_attrs,
    lua_script = label_lua_script
    #lua_script = """
    #  function calculate()
    #    --------------------------------------
    #    -- 输入 attr 的缺省值处理（十分必要！）
    #    --------------------------------------
    #    local show = show or 0
    #    local click = click or 0
    #    --------------------------------------
    #    local ctr_sample_flag = 0
    #    local ctr_label = 0
    #    local ctr_label_value = 0
    #    if show > 0 then
    #      ctr_sample_flag = 1
    #    end
    #    if click > 0 then
    ...
    #      ctr_label_value = 1
    #    end
    #    return ctr_sample_flag, ctr_label, ctr_label_value
    #  """
    )
