...
def self_attention(loss, query, data, n, nh, dim, mask):
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
      ...
      #Q = tf.layers.dense(query, nh * dim)  # [batch_size, query_length, hidden_dim]
      #K = tf.layers.dense(data, nh * dim)  # [batch_size, sequence_length, hidden_dim]
