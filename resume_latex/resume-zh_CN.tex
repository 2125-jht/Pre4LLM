...
\begin{itemize}
  ...
  将现有逐 item 预估、规则累加的评估范式升级为消费过程感知的 List-wise Value Model。基于 Causal Prefix Transformer 建模序列上下文，将消费长度分解为逐位置继续概率并递推 $P(K)$，联合预测各 Prefix 的累计观看时长、有效 VV 与互动价值；针对曝光截断、长尾价值标签及未曝光后缀缺少反事实监督等问题，设计右截断 Continuation NLL、Prefix/List 双层价值回归、累计价值单调约束与反馈偏好 RankNet Loss，最终通过
